...
static void print_string_list(FILE *out, const char *const *list,
                              const char *prefix);
...
static int get_argv_utf8(int *argc_ptr, char ***argv_ptr) {
  ...
  if (argv_utf16) {
    ...
    for (i = 0; i < argc; i++)
      size += WideCharToMultiByte(CP_UTF8, 0, argv_utf16[i], -1, NULL, 0, NULL,
                                  NULL);
    ...
    if (argv != NULL) {
      for (i = 0; i < argc; i++) {
        ...
        offset += WideCharToMultiByte(CP_UTF8, 0, argv_utf16[i], -1, argv[i],
...
void EventLoop(hb_handle_t *h, hb_dict_t *preset_dict) {
  ...
  while (!die && !work_done) {
    ...
    if (ret > 0) {
      ...
      while (size < 256 && read(STDIN_FILENO, &buf[size], 1) > 0) {
        if (buf[size] == '\n') {
          break;
        }
        size++;
...
int main(int argc, char **argv) {
  ...
  /* Parse command line */
  if (ParseOptions(argc, argv) || CheckOptions(argc, argv)) {
  ...
  /* Show version */
  fprintf(stderr, "%s - %s - %s\n", HB_PROJECT_TITLE, HB_PROJECT_HOST_TITLE,
...
static void PrintTitleInfo(hb_title_t *title, int feature) {
  ...
  fprintf(
      ...
      title->geometry.width, title->geometry.height, title->geometry.par.num,
      title->geometry.par.den, (float)title->dar.num / title->dar.den,
      (float)title->vrate.num / title->vrate.den);
  fprintf(stderr, "  + autocrop: %d/%d/%d/%d\n", title->crop[0], title->crop[1],
  ...
  for (i = 0; i < hb_list_count(title->list_audio); i++) {
    ...
    if ((audio->in.codec == HB_ACODEC_AC3) ||
        (audio->in.codec == HB_ACODEC_DCA)) {
      fprintf(stderr, "    + %d, %s (iso639-2: %s), %dHz, %dbps\n", i + 1,
              audio->lang.description, audio->lang.iso639_2,
              audio->in.samplerate, audio->in.bitrate);
    } else {
...
void write_chapter_names(hb_dict_t *job_dict, const char *marker_file) {
  ...
  chapter_array =
      hb_dict_get(hb_dict_get(job_dict, "Destination"), "ChapterList");
  ...
  while (NULL != (cell = hb_read_next_cell(file))) {
    ...
    if (cell->cell_col == 1 && row == cell->cell_row) {
      ...
      if (chapter_dict != NULL) {
        hb_dict_set(chapter_dict, "Name", hb_value_string(cell->cell_text));
      }
    }
  ...
  case HB_STATE_SCANNING:
    ...
    if (p.preview_cur) {
      fprintf(stderr, "%sScanning title %d of %d, preview %d, %.2f %%",
              stderr_sep, p.title_cur, p.title_count, p.preview_cur,
              100 * p.progress);
    } else {
      fprintf(stderr, "%sScanning title %d of %d, %.2f %%", stderr_sep,
...
#define p s.param.working
  case HB_STATE_SEARCHING:
    ...
    fprintf(stdout,
            "%sEncoding: task %d of %d, Searching for start time, %.2f %%",
            stdout_sep, p.pass, p.pass_count, 100.0 * p.progress);
    if (p.seconds > -1) {
      fprintf(stdout, " (ETA %02dh%02dm%02ds)", p.hours, p.minutes, p.seconds);
    }
    fflush(stdout);
    break;
  ...
  case HB_STATE_WORKING:
    ...
    fprintf(stdout, "%sEncoding: task %d of %d, %.2f %%", stdout_sep, p.pass,
...
static void ShowHelp() {
  ...
  fprintf(
      ...
      "                           JSON format\n"
      "   -v, --verbose[=number]  Be verbose (optional argument: logging "
      "level)\n"
      "   -Z. --preset <string>   Select preset by name (case-sensitive)\n"
      "                           Enclose names containing spaces in double "
      "quotation\n"
      ...
      "options and\n"
      "                           write a json representation of the preset to "
      "the\n"
      "                           console or a file if '--preset-export-file' "
      "is\n"
      "                           specified. The required argument will be the "
      "name\n"
      "                           of the new preset.\n"
      "   --preset-export-file <filename>\n"
      "                           Write new preset generated by "
      "'--preset-export'\n"
      "                           to file 'filename'.\n"
      "   --preset-export-description <string>\n"
      "                           Add a description to the new preset created "
      "with\n"
      "                           '--preset-export'\n"
      "   --queue-import-file <filename>\n"
      "                           Import an encode queue file created by the "
      "GUI\n"
      ...
      "   -t, --title <number>    Select a title to encode (0 to scan all "
      "titles\n"
      "                           only, default: 1)\n"
      "       --min-duration      Set the minimum title duration (in "
      "seconds).\n"
      "                           Shorter titles will be ignored (default: "
      "10).\n"
      ...
      "       --angle <number>    Select the video angle (DVD or Blu-ray "
      "only)\n"
      "       --previews <number:boolean>\n"
      "                           Select how many preview images are "
      "generated,\n"
      ...
      "                           Start encoding at a given offset in "
      "seconds,\n"
      "                           frames, or pts (on a 90kHz clock)\n"
      "                           (e.g. seconds:10, frames:300, pts:900000).\n"
      "   --stop-at  <string:number>\n"
      "                           Stop encoding after a given duration in "
      "seconds,\n"
      "                           frames, or pts (on a 90kHz clock) has "
      "passed\n"
      "                           (e.g. seconds:10, frames:300, pts:900000).\n"
      "                           Duration is relative to --start-at, if "
      "specified.\n"
      "\n"
      "\n"
      "Destination Options "
  ...
  fprintf(
      out,
      "                           default: auto-detected from destination file "
      "name)\n"
      "   -m, --markers           Add chapter markers\n"
      "       --no-markers        Disable preset chapter markers\n"
      "   -O, --optimize          Optimize MP4 files for HTTP streaming (fast "
      "start,\n"
      "                           s.s. rewrite file to place MOOV atom at "
      "beginning)\n"
      "       --no-optimize       Disable preset 'optimize'\n"
      "   -I, --ipod-atom         Add iPod 5G compatibility atom to MP4 "
      "container\n"
      "       --no-ipod-atom      Disable iPod 5G atom\n"
      "       --align-av          Add audio silence or black video frames to "
      "start\n"
      "                           of streams so that all streams start at "
      "exactly\n"
      "                           the same time\n"
      "   --inline-parameter-sets Create adaptive streaming compatible "
      "output.\n"
  ...
  fprintf(
      ...
      "                           Adjust video encoding settings for a "
      "particular\n"
      "                           speed/efficiency tradeoff "
      "(encoder-specific)\n"
      "   --encoder-preset-list <string>\n"
      "                           List supported --encoder-preset values for "
      "the\n"
      "                           specified video encoder\n"
      "       --encoder-tune <string>\n"
      "                           Adjust video encoding settings for a "
      "particular\n"
      "                           type of source or situation "
      "(encoder-specific)\n"
      "   --encoder-tune-list <string>\n"
      "                           List supported --encoder-tune values for "
      "the\n"
      "                           specified video encoder\n"
      "   -x, --encopts <string>  Specify advanced encoding options in the "
      "same\n"
      "                           style as mencoder (all encoders except "
      "theora):\n"
      ...
      "                           List supported --encoder-profile values for "
      "the\n"
      "                           specified video encoder\n"
      ...
      "                           List supported --encoder-level values for "
      "the\n"
      "                           specified video encoder\n"
      ...
      "   -T, --turbo             When using 2-pass use \"turbo\" options on "
      "the\n"
  ...
  fprintf(
      out,
      ").\n"
      "                           Be aware that not specifying a framerate "
      "lets\n"
      "                           HandBrake preserve a source's time stamps,\n"
      "                           potentially creating variable framerate "
      "video\n"
      "   --vfr, --cfr, --pfr     Select variable, constant or peak-limited\n"
      "                           frame rate control. VFR preserves the "
      "source\n"
      "                           timing. CFR makes the output constant rate "
      "at\n"
      "                           the rate given by the -r flag (or the "
      "source's\n"
      "                           average rate if no -r is given). PFR "
      "doesn't\n"
      "                           allow the rate to go over the rate "
      "specified\n"
      "                           with the -r flag but won't change the "
      "source\n"
      "                           timing if it's below that rate.\n"
      "                           If none of these flags are given, the "
      "default\n"
      "                           is --pfr when -r is given and --vfr "
      "otherwise\n"
      "\n"
      "\n"
      "Audio Options "
      ...
      "                           matching each language will be added to "
      "your\n"
      "                           output. Provide the language's ISO 639-2 "
      "code\n"
      "                           (e.g. fre, eng, spa, dut, et cetera)\n"
      "                           Use code 'und' (Unknown) to match all "
      "languages.\n"
      "       --all-audio         Select all audio tracks matching languages "
      "in\n"
      "                           the specified language list "
      "(--audio-lang-list).\n"
      "                           Any language if list is not specified.\n"
      "       --first-audio       Select first audio track matching languages "
      "in\n"
      "                           the specified language list "
      "(--audio-lang-list).\n"
      "                           Any language if list is not specified.\n"
      "   -a, --audio <string>    Select audio track(s), separated by commas\n"
      "                           (\"none\" for no audio, \"1,2,3\" for "
      "multiple\n"
      "                           tracks, default: first one).\n"
      "                           Multiple output tracks can be used for one "
      "input.\n"
  ...
  fprintf(out, "                           \"copy:<type>\" will pass through "
               "the corresponding\n"
               "                           audio track without modification, "
               "if pass through\n"
  ...
  fprintf(
      ...
      "                           Set audio codecs that are permitted when "
      "the\n"
  ...
  fprintf(
      out,
      ")\n"
      "                           Separated by commas for multiple allowed "
      "options.\n"
      "       --audio-fallback <string>\n"
      "                           Set audio codec to use when it is not "
      "possible\n"
      "                           to copy an audio track without re-encoding.\n"
      "   -B, --ab <number>       Set audio track bitrate(s) in kbit/s.\n"
      "                           (default: determined by the selected codec, "
      "mixdown,\n"
  ...
  fprintf(out, "                           Separate tracks by commas.\n"
               "                           Defaults:\n");
  encoder = NULL;
  while ((encoder = hb_audio_encoder_get_next(encoder)) != NULL) {
    if (!(encoder->codec & HB_ACODEC_PASS_FLAG)) {
  ...
  fprintf(out,
          "       --normalize-mix     Normalize audio mix levels to prevent "
          "clipping.\n"
  ...
  fprintf(
      ...
      "   -D, --drc <float>       Apply extra dynamic range compression to "
      "the\n"
      "                           audio, making soft sounds louder. Range is "
      "1.0\n"
      "                           to 4.0 (too loud), with 1.5 - 2.5 being a "
      "useful\n"
      "                           range.\n"
      "                           Separate tracks by commas.\n"
      "       --gain <float>      Amplify or attenuate audio before encoding.  "
      "Does\n"
      "                           NOT work with audio passthru (copy). Values "
      "are\n"
      "                           in dB.  Negative values attenuate, positive\n"
      "                           values amplify. A 1 dB difference is barely\n"
      "                           audible.\n"
      "       --adither <string>  Select dithering to apply before encoding "
      "audio:\n");
  dither = NULL;
  ...
  fprintf(
      ...
      "   --auto-anamorphic       Store pixel aspect ratio that maximizes "
      "storage\n"
      "                           resolution\n"
      "   --loose-anamorphic      Store pixel aspect ratio that is as close "
      "as\n"
      "                           possible to the source video pixel aspect "
      "ratio\n"
      "   --custom-anamorphic     Store pixel aspect ratio in video stream "
      "and\n"
      "                           directly control all parameters.\n"
      "   --display-width <number>\n"
      "                           Set display width in pixels, for custom "
      "anamorphic.\n"
      "                           This determines the display aspect during "
      "playback,\n"
      ...
      "                           (--display-width and --pixel-aspect are "
      "mutually\n"
      "                           exclusive.\n"
      "   --itu-par               Use wider ITU pixel aspect values for loose "
      "and\n"
      ...
      "                           If not accompanied by the decomb or "
      "deinterlace\n"
      "                           filters, this filter only logs the "
      "interlaced\n"
      "                           frame count to the activity log.\n"
      "                           If accompanied by the decomb or deinterlace\n"
      "                           filters, it causes these filters to "
      "selectively\n"
      "                           deinterlace only those frames where "
      "interlacing\n"
      "                           is detected.\n");
  ...
  fprintf(out,
          ...
          "   -5, --decomb[=string]   Deinterlace video using a combination of "
          "yadif,\n"
  ...
  fprintf(
      ...
      "                           restore the pre-telecine framerate, unless "
      "you\n"
  ...
  fprintf(out, "   --no-detelecine         Disable preset detelecine filter\n"
  ...
  fprintf(out, "                           Applies to NLMeans presets only "
               "(does not affect\n"
               "                           custom settings)\n"
               "   --chroma-smooth[=string]      Sharpen video with chroma "
               "smooth filter\n");
  ...
  fprintf(out,
          "                                 Applies to chroma smooth presets "
          "only (does\n"
  ...
  fprintf(out,
          "                           Applies to unsharp presets only (does "
          "not affect\n"
          "                           custom settings)\n"
  ...
  fprintf(out,
          "                           Applies to lapsharp presets only (does "
          "not affect\n"
          "                           custom settings)\n"
  ...
  fprintf(out, "   --no-deblock            Disable preset deblock filter\n"
               "   --deblock-tune <string>\n"
               "                           Tune deblock filter\n");
  showFilterTunes(out, HB_FILTER_DEBLOCK);
  fprintf(out,
          "                           Applies to deblock presets only (does "
          "not affect\n"
          "                           custom settings)\n"
          "   --rotate[=string]       Rotate image or flip its axes.\n"
          "                           angle rotates clockwise, can be one of:\n"
          "                               0, 90, 180, 270\n"
          "                           hflip=1 flips the image on the x axis "
          "(horizontally).\n");
  ...
  fprintf(
      ...
      "                           The padding color may be set (default "
      "black).\n"
      "                           Color may be an HTML color name or RGB "
      "value.\n"
      "                           The position of image in pad may also be "
      "set.\n");
  ...
  fprintf(
      ...
      "                           languages you would like to select from the\n"
      "                           source title. By default, the first "
      "subtitle\n"
      "                           matching each language will be added to "
      "your\n"
      "                           output. Provide the language's ISO 639-2 "
      "code\n"
      "                           (e.g. fre, eng, spa, dut, et cetera)\n"
      "      --all-subtitles      Select all subtitle tracks matching "
      "languages in\n"
      "                           the specified language list\n"
      "                           (--subtitle-lang-list).\n"
      "                           Any language if list is not specified.\n"
      "      --first-subtitle     Select first subtitle track matching "
      "languages in\n"
      "                           the specified language list\n"
      "                           (--subtitle-lang-list).\n"
      "                           Any language if list is not specified.\n"
      "  -s, --subtitle <string>  Select subtitle track(s), separated by "
      "commas\n"
      "                           More than one output track can be used for "
      "one\n"
      "                           input. \"none\" for no subtitles.\n"
      "                           Example: \"1,2,3\" for multiple tracks.\n"
      "                           A special track name \"scan\" adds an extra "
      "first\n"
      "                           pass. This extra pass scans subtitles "
      "matching\n"
      "                           the language of the first audio or the "
      "language \n"
      "                           selected by --native-language.\n"
      "                           The one that's only used 10 percent of the "
      "time\n"
      "                           or less is selected. This should locate "
      "subtitles\n"
      "                           for short foreign language segments. Best "
      "used in\n"
      "                           conjunction with --subtitle-forced.\n"
      "  -S, --subname <string>   Set subtitle track name(s).\n"
      "                           Separate tracks by commas.\n"
      "  -F, --subtitle-forced[=string]\n"
      "                           Only display subtitles from the selected "
      "stream\n"
      "                           if the subtitle has the forced flag set. "
      "The\n"
      ...
      "                           \"Burn\" the selected subtitle into the "
      "video\n"
      "                           track. If \"subtitle\" is omitted, the "
      "first\n"
      "                           track is burned. \"subtitle\" is an index "
      "into\n"
      "                           the subtitle list specified with "
      "'--subtitle'\n"
      "                           or \"native\" to burn the subtitle track "
      "that may\n"
      "                           be added by the 'native-language' option.\n"
      "      --subtitle-default[=number or \"none\"]\n"
      "                           Flag the selected subtitle as the default\n"
      "                           subtitle to be displayed upon playback.  "
      "Setting\n"
      "                           no default means no subtitle will be "
      "displayed\n"
      "                           automatically. 'number' is an index into "
      "the\n"
      "                           subtitle list specified with '--subtitle'.\n"
      "                           \"none\" may be used to override an "
      "automatically\n"
      "                           selected default subtitle track.\n"
      "  -N, --native-language <string>\n"
      "                           Specify your language preference. When the "
      "first\n"
      "                           audio track does not match your native "
      "language\n"
      "                           then select the first subtitle that does. "
      "When\n"
      "                           used in conjunction with --native-dub the "
      "audio\n"
      "                           track is changed in preference to "
      "subtitles.\n"
      "                           Provide the language's ISO 639-2 code\n"
      "                           (e.g. fre, eng, spa, dut, et cetera)\n"
      "      --native-dub         Used in conjunction with --native-language\n"
      "                           requests that if no audio tracks are "
      "selected the\n"
      "                           default selected audio track will be the "
      "first\n"
      "                           one that matches the --native-language. If "
      "there\n"
      "                           are no matching audio tracks then the first\n"
      "                           matching subtitle track is used instead.\n"
      "     --srt-file <string>   SubRip SRT filename(s), separated by "
      "commas.\n"
      "     --srt-codeset <string>\n"
      "                           Character codeset(s) that the SRT file(s) "
      "are\n"
      "                           encoded as, separated by commas.\n"
      "                           If not specified, 'latin1' is assumed.\n"
      "                           Command 'iconv -l' provides a list of valid "
      "codesets.\n"
      "     --srt-offset <string> Offset (in milliseconds) to apply to the "
      "SRT\n"
      "                           file(s), separated by commas. If not "
      "specified,\n"
      ...
      "                           Separate by commas.\n"
      "     --srt-default[=number]\n"
      "                           Flag the selected SRT as the default "
      "subtitle\n"
      "                           to be displayed during playback.\n"
      "                           Setting no default means no subtitle will "
      "be\n"
      "                           automatically displayed. If 'number' is "
      "omitted,\n"
      "                           the first SRT is the default.\n"
      "                           'number' is a 1-based index into the "
      "'srt-file' list\n"
      "     --srt-burn[=number]   \"Burn\" the selected SRT subtitle into\n"
      "                           the video track.\n"
      "                           If 'number' is omitted, the first SRT is "
      "burned.\n"
      "                           'number' is a 1-based index into the "
      "'srt-file' list\n"
      "     --ssa-file <string>   SubStationAlpha SSA filename(s), separated "
      "by\n"
      "                           commas.\n"
      "     --ssa-offset <string> Offset (in milliseconds) to apply to the "
      "SSA\n"
      "                           file(s), separated by commas. If not "
      "specified,\n"
      "                           zero is assumed. Offsets may be negative.\n"
      ...
      "                           Separate by commas.\n"
      "     --ssa-default[=number]\n"
      "                           Flag the selected SSA as the default "
      "subtitle\n"
      "                           to be displayed during playback.\n"
      "                           Setting no default means no subtitle will "
      "be\n"
      ...
      "                           the video track.\n"
      "                           If 'number' is omitted, the first SSA is "
      "burned.\n"
      "                           'number' is a 1-based index into the "
      "'ssa-file' list\n"
      "\n");
  ...
  if (hb_qsv_available()) {
    fprintf(
        ...
        "--------------------------------------------\n"
        "\n"
        "   --enable-qsv-decoding   Allow QSV hardware decoding of the video "
        "track\n"
        "   --disable-qsv-decoding  Disable QSV hardware decoding of the video "
        "track,\n"
        "                           forcing software decoding instead\n"
        "   --qsv-async-depth[=number]\n"
        "                           Set the number of asynchronous operations "
        "that\n"
 ...
 * ShowPresets:
 ****************************************************************************/
static const char *reverse_search_char(const char *front, const char *back,
                                       char delim) {
...
static void ShowPresets(hb_value_array_t *presets, int indent,
                        int descriptions) {
  if (presets == NULL)
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    if (hb_value_get_bool(hb_dict_get(preset_dict, "Folder"))) {
      ...
      if (descriptions) {
        const char *desc;
        desc =
            hb_value_get_string(hb_dict_get(preset_dict, "PresetDescription"));
...
#define FILTER_DEBLOCK_TUNE 324
  ...
  for (;;) {
    static struct option long_options[] = {
      ...
      {"chroma-smooth-tune", required_argument, NULL,
       FILTER_CHROMA_SMOOTH_TUNE},
      ...
      {"custom-anamorphic", no_argument, &anamorphic_mode,
      ...
      {"preset-export-description", required_argument, NULL,
       PRESET_EXPORT_DESC},
    ...
    case 'i':
      ...
      if (devName != NULL) {
        if (device_is_dvd(devName)) {
          ...
          if (input == NULL) {
            fprintf(stderr, "ERROR: malloc() failed while attempting to set "
                            "device path.\n");
            free(devName);
            return -1;
          }
    ...
    case SUB_BURNED:
      if (optarg != NULL) {
        if (!strcasecmp(optarg, "native") || !strcasecmp(optarg, "scan")) {
          subburn_native = 1;
    ...
    case ENCODER_PRESET_LIST:
      fprintf(stderr, "Available --encoder-preset values for '%s' encoder:\n",
              hb_video_encoder_get_short_name(
                  hb_video_encoder_get_from_name(optarg)));
      print_string_list(
          stderr,
    ...
    case STOP_AT: {
      ...
      if (!strcmp(stop_at_token, "frame") || !strcmp(stop_at_token, "frames")) {
    ...
    case 'M':
      if (optarg != NULL) {
        if (!strcmp(optarg, "601") || !strcmp(optarg, "ntsc"))
          color_matrix_code = 1;
  ...
  if (deblock != NULL) {
    if (deblock_disable) {
      fprintf(stderr, "Incompatible options --deblock and --no-deblock\n");
      return -1;
    }
    if (!hb_validate_filter_preset(HB_FILTER_DEBLOCK, deblock, deblock_tune,
                                   NULL)) {
  ...
  if (detelecine != NULL) {
    ...
    if (!hb_validate_filter_preset(HB_FILTER_DETELECINE, detelecine, NULL,
                                   NULL)) {
      // Nothing to do, but must validate preset before
      // attempting to validate custom settings to prevent potential
      // false positive
  ...
  if (pad != NULL) {
    if (pad_disable) {
      fprintf(stderr, "Incompatible options --pad and --no-pad\n");
      return -1;
  ...
  if (deinterlace != NULL) {
    ...
    if (!hb_validate_filter_preset(HB_FILTER_DEINTERLACE, deinterlace, NULL,
                                   NULL)) {
      // Nothing to do, but must validate preset before
      // attempting to validate custom settings to prevent potential
      // false positive
  ...
  if (comb_detect != NULL) {
    ...
    if (!hb_validate_filter_preset(HB_FILTER_COMB_DETECT, comb_detect, NULL,
                                   NULL)) {
      // Nothing to do, but must validate preset before
      // attempting to validate custom settings to prevent potential
      // false positive
  ...
  if (decomb != NULL) {
    if (decomb_disable) {
      fprintf(stderr, "Incompatible options --decomb and --no-decomb\n");
      return -1;
    }
  ...
  if (hqdn3d != NULL) {
    if (hqdn3d_disable) {
      fprintf(stderr, "Incompatible options --hqdn3d and --no-hqdn3d\n");
      return -1;
    }
  ...
  if (nlmeans != NULL) {
    if (nlmeans_disable) {
      fprintf(stderr, "Incompatible options --nlmeans and --no-nlmeans\n");
      return -1;
    }
  ...
  if (chroma_smooth != NULL) {
    ...
    if (!hb_validate_filter_preset(HB_FILTER_CHROMA_SMOOTH, chroma_smooth,
      ...
      // attempting to validate custom settings to prevent potential
      // false positive
    } else if (!hb_validate_filter_string(HB_FILTER_CHROMA_SMOOTH,
                                          chroma_smooth)) {
  ...
  if (unsharp != NULL) {
    if (unsharp_disable) {
      fprintf(stderr, "Incompatible options --unsharp and --no-unsharp\n");
      return -1;
    }
    if (!hb_validate_filter_preset(HB_FILTER_UNSHARP, unsharp, unsharp_tune,
                                   NULL)) {
      // Nothing to do, but must validate preset before
      // attempting to validate custom settings to prevent potential
      // false positive
  ...
  if (lapsharp != NULL) {
    if (lapsharp_disable) {
      fprintf(stderr, "Incompatible options --lapsharp and --no-lapsharp\n");
      return -1;
    }
    if (!hb_validate_filter_preset(HB_FILTER_LAPSHARP, lapsharp, lapsharp_tune,
                                   NULL)) {
      // Nothing to do, but must validate preset before
      // attempting to validate custom settings to prevent potential
      // false positive
...
static int CheckOptions(int argc, char **argv) {
  ...
  if (subtitle_track_count > 0 && subtitle_all != -1) {
    fprintf(stderr, "Incompatible options: --all-subtitles/--first-subtitle "
                    "and --subtitle\n");
    return 1;
  }

  if (atracks != NULL && audio_lang_list != NULL) {
    fprintf(stderr, "Incompatible options: --audio-lang-list and --audio\n");
    return 1;
  }
...
static hb_dict_t *PreparePreset(const char *preset_name) {
  ...
  if (preset_name != NULL) {
    preset = hb_preset_search(preset_name, 1 /*recurse*/, HB_PRESET_TYPE_ALL);
    if (preset == NULL) {
      fprintf(stderr,
              "Invalid preset %s\n"
              "Valid presets are:\n",
  ...
  if (native_language != NULL) {
    ...
    hb_dict_set(preset, "SubtitleAddForeignAudioSubtitle", hb_value_bool(1));
  }
  ...
  // Audio overrides
  if (atracks == NULL && audio_all != 1 &&
      (acodecs != NULL || abitrates != NULL || arates != NULL ||
       mixdowns != NULL || normalize_mix_level != NULL ||
       audio_dither != NULL || dynamic_range_compression != NULL ||
       audio_gain != NULL || aqualities != NULL || acompressions != NULL ||
       anames != NULL)) {
    ...
    if (hb_str_vlen(acodecs) > 0) {
      int last = -1;
      for (ii = 0; acodecs[ii] != NULL && acodecs[ii][0] != 0; ii++) {
        audio_dict = hb_value_array_get(list, ii);
        hb_dict_set(audio_dict, "AudioEncoder", hb_value_string(acodecs[ii]));
        last = ii;
      }
    ...
    if (hb_str_vlen(abitrates) > 0) {
      for (ii = 0; abitrates[ii] != NULL && abitrates[ii][0] != 0; ii++) {
        audio_dict = hb_value_array_get(list, ii);
    ...
    if (hb_str_vlen(aqualities) > 0) {
      for (ii = 0; aqualities[ii] != NULL && aqualities[ii][0] != 0; ii++) {
        audio_dict = hb_value_array_get(list, ii);
        hb_dict_set(audio_dict, "AudioTrackQualityEnable", hb_value_bool(1));
    ...
    if (hb_str_vlen(arates) > 0) {
      int last = -1;
      for (ii = 0; arates[ii] != NULL && arates[ii][0] != 0; ii++) {
        audio_dict = hb_value_array_get(list, ii);
        hb_dict_set(audio_dict, "AudioSamplerate", hb_value_string(arates[ii]));
        last = ii;
      }
      if (last_audio_dict == NULL && last >= 0) {
        // No defaults exist in original preset.
    ...
    if (hb_str_vlen(mixdowns) > 0) {
      int last = -1;
      for (ii = 0; mixdowns[ii] != NULL && mixdowns[ii][0] != 0; ii++) {
        audio_dict = hb_value_array_get(list, ii);
        hb_dict_set(audio_dict, "AudioMixdown", hb_value_string(mixdowns[ii]));
        last = ii;
      }
      if (last_audio_dict == NULL && last >= 0) {
        // No defaults exist in original preset.
    ...
    if (hb_str_vlen(normalize_mix_level) > 0) {
      int last = -1;
      for (ii = 0;
           normalize_mix_level[ii] != NULL && normalize_mix_level[ii][0] != 0;
           ii++) {
        audio_dict = hb_value_array_get(list, ii);
    ...
    if (hb_str_vlen(dynamic_range_compression) > 0) {
      ...
      for (ii = 0; dynamic_range_compression[ii] != NULL &&
                   dynamic_range_compression[ii][0] != 0;
           ii++) {
        audio_dict = hb_value_array_get(list, ii);
        hb_dict_set(
            audio_dict, "AudioTrackDRCSlider",
            hb_value_double(strtod(dynamic_range_compression[ii], NULL)));
        last = ii;
      }
      if (last_audio_dict == NULL && last >= 0) {
        // No defaults exist in original preset.
        ...
        for (; ii < count; ii++) {
          ...
          hb_dict_set(
              audio_dict, "AudioTrackDRCSlider",
              hb_value_double(strtod(dynamic_range_compression[last], NULL)));
        }
      }
    }
    ...
    if (hb_str_vlen(audio_gain) > 0) {
      int last = -1;
      for (ii = 0; audio_gain[ii] != NULL && audio_gain[ii][0] != 0; ii++) {
        audio_dict = hb_value_array_get(list, ii);
        hb_dict_set(audio_dict, "AudioTrackGainSlider",
                    hb_value_double(strtod(audio_gain[ii], NULL)));
        last = ii;
      }
      if (last_audio_dict == NULL && last >= 0) {
        // No defaults exist in original preset.
    ...
    if (hb_str_vlen(audio_dither) > 0) {
      int last = -1;
      for (ii = 0; audio_dither[ii] != NULL && audio_dither[ii][0] != 0; ii++) {
        audio_dict = hb_value_array_get(list, ii);
    ...
    if (hb_str_vlen(acompressions) > 0) {
      int last = -1;
      for (ii = 0; acompressions[ii] != NULL && acompressions[ii][0] != 0;
           ii++) {
        audio_dict = hb_value_array_get(list, ii);
        hb_dict_set(audio_dict, "AudioCompressionLevel",
                    hb_value_double(strtod(acompressions[ii], NULL)));
        last = ii;
      }
      if (last_audio_dict == NULL && last >= 0) {
        // No defaults exist in original preset.
    ...
    if (hb_str_vlen(anames) > 0) {
      for (ii = 0; anames[ii] != NULL && anames[ii][0] != 0; ii++) {
        audio_dict = hb_value_array_get(list, ii);
        hb_dict_set(audio_dict, "AudioTrackName", hb_value_string(anames[ii]));
      }
    }
  }
  ...
  if (atracks != NULL) {
    ...
    hb_dict_set(preset, "AudioTrackSelectionBehavior", hb_value_string("none"));
  }
  ...
  if (cfr != -1) {
    hb_dict_set(preset, "VideoFramerateMode",
                hb_value_string(cfr == 0 ? "vfr" : cfr == 1 ? "cfr" : "pfr"));
  }
  ...
  if (qsv_async_depth >= 0) {
    hb_dict_set(preset, "VideoQSVAsyncDepth", hb_value_int(qsv_async_depth));
  }
  if (qsv_decode != -1) {
  ...
  if (keep_display_aspect != -1) {
    hb_dict_set(preset, "PictureKeepRatio", hb_value_bool(keep_display_aspect));
  }
  if (itu_par != -1) {
  ...
  if (comb_detect != NULL) {
    if (!comb_detect_custom) {
      ...
      hb_dict_set(preset, "PictureCombDetectPreset", hb_value_string("custom"));
  ...
  if (decomb != NULL) {
    hb_dict_set(preset, "PictureDeinterlaceFilter", hb_value_string("decomb"));
    if (!decomb_custom) {
      hb_dict_set(preset, "PictureDeinterlacePreset", hb_value_string(decomb));
    } else {
      hb_dict_set(preset, "PictureDeinterlacePreset",
                  hb_value_string("custom"));
      hb_dict_set(preset, "PictureDeinterlaceCustom", hb_value_string(decomb));
    }
  }
  if (detelecine_disable) {
  ...
  if (detelecine != NULL) {
    if (!detelecine_custom) {
      hb_dict_set(preset, "PictureDetelecine", hb_value_string(detelecine));
    } else {
  ...
  if (hqdn3d != NULL) {
    ...
    if (!hqdn3d_custom) {
      hb_dict_set(preset, "PictureDenoisePreset", hb_value_string(hqdn3d));
    } else {
      hb_dict_set(preset, "PictureDenoisePreset", hb_value_string("custom"));
      hb_dict_set(preset, "PictureDenoiseCustom", hb_value_string(hqdn3d));
    }
  }
  ...
  if (nlmeans != NULL) {
    ...
    if (!nlmeans_custom) {
      hb_dict_set(preset, "PictureDenoisePreset", hb_value_string(nlmeans));
      ...
      hb_dict_set(preset, "PictureDenoisePreset", hb_value_string("custom"));
      hb_dict_set(preset, "PictureDenoiseCustom", hb_value_string(nlmeans));
    }
  }
  if (chroma_smooth_disable) {
  ...
  if (unsharp != NULL) {
    ...
    if (!unsharp_custom) {
      hb_dict_set(preset, "PictureSharpenPreset", hb_value_string(unsharp));
      if (unsharp_tune != NULL) {
        hb_dict_set(preset, "PictureSharpenTune",
                    hb_value_string(unsharp_tune));
      }
    } else {
      hb_dict_set(preset, "PictureSharpenPreset", hb_value_string("custom"));
      hb_dict_set(preset, "PictureSharpenCustom", hb_value_string(unsharp));
    }
  }
  ...
  if (lapsharp != NULL) {
    ...
    if (!lapsharp_custom) {
      hb_dict_set(preset, "PictureSharpenPreset", hb_value_string(lapsharp));
      ...
      hb_dict_set(preset, "PictureSharpenPreset", hb_value_string("custom"));
      hb_dict_set(preset, "PictureSharpenCustom", hb_value_string(lapsharp));
    }
  }
  if (deblock != NULL) {
    if (!deblock_custom) {
      ...
      if (deblock_tune != NULL) {
        hb_dict_set_string(preset, "PictureDeblockTune", deblock_tune);
      }
    } else {
...
static int add_sub(hb_value_array_t *list, hb_title_t *title, int track,
                   int out_track, int *one_burned) {
  hb_subtitle_t *subtitle;
  ...
  *one_burned |= burn;
  ...
  if (!burn && !hb_subtitle_can_pass(subtitle->source, mux)) {
    // Only allow one subtitle to be burned into video
    if (*one_burned) {
      fprintf(stderr,
              "Warning: Skipping subtitle track %d, can't have more than one "
              "track burnt in\n",
              track + 1);
      return -1;
    }
...
static int add_srt(hb_value_array_t *list, int track, int *one_burned) {
  ...
  char *iso639_2 = "und";
  int burn =
      !*one_burned && srtburn == track + 1 && hb_subtitle_can_burn(IMPORTSRT);
  *one_burned |= burn;
...
static int add_ssa(hb_value_array_t *list, int track, int *one_burned) {
  ...
  char *iso639_2 = "und";
  int burn =
      !*one_burned && ssaburn == track + 1 && hb_subtitle_can_burn(IMPORTSRT);
  *one_burned |= burn;
...
static hb_dict_t *PrepareJob(hb_handle_t *h, hb_title_t *title,
  ...
  if (chapter_start && chapter_end && !start_at_pts && !stop_at_pts &&
      !start_at_frame && !stop_at_frame && !start_at_preview) {
    range_type = "chapter";
  ...
  if (range_start || range_end) {
    hb_dict_t *range_dict =
        hb_dict_get(hb_dict_get(job_dict, "Source"), "Range");
    ...
    if (range_seek_points)
      hb_dict_set(range_dict, "SeekPoints", hb_value_int(range_seek_points));
  }
  ...
  if (atracks != NULL || audio_all == 1) {
    ...
    if (track_count > 0) {
      ...
      if (audio != NULL) {
        hb_preset_job_add_subtitles(h, title->index, preset_dict, job_dict);
      }
    }
    ...
    if (acompressions != NULL) {
      ...
      if (acompressions[ii] != NULL) {
        fprintf(stderr, "Dropping excess audio compression levels\n");
      }
    }
    ...
    if (audio_dither != NULL) {
      for (; audio_dither[ii] != NULL && ii < track_count; ii++) {
        if (*audio_dither[ii]) {
          ...
          audio_dict = hb_value_array_get(audio_array, ii);
          hb_dict_set(audio_dict, "DitherMethod", hb_value_int(dither));
        }
      }
    ...
    if (ii == 1)
      for (; ii < track_count; ii++) {
        ...
        if (hb_audio_dither_is_supported(codec)) {
          hb_dict_set(audio_dict, "DitherMethod", hb_value_double(dither));
        }
      }
    ...
    if (normalize_mix_level) {
      ...
      if (nmls[ii] != NULL) {
        fprintf(stderr, "Dropping excess audio mixdown normalizations\n");
      }
    }
    ...
    if (anames != NULL) {
      for (; anames[ii] != NULL && ii < track_count; ii++) {
        if (*anames[ii]) {
          audio_dict = hb_value_array_get(audio_array, ii);
          hb_dict_set(audio_dict, "Name", hb_value_string(anames[ii]));
        }
      }
      if (anames[ii] != NULL) {
  ...
  if (subtracks != NULL) {
    ...
    for (ii = 0; subtracks[ii] != NULL; ii++) {
      ...
      if (sscanf(subtracks[ii], "%d-%d", &first, &last) == 2) {
        for (track = first - 1; track < last; track++) {
          if (add_sub(subtitle_array, title, track - 1, out_track + 1,
                      &one_burned) == 0) {
            out_track++;
          }
        }
    ...
    if (subnames != NULL) {
      for (; subnames[ii] != NULL && ii < track_count; ii++) {
        if (*subnames[ii]) {
          ...
          hb_dict_set(subtitle_dict, "Name", hb_value_string(subnames[ii]));
        }
      }
      if (subnames[ii] != NULL) {
...
static char *bsd_name_for_path(char *path) {
  ...
  size_t lenpre = strlen(prefix), lenstr = strlen(s.f_mntfromname);
...
static int is_dvd_service(io_service_t service) {
  ...
  kernResult = IORegistryEntryCreateIterator(
...
static int is_whole_media_service(io_service_t service) {
  ...
  if (IOObjectConformsTo(service, kIOMediaClass)) {
    CFTypeRef wholeMedia = IORegistryEntryCreateCFProperty(
        service, CFSTR(kIOMediaWholeKey), kCFAllocatorDefault, 0);
    if (!wholeMedia) {
      return 0;
    }
