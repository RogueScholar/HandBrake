...
#if defined(_ENABLE_GST)
G_MODULE_EXPORT gboolean live_preview_cb(GstBus *bus, GstMessage *msg,
                                         gpointer data);
#endif
...
void preview_set_size(signal_user_data_t *ud, int width, int height) {
  ud->preview->width = width;
  ud->preview->height = height;
}

void preview_set_render_size(signal_user_data_t *ud, int width, int height) {
  GtkWidget *widget;
  GtkWindow *window;
  GhbSurface *ss;
  GdkGeometry geo;
...
void ghb_preview_init(signal_user_data_t *ud) {
  ...
  if (ud->preview->play == NULL || ud->preview->vsink == NULL) {
    ...
    g_object_set(ud->preview->vsink, "qos", FALSE, "max-lateness", (gint64)-1,
                 NULL);
    g_object_set(ud->preview->play, "video-sink", ud->preview->vsink, NULL);
    g_object_set(ud->preview->play, "subtitle-font-desc", "sans bold 20", NULL);
...
#if defined(_ENABLE_GST)
void live_preview_start(signal_user_data_t *ud) {
  GtkImage *img;
  gchar *uri;
...
void live_preview_pause(signal_user_data_t *ud) {
  GtkImage *img;
...
#if defined(_ENABLE_GST)
static void caps_set(GstCaps *caps, signal_user_data_t *ud) {
  GstStructure *ss;
  ...
  if (ss) {
    ...
    ghb_screen_par(ud, &disp_par_n, &disp_par_d);
    gst_video_calculate_display_ratio(&num, &den, width, height, par_n, par_d,
    ...
    if (ghb_dict_get_bool(ud->prefs, "reduce_hd_preview")) {
      ...
      ghb_monitor_get_size(GHB_WIDGET(ud->builder, "preview_window"), &s_w,
                           &s_h);
      if (s_w > 0 && s_h > 0) {
        if (width > s_w * 80 / 100) {
          width = s_w * 80 / 100;
...
static void update_stream_info(signal_user_data_t *ud) {
  GstPad *vpad = NULL;
  gint n_video;
...
G_MODULE_EXPORT gboolean live_preview_cb(GstBus *bus, GstMessage *msg,
  ...
  case GST_MESSAGE_EOS: {
    ...
    // printf("eos\n");
    live_preview_stop(ud);
    gst_element_seek(ud->preview->play, 1.0, GST_FORMAT_TIME,
  ...
  case GST_MESSAGE_ELEMENT: {
    ...
    if (gst_is_missing_plugin_message(msg)) {
      ...
      message =
          g_strdup_printf(_("Missing GStreamer plugin\n"
                            "Audio or Video may not play as expected\n\n%s"),
                          desc);
      ghb_message_dialog(hb_window, GTK_MESSAGE_WARNING, message, "Ok", NULL);
      g_free(message);
      gst_element_set_state(ud->preview->play, GST_STATE_PLAYING);
      ...
      if (gstStruct != NULL &&
        ...
        if (val != NULL) {
          ...
          if (width != ud->preview->width || height != ud->preview->height ||
            ...
            xscale = (double)ud->preview->render_width / ud->preview->width;
            yscale = (double)ud->preview->render_height / ud->preview->height;
            if (xscale <= yscale) {
              width = ud->preview->render_width;
            ...
            ud->preview->scaled_pix = gdk_pixbuf_scale_simple(
...
void ghb_live_encode_done(signal_user_data_t *ud, gboolean success) {
  ...
  if (success && ud->preview->encode_frame == ud->preview->frame) {
...
#if defined(_ENABLE_GST)
G_MODULE_EXPORT gboolean unlock_progress_cb(signal_user_data_t *ud) {
...
#if defined(_ENABLE_GST)
G_MODULE_EXPORT gboolean unlock_seek_cb(signal_user_data_t *ud) {
...
static void set_mini_preview_image(signal_user_data_t *ud, GdkPixbuf *pix) {
  ...
  if ((height >= 16) && (width >= 16)) {
    scaled_preview =
        gdk_pixbuf_scale_simple(pix, width, height, GDK_INTERP_NEAREST);
...
void init_preview_image(signal_user_data_t *ud) {
  GtkWidget *widget;
  gint width, height;
...
#if GTK_CHECK_VERSION(3, 90, 0)
G_MODULE_EXPORT void preview_draw_cb(GtkDrawingArea *da, cairo_t *cr, int width,
                                     int height, signal_user_data_t *ud) {
  if (ud->preview->scaled_pix != NULL) {
...
G_MODULE_EXPORT gboolean preview_draw_cb(GtkWidget *widget, cairo_t *cr,
                                         signal_user_data_t *ud) {
  if (ud->preview->scaled_pix != NULL) {
    _draw_pixbuf(ud, cr, ud->preview->scaled_pix);
  }
...
static void update_preview_labels(signal_user_data_t *ud, gboolean active) {
...
G_MODULE_EXPORT void preview_reset_clicked_cb(GtkWidget *toggle,
                                              signal_user_data_t *ud) {
...
static gboolean hud_timeout(signal_user_data_t *ud) {
  GtkWidget *widget;
...
#if GTK_CHECK_VERSION(3, 90, 0)
G_MODULE_EXPORT void hud_enter_cb(GtkEventControllerMotion *econ, double x,
                                  double y, GdkCrossingMode cross,
...
#if GTK_CHECK_VERSION(3, 90, 0)
G_MODULE_EXPORT void hud_leave_cb(GtkEventControllerMotion *econ,
                                  GdkCrossingMode cross, GdkNotifyType notify,
                                  signal_user_data_t *ud)
#else
G_MODULE_EXPORT gboolean hud_leave_cb(GtkWidget *widget,
                                      GdkEventCrossing *event,
                                      signal_user_data_t *ud)
#endif
{
...
preview_leave_cb(GtkEventControllerMotion *econ, GdkCrossingMode cross,
                 GdkNotifyType notify, signal_user_data_t *ud)
#else
...
#if GTK_CHECK_VERSION(3, 90, 0)
G_MODULE_EXPORT void preview_motion_cb(GtkEventControllerMotion *econ,
                                       gdouble x, gdouble y,
                                       signal_user_data_t *ud)
#else
...
G_MODULE_EXPORT gboolean preview_state_cb(GtkWidget *widget, GdkEvent *event,
                                          signal_user_data_t *ud) {
...
G_MODULE_EXPORT void preview_resize_cb(GtkWidget *widget,
#if GTK_CHECK_VERSION(3, 90, 0)
...
G_MODULE_EXPORT void show_crop_changed_cb(GtkWidget *widget,
                                          signal_user_data_t *ud) {
