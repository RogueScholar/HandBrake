...
static void chapter_changed_cb(GtkEditable *edit, signal_user_data_t *ud);
...
#else
static gboolean chapter_keypress_cb(GtkWidget *widget, GdkEvent *event,
                                    signal_user_data_t *ud);
#endif
...
static GtkWidget *create_chapter_row(int index, int64_t start, int64_t duration,
...
#else
  g_signal_connect(entry, "key-press-event", G_CALLBACK(chapter_keypress_cb),
                   ud);
#endif
...
static void ghb_clear_chapter_list_ui(signal_user_data_t *ud) {
  GtkListBox *lb;
  GtkListBoxRow *row;
...
static void chapter_refresh_list_ui(signal_user_data_t *ud) {
...
static gboolean chapter_keypress(GtkWidget *widget, guint keyval,
                                 signal_user_data_t *ud) {
  GtkWidget *entry;
  GtkListBoxRow *row;
  GtkListBox *lb;
