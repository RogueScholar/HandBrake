...
GtkBuilder *create_builder_or_die(const gchar *name) {
  ...
  GBytes *gbytes =
      g_resource_lookup_data(ui_res, "/fr/handbrake/ghb/ui/ghb.ui", 0, NULL);
  ghb_ui = g_bytes_get_data(gbytes, &data_size);

  const gchar *markup = N_("<b><big>Unable to create %s.</big></b>\n"
                           "\n"
  ...
  if (!xml || !res) {
    GtkWidget *dialog = gtk_message_dialog_new_with_markup(
        ...
        gettext(markup), name, error->message);
...
static GCallback self_symbol_lookup(const gchar *symbol_name) {
...
static void MyConnect(GtkBuilder *builder, GObject *object,
  ...
  if (connect_object) {
    g_signal_connect_object(object, signal_name, callback, connect_object,
                            flags);
  } else {
...
static void bind_audio_tree_model(signal_user_data_t *ud) {
  ...
  treestore = gtk_tree_store_new(6, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
...
// Also, connect up the signal that lets us know the selection has changed
static void bind_subtitle_tree_model(signal_user_data_t *ud) {
  GtkCellRenderer *source_cell;
  GtkCellRenderer *arrow_cell;
  GtkCellRenderer *output_cell;
  GtkCellRenderer *edit_cell;
  ...
  g_signal_connect(selection, "changed", subtitle_list_selection_changed_cb,
                   ud);
...
static const char *presets_drag_entries[] = {"widget/presets-list-row-drop"};
#else
...
// Also, connect up the signal that lets us know the selection has changed
static void bind_presets_tree_model(signal_user_data_t *ud) {
  ...
  gtk_tree_view_enable_model_drag_source(treeview, GDK_BUTTON1_MASK, targets,
                                         GDK_ACTION_MOVE);
  ...
  gtk_tree_view_enable_model_drag_source(
      treeview, GDK_BUTTON1_MASK, presets_drag_entries, 1, GDK_ACTION_MOVE);
#endif

  g_signal_connect(treeview, "drag_data_received",
                   presets_drag_data_received_cb, ud);
...
static void IoRedirect(signal_user_data_t *ud) {
  GIOChannel *channel;
  gint pfd[2];
...
static GOptionEntry entries[] = {
    {"device", 'd', 0, G_OPTION_ARG_FILENAME, &dvd_device,
     N_("The device or file to encode"), NULL},
    {"preset", 'p', 0, G_OPTION_ARG_STRING, &arg_preset,
     N_("The preset values to use for encoding"), NULL},
    {"debug", 'x', 0, G_OPTION_ARG_NONE, &ghb_debug, N_("Spam a lot"), NULL},
    {"config", 'o', 0, G_OPTION_ARG_STRING, &arg_config_dir,
     N_("The path to override user config dir"), NULL},
#if defined(_WIN32)
    {"console", 'c', 0, G_OPTION_ARG_NONE, &win32_console,
     N_("Open a console for debug output"), NULL},
#endif
    {NULL}};
...
#if defined(_WIN32)
G_MODULE_EXPORT GdkFilterReturn win_message_cb(GdkXEvent *wmevent,
...
G_MODULE_EXPORT void video_option_changed_cb(GtkWidget *widget,
                                             signal_user_data_t *ud);
...
const gchar *MyCSS = "@define-color black  #000000;"
...
extern G_MODULE_EXPORT void ghb_shutdown_cb(GApplication *app,
                                            signal_user_data_t *ud) {}
...
G_MODULE_EXPORT void show_queue_action_cb(GSimpleAction *action,
                                          GVariant *value, gpointer ud);
G_MODULE_EXPORT void show_preview_action_cb(GSimpleAction *action,
                                            GVariant *value, gpointer ud);
G_MODULE_EXPORT void show_activity_action_cb(GSimpleAction *action,
                                             GVariant *value, gpointer ud);
G_MODULE_EXPORT void preset_save_action_cb(GSimpleAction *action,
                                           GVariant *param, gpointer ud);
G_MODULE_EXPORT void preset_save_as_action_cb(GSimpleAction *action,
                                              GVariant *param, gpointer ud);
G_MODULE_EXPORT void preset_rename_action_cb(GSimpleAction *action,
                                             GVariant *param, gpointer ud);
G_MODULE_EXPORT void preset_remove_action_cb(GSimpleAction *action,
                                             GVariant *param, gpointer ud);
G_MODULE_EXPORT void preset_default_action_cb(GSimpleAction *action,
                                              GVariant *param, gpointer ud);
G_MODULE_EXPORT void preset_export_action_cb(GSimpleAction *action,
                                             GVariant *param, gpointer ud);
G_MODULE_EXPORT void preset_import_action_cb(GSimpleAction *action,
                                             GVariant *param, gpointer ud);
G_MODULE_EXPORT void presets_reload_action_cb(GSimpleAction *action,
                                              GVariant *param, gpointer ud);
...
G_MODULE_EXPORT void guide_action_cb(GSimpleAction *action, GVariant *param,
                                     gpointer ud);
G_MODULE_EXPORT void preset_select_action_cb(GSimpleAction *action,
                                             GVariant *param, gpointer ud);
G_MODULE_EXPORT void preset_reload_action_cb(GSimpleAction *action,
                                             GVariant *param, gpointer ud);
...
static void map_actions(GApplication *app, signal_user_data_t *ud) {
  const GActionEntry entries[] = {
      ...
      {"show-presets", NULL, NULL, "false", show_presets_action_cb},
      {"show-queue", NULL, NULL, "false", show_queue_action_cb},
      {"show-preview", NULL, NULL, "false", show_preview_action_cb},
      {"show-activity", NULL, NULL, "false", show_activity_action_cb},
...
gboolean ghb_idle_ui_init(signal_user_data_t *ud) {
...
extern G_MODULE_EXPORT void preview_draw_cb(GtkDrawingArea *, cairo_t *, int,
                                            int, gpointer);
extern G_MODULE_EXPORT void hud_enter_cb(void);
extern G_MODULE_EXPORT void hud_leave_cb(void);
#endif

extern G_MODULE_EXPORT void ghb_activate_cb(GApplication *app,
                                            signal_user_data_t *ud) {
  ...
  gtk_widget_set_name(GHB_WIDGET(ud->builder, "preview_frame"),
                      "preview_frame");
  gtk_widget_set_name(GHB_WIDGET(ud->builder, "live_preview_play"),
                      "live_preview_play");
  gtk_widget_set_name(GHB_WIDGET(ud->builder, "live_preview_progress"),
                      "live_preview_progress");
  gtk_widget_set_name(GHB_WIDGET(ud->builder, "live_encode_progress"),
                      "live_encode_progress");
  gtk_widget_set_name(GHB_WIDGET(ud->builder, "live_duration"),
                      "live_duration");
  gtk_widget_set_name(GHB_WIDGET(ud->builder, "preview_show_crop"),
                      "preview_show_crop");
  gtk_widget_set_name(GHB_WIDGET(ud->builder, "preview_reset"),
                      "preview_reset");
  gtk_widget_set_name(GHB_WIDGET(ud->builder, "activity_view"),
                      "activity_view");

  // Redirect stderr to the activity window
  ghb_preview_init(ud);
  IoRedirect(ud);
  ghb_log("%s - %s - %s", HB_PROJECT_TITLE, HB_PROJECT_HOST_TITLE,
  ...
  */
  ...
  ghb_set_custom_filter_tooltip(ud, "PictureDetelecineCustom", "detelecine",
  ...
  gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(widget), preview_draw_cb, ud,
                                 NULL);
...
extern G_MODULE_EXPORT void ghb_open_file_cb(GApplication *app, gpointer pfiles,
                                             gint nfiles, gchar *hint,
                                             signal_user_data_t *ud) {
  GFile **files = pfiles;
...
int main(int argc, char *argv[]) {
  ...
  ud->app =
      gtk_application_new("fr.handbrake.ghb", G_APPLICATION_NON_UNIQUE |
