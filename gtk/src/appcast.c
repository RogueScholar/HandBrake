...
static const gchar *lookup_attr_value(const gchar *name,
...
static void start_element(GMarkupParseContext *ctx, const gchar *tag,
                          const gchar **attr_names, const gchar **attr_values,
                          gpointer ud, GError **error) {
  ...
  case A_ENCLOSURE: {
    ...
    build = lookup_attr_value("sparkle:version", attr_names, attr_values);
    version = lookup_attr_value("sparkle:shortVersionString", attr_names,
                                attr_values);
    if (build)
...
static void end_element(GMarkupParseContext *ctx, const gchar *tag, gpointer ud,
                        GError **error) {
  parse_data_t *pd = (parse_data_t *)ud;
  gint id;
  union {
...
static void text_data(GMarkupParseContext *ctx, const gchar *text, gsize len,
                      gpointer ud, GError **error) {
  parse_data_t *pd = (parse_data_t *)ud;
  union {
    gint id;
...
static void passthrough(GMarkupParseContext *ctx, const gchar *text, gsize len,
                        gpointer ud, GError **error) {
  // parse_data_t *pd = (parse_data_t*)ud;
...
static void parse_error(GMarkupParseContext *ctx, GError *error, gpointer ud) {
  g_warning("Resource parse error: %s", error->message);
}

// This is required or the parser crashes
static void destroy_notify(gpointer data) { // Do nothing
...
void ghb_appcast_parse(gchar *buf, gchar **desc, gchar **build,
  ...
  ctx = g_markup_parse_context_new(&parser, G_MARKUP_TREAT_CDATA_AS_TEXT, &pd,
                                   destroy_notify);
