...
void ghb_load_icons() {
  ...
  char **children = g_resource_enumerate_children(
      icon_res, "/fr/handbrake/ghb/icons/scalable/apps", 0, NULL);
  ...
  for (ii = 0; children[ii] != NULL; ii++) {
    ...
    for (jj = 0; sizes[jj]; jj++) {
      ...
      pb = gdk_pixbuf_new_from_stream_at_scale(gis, sizes[jj], sizes[jj], TRUE,
                                               NULL, NULL);
