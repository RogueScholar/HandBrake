...
#if !defined(_WIN32)
static GDBusProxy *ghb_get_session_dbus_proxy(const gchar *name,
                                              const gchar *path,
  ...
  proxy = g_dbus_proxy_new_sync(conn, G_DBUS_PROXY_FLAGS_NONE, NULL, name, path,
                                interface, NULL, NULL);
  if (proxy == NULL)
...
static gboolean ghb_can_suspend_gpm() {
  ...
  proxy = ghb_get_session_dbus_proxy(GPM_DBUS_PM_SERVICE, GPM_DBUS_PM_PATH,
                                     GPM_DBUS_PM_INTERFACE);
  if (proxy == NULL)
    return FALSE;
...
static void ghb_suspend_gpm() {
  ...
  res = g_dbus_proxy_call_sync(proxy, "Suspend", NULL, G_DBUS_CALL_FLAGS_NONE,
                               -1, NULL, &error);
  if (!res) {
    if (error != NULL) {
...
static void ghb_shutdown_gpm() {
  ...
  res = g_dbus_proxy_call_sync(proxy, "Shutdown", NULL, G_DBUS_CALL_FLAGS_NONE,
                               -1, NULL, &error);
  if (!res) {
    if (error != NULL) {
...
guint ghb_inhibit_gpm() {
  ...
  proxy = ghb_get_session_dbus_proxy(GPM_DBUS_PM_SERVICE, GPM_DBUS_INHIBIT_PATH,
...
void ghb_uninhibit_gpm(guint cookie) {
#if !defined(_WIN32)
  GDBusProxy *proxy;
  GError *error = NULL;
  GVariant *res;
  ...
  res = g_dbus_proxy_call_sync(proxy, "UnInhibit", g_variant_new("(u)", cookie),
                               G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
  if (!res) {
    if (error != NULL) {
      g_error_free(error);
...
static gboolean ghb_can_shutdown_gsm() {
  ...
  proxy = ghb_get_session_dbus_proxy(GPM_DBUS_SM_SERVICE, GPM_DBUS_SM_PATH,
                                     GPM_DBUS_SM_INTERFACE);
  if (proxy == NULL)
    return FALSE;
...
#if !GTK_CHECK_VERSION(3, 4, 0)
guint ghb_inhibit_gsm(signal_user_data_t *ud) {
  guint cookie = -1;
  ...
  res = g_dbus_proxy_call_sync(
      proxy, "Inhibit", g_variant_new("(susu)", "ghb", xid, "Encoding", 1 | 4),
      G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
  g_object_unref(G_OBJECT(proxy));
  if (!res) {
    if (error != NULL) {
...
void ghb_uninhibit_gsm(guint cookie) {
#if !defined(_WIN32)
  GDBusProxy *proxy;
  GError *error = NULL;
  GVariant *res;
...
void ghb_inhibit_suspend(signal_user_data_t *ud) {
  if (suspend_inhibited != GHB_SUSPEND_UNINHIBITED) {
    // Already inhibited
    return;
  }
#if GTK_CHECK_VERSION(3, 4, 0)
  suspend_cookie = gtk_application_inhibit(
      ud->app, NULL, GTK_APPLICATION_INHIBIT_SUSPEND, "Encoding");
  if (suspend_cookie != 0) {
...
void ghb_uninhibit_suspend(signal_user_data_t *ud) {
...
static gboolean dep_check(signal_user_data_t *ud, const gchar *name,
                          gboolean *out_hide) {
  GtkWidget *widget;
  GObject *dep_object;
  gint ii;
...
G_MODULE_EXPORT void quit_action_cb(GSimpleAction *action, GVariant *param,
...
#if defined(_WIN32)
static gchar *get_dvd_device_name(gchar *device) { return g_strdup(device); }
#else
static gchar *get_dvd_device_name(GDrive *gd) {
...
static void free_volname_key(gpointer data) {
  if (data != NULL)
    g_free(data);
}

static void free_volname_value(gpointer data) {
  if (data != NULL)
    g_free(data);
}

#if defined(_WIN32)
static gchar *get_direct_dvd_volume_name(const gchar *drive) {
  gchar *result = NULL;
...
static gchar *get_dvd_volume_name(gpointer gd) {
  gchar *label = NULL;
  gchar *result;
  gchar *drive;
...
static void free_drive(gpointer drive) {
#if defined(_WIN32)
  g_free(drive);
#else
...
static const gchar *get_extension(signal_user_data_t *ud, GhbValue *settings) {
  const char *mux_id;
...
static gboolean check_name_template(signal_user_data_t *ud, const char *str) {
...
static int parse_datestring(const char *src, struct tm *tm) {
  datemap ymdThmsZ = {
      "[0-9]{4}-[0-1]?[0-9]-[0-3]?[0-9]T[0-9]{2}:[0-9]{2}:[0-9]{2}Z",
      "%Y-%m-%dT%H:%M:%SZ"};
...
static char *get_creation_date(const char *pattern, const char *metaValue,
                               const char *file) {
  char date[11] = "";
...
static void make_unique_dest(const gchar *dest_dir, GString *str,
  ...
  while (g_file_test(uniq->str, G_FILE_TEST_EXISTS)) {
    g_string_printf(uniq, "%s/%s (%d).%s", dest_dir, str->str, ++copy,
                    extension);
  }
  if (copy) {
...
static void set_destination_settings(signal_user_data_t *ud,
                                     GhbValue *settings) {
  ...
  ghb_dict_set(settings, "dest_dir",
               ghb_value_dup(ghb_dict_get_value(ud->prefs, "destination_dir")));
  if (ghb_dict_get_bool(ud->prefs, "auto_name")) {
    GString *str = g_string_new("");
    const gchar *p;
    ...
    while (*p) {
      if (!strncasecmp(p, "{source}", strlen("{source}"))) {
        ...
        p += strlen("{date}");
      } else if (!strncasecmp(p, "{creation-date}",
                              strlen("{creation-date}"))) {
        gchar *val;
        const gchar *source = ghb_dict_get_string(ud->globals, "scan_source");
        val = get_creation_date(
            "%Y-%m-%d", ghb_dict_get_string(settings, "MetaReleaseDate"),
            source);
        g_string_append_printf(str, "%s", val);
        p += strlen("{creation-date}");
        g_free(val);
      } else if (!strncasecmp(p, "{creation-time}",
                              strlen("{creation-time}"))) {
        gchar *val;
        const gchar *source = ghb_dict_get_string(ud->globals, "scan_source");
        val = get_creation_date(
            "%H:%M", ghb_dict_get_string(settings, "MetaReleaseDate"), source);
        g_string_append_printf(str, "%s", val);
...
static void set_destination(signal_user_data_t *ud) {
  set_destination_settings(ud, ud->settings);
  ghb_ui_update(ud, "dest_file", ghb_dict_get_value(ud->settings, "dest_file"));
}

G_MODULE_EXPORT void chooser_file_selected_cb(GtkFileChooser *dialog,
                                              signal_user_data_t *ud) {
...
G_MODULE_EXPORT void dvd_device_changed_cb(GtkComboBoxText *combo,
                                           signal_user_data_t *ud) {
  GtkWidget *dialog;
  gint ii;
...
static void source_dialog_extra_widgets(signal_user_data_t *ud,
                                        GtkWidget *dialog) {
...
void ghb_break_pts_duration(gint64 ptsDuration, gint *hh, gint *mm,
                            gdouble *ss) {
...
gint64 ghb_title_range_get_duration(GhbValue *settings,
...
static void update_title_duration(signal_user_data_t *ud) {
  gint hh, mm, ss;
  gint64 duration;
  gchar *text;
  GtkWidget *widget;
...
static void adjustment_configure(GtkAdjustment *adj, double val, double min,
                                 double max, double step, double page,
                                 double page_sz) {
  gtk_adjustment_configure(adj, val, min, max, step, page, page_sz);
}

static void spin_configure(signal_user_data_t *ud, char *name, double val,
...
void ghb_set_widget_ranges(signal_user_data_t *ud, GhbValue *settings) {
  int title_id, titleindex;
  const hb_title_t *title;
  double val;
  ...
  if (title != NULL) {
    ...
    if (ghb_settings_combo_int(ud->settings, "PtoPType") == 0) {
      ...
      gdouble max_frames;
      max_frames = (gdouble)duration * title->vrate.num / title->vrate.den;
  ...
  ghb_scale_configure(ud, "VideoQualitySlider", val, vqmin, vqmax, step, page,
                      digits, inverted);
}

static void check_chapter_markers(signal_user_data_t *ud) {
  GtkWidget *widget;
  gint start, end;
...
static void show_scan_progress(signal_user_data_t *ud) {
  GtkWidget *widget;
...
static void hide_scan_progress(signal_user_data_t *ud) {
  GtkWidget *widget;
  GtkProgressBar *progress;
...
static void start_scan(signal_user_data_t *ud, const gchar *path, gint title_id,
                       gint preview_count) {
  GtkWidget *widget;
  ghb_status_t status;
...
void ghb_do_scan(signal_user_data_t *ud, const gchar *filename, gint title_id,
                 gboolean force) {
  int titleindex;
  const hb_title_t *title;
...
static void do_source_dialog(gboolean single, signal_user_data_t *ud) {
  GtkWidget *dialog;
...
#else
G_MODULE_EXPORT void source_action_cb(GSimpleAction *action, GVariant *param,
                                      signal_user_data_t *ud) {
  ghb_status_t status;
  ghb_get_status(&status);
  if (status.scan.state & GHB_STATE_SCANNING) {
...
G_MODULE_EXPORT void single_title_action_cb(GSimpleAction *action,
                                            GVariant *param,
                                            signal_user_data_t *ud) {
  do_source_dialog(TRUE, ud);
}

G_MODULE_EXPORT void dvd_source_activate_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
...
void ghb_update_destination_extension(signal_user_data_t *ud) {
  static gchar *containers[] = {".mkv",  ".mp4",   ".m4v",
                                ".webm", ".error", NULL};
  gchar *filename;
  const gchar *extension;
  gint ii;
...
static void destination_select_title(GtkEntry *entry) {
  const gchar *dest;
  gint start, end;
...
G_MODULE_EXPORT gboolean destination_grab_cb(GtkEntry *entry,
                                             signal_user_data_t *ud) {
  destination_select_title(entry);
  return FALSE;
}

static void update_default_destination(signal_user_data_t *ud) {
...
G_MODULE_EXPORT void dest_dir_set_cb(GtkFileChooserButton *dest_chooser,
                                     signal_user_data_t *ud) {
...
G_MODULE_EXPORT void dest_file_changed_cb(GtkEntry *entry,
                                          signal_user_data_t *ud) {
  const gchar *dest_file, *dest_dir;
  gchar *dest;
...
G_MODULE_EXPORT void destination_action_cb(GSimpleAction *action,
                                           GVariant *param,
                                           signal_user_data_t *ud) {
  GtkWidget *dialog;
  GtkEntry *entry;
  ...
  dialog = gtk_file_chooser_dialog_new(
      "Choose Destination", hb_window, GTK_FILE_CHOOSER_ACTION_SAVE,
      GHB_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GHB_STOCK_SAVE,
...
G_MODULE_EXPORT gboolean window_destroy_event_cb(GtkWidget *widget,
...
G_MODULE_EXPORT gboolean window_delete_event_cb(GtkWidget *widget,
#if !GTK_CHECK_VERSION(3, 90, 0)
                                                GdkEvent *event,
#endif
                                                signal_user_data_t *ud) {
...
static void update_acodec(signal_user_data_t *ud) {
  ghb_audio_list_refresh_all(ud);
  ghb_grey_combo_options(ud);
}

G_MODULE_EXPORT void container_changed_cb(GtkWidget *widget,
                                          signal_user_data_t *ud) {
...
static gchar *get_aspect_string(gint aspect_n, gint aspect_d) {
  gchar *aspect;
...
static void update_aspect_info(signal_user_data_t *ud) {}

static void update_crop_info(signal_user_data_t *ud) {
  GtkWidget *widget;
  gchar *text;
  gint width, height,
      crop[4] = {
...
static void update_scale_info(signal_user_data_t *ud) {}
...
void ghb_update_title_info(signal_user_data_t *ud) {
  ...
  rate = g_strdup_printf("%.6g", (gdouble)title->vrate.num / title->vrate.den);
  ...
  text = g_strdup_printf(
      ...
      geo->width, geo->height, geo->width * geo->par.num / geo->par.den,
      geo->height, aspect, rate, audio_count, audio_count == 1 ? "" : "s",
...
void ghb_update_summary_info(signal_user_data_t *ud) {
  ...
  if (show < count) {
    g_string_append_printf(str, "\n+ %d more subtitle track%s", count - show,
                           count - show > 1 ? "s" : "");
  }
  ...
  display_aspect = ghb_get_display_aspect_string(display_width, display_height);
...
void ghb_set_title_settings(signal_user_data_t *ud, GhbValue *settings) {
  ...
  if (title != NULL) {
    ...
    if (!keep_aspect || pic_par == HB_ANAMORPHIC_NONE ||
        pic_par == HB_ANAMORPHIC_AUTO || pic_par == HB_ANAMORPHIC_CUSTOM) {
    ...
    if (title->metadata) {
      if (title->metadata->name) {
        ghb_dict_set_string(settings, "MetaName", title->metadata->name);
        update_meta(settings, "Name", title->metadata->name);
      }
      ghb_dict_set_string(settings, "MetaArtist", title->metadata->artist);
      ...
      ghb_dict_set_string(settings, "MetaComment", title->metadata->comment);
      update_meta(settings, "Comment", title->metadata->comment);
      if (!title->metadata->name && title->metadata->album) {
        ghb_dict_set_string(settings, "MetaName", title->metadata->album);
      ...
      ghb_dict_set_string(settings, "MetaGenre", title->metadata->genre);
...
G_MODULE_EXPORT void title_changed_cb(GtkWidget *widget,
                                      signal_user_data_t *ud) {
...
G_MODULE_EXPORT void ptop_widget_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
  gint title_id, titleindex;
  const hb_title_t *title;
  ...
  if (ghb_settings_combo_int(ud->settings, "PtoPType") == 0) {
    ...
    gdouble max_frames =
        (gdouble)duration * title->vrate.num / title->vrate.den;
    spin_configure(ud, "start_point", 1, 1, max_frames * 2);
    spin_configure(ud, "end_point", max_frames, 1, max_frames * 2);
  }
}

G_MODULE_EXPORT void setting_widget_changed_cb(GtkWidget *widget,
                                               signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_update_summary_info(ud);
...
G_MODULE_EXPORT void nonsetting_widget_changed_cb(GtkWidget *widget,
                                                  signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_update_summary_info(ud);
...
G_MODULE_EXPORT void comb_detect_widget_changed_cb(GtkWidget *widget,
                                                   signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_clear_presets_selection(ud);
  ...
  if (strcasecmp(comb_detect, "off")) {
    ...
    if (!strcasecmp(deint, "off")) {
      ghb_ui_update(ud, "PictureDeinterlaceFilter", ghb_string_value("decomb"));
    }
  }
  ghb_update_summary_info(ud);
}

G_MODULE_EXPORT void deint_filter_changed_cb(GtkWidget *widget,
                                             signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_clear_presets_selection(ud);
  ...
  deint = ghb_dict_get_string(ud->settings, "PictureDeinterlaceFilter");
  if (!strcasecmp(deint, "off")) {
    ghb_ui_update(ud, "PictureCombDetectPreset", ghb_string_value("off"));
  }
  ghb_update_summary_info(ud);
}

G_MODULE_EXPORT void denoise_filter_changed_cb(GtkWidget *widget,
                                               signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_clear_presets_selection(ud);
...
G_MODULE_EXPORT void sharpen_filter_changed_cb(GtkWidget *widget,
                                               signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_clear_presets_selection(ud);
...
G_MODULE_EXPORT void title_angle_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_live_reset(ud);
...
G_MODULE_EXPORT void meta_name_changed_cb(GtkWidget *widget,
                                          signal_user_data_t *ud) {
  const char *val;
...
G_MODULE_EXPORT void meta_artist_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
  const char *val;
...
G_MODULE_EXPORT void meta_album_artist_changed_cb(GtkWidget *widget,
                                                  signal_user_data_t *ud) {
  const char *val;
...
G_MODULE_EXPORT void meta_release_date_changed_cb(GtkWidget *widget,
                                                  signal_user_data_t *ud) {
  const char *val;
...
G_MODULE_EXPORT void meta_comment_changed_cb(GtkWidget *widget,
                                             signal_user_data_t *ud) {
  const char *val;
...
G_MODULE_EXPORT void meta_genre_changed_cb(GtkWidget *widget,
                                           signal_user_data_t *ud) {
  const char *val;
...
G_MODULE_EXPORT void meta_description_changed_cb(GtkWidget *widget,
                                                 signal_user_data_t *ud) {
  const char *val;
...
G_MODULE_EXPORT void plot_changed_cb(GtkWidget *widget,
                                     signal_user_data_t *ud) {
  GtkWidget *textview;
  const char *val;
...
G_MODULE_EXPORT void chapter_markers_changed_cb(GtkWidget *widget,
                                                signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_clear_presets_selection(ud);
...
G_MODULE_EXPORT void vquality_type_changed_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_clear_presets_selection(ud);
...
G_MODULE_EXPORT void vbitrate_changed_cb(GtkWidget *widget,
                                         signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_clear_presets_selection(ud);
...
G_MODULE_EXPORT void vquality_changed_cb(GtkWidget *widget,
                                         signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
  ghb_clear_presets_selection(ud);
...
G_MODULE_EXPORT gboolean ptop_input_cb(GtkWidget *widget, gdouble *val,
                                       signal_user_data_t *ud) {
  if (ghb_settings_combo_int(ud->settings, "PtoPType") != 1)
    return FALSE;
...
G_MODULE_EXPORT gboolean ptop_output_cb(GtkWidget *widget,
                                        signal_user_data_t *ud) {
  if (ghb_settings_combo_int(ud->settings, "PtoPType") != 1)
    return FALSE;
...
G_MODULE_EXPORT void start_point_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
  int64_t start, end;
...
G_MODULE_EXPORT void end_point_changed_cb(GtkWidget *widget,
                                          signal_user_data_t *ud) {
  int64_t start, end;
...
G_MODULE_EXPORT void scale_width_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
...
G_MODULE_EXPORT void scale_height_changed_cb(GtkWidget *widget,
                                             signal_user_data_t *ud) {
...
G_MODULE_EXPORT void crop_changed_cb(GtkWidget *widget,
                                     signal_user_data_t *ud) {
...
G_MODULE_EXPORT void display_width_changed_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
...
G_MODULE_EXPORT void display_height_changed_cb(GtkWidget *widget,
                                               signal_user_data_t *ud) {
...
G_MODULE_EXPORT void par_changed_cb(GtkWidget *widget, signal_user_data_t *ud) {
...
G_MODULE_EXPORT void scale_changed_cb(GtkWidget *widget,
                                      signal_user_data_t *ud) {
...
G_MODULE_EXPORT void generic_entry_changed_cb(GtkEntry *entry,
                                              signal_user_data_t *ud) {
...
G_MODULE_EXPORT void preferences_action_cb(GSimpleAction *action,
                                           GVariant *param,
                                           signal_user_data_t *ud) {
  GtkWidget *dialog;
...
static gboolean quit_cb(countdown_t *cd) {
  gchar *str;
  ...
  str = g_strdup_printf(_("%s\n\n%s in %d seconds ..."), cd->msg, cd->action,
                        cd->timeout);
...
static gboolean shutdown_cb(countdown_t *cd) {
  gchar *str;
...
static gboolean suspend_cb(countdown_t *cd) {
  gchar *str;
...
void ghb_countdown_dialog(GtkMessageType type, const gchar *message,
                          const gchar *action, const gchar *cancel,
                          GSourceFunc action_func, signal_user_data_t *ud,
                          gint timeout) {
  GtkWindow *hb_window;
  GtkWidget *dialog;
  ...
  dialog = gtk_message_dialog_new(
      hb_window, GTK_DIALOG_MODAL, type, GTK_BUTTONS_NONE,
      _("%s\n\n%s in %d seconds ..."), message, action, timeout);
  gtk_dialog_add_buttons(GTK_DIALOG(dialog), cancel, GTK_RESPONSE_CANCEL, NULL);
...
gboolean ghb_message_dialog(GtkWindow *parent, GtkMessageType type,
  ...
  gtk_dialog_add_buttons(GTK_DIALOG(dialog), no, GTK_RESPONSE_NO, yes,
...
void ghb_cancel_encode(signal_user_data_t *ud, const gchar *extra_msg) {
  ...
  hb_window = GTK_WINDOW(GHB_WIDGET(ud->builder, "hb_window"));
  dialog = gtk_message_dialog_new(
      hb_window, GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING, GTK_BUTTONS_NONE,
      _("%sYour movie will be lost if you don't continue encoding."),
      extra_msg);
  gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("Cancel Current and Stop"), 1,
                         _("Cancel Current, Start Next"), 2,
                         _("Finish Current, then Stop"), 3,
                         _("Continue Encoding"), 4, NULL);
  response = gtk_dialog_run(GTK_DIALOG(dialog));
  gtk_widget_destroy(dialog);
  switch ((int)response) {
  case 1:
...
static void start_new_log(signal_user_data_t *ud, GhbValue *uiDict) {
  time_t _now;
  struct tm *now;
  ...
  log_path =
      g_strdup_printf("%s/%s %d-%02d-%02d %02d-%02d-%02d.log", dest_dir,
                      basename, now->tm_year + 1900, now->tm_mon + 1,
  ...
  if (ud->job_activity_log) {
    ...
    g_io_channel_write_chars(ud->job_activity_log, ver_str, -1, NULL, NULL);
    g_free(ver_str);
...
static void submit_job(signal_user_data_t *ud, GhbValue *queueDict) {
  gchar *type, *modified;
  const char *name;
  GhbValue *uiDict;
  gboolean preset_modified;
...
static gint queue_pending_count(GhbValue *queue) {
  gint nn, ii, count;
...
static gchar *working_status_string(signal_user_data_t *ud,
  ...
  if (status->pass_count > 1) {
    if (status->pass_id == HB_PASS_SUBTITLE) {
      ...
      task_str = g_strdup_printf(_("pass %d of %d, "), status->pass,
                                 status->pass_count);
    }
  } else {
    task_str = g_strdup("");
  ...
  if (status->seconds > -1) {
    if (status->rate_cur > 0.0) {
      status_str = g_strdup_printf(
          ...
          job_str, task_str, 100.0 * status->progress, status->rate_cur,
          status->rate_avg, status->hours, status->minutes, status->seconds);
    } else {
      status_str =
          g_strdup_printf(_("Encoding: %s%s%.2f %%"
                            " (ETA %02dh%02dm%02ds)"),
                          job_str, task_str, 100.0 * status->progress,
                          status->hours, status->minutes, status->seconds);
    }
  } else {
    status_str = g_strdup_printf(_("Encoding: %s%s%.2f %%"), job_str, task_str,
...
static gchar *searching_status_string(signal_user_data_t *ud,
                                      ghb_instance_status_t *status) {
  gchar *task_str, *job_str, *status_str;
  gint qcount;
  gint index;
...
static void ghb_backend_events(signal_user_data_t *ud) {
  ghb_status_t status;
  gchar *status_str;
  GtkProgressBar *progress;
  ...
  if (status.scan.state & GHB_STATE_SCANNING) {
    ...
    if (status.scan.title_cur == 0) {
      ...
      if (status.scan.preview_cur == 0)
        status_str =
            g_strdup_printf(_("Scanning title %d of %d..."),
                            status.scan.title_cur, status.scan.title_count);
      else
        status_str = g_strdup_printf(
            _("Scanning title %d of %d preview %d..."), status.scan.title_cur,
            status.scan.title_count, status.scan.preview_cur);
    }
...
G_MODULE_EXPORT gboolean ghb_timer_cb(gpointer data) {
  ...
  if (!appcast_busy) {
    ...
    if (duration != 0) {
      ...
      time(&tt);
      if (last + duration < tt) {
        ghb_dict_set_int(ud->prefs, "last_update_check", tt);
...
static void append_activity(GtkTextBuffer *tb, const char *text, gsize length) {
...
G_MODULE_EXPORT gboolean ghb_log_cb(GIOChannel *source, GIOCondition cond,
                                    gpointer data) {
  gchar *text = NULL;
  ...
  if (text != NULL && length > 0) {
    ...
    if (utf8_text != NULL) {
      ...
      if (ud->activity_log != NULL) {
        g_io_channel_write_chars(ud->activity_log, utf8_text, length,
                                 &outlength, NULL);
#if defined(_WIN32)
      ...
      if (ud->job_activity_log) {
        g_io_channel_write_chars(ud->job_activity_log, utf8_text, length,
                                 &outlength, NULL);
#if defined(_WIN32)
        g_io_channel_write_chars(ud->activity_log, "\n", one, &outlength, NULL);
#endif
...
static void update_activity_labels(signal_user_data_t *ud, gboolean active) {
...
G_MODULE_EXPORT void show_activity_action_cb(GSimpleAction *action,
                                             GVariant *value,
                                             signal_user_data_t *ud) {
...
G_MODULE_EXPORT gboolean activity_window_delete_cb(GtkWidget *xwidget,
#if !GTK_CHECK_VERSION(3, 90, 0)
                                                   GdkEvent *event,
#endif
                                                   signal_user_data_t *ud) {
...
void ghb_browse_uri(signal_user_data_t *ud, const gchar *uri) {
  ...
  if (result)
    return;
  char *argv[] = {"xdg-open", NULL, NULL, NULL};
  argv[1] = (gchar *)uri;
  result = g_spawn_async(NULL, argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL,
                         NULL, NULL);
  if (result)
    return;
...
G_MODULE_EXPORT void about_action_cb(GSimpleAction *action, GVariant *param,
                                     signal_user_data_t *ud) {
...
G_MODULE_EXPORT void guide_action_cb(GSimpleAction *action, GVariant *param,
                                     signal_user_data_t *ud) {
...
static void presets_window_set_visible(signal_user_data_t *ud,
                                       gboolean visible) {
...
G_MODULE_EXPORT void show_presets_action_cb(GSimpleAction *action,
                                            GVariant *value,
                                            signal_user_data_t *ud) {
  gboolean state = g_variant_get_boolean(value);
...
G_MODULE_EXPORT void hbfd_action_cb(GSimpleAction *action, GVariant *value,
                                    signal_user_data_t *ud) {
  gboolean state = g_variant_get_boolean(value);
...
G_MODULE_EXPORT void activity_font_changed_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
...
G_MODULE_EXPORT void when_complete_changed_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
...
G_MODULE_EXPORT void queue_when_complete_changed_cb(GtkWidget *widget,
                                                    signal_user_data_t *ud) {
  GhbValue *value = ghb_widget_value(widget);
...
G_MODULE_EXPORT void pref_changed_cb(GtkWidget *widget,
                                     signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->prefs, widget);
...
G_MODULE_EXPORT void log_level_changed_cb(GtkWidget *widget,
                                          signal_user_data_t *ud) {
...
G_MODULE_EXPORT void use_m4v_changed_cb(GtkWidget *widget,
                                        signal_user_data_t *ud) {
...
G_MODULE_EXPORT void vqual_granularity_changed_cb(GtkWidget *widget,
                                                  signal_user_data_t *ud) {
...
G_MODULE_EXPORT void tweaks_changed_cb(GtkWidget *widget,
                                       signal_user_data_t *ud) {
...
G_MODULE_EXPORT void hbfd_feature_changed_cb(GtkWidget *widget,
                                             signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->prefs, widget);
  const gchar *name = ghb_get_setting_key(widget);
  ghb_pref_set(ud->prefs, name);
...
gboolean ghb_file_menu_add_dvd(signal_user_data_t *ud) {
  ...
  if (drives != NULL) {
    ...
    while (link != NULL) {
      ...
      g_signal_connect(widget, "activate", (GCallback)dvd_source_activate_cb,
                       ud);
      g_free(name);
      g_free(drive);
      free_drive(link->data);
  ...
  for (ii = 0; ii < 26; ii++) {
    if (drives & 0x01) {
      ...
      if (dtype == DRIVE_CDROM) {
        dvd_devices = g_list_append(dvd_devices, (gpointer)g_strdup(drive));
      }
    }
    drives >>= 1;
  }
...
#if defined(_WIN32)
static void handle_media_change(const gchar *device, gboolean insert,
                                signal_user_data_t *ud) {
  guint dtype;
...
static gchar FindDriveFromMask(ULONG unitmask) {
  gchar cc;
  for (cc = 0; cc < 26; cc++) {
    if (unitmask & 0x01)
      return 'A' + cc;
...
G_MODULE_EXPORT void drive_changed_cb(GVolumeMonitor *gvm, GDrive *gd,
                                      signal_user_data_t *ud) {
  gchar *device;
  gint state;
  ...
  device = g_drive_get_identifier(gd, G_VOLUME_IDENTIFIER_KIND_UNIX_DEVICE);
  if (ud->current_dvd_device == NULL ||
      strcmp(device, ud->current_dvd_device) != 0 || state != GHB_STATE_IDLE) {
    return;
  }
...
static gboolean easter_egg_timeout_cb(button_click_t *bc) {
  if (bc->count == 1) {
    GtkWidget *widget;
...
G_MODULE_EXPORT void easter_egg_multi_cb(GtkGestureMultiPress *gest,
...
G_MODULE_EXPORT gboolean easter_egg_cb(GtkWidget *widget, GdkEvent *event,
                                       signal_user_data_t *ud) {
...
G_MODULE_EXPORT gchar *format_deblock_cb(GtkScale *scale, gdouble val,
                                         signal_user_data_t *ud) {
  if (val < 5.0) {
...
static void process_appcast(signal_user_data_t *ud) {
  ...
  msg =
      g_strdup_printf(_("HandBrake %s/%s is now available (you have %s/%d)."),
...
void ghb_net_close(GIOChannel *ioc) {
  gint fd;
...
G_MODULE_EXPORT gboolean ghb_net_recv_cb(GIOChannel *ioc, GIOCondition cond,
                                         gpointer data) {
  gchar buf[2048];
  gsize len;
  GError *gerror = NULL;
  ...
  if ((status == G_IO_STATUS_NORMAL || status == G_IO_STATUS_EOF) && len > 0) {
...
GIOChannel *ghb_net_open(signal_user_data_t *ud, gchar *address, gint port) {
  GIOChannel *ioc;
  gint fd;
...
void ghb_notify_done(signal_user_data_t *ud) {
  ...
  g_notification_set_body(
      notification, _("Put down that cocktail, Your HandBrake queue is done!"));
  icon = g_themed_icon_new("fr.handbrake.ghb");
  g_notification_set_icon(notification, icon);

  g_application_send_notification(G_APPLICATION(ud->app), "cocktail",
                                  notification);
  ...
  if (ghb_settings_combo_int(ud->prefs, "WhenComplete") == 3) {
    if (ghb_can_shutdown_gsm()) {
      ghb_countdown_dialog(GTK_MESSAGE_WARNING, _("Your encode is complete."),
                           _("Shutting down the computer"), _("Cancel"),
                           (GSourceFunc)shutdown_cb, ud, 60);
    }
  }
  if (ghb_settings_combo_int(ud->prefs, "WhenComplete") == 2) {
    if (ghb_can_suspend_gpm()) {
      ghb_countdown_dialog(GTK_MESSAGE_WARNING, _("Your encode is complete."),
                           _("Putting computer to sleep"), _("Cancel"),
                           (GSourceFunc)suspend_cb, ud, 60);
    }
  }
  if (ghb_settings_combo_int(ud->prefs, "WhenComplete") == 4) {
    ghb_countdown_dialog(GTK_MESSAGE_WARNING, _("Your encode is complete."),
                         _("Quitting Handbrake"), _("Cancel"),
                         (GSourceFunc)quit_cb, ud, 60);
  }
}

G_MODULE_EXPORT gboolean window_map_cb(GtkWidget *widget,
#if !GTK_CHECK_VERSION(3, 90, 0)
                                       GdkEventAny *event,
#endif
                                       signal_user_data_t *ud) {
...
G_MODULE_EXPORT void hb_win_sz_alloc_cb(GtkWidget *widget,
#if GTK_CHECK_VERSION(3, 90, 0)
...
static void lang_combo_search(GtkComboBox *combo, guint keyval,
                              signal_user_data_t *ud) {
  if (combo == NULL) {
    return;
  }
...
#if GTK_CHECK_VERSION(3, 90, 0)
G_MODULE_EXPORT gboolean combo_search_key_press_cb(
    GtkEventControllerKey *keycon, guint keyval, guint keycode,
    GdkModifierType state, signal_user_data_t *ud) {
  GtkComboBox *combo;

  combo = GTK_COMBO_BOX(
      gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(keycon)));
  lang_combo_search(combo, keyval, ud);
  return FALSE;
}
#else
G_MODULE_EXPORT gboolean combo_search_key_press_cb(GtkWidget *widget,
                                                   GdkEvent *event,
                                                   signal_user_data_t *ud) {
  guint keyval;
