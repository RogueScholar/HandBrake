...
G_MODULE_EXPORT void vcodec_changed_cb(GtkWidget *widget,
  ...
  while (video_presets && video_presets[count])
    count++;
...
static void update_adv_settings_tooltip(signal_user_data_t *ud) {
  ...
  if (encoder & HB_VCODEC_X264_MASK) {
    ...
    if (ghb_dict_get_bool(ud->settings, "x264ZeroLatency")) {
      g_string_append_printf(str, "%s%s", str->str[0] ? "," : "",
                             "zerolatency");
    }
...
G_MODULE_EXPORT void video_preset_slider_changed_cb(GtkWidget *widget,
                                                    signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
...
G_MODULE_EXPORT void video_setting_changed_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
...
G_MODULE_EXPORT gchar *format_video_preset_cb(GtkScale *scale, gdouble val,
                                              signal_user_data_t *ud) {
  const char *const *video_presets;
  const char *preset;
  int encoder = ghb_get_video_encoder(ud->settings);
...
G_MODULE_EXPORT gchar *format_vquality_cb(GtkScale *scale, gdouble val,
                                          signal_user_data_t *ud) {
  gint vcodec;
  const char *vqname;
  char *result;
  ...
  case HB_VCODEC_X264_8BIT: {
    if (val == 0.0) {
      result = g_strdup_printf(_("%s: %.4g (Warning: lossless)"), vqname, val);
      break;
    }
...
G_MODULE_EXPORT void framerate_changed_cb(GtkWidget *widget,
                                          signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_update_summary_info(ud);
...
G_MODULE_EXPORT void framerate_mode_changed_cb(GtkWidget *widget,
                                               signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_update_summary_info(ud);
  ghb_check_dependency(ud, widget, NULL);
