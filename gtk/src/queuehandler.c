...
G_MODULE_EXPORT void queue_drag_begin_cb(GtkWidget *widget, GdkDrag *context,
                                         signal_user_data_t *ud);
G_MODULE_EXPORT void queue_drag_end_cb(GtkWidget *widget, GdkDrag *context,
                                       signal_user_data_t *ud);
G_MODULE_EXPORT void queue_drag_data_get_cb(GtkWidget *widget, GdkDrag *context,
                                            GtkSelectionData *selection_data,
                                            signal_user_data_t *ud);

G_MODULE_EXPORT gboolean queue_row_key_cb(GtkEventControllerKey *keycon,
                                          guint keyval, guint keycode,
                                          GdkModifierType state,
                                          signal_user_data_t *ud);
#else
G_MODULE_EXPORT void queue_drag_begin_cb(GtkWidget *widget,
                                         GdkDragContext *context,
                                         signal_user_data_t *ud);
G_MODULE_EXPORT void queue_drag_end_cb(GtkWidget *widget,
                                       GdkDragContext *context,
                                       signal_user_data_t *ud);
G_MODULE_EXPORT void queue_drag_data_get_cb(GtkWidget *widget,
                                            GdkDragContext *context,
                                            GtkSelectionData *selection_data,
                                            guint info, guint time,
                                            signal_user_data_t *ud);
#endif
G_MODULE_EXPORT void title_selected_cb(GtkWidget *widget,
                                       signal_user_data_t *ud);
G_MODULE_EXPORT void title_dest_file_cb(GtkWidget *widget,
                                        signal_user_data_t *ud);
G_MODULE_EXPORT void title_dest_dir_cb(GtkWidget *widget,
                                       signal_user_data_t *ud);

#if GTK_CHECK_VERSION(3, 90, 0)
static const char *queue_drag_entries[] = {"application/queue-list-row-drop"};
...
static void queue_update_summary(GhbValue *queueDict, signal_user_data_t *ud) {
  ...
  if (!strcmp(rangeType, "chapter")) {
    g_string_append_printf(str, "Chapters: %ld to %ld", rangeStart, rangeEnd);
    ...
    g_string_append_printf(str, "Time: %02d:%02d:%05.2f to %02d:%02d:%05.2f",
                           start_hh, start_mm, start_ss, end_hh, end_mm,
                           end_ss);
  } else if (!strcmp(rangeType, "frame")) {
    g_string_append_printf(str, "Frames: %ld to %ld", rangeStart, rangeEnd);
  }
  text = g_string_free(str, FALSE);
  ...
  text = g_strdup_printf(_("%d:%d:%d:%d Crop\n"
                         ...
                         (int)display_width, display_height, par_width,
                         par_height, display_aspect);
  widget = GHB_WIDGET(ud->builder, "queue_summary_dimensions");
  gtk_label_set_text(GTK_LABEL(widget), text);
  ...
  if (!vqtype) {
    ...
    if (!two_pass) {
      g_string_append_printf(str, _("%s, Bitrate %dkbps"), video_encoder->name,
                             br);
    } else {
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    if (audio_encoder->codec & HB_ACODEC_PASS_FLAG) {
      g_string_append_printf(str, "%s%s, %s", sep, lang, audio_encoder->name);
    } else {
...
void queue_update_current_stats(signal_user_data_t *ud) {
  ...
  if (row != NULL) {
    ...
    if (index < 0 || index >= ghb_array_len(ud->queue)) { // Should never happen
      return;
    }
...
void ghb_queue_select_log(signal_user_data_t *ud) {
  ...
  if (row != NULL) {
    ...
    if (status != GHB_QUEUE_PENDING && log_path != NULL) {
      ghb_ui_update(ud, "queue_activity_location", ghb_string_value(log_path));
    } else {
...
void ghb_queue_progress_set_fraction(signal_user_data_t *ud, int index,
  ...
  progress =
      GTK_PROGRESS_BAR(find_widget(GTK_WIDGET(row), "queue_item_progress"));
...
void ghb_queue_update_status_icon(signal_user_data_t *ud, int index) {
  ...
  ghb_image_set_from_icon_name(status_icon, icon_name, GHB_ICON_SIZE_BUTTON);
}
...
static void save_queue_file(signal_user_data_t *ud) {
  ...
  dialog = gtk_file_chooser_dialog_new(
      "Queue Destination", hb_window, GTK_FILE_CHOOSER_ACTION_SAVE,
...
static void add_to_queue_list(signal_user_data_t *ud, GhbValue *queueDict) {
  GtkListBox *lb;
  GtkWidget *row;
  ...
  status_icon = ghb_image_new_from_icon_name("hb-source", GHB_ICON_SIZE_BUTTON);
  ...
  g_signal_connect(delete_button, "clicked", (GCallback)queue_remove_clicked_cb,
                   ud);
  ...
  g_signal_connect(ebox, "drag-data-get", G_CALLBACK(queue_drag_data_get_cb),
                   NULL);
...
static void open_queue_file(signal_user_data_t *ud) {
  ...
  hb_window = GTK_WINDOW(GHB_WIDGET(ud->builder, "hb_window"));
  dialog = gtk_file_chooser_dialog_new(
      "Queue Destination", hb_window, GTK_FILE_CHOOSER_ACTION_OPEN,
      GHB_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GHB_STOCK_OPEN,
      GTK_RESPONSE_ACCEPT, NULL);
...
gint64 ghb_dest_free_space(GhbValue *settings) {
  GFile *gfile;
  GFileInfo *info;
  guint64 size = -1;
  ...
  info = g_file_query_filesystem_info(gfile, G_FILE_ATTRIBUTE_FILESYSTEM_FREE,
                                      NULL, NULL);
  if (info != NULL) {
...
void ghb_low_disk_check(signal_user_data_t *ud) {
  ...
  gtk_dialog_add_buttons(GTK_DIALOG(dialog),
                         ...
                         _("Cancel Current and Stop"), 3, NULL);
...
static gboolean validate_settings(signal_user_data_t *ud, GhbValue *settings,
                                  gint batch) {
  ...
  if (!g_file_test(destdir, G_FILE_TEST_IS_DIR)) {
    message = g_strdup_printf(_("Destination: %s\n\n"
                              ...
                              destdir);
    ghb_message_dialog(hb_window, GTK_MESSAGE_ERROR, message, _("Cancel"),
                       NULL);
    g_free(message);
    g_free(destdir);
    return FALSE;
...
static gboolean queue_add(signal_user_data_t *ud, GhbValue *settings,
                          gint batch) {
...
static gboolean title_multiple_can_select(GhbValue *settings_array, int index) {
  gint count, ii;
...
static void title_add_multiple_set_sensitive(GtkWidget *row,
                                             gboolean sensitive) {
  GtkWidget *widget;
...
static gboolean title_add_multiple_are_conflicts(signal_user_data_t *ud) {
  GtkListBox *list;
  GtkWidget *row;
  gint count, ii;
...
static void title_add_multiple_set_conflict_label(signal_user_data_t *ud,
  ...
  if (are_conflicts) {
    msg = "<span foreground='black' weight='bold'>"
          "Duplicate destination files detected.\n"
          "Duplicates will not be added to the queue."
          "</span>";
    msg_type = GTK_MESSAGE_WARNING;
  } else {
    msg = "Destination files OK.  No duplicates detected.";
  ...
  if (are_conflicts ^ conflict_showing) {
    ...
    info = GTK_INFO_BAR(GHB_WIDGET(ud->builder, "title_add_multiple_infobar"));
...
static void title_add_multiple_check_conflicts(signal_user_data_t *ud) {
  gint count, ii;
  GhbValue *settings;
  GtkWidget *row;
  GtkListBox *list;
...
static void add_multiple_titles(signal_user_data_t *ud) {
  gint count, ii;
...
GtkWidget *title_create_row(signal_user_data_t *ud) {
  ...
  gtk_widget_set_tooltip_markup(
      GTK_WIDGET(selected), _("Select this title for adding to the queue.\n"));
  ...
  dest_dir = GTK_FILE_CHOOSER_BUTTON(gtk_file_chooser_button_new(
      _("Destination Directory"), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER));
  g_signal_connect(dest_dir, "selection-changed", (GCallback)title_dest_dir_cb,
                   ud);
...
static void ghb_queue_remove_row_internal(signal_user_data_t *ud, int index) {
  GtkListBox *lb;
  GtkListBoxRow *row;
  GhbValue *queueDict, *uiDict;
...
void ghb_queue_remove_row(signal_user_data_t *ud, int index) {
...
void ghb_queue_buttons_grey(signal_user_data_t *ud) {
  ...
  show_stop =
      queue_state & (GHB_STATE_WORKING | GHB_STATE_SEARCHING |
                     GHB_STATE_SCANNING | GHB_STATE_MUXING | GHB_STATE_PAUSED);
  allow_start =
      !(scan_state & GHB_STATE_SCANNING) && (title != NULL || queue_count > 0);
  allow_add = !(scan_state & GHB_STATE_SCANNING) && title != NULL;

  paused = queue_state & GHB_STATE_PAUSED;

  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-export"));
  g_simple_action_set_enabled(action, !!queue_count);
  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-add"));
  g_simple_action_set_enabled(action, allow_add);
  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-add-all"));
  g_simple_action_set_enabled(action, allow_add);
  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-start"));
  g_simple_action_set_enabled(action, allow_start || show_stop);
  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-pause"));
  g_simple_action_set_enabled(action, show_stop);

  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-reset"));
  g_simple_action_set_enabled(action, row != NULL);

  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-edit"));
  g_simple_action_set_enabled(action, row != NULL);

  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-open-source"));
  g_simple_action_set_enabled(action, row != NULL);

  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-open-dest"));
  g_simple_action_set_enabled(action, row != NULL);

  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-open-log-dir"));
  g_simple_action_set_enabled(action, status != GHB_QUEUE_PENDING);

  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "queue-open-log"));
  g_simple_action_set_enabled(action, status != GHB_QUEUE_PENDING);
...
find_pid:
  ...
  for (ii = count - 1; ii >= 0; ii--) {
    queueDict = ghb_array_get(queue, ii);
    uiDict = ghb_dict_get(queueDict, "uiSettings");
    if (uiDict == NULL || ghb_dict_get_value(uiDict, "job_status") == NULL) {
...
static gboolean queue_row_key(GtkListBoxRow *row, guint keyval,
                              signal_user_data_t *ud) {
  gint index;
...
#else
G_MODULE_EXPORT gboolean queue_key_press_cb(GtkWidget *widget, GdkEvent *event,
                                            signal_user_data_t *ud) {
  GtkListBoxRow *row;
  guint keyval;
...
G_MODULE_EXPORT void show_queue_action_cb(GSimpleAction *action,
                                          GVariant *value,
                                          signal_user_data_t *ud) {
...
G_MODULE_EXPORT gboolean queue_window_delete_cb(GtkWidget *xwidget,
#if !GTK_CHECK_VERSION(3, 90, 0)
                                                GdkEvent *event,
#endif
                                                signal_user_data_t *ud) {
...
G_MODULE_EXPORT void queue_edit_action_cb(GSimpleAction *action,
                                          GVariant *param,
                                          signal_user_data_t *ud) {
  GtkListBox *lb;
  GtkListBoxRow *row;
  ...
  if (row != NULL) {
    ...
    ghb_dict_set(ghb_queue_edit_settings, "Job",
                 ghb_value_dup(ghb_dict_get(queueDict, "Job")));
    ghb_dict_set(ghb_queue_edit_settings, "Title",
                 ghb_value_dup(ghb_dict_get(queueDict, "Title")));
    status = ghb_dict_get_int(uiDict, "job_status");
    if (status == GHB_QUEUE_PENDING) {
...
G_MODULE_EXPORT void queue_export_action_cb(GSimpleAction *action,
                                            GVariant *param,
                                            signal_user_data_t *ud) {
  save_queue_file(ud);
}

G_MODULE_EXPORT void queue_import_action_cb(GSimpleAction *action,
                                            GVariant *param,
                                            signal_user_data_t *ud) {
  open_queue_file(ud);
}

G_MODULE_EXPORT void queue_open_clicked_cb(GtkWidget *widget,
                                           signal_user_data_t *ud) {
  open_queue_file(ud);
}

G_MODULE_EXPORT void queue_open_source_action_cb(GSimpleAction *action,
                                                 GVariant *param,
                                                 signal_user_data_t *ud) {
  GtkListBox *lb;
  GtkListBoxRow *row;
...
G_MODULE_EXPORT void queue_open_dest_action_cb(GSimpleAction *action,
                                               GVariant *param,
                                               signal_user_data_t *ud) {
  GtkListBox *lb;
  GtkListBoxRow *row;
...
G_MODULE_EXPORT void queue_open_log_action_cb(GSimpleAction *action,
                                              GVariant *param,
                                              signal_user_data_t *ud) {
  GtkListBox *lb;
  GtkListBoxRow *row;
...
G_MODULE_EXPORT void queue_open_log_dir_action_cb(GSimpleAction *action,
                                                  GVariant *param,
                                                  signal_user_data_t *ud) {
  GtkListBox *lb;
  GtkListBoxRow *row;
...
G_MODULE_EXPORT void queue_list_selection_changed_cb(GtkListBox *box,
                                                     GtkListBoxRow *row,
                                                     signal_user_data_t *ud) {
...
G_MODULE_EXPORT void queue_add_action_cb(GSimpleAction *action, GVariant *param,
                                         signal_user_data_t *ud) {
...
G_MODULE_EXPORT void queue_add_all_action_cb(GSimpleAction *action,
                                             GVariant *param,
                                             signal_user_data_t *ud) {
  GtkListBox *list;
  GtkWidget *row;
  ...
  clear_all = GTK_TOGGLE_BUTTON(
      GHB_WIDGET(ud->builder, "title_add_multiple_clear_all"));
  select_all = GTK_TOGGLE_BUTTON(
      GHB_WIDGET(ud->builder, "title_add_multiple_select_all"));
...
G_MODULE_EXPORT void queue_delete_all_action_cb(GSimpleAction *action,
                                                GVariant *param,
                                                signal_user_data_t *ud) {
  int ii, count;
...
G_MODULE_EXPORT void queue_delete_complete_action_cb(GSimpleAction *action,
                                                     GVariant *param,
                                                     signal_user_data_t *ud) {
  int ii, count, status;
  GhbValue *queueDict, *uiDict;
...
G_MODULE_EXPORT void queue_reset_all_action_cb(GSimpleAction *action,
                                               GVariant *param,
                                               signal_user_data_t *ud) {
...
G_MODULE_EXPORT void queue_reset_fail_action_cb(GSimpleAction *action,
                                                GVariant *param,
                                                signal_user_data_t *ud) {
  int ii, count, status;
  GhbValue *queueDict, *uiDict;
...
G_MODULE_EXPORT void queue_reset_action_cb(GSimpleAction *action,
                                           GVariant *param,
                                           signal_user_data_t *ud) {
  GtkListBox *lb;
  GtkListBoxRow *row;
...
G_MODULE_EXPORT void queue_start_action_cb(GSimpleAction *action,
                                           GVariant *param,
                                           signal_user_data_t *ud) {
  GhbValue *queueDict, *uiDict;
  ...
  if (state & (GHB_STATE_WORKING | GHB_STATE_SEARCHING | GHB_STATE_SCANNING |
               GHB_STATE_MUXING)) {
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    uiDict = ghb_dict_get(queueDict, "uiSettings");
    status = ghb_dict_get_int(uiDict, "job_status");
    if ((status == GHB_QUEUE_RUNNING) || (status == GHB_QUEUE_PENDING)) {
      running = TRUE;
      break;
    }
  }
...
G_MODULE_EXPORT void queue_pause_action_cb(GSimpleAction *action,
                                           GVariant *param,
                                           signal_user_data_t *ud) {
...
G_MODULE_EXPORT void title_add_multiple_select_all_cb(GtkWidget *widget,
                                                      signal_user_data_t *ud) {
  gint count, ii;
  GhbValue *settings;
  GtkWidget *row;
...
G_MODULE_EXPORT void title_add_multiple_clear_all_cb(GtkWidget *widget,
                                                     signal_user_data_t *ud) {
  gint count, ii;
  GhbValue *settings;
  GtkWidget *row;
...
#if GTK_CHECK_VERSION(3, 90, 0)
queue_drag_begin_cb(GtkWidget *widget, GdkDrag *context, signal_user_data_t *ud)
#else
...
#endif
{
  ...
  surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, alloc.width,
                                       alloc.height);
...
G_MODULE_EXPORT void
#if GTK_CHECK_VERSION(3, 90, 0)
queue_drag_end_cb(GtkWidget *widget, GdkDrag *context, signal_user_data_t *ud)
#else
...
queue_drag_data_get_cb(GtkWidget *widget, GdkDrag *context,
                       GtkSelectionData *selection_data, signal_user_data_t *ud)
#else
...
#endif
{
  ...
  row = gtk_widget_get_ancestor(widget, GTK_TYPE_LIST_BOX_ROW);
  gtk_selection_data_set(selection_data, ghb_atom_string("GTK_LIST_BOX_ROW"),
                         32, (const guchar *)&row, sizeof(gpointer));
}

G_MODULE_EXPORT void
#if GTK_CHECK_VERSION(3, 90, 0)
queue_drag_leave_cb(GtkListBox *lb, GdkDrop *ctx, signal_user_data_t *ud)
#else
queue_drag_leave_cb(GtkListBox *lb, GdkDragContext *ctx, guint time,
                    signal_user_data_t *ud)
#endif
{
  GtkWidget *drag_row;
...
#if GTK_CHECK_VERSION(3, 90, 0)
queue_drag_motion_cb(GtkListBox *lb, GdkDrop *ctx, gint x, gint y,
                     signal_user_data_t *ud)
#else
queue_drag_motion_cb(GtkListBox *lb, GdkDragContext *ctx, gint x, gint y,
...
#endif
{
  ...
  if (row_after) {
    gtk_style_context_add_class(gtk_widget_get_style_context(row_after),
                                "drag-hover-top");
  }
...
G_MODULE_EXPORT void
#if GTK_CHECK_VERSION(3, 90, 0)
queue_drag_data_received_cb(GtkListBox *lb, GdkDrop *context,
                            GtkSelectionData *selection_data,
                            signal_user_data_t *ud)
#else
queue_drag_data_received_cb(GtkListBox *lb, GdkDragContext *context, gint x,
                            gint y, GtkSelectionData *selection_data,
                            guint info, guint32 time, signal_user_data_t *ud)
#endif
{
  GtkWidget *row_before;
  GtkWidget *row_after;
  ...
  src_row =
      GTK_WIDGET(*(gpointer *)gtk_selection_data_get_data(selection_data));
