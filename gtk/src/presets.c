...
hb_preset_index_t *ghb_tree_get_index(GtkTreeModel *store, GtkTreeIter *iter) {
...
hb_preset_index_t *ghb_tree_path_get_index(GtkTreePath *treepath) {
  int *indices, len;
...
GtkTreePath *ghb_tree_path_new_from_index(const hb_preset_index_t *path) {
  ...
  switch (path->depth) {
  case 1:
    return gtk_tree_path_new_from_indices(path->index[0], -1);
  case 2:
    return gtk_tree_path_new_from_indices(path->index[0], path->index[1], -1);
  case 3:
    return gtk_tree_path_new_from_indices(path->index[0], path->index[1],
                                          path->index[2], -1);
  case 4:
    return gtk_tree_path_new_from_indices(path->index[0], path->index[1],
                                          path->index[2], path->index[3], -1);
  case 5:
    return gtk_tree_path_new_from_indices(path->index[0], path->index[1],
  ...
  case 6:
    return gtk_tree_path_new_from_indices(path->index[0], path->index[1],
                                          path->index[2], path->index[3],
                                          path->index[4], path->index[5], -1);
  case 7:
    return gtk_tree_path_new_from_indices(
        path->index[0], path->index[1], path->index[2], path->index[3],
        path->index[4], path->index[5], path->index[6], -1);
  case 8:
    return gtk_tree_path_new_from_indices(
        path->index[0], path->index[1], path->index[2], path->index[3],
        path->index[4], path->index[5], path->index[6], path->index[7], -1);
  default:
...
void ghb_presets_list_show_default(signal_user_data_t *ud) {
  hb_preset_index_t *path;
  ...
  if (treepath) {
    if (gtk_tree_model_get_iter(GTK_TREE_MODEL(store), &iter, treepath)) {
      gtk_tree_store_set(store, &iter, 1, 800, 2, 2, -1);
    }
    gtk_tree_path_free(treepath);
  }
  free(path);
...
void ghb_presets_list_clear_default(signal_user_data_t *ud) {
  hb_preset_index_t *path;
  ...
  if (treepath) {
    if (gtk_tree_model_get_iter(GTK_TREE_MODEL(store), &iter, treepath)) {
      gtk_tree_store_set(store, &iter, 1, 400, 2, 0, -1);
    }
    gtk_tree_path_free(treepath);
  }
  free(path);
...
static gint preset_get_type(hb_preset_index_t *path) {
  GhbValue *dict;
...
static gboolean preset_is_folder(hb_preset_index_t *path) {
  GhbValue *dict;
...
void ghb_preset_to_settings(GhbValue *settings, GhbValue *preset) {
  ...
  ghb_dict_set(settings, "scale_height",
               ghb_value_dup(ghb_dict_get_value(settings, "PictureHeight")));

  ghb_dict_set(settings, "scale_width",
               ghb_value_dup(ghb_dict_get_value(settings, "PictureWidth")));
...
static const char *widget_priority_list[] = {"preview_count",
                                             "PtoPType",
                                             "VideoEncoder",
...
char *preset_get_fullname(hb_preset_index_t *path, const char *sep,
                          gboolean escape) {
  int ii;
  GString *gstr;
...
static void set_preset_menu_button_label(signal_user_data_t *ud,
                                         hb_preset_index_t *path) {
  char *fullname, *text;
  ...
  text = g_strdup_printf(
      "%s%s", type == HB_PRESET_TYPE_CUSTOM ? "Custom" : "Official", fullname);
...
static void select_preset2(signal_user_data_t *ud, hb_preset_index_t *path) {
  GtkTreeView *treeview;
  ...
  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "preset-rename"));
  g_simple_action_set_enabled(action, type == HB_PRESET_TYPE_CUSTOM);
  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "preset-save"));
  g_simple_action_set_enabled(action, type == HB_PRESET_TYPE_CUSTOM);
}
...
void ghb_override_user_config_dir(char *dir) { override_user_config_dir = dir; }

static void write_config_file(const gchar *name, GhbValue *dict) {
  gchar *config, *path;
...
static int presets_add_config_file(const gchar *name) {
  gchar *config, *path;
  hb_value_t *preset;
  ...
  if (preset != NULL) {
    ...
    if (major != hb_major) {
      ...
      config = ghb_get_user_config_dir(NULL);
      path = g_strdup_printf("%s/presets.%d.%d.%d.json", config, major, minor,
                             micro);
...
static GhbValue *read_config_file(const gchar *name) {
  gchar *config, *path;
  GhbValue *gval = NULL;
...
static void remove_config_file(const gchar *name) {
  gchar *config, *path;
...
static const gchar *get_preset_color(gint type, gboolean is_folder) {
  const gchar *color;
...
hb_preset_index_t *get_selected_path(signal_user_data_t *ud) {
  GtkTreeView *treeview;
  GtkTreeSelection *selection;
  GtkTreeModel *store;
  GtkTreeIter iter;
...
G_MODULE_EXPORT gboolean presets_window_delete_cb(GtkWidget *xwidget,
  ...
  action = G_SIMPLE_ACTION(
      g_action_map_lookup_action(G_ACTION_MAP(ud->app), "show-presets"));
  g_action_change_state(G_ACTION(action), state);
  return TRUE;
}

G_MODULE_EXPORT void presets_sz_alloc_cb(GtkWidget *widget,
#if GTK_CHECK_VERSION(3, 90, 0)
...
void ghb_presets_menu_init(signal_user_data_t *ud) {
  ...
  for (type = 0; type < 2; type++) {
    ...
    for (ii = 0; ii < menu_count; ii++) {
      ...
      g_string_append_printf(folder_str, "%d/%s", folder_type, folder_name);
      if (is_folder) {
        ...
        for (jj = 0; jj < submenu_count; jj++) {
          ...
          detail_action =
              g_strdup_printf("app.preset-select(\"%s\")", preset_path);
        ...
        g_menu_append_submenu(section, menu_item_name, G_MENU_MODEL(submenu));
        g_free(menu_item_name);
      }
      g_string_free(folder_str, TRUE);
    }
    g_menu_append_section(menu,
                          type == HB_PRESET_TYPE_CUSTOM ? "Custom" : "Official",
...
void ghb_presets_menu_clear(signal_user_data_t *ud) {
  GtkMenuButton *mb;
  GMenuModel *mm;
...
void ghb_presets_list_init(signal_user_data_t *ud,
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    gtk_tree_store_set(store, &iter, 0, name, 1, def ? 800 : 400, 2,
                       def ? 2 : 0, 3, color, 4, description, 5,
                       type == HB_PRESET_TYPE_OFFICIAL ? 0 : 1, -1);
    if (is_folder) {
...
static void presets_list_clear(signal_user_data_t *ud) {
  GtkTreeView *treeview;
  GtkTreeModel *store;
  GtkTreeIter iter;
  gboolean valid;
...
static void presets_list_update_item(signal_user_data_t *ud,
                                     const hb_preset_index_t *path,
                                     gboolean recurse) {
  GhbValue *dict;
  GtkTreeView *treeview;
  ...
  gtk_tree_store_set(store, &iter, 0, name, 1, def ? 800 : 400, 2, def ? 2 : 0,
                     3, color, 4, description, 5,
                     type == HB_PRESET_TYPE_OFFICIAL ? 0 : 1, -1);
...
static void presets_list_append(signal_user_data_t *ud,
                                const hb_preset_index_t *path) {
...
static void presets_list_remove(signal_user_data_t *ud,
                                hb_preset_index_t *path) {
  GtkTreeView *treeview;
  GtkTreePath *treepath;
  GtkTreeIter iter;
...
static gboolean delayed_store_prefs(gpointer data) {
  write_config_file("preferences.json", prefsDict);
  prefs_timeout_id = 0;
  return FALSE;
}

static void store_presets() {
  gchar *config, *path;
  hb_value_t *presets;
...
static void store_prefs(void) {
  ...
  prefs_timeout_id =
      g_timeout_add_seconds(1, (GSourceFunc)delayed_store_prefs, NULL);
}
...
void ghb_presets_load(signal_user_data_t *ud) {
  ...
  if (result == -2) {
    ...
    gchar *message =
        g_strdup_printf(_("Presets found are newer than what is supported by "
                          "this version of HandBrake!\n\n"
                          "Would you like to continue?"));
    if (!ghb_message_dialog(hb_window, GTK_MESSAGE_WARNING, message,
                            _("Get me out of here!"),
                            _("Load backup presets"))) {
      g_free(message);
      exit(1);
    }
    g_free(message);
...
static void settings_save(signal_user_data_t *ud, const char *category,
...
G_MODULE_EXPORT void preset_import_action_cb(GSimpleAction *action,
  ...
  dialog = gtk_file_chooser_dialog_new(
      "Import Preset", hb_window, GTK_FILE_CHOOSER_ACTION_OPEN,
...
G_MODULE_EXPORT void preset_export_action_cb(GSimpleAction *action,
  ...
  hb_window = GTK_WINDOW(GHB_WIDGET(ud->builder, "hb_window"));
  dialog = gtk_file_chooser_dialog_new(
      _("Export Preset"), hb_window, GTK_FILE_CHOOSER_ACTION_SAVE,
...
static void preset_save_set_ok_sensitive(signal_user_data_t *ud) {
  GtkEntry *entry;
  GtkWidget *ok_button;
  const char *name;
  const char *category;
...
G_MODULE_EXPORT void preset_category_changed_cb(GtkWidget *widget,
                                                signal_user_data_t *ud) {
...
G_MODULE_EXPORT void preset_name_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
  preset_save_set_ok_sensitive(ud);
}
...
G_MODULE_EXPORT void preset_remove_action_cb(GSimpleAction *action,
  ...
  dialog = gtk_message_dialog_new(hb_window, GTK_DIALOG_MODAL,
                                  ...
                                  is_folder ? _("folder") : _("preset"), name);
  response = gtk_dialog_run(GTK_DIALOG(dialog));
  gtk_widget_destroy(dialog);
...
#if GTK_CHECK_VERSION(3, 90, 0)
presets_drag_motion_cb(GtkTreeView *tv, GdkDrop *ctx, gint x, gint y,
                       signal_user_data_t *ud)
#else
presets_drag_motion_cb(GtkTreeView *tv, GdkDragContext *ctx, gint x, gint y,
...
#if GTK_CHECK_VERSION(3, 90, 0)
presets_drag_data_received_cb(GtkTreeView *tv, GdkDrop *dc,
                              GtkSelectionData *selection_data,
                              signal_user_data_t *ud)
#else
presets_drag_data_received_cb(GtkTreeView *tv, GdkDragContext *dc, gint x,
                              gint y, GtkSelectionData *selection_data,
                              guint info, guint t, signal_user_data_t *ud)
#endif
{
  ...
  drop_pos =
      (GtkTreeViewDropPosition)g_object_get_data(G_OBJECT(tv), "dst-drop-pos");
  g_object_set_data(G_OBJECT(tv), "dst-tree-path", NULL);
  ...
  if (gtk_tree_model_get_iter(dst_model, &dst_iter, dst_treepath)) {
    ...
    case GTK_TREE_VIEW_DROP_BEFORE:
      gtk_tree_store_insert_before(GTK_TREE_STORE(dst_model), &iter, NULL,
                                   &dst_iter);
      break;

    case GTK_TREE_VIEW_DROP_INTO_OR_BEFORE:
      gtk_tree_store_insert(GTK_TREE_STORE(dst_model), &iter, &dst_iter, 0);
      break;

    case GTK_TREE_VIEW_DROP_AFTER:
      gtk_tree_store_insert_after(GTK_TREE_STORE(dst_model), &iter, NULL,
                                  &dst_iter);
      break;

    case GTK_TREE_VIEW_DROP_INTO_OR_AFTER:
      gtk_tree_store_insert_after(GTK_TREE_STORE(dst_model), &iter, &dst_iter,
                                  NULL);
      break;
...
void presets_row_expanded_cb(GtkTreeView *treeview, GtkTreeIter *iter,
                             GtkTreePath *treepath, signal_user_data_t *ud) {
  hb_preset_index_t *path;
  gboolean expanded;
  GhbValue *dict;
...
G_MODULE_EXPORT void
presets_list_selection_changed_cb(GtkTreeSelection *selection,
                                  signal_user_data_t *ud) {
  hb_preset_index_t *path;
  ...
  if (path != NULL) {
    ...
    if (!ghb_dict_get_bool(dict, "Folder")) {
      ...
      set_preset_menu_button_label(ud, path);
      action = G_SIMPLE_ACTION(
          g_action_map_lookup_action(G_ACTION_MAP(ud->app), "preset-reload"));
...
G_MODULE_EXPORT void preset_edited_cb(GtkCellRendererText *cell,
                                      gchar *treepath_s, gchar *text,
                                      signal_user_data_t *ud) {
  GtkTreePath *treepath;
  GtkTreeStore *store;
  GtkTreeView *treeview;
...
G_MODULE_EXPORT void preset_widget_changed_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
}
