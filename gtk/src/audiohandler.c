...
static void ghb_add_audio_to_ui(signal_user_data_t *ud,
                                const GhbValue *settings);
static GhbValue *audio_get_selected_settings(signal_user_data_t *ud,
                                             int *index);
...
static void ghb_sanitize_audio_settings(GhbValue *settings,
                                        GhbValue *asettings) {
  // Sanitize codec
  const char *mux_name;
  ...
  acodec = ghb_select_audio_codec(mux, in_codec, acodec, fallback, copy_mask);
...
static void ghb_adjust_audio_rate_combos(signal_user_data_t *ud,
  ...
  if (asettings != NULL) {
    ...
    ghb_ui_update(ud, "AudioEncoder", ghb_dict_get_value(asettings, "Encoder"));
    ghb_ui_update(ud, "AudioBitrate", ghb_dict_get_value(asettings, "Bitrate"));
    ...
    ghb_ui_update(ud, "AudioMixdown", ghb_dict_get_value(asettings, "Mixdown"));
...
static void enable_quality_widgets(signal_user_data_t *ud,
                                   gboolean quality_enable, int acodec, int sr,
                                   int mix) {
  GtkWidget *widget;
  ...
  if (!quality_sensitive) {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), !quality_sensitive);
  }
...
static void audio_deps(signal_user_data_t *ud, GhbValue *asettings,
                       GtkWidget *widget) {
...
static void audio_update_dialog_widgets(signal_user_data_t *ud,
                                        GhbValue *asettings) {
  if (asettings != NULL) {
    ...
    block_updates = TRUE;
    ghb_ui_update(ud, "AudioTrack", ghb_dict_get_value(asettings, "Track"));
    ghb_ui_update(ud, "AudioEncoder", ghb_dict_get_value(asettings, "Encoder"));
    ghb_ui_update(ud, "AudioBitrate", ghb_dict_get_value(asettings, "Bitrate"));
    ...
    if (qe) {
      ghb_ui_update(ud, "AudioTrackQualityEnable", ghb_bool_value_new(qe));
    } else {
      ghb_ui_update(ud, "AudioTrackBitrateEnable", ghb_bool_value_new(!qe));
    }
    block_updates = FALSE;
  }
...
static GhbValue *audio_add_track(GhbValue *settings, int track, int encoder,
                                 gdouble quality, int bitrate, int samplerate,
                                 int mix, gdouble drc, gdouble gain) {
  GhbValue *asettings;
  GhbValue *atrack;
...
static GhbValue *audio_select_and_add_track(const hb_title_t *title,
                                            GhbValue *settings,
                                            GhbValue *pref_audio,
                                            const char *lang, int pref_index,
                                            int start_track) {
  ...
  if (track >= 0) {
    ...
    select_acodec = ghb_select_audio_codec(mux->format, aconfig->in.codec,
                                           acodec, fallback, copy_mask);

    asettings = audio_add_track(settings, track, select_acodec, quality,
                                bitrate, samplerate, mix, drc, gain);
  }
  return asettings;
}
...
static void audio_refresh_list_row_ui(GtkTreeModel *tm, GtkTreeIter *ti,
  ...
  if (qe && quality != HB_INVALID_AUDIO_QUALITY) {
    char *tmp = ghb_format_quality(_("Quality: "), encoder->codec, quality);
  ...
  info_src = g_strdup_printf(_("<small>%d - %s (%.4gkHz)</small>"), track + 1,
                             s_track, (double)aconfig->in.samplerate / 1000);
  if (aconfig->in.bitrate > 0) {
    info_src_2 = g_strdup_printf(_("Bitrate: %.4gkbps"),
  ...
  if (ghb_audio_is_passthru(encoder->codec)) {
    ...
    info_dst = g_strdup_printf("<small>%s (%s) (%s)</small>", encoder->name,
                               mix->name, s_sr);
  ...
  gtk_tree_store_set(GTK_TREE_STORE(tm), ti,
                     ...
                     0, info_src, 1, "-->", 2, info_dst, 3, "hb-edit", 4,
                     "hb-remove", 5, 0.5, -1);
  ...
  if (info_src_2 != NULL || info_dst_2 != NULL) {
    ...
    gtk_tree_store_set(GTK_TREE_STORE(tm), &cti,
                       // These are displayed in list
                       0, info_src_2, 2, info_dst_2, 5, 0.0, -1);
  } else {
    if (gtk_tree_model_iter_children(tm, &cti, ti)) {
...
static void audio_refresh_list_ui(signal_user_data_t *ud) {
  GhbValue *audio_list;
  GhbValue *asettings;
...
void audio_update_setting(GhbValue *val, const char *name,
                          signal_user_data_t *ud) {
  GhbValue *asettings;
...
G_MODULE_EXPORT void audio_codec_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
...
G_MODULE_EXPORT void audio_track_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
...
G_MODULE_EXPORT void audio_mix_changed_cb(GtkWidget *widget,
                                          signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  audio_update_setting(ghb_widget_value(widget), "Mixdown", ud);
}

G_MODULE_EXPORT void audio_name_changed_cb(GtkWidget *widget,
                                           signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
...
G_MODULE_EXPORT void audio_bitrate_changed_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
  int br;
...
G_MODULE_EXPORT void audio_samplerate_changed_cb(GtkWidget *widget,
                                                 signal_user_data_t *ud) {
  int sr;
...
G_MODULE_EXPORT void audio_quality_radio_changed_cb(GtkWidget *widget,
                                                    signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
...
G_MODULE_EXPORT void audio_passthru_widget_changed_cb(GtkWidget *widget,
                                                      signal_user_data_t *ud) {
...
G_MODULE_EXPORT gchar *format_drc_cb(GtkScale *scale, gdouble val,
                                     signal_user_data_t *ud) {
  if (val < 1.0)
...
G_MODULE_EXPORT void quality_widget_changed_cb(GtkWidget *widget,
                                               gdouble quality,
                                               signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
...
G_MODULE_EXPORT void drc_widget_changed_cb(GtkWidget *widget, gdouble drc,
                                           signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
...
G_MODULE_EXPORT gchar *format_gain_cb(GtkScale *scale, gdouble val,
                                      signal_user_data_t *ud) {
  if (val >= 21.0)
...
G_MODULE_EXPORT void gain_widget_changed_cb(GtkWidget *widget, gdouble gain,
                                            signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
...
void ghb_clear_audio_list_settings(GhbValue *settings) {
  GhbValue *audio_list;
...
void ghb_clear_audio_list_ui(GtkBuilder *builder) {
  GtkTreeView *tv;
  GtkTreeStore *ts;
  GtkTreeSelection *tsel;
...
G_MODULE_EXPORT void audio_list_selection_changed_cb(GtkTreeSelection *ts,
                                                     signal_user_data_t *ud) {
  GtkTreeModel *tm;
  GtkTreeIter ti;
  GhbValue *asettings = NULL;
...
G_MODULE_EXPORT void audio_add_clicked_cb(GtkWidget *xwidget,
                                          signal_user_data_t *ud) {
  ...
  asettings =
      audio_select_and_add_track(title, ud->settings, pref_audio, "und", 0, 0);
...
G_MODULE_EXPORT void audio_add_all_clicked_cb(GtkWidget *xwidget,
                                              signal_user_data_t *ud) {
  int title_id, titleindex;
  const hb_title_t *title;
...
static void audio_edit(GtkTreeView *tv, GtkTreePath *tp,
                       signal_user_data_t *ud) {
  GtkTreeModel *tm;
  GtkTreeSelection *ts;
  GtkTreeIter ti;
...
G_MODULE_EXPORT void audio_edit_clicked_cb(GtkWidget *widget, gchar *path,
                                           signal_user_data_t *ud) {
  GtkTreeView *tv;
  GtkTreePath *tp;
...
G_MODULE_EXPORT void audio_row_activated_cb(GtkTreeView *tv, GtkTreePath *tp,
                                            GtkTreeViewColumn *col,
                                            signal_user_data_t *ud) {
  audio_edit(tv, tp, ud);
}

G_MODULE_EXPORT void audio_remove_clicked_cb(GtkWidget *widget, gchar *path,
                                             signal_user_data_t *ud) {
  GtkTreeView *tv;
  GtkTreePath *tp;
  GtkTreeModel *tm;
...
G_MODULE_EXPORT void audio_reset_clicked_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
...
static GtkListBoxRow *audio_settings_get_row(GtkWidget *widget) {
...
static void audio_def_settings_init_row(GhbValue *adict, GtkWidget *row) {
  GtkWidget *widget;
...
G_MODULE_EXPORT void audio_def_setting_add_cb(GtkWidget *w,
                                              signal_user_data_t *ud);
G_MODULE_EXPORT void audio_def_setting_remove_cb(GtkWidget *w,
                                                 signal_user_data_t *ud);
G_MODULE_EXPORT void audio_def_encoder_changed_cb(GtkWidget *w,
                                                  signal_user_data_t *ud);
G_MODULE_EXPORT void
audio_def_encode_setting_changed_cb(GtkWidget *w, signal_user_data_t *ud);
G_MODULE_EXPORT void audio_def_drc_changed_cb(GtkWidget *w, gdouble drc,
                                              signal_user_data_t *ud);
G_MODULE_EXPORT void audio_def_gain_changed_cb(GtkWidget *w, gdouble gain,
                                               signal_user_data_t *ud);
G_MODULE_EXPORT void audio_def_quality_changed_cb(GtkWidget *w, gdouble quality,
                                                  signal_user_data_t *ud);
G_MODULE_EXPORT void
...
GtkWidget *ghb_create_audio_settings_row(signal_user_data_t *ud) {
  ...
  gtk_widget_set_tooltip_markup(GTK_WIDGET(button),
                                _("Add an audio encoder.\n"
                                  "Each selected source track will be encoded "
                                  "with all selected encoders."));
  ...
  gtk_widget_set_tooltip_markup(
      GTK_WIDGET(combo), _("Set the audio codec to encode this track with."));
  gtk_widget_set_valign(GTK_WIDGET(combo), GTK_ALIGN_CENTER);
  gtk_widget_set_name(GTK_WIDGET(combo), "AudioEncoder");
  gtk_widget_show(GTK_WIDGET(combo));
  g_signal_connect(combo, "changed", (GCallback)audio_def_encoder_changed_cb,
                   ud);
  ...
  g_signal_connect(radio, "toggled",
                   (GCallback)audio_def_quality_enable_changed_cb, ud);
  gtk_widget_show(GTK_WIDGET(radio));
  ghb_box_append_child(vbox, GTK_WIDGET(radio));
  ...
  gtk_widget_set_tooltip_markup(
      GTK_WIDGET(combo), _("Set the bitrate to encode this track with."));
  gtk_widget_set_valign(GTK_WIDGET(combo), GTK_ALIGN_CENTER);
  gtk_widget_set_name(GTK_WIDGET(combo), "AudioBitrate");
  gtk_widget_show(GTK_WIDGET(combo));
  g_signal_connect(combo, "changed",
                   (GCallback)audio_def_encode_setting_changed_cb, ud);
  ...
  const gchar *quality_icons[] = {"weather-storm",
                                  "weather-clear",
                                  "weather-storm",
  ...
  g_signal_connect(scale, "value-changed",
                   (GCallback)audio_def_quality_changed_cb, ud);
  ...
  gtk_widget_set_tooltip_markup(
      GTK_WIDGET(combo), _("Set the mixdown of the output audio track."));
  gtk_widget_set_valign(GTK_WIDGET(combo), GTK_ALIGN_CENTER);
  ...
  gtk_widget_set_tooltip_markup(
      GTK_WIDGET(combo), _("Set the sample rate of the output audio track."));
  gtk_widget_set_valign(GTK_WIDGET(combo), GTK_ALIGN_CENTER);
  ...
  const gchar *gain_icons[] = {"audio-volume-muted", "audio-volume-high",
                               "audio-volume-low", "audio-volume-medium", NULL};
  ...
  gtk_widget_show(GTK_WIDGET(scale));
  g_signal_connect(scale, "value-changed", (GCallback)audio_def_gain_changed_cb,
                   ud);
  ...
  // Audio DRC ComboBox
  const gchar *drc_icons[] = {"audio-input-microphone", NULL};
  ...
  gtk_widget_show(GTK_WIDGET(scale));
  g_signal_connect(scale, "value-changed", (GCallback)audio_def_drc_changed_cb,
                   ud);
  ghb_box_append_child(box3, GTK_WIDGET(scale));
  ...
  g_signal_connect(button, "clicked", (GCallback)audio_def_setting_remove_cb,
                   ud);
...
static void audio_def_setting_update(signal_user_data_t *ud,
                                     GtkWidget *widget) {
...
void audio_add_lang_iter(GtkTreeModel *tm, GtkTreeIter *iter,
                         signal_user_data_t *ud) {
...
G_MODULE_EXPORT void audio_avail_lang_activated_cb(GtkTreeView *tv,
                                                   GtkTreePath *tp,
...
G_MODULE_EXPORT void audio_add_lang_clicked_cb(GtkWidget *widget,
                                               signal_user_data_t *ud) {
  GtkTreeView *avail;
...
void audio_remove_lang_iter(GtkTreeModel *tm, GtkTreeIter *iter,
                            signal_user_data_t *ud) {
  GtkTreeView *avail;
...
G_MODULE_EXPORT void audio_selected_lang_activated_cb(GtkTreeView *tv,
                                                      GtkTreePath *tp,
                                                      GtkTreeViewColumn *column,
                                                      signal_user_data_t *ud) {
  GtkTreeIter iter;
...
G_MODULE_EXPORT void audio_remove_lang_clicked_cb(GtkWidget *widget,
                                                  signal_user_data_t *ud) {
  GtkTreeView *selected;
...
static void audio_quality_update_limits(GtkWidget *widget, int encoder,
                                        gboolean set_default, gdouble value) {
  float low, high, gran;
  int dir;
...
void audio_def_set_limits(signal_user_data_t *ud, GtkWidget *widget,
  ...
  int fallback =
      ghb_settings_audio_encoder_codec(ud->settings, "AudioEncoderFallback");
  ...
  if (sensitive) {
    ...
    audio_quality_update_limits(
        find_widget(GTK_WIDGET(row), "AudioTrackQualityX"), enc, set_default,
        quality);
  }
...
void audio_def_set_all_limits(signal_user_data_t *ud) {
  ...
  gtk_container_foreach(GTK_CONTAINER(list_box), audio_def_set_all_limits_cb,
                        (gpointer)ud);
}

G_MODULE_EXPORT void audio_def_widget_changed_cb(GtkWidget *widget,
                                                 signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_clear_presets_selection(ud);
}

G_MODULE_EXPORT void audio_fallback_widget_changed_cb(GtkWidget *widget,
                                                      signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  GhbValue *audio = ghb_get_job_audio_settings(ud->settings);
  ghb_dict_set(
      audio, "FallbackEncoder",
      ghb_value_dup(ghb_dict_get(ud->settings, "AudioEncoderFallback")));
...
audio_def_quality_enable_changed_cb(GtkWidget *widget, signal_user_data_t *ud) {
  ...
  audio_def_settings_bitrate_show(
      GTK_WIDGET(row), !ghb_dict_get_bool(adict, "AudioTrackQualityEnable"));
  ghb_clear_presets_selection(ud);
}

G_MODULE_EXPORT void audio_def_quality_changed_cb(GtkWidget *widget,
                                                  gdouble quality,
                                                  signal_user_data_t *ud) {

  GtkListBoxRow *row = audio_settings_get_row(widget);
  GtkWidget *quality_label =
      find_widget(GTK_WIDGET(row), "AudioTrackQualityValue");
  gint index = gtk_list_box_row_get_index(row);
...
G_MODULE_EXPORT void audio_def_gain_changed_cb(GtkWidget *widget, gdouble gain,
                                               signal_user_data_t *ud) {
  audio_def_setting_update(ud, widget);
...
G_MODULE_EXPORT void audio_def_drc_changed_cb(GtkWidget *widget, gdouble drc,
                                              signal_user_data_t *ud) {
  audio_def_setting_update(ud, widget);
...
G_MODULE_EXPORT void audio_def_setting_add_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
  GtkListBoxRow *row = audio_settings_get_row(widget);
...
G_MODULE_EXPORT void audio_def_setting_remove_cb(GtkWidget *widget,
                                                 signal_user_data_t *ud) {
  GtkListBoxRow *row = audio_settings_get_row(widget);
  gint index = gtk_list_box_row_get_index(row);
...
G_MODULE_EXPORT void audio_def_encoder_changed_cb(GtkWidget *widget,
                                                  signal_user_data_t *ud) {
  audio_def_setting_update(ud, widget);
...
gboolean ghb_find_lang_row(GtkTreeModel *model, GtkTreeIter *iter,
                           int lang_idx) {
...
G_MODULE_EXPORT void audio_list_toggled_cb(GtkWidget *widget,
                                           signal_user_data_t *ud) {
  gboolean active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  GtkToggleButton *selection =
      GTK_TOGGLE_BUTTON(GHB_WIDGET(ud->builder, "audio_selection_toggle"));
...
G_MODULE_EXPORT void audio_selection_toggled_cb(GtkWidget *widget,
                                                signal_user_data_t *ud) {
  gboolean active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  GtkToggleButton *list =
      GTK_TOGGLE_BUTTON(GHB_WIDGET(ud->builder, "audio_list_toggle"));
