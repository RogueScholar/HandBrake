...
combo_opts_t subtitle_track_sel_opts = {sizeof(d_subtitle_track_sel_opts) /
                                            sizeof(options_map_t),
...
combo_opts_t subtitle_burn_opts = {
    sizeof(d_subtitle_burn_opts) / sizeof(options_map_t), d_subtitle_burn_opts};
...
combo_opts_t audio_track_sel_opts = {sizeof(d_audio_track_sel_opts) /
                                         sizeof(options_map_t),
...
combo_opts_t point_to_point_opts = {sizeof(d_point_to_point_opts) /
                                        sizeof(options_map_t),
...
combo_opts_t when_complete_opts = {
    sizeof(d_when_complete_opts) / sizeof(options_map_t), d_when_complete_opts};
...
combo_opts_t par_opts = {sizeof(d_par_opts) / sizeof(options_map_t),
                         d_par_opts};
...
combo_opts_t alignment_opts = {sizeof(d_alignment_opts) / sizeof(options_map_t),
                               d_alignment_opts};
...
combo_opts_t logging_opts = {sizeof(d_logging_opts) / sizeof(options_map_t),
                             d_logging_opts};
...
combo_opts_t log_longevity_opts = {
    sizeof(d_log_longevity_opts) / sizeof(options_map_t), d_log_longevity_opts};
...
combo_opts_t appcast_update_opts = {sizeof(d_appcast_update_opts) /
                                        sizeof(options_map_t),
...
combo_opts_t vqual_granularity_opts = {sizeof(d_vqual_granularity_opts) /
                                           sizeof(options_map_t),
...
combo_opts_t deint_opts = {sizeof(d_deint_opts) / sizeof(options_map_t),
                           d_deint_opts};
...
combo_opts_t denoise_opts = {sizeof(d_denoise_opts) / sizeof(options_map_t),
                             d_denoise_opts};
...
combo_opts_t sharpen_opts = {sizeof(d_sharpen_opts) / sizeof(options_map_t),
                             d_sharpen_opts};
...
combo_opts_t rotate_opts = {sizeof(d_rotate_opts) / sizeof(options_map_t),
                            d_rotate_opts};
...
combo_opts_t direct_opts = {sizeof(d_direct_opts) / sizeof(options_map_t),
                            d_direct_opts};
...
combo_opts_t badapt_opts = {sizeof(d_badapt_opts) / sizeof(options_map_t),
                            d_badapt_opts};
...
combo_opts_t bpyramid_opts = {sizeof(d_bpyramid_opts) / sizeof(options_map_t),
                              d_bpyramid_opts};
...
combo_opts_t weightp_opts = {sizeof(d_weightp_opts) / sizeof(options_map_t),
                             d_weightp_opts};
...
combo_opts_t me_opts = {sizeof(d_me_opts) / sizeof(options_map_t), d_me_opts};
...
combo_opts_t subme_opts = {sizeof(d_subme_opts) / sizeof(options_map_t),
                           d_subme_opts};
...
combo_opts_t analyse_opts = {sizeof(d_analyse_opts) / sizeof(options_map_t),
                             d_analyse_opts};
...
combo_opts_t trellis_opts = {sizeof(d_trellis_opts) / sizeof(options_map_t),
                             d_trellis_opts};
...
static filter_opts_t deblock_preset_opts = {.filter_id = HB_FILTER_DEBLOCK,
                                            .preset = TRUE};

static filter_opts_t deblock_tune_opts = {.filter_id = HB_FILTER_DEBLOCK,
                                          .preset = FALSE};

static filter_opts_t deint_preset_opts = {.filter_id = HB_FILTER_DECOMB,
                                          .preset = TRUE};

static filter_opts_t nlmeans_preset_opts = {.filter_id = HB_FILTER_NLMEANS,
                                            .preset = TRUE};

static filter_opts_t nlmeans_tune_opts = {.filter_id = HB_FILTER_NLMEANS,
                                          .preset = FALSE};

static filter_opts_t sharpen_preset_opts = {.filter_id = HB_FILTER_UNSHARP,
                                            .preset = TRUE};

static filter_opts_t sharpen_tune_opts = {.filter_id = HB_FILTER_UNSHARP,
                                          .preset = FALSE};
...
static filter_opts_t comb_detect_opts = {.filter_id = HB_FILTER_COMB_DETECT,
                                         .preset = TRUE};

static filter_opts_t detel_opts = {.filter_id = HB_FILTER_DETELECINE,
                                   .preset = TRUE};
...
static void video_framerate_opts_set(signal_user_data_t *ud, const gchar *name,
                                     void *opts, const void *data);
static void mix_opts_set(signal_user_data_t *ud, const gchar *name, void *opts,
                         const void *data);
...
combo_name_map_t combo_name_map[] = {
    {"SubtitleTrackSelectionBehavior", &subtitle_track_sel_opts, small_opts_set,
     generic_opt_get},
    {"SubtitleBurnBehavior", &subtitle_burn_opts, small_opts_set,
     generic_opt_get},
    {"AudioTrackSelectionBehavior", &audio_track_sel_opts, small_opts_set,
     generic_opt_get},
    {"PtoPType", &point_to_point_opts, small_opts_set, generic_opt_get},
    {"WhenComplete", &when_complete_opts, small_opts_set, generic_opt_get},
    {"QueueWhenComplete", &when_complete_opts, small_opts_set, generic_opt_get},
    {"PicturePAR", &par_opts, small_opts_set, generic_opt_get},
    {"PictureModulus", &alignment_opts, small_opts_set, generic_opt_get},
    {"LoggingLevel", &logging_opts, small_opts_set, generic_opt_get},
    {"LogLongevity", &log_longevity_opts, small_opts_set, generic_opt_get},
    {"check_updates", &appcast_update_opts, small_opts_set, generic_opt_get},
    {"VideoQualityGranularity", &vqual_granularity_opts, small_opts_set,
     generic_opt_get},
    {"PictureCombDetectPreset", &comb_detect_opts, filter_opts_set,
     filter_opt_get},
    {"PictureDeinterlaceFilter", &deint_opts, small_opts_set, generic_opt_get},
    {"PictureDeinterlacePreset", &deint_preset_opts, deint_opts_set,
     filter_opt_get},
    {"PictureDetelecine", &detel_opts, filter_opts_set, filter_opt_get},
    {"PictureDeblockPreset", &deblock_preset_opts, filter_opts_set,
     filter_opt_get},
    {"PictureDeblockTune", &deblock_tune_opts, filter_opts_set, filter_opt_get},
    {"PictureDenoiseFilter", &denoise_opts, small_opts_set, generic_opt_get},
    {"PictureDenoisePreset", &nlmeans_preset_opts, denoise_opts_set,
     filter_opt_get},
    {"PictureDenoiseTune", &nlmeans_tune_opts, filter_opts_set, filter_opt_get},
    {"PictureSharpenFilter", &sharpen_opts, small_opts_set, generic_opt_get},
    {"PictureSharpenPreset", &sharpen_preset_opts, sharpen_opts_set,
     filter_opt_get},
    {"PictureSharpenTune", &sharpen_tune_opts, sharpen_opts_set,
     filter_opt_get},
    {"PictureRotate", &rotate_opts, small_opts_set, generic_opt_get},
    {"AudioBitrate", NULL, audio_bitrate_opts_set, NULL},
    {"AudioSamplerate", NULL, audio_samplerate_opts_set, NULL},
    {"VideoFramerate", NULL, video_framerate_opts_set, NULL},
    {"AudioMixdown", NULL, mix_opts_set, NULL},
    {"VideoEncoder", NULL, video_encoder_opts_set, NULL},
    {"AudioEncoder", NULL, audio_encoder_opts_set, NULL},
    {"AudioEncoderFallback", NULL, acodec_fallback_opts_set, NULL},
    {"ImportLanguage", NULL, language_opts_set, NULL},
    {"SrtCodeset", NULL, srt_codeset_opts_set, NULL},
    {"title", NULL, title_opts_set, NULL},
    {"AudioTrack", NULL, audio_track_opts_set, NULL},
    {"SubtitleTrack", NULL, subtitle_track_opts_set, NULL},
    {"VideoTune", NULL, video_tune_opts_set, NULL},
    {"VideoProfile", NULL, video_profile_opts_set, NULL},
    {"VideoLevel", NULL, video_level_opts_set, NULL},
    {"FileFormat", NULL, container_opts_set, NULL},
    {"PresetCategory", NULL, preset_category_opts_set, NULL},
    {NULL, NULL, NULL, NULL}};

const gchar *srt_codeset_table[] = {
    "ANSI_X3.4-1968", "ANSI_X3.4-1986", "ANSI_X3.4",   "ANSI_X3.110-1983",
    "ANSI_X3.110",    "ASCII",          "ECMA-114",    "ECMA-118",
    "ECMA-128",       "ECMA-CYRILLIC",  "IEC_P27-1",   "ISO-8859-1",
    "ISO-8859-2",     "ISO-8859-3",     "ISO-8859-4",  "ISO-8859-5",
    "ISO-8859-6",     "ISO-8859-7",     "ISO-8859-8",  "ISO-8859-9",
    "ISO-8859-9E",    "ISO-8859-10",    "ISO-8859-11", "ISO-8859-13",
    "ISO-8859-14",    "ISO-8859-15",    "ISO-8859-16", "UTF-7",
    "UTF-8",          "UTF-16",         "UTF-16LE",    "UTF-16BE",
    "UTF-32",         "UTF-32LE",       "UTF-32BE",    NULL};
...
static void del_tree(const gchar *name, gboolean del_top) {
  const gchar *file;
...
const gchar *ghb_version() { return hb_get_version(NULL); }
...
void ghb_vquality_range(signal_user_data_t *ud, float *min, float *max,
                        float *step, float *page, gint *digits,
                        int *direction) {
  float min_step;
  gint vcodec;
  vcodec = ghb_settings_video_encoder_codec(ud->settings, "VideoEncoder");
...
gint find_opt_entry(const combo_opts_t *opts, const GhbValue *gval) {
  gint ii;
...
const hb_filter_param_t *find_param_entry(const hb_filter_param_t *param,
                                          const GhbValue *gval) {
  gint ii;
...
static gint lookup_generic_int(const combo_opts_t *opts, const GhbValue *gval) {
  gint ii;
  gint result = -1;
...
static gdouble lookup_generic_double(const combo_opts_t *opts,
                                     const GhbValue *gval) {
  gint ii;
  gdouble result = -1;
...
static const gchar *lookup_generic_option(const combo_opts_t *opts,
                                          const GhbValue *gval) {
  gint ii;
  const gchar *result = "";
...
static gint lookup_param_int(const hb_filter_param_t *param,
                             const GhbValue *gval) {
  gint result = -1;
...
static const gchar *lookup_param_option(const hb_filter_param_t *param,
                                        const GhbValue *gval) {
  const gchar *result = "";
...
gint ghb_find_closest_audio_samplerate(gint irate) {
...
hb_handle_t *ghb_scan_handle(void) { return h_scan; }

hb_handle_t *ghb_queue_handle(void) { return h_queue; }

hb_handle_t *ghb_live_handle(void) { return h_live; }

gchar *ghb_get_tmp_dir() { return hb_get_temporary_directory(); }
...
static gboolean find_combo_item_by_int(GtkTreeModel *store, gint value,
                                       GtkTreeIter *iter);

static void grey_combo_box_item(GtkComboBox *combo, gint value, gboolean grey) {
  GtkListStore *store;
  GtkTreeIter iter;

  store = GTK_LIST_STORE(gtk_combo_box_get_model(combo));
  if (find_combo_item_by_int(GTK_TREE_MODEL(store), value, &iter)) {
    gtk_list_store_set(store, &iter, 1, !grey, -1);
  }
}

static void grey_builder_combo_box_item(GtkBuilder *builder, const gchar *name,
                                        gint value, gboolean grey) {
...
static void grey_mix_opts(signal_user_data_t *ud, gint acodec, gint64 layout) {
  g_debug("grey_mix_opts()\n");

  const hb_mixdown_t *mix;
  for (mix = hb_mixdown_get_next(NULL); mix != NULL;
       mix = hb_mixdown_get_next(mix)) {
    grey_builder_combo_box_item(
        ud->builder, "AudioMixdown", mix->amixdown,
...
static void grey_passthru(signal_user_data_t *ud, hb_audio_config_t *aconfig) {
  ...
  for (enc = hb_audio_encoder_get_next(NULL); enc != NULL;
    ...
    if (!(enc->codec & HB_ACODEC_PASS_FLAG))
      continue;
    if ((enc->codec & HB_ACODEC_MASK) != (aconfig->in.codec & HB_ACODEC_MASK)) {
      grey_builder_combo_box_item(ud->builder, "AudioEncoder", enc->codec,
                                  TRUE);
    }
  }
}
...
void ghb_grey_combo_options(signal_user_data_t *ud) {
  ...
  for (enc = hb_video_encoder_get_next(NULL); enc != NULL;
    ...
    if (!(mux->format & enc->muxers)) {
      grey_builder_combo_box_item(ud->builder, "VideoEncoder", enc->codec,
                                  TRUE);
    } else {
      grey_builder_combo_box_item(ud->builder, "VideoEncoder", enc->codec,
                                  FALSE);
  ...
  acodec = ghb_select_audio_codec(mux->format, in_codec, acodec, fallback,
                                  copy_mask);
...
void ghb_init_combo_box(GtkComboBox *combo) {
  ...
  store = gtk_list_store_new(4, G_TYPE_STRING, G_TYPE_BOOLEAN, G_TYPE_STRING,
                             G_TYPE_DOUBLE);
  ...
  if (!gtk_combo_box_get_has_entry(combo)) {
    ...
    gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combo), cell, "markup", 0,
                                   "sensitive", 1, NULL);
  } else { // Combo box entry
    gtk_combo_box_set_entry_text_column(GTK_COMBO_BOX(combo), 0);
  }
}

// Set up the model for the combo box
static void init_combo_box(GtkBuilder *builder, const gchar *name) {
  GtkComboBox *combo;
...
void ghb_audio_samplerate_opts_set(GtkComboBox *combo) {
  ...
  gtk_list_store_set(store, &iter, 0, str, 1, TRUE, 2, "auto", 3, 0.0, -1);
  g_free(str);
  ...
  for (rate = hb_audio_samplerate_get_next(NULL); rate != NULL;
    ...
    gtk_list_store_set(store, &iter, 0, str, 1, TRUE, 2, rate->name, 3,
                       (gdouble)rate->rate, -1);
    g_free(str);
  }
}
...
void ghb_audio_samplerate_opts_filter(GtkComboBox *combo, gint acodec) {
  ...
  do {
    ...
    gtk_list_store_set(
        store, &iter, 1,
        irate == 0.0 ? TRUE : hb_audio_samplerate_is_supported(irate, acodec),
        -1);
...
int ghb_settings_audio_samplerate_rate(const GhbValue *settings,
                                       const char *name) {
  int result;
...
const hb_rate_t *ghb_settings_audio_samplerate(const GhbValue *settings,
                                               const char *name) {
...
static void video_framerate_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  // Add an item for "Same As Source"
  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter, 0, _("Same as source"), 1, TRUE, 2, "auto",
                     3, 0.0, -1);
  ...
  for (rate = hb_video_framerate_get_next(NULL); rate != NULL;
    ...
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, option, 1, TRUE, 2, rate->name, 3,
                       (gdouble)rate->rate, -1);
    g_free(option);
  }
}
...
int ghb_settings_video_framerate_rate(const GhbValue *settings,
                                      const char *name) {
  int result;
  char *rate_id = ghb_dict_get_string_xform(settings, name);
...
const hb_rate_t *ghb_settings_video_framerate(const GhbValue *settings,
                                              const char *name) {
  const hb_rate_t *result;
  char *rate_id = ghb_dict_get_string_xform(settings, name);
...
static void video_encoder_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  for (enc = hb_video_encoder_get_next(NULL); enc != NULL;
    ...
    gtk_list_store_set(store, &iter, 0, str, 1, TRUE, 2, enc->short_name, 3,
                       (gdouble)enc->codec, -1);
    g_free(str);
  }
}
...
int ghb_settings_video_encoder_codec(const GhbValue *settings,
                                     const char *name) {
  const char *encoder_id = ghb_dict_get_string(settings, name);
  return ghb_lookup_video_encoder_codec(encoder_id);
}

const hb_encoder_t *ghb_settings_video_encoder(const GhbValue *settings,
                                               const char *name) {
  const char *encoder_id = ghb_dict_get_string(settings, name);
  return ghb_lookup_video_encoder(encoder_id);
}

void ghb_audio_encoder_opts_set_with_mask(GtkComboBox *combo, int mask,
                                          int neg_mask) {
  GtkTreeIter iter;
  GtkListStore *store;
  gchar *str;
...
int ghb_settings_audio_encoder_codec(const GhbValue *settings,
                                     const char *name) {
  const char *encoder_id = ghb_dict_get_string(settings, name);
  return ghb_lookup_audio_encoder_codec(encoder_id);
}

const hb_encoder_t *ghb_settings_audio_encoder(const GhbValue *settings,
                                               const char *name) {
  const char *encoder_id = ghb_dict_get_string(settings, name);
  return ghb_lookup_audio_encoder(encoder_id);
}

static void audio_encoder_opts_set_with_mask(GtkBuilder *builder,
                                             const gchar *name, int mask,
                                             int neg_mask) {
  GtkComboBox *combo = GTK_COMBO_BOX(GHB_WIDGET(builder, name));
...
void ghb_mix_opts_set(GtkComboBox *combo) {
  ...
  for (mix = hb_mixdown_get_next(NULL); mix != NULL;
    ...
    gtk_list_store_set(store, &iter, 0, str, 1, TRUE, 2, mix->short_name, 3,
                       (gdouble)mix->amixdown, -1);
    g_free(str);
  }
}
...
const hb_mixdown_t *ghb_settings_mixdown(const GhbValue *settings,
                                         const char *name) {
  const char *mixdown_id = ghb_dict_get_string(settings, name);
...
static void container_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  for (mux = hb_container_get_next(NULL); mux != NULL;
    ...
    gtk_list_store_set(store, &iter, 0, str, 1, TRUE, 2, mux->short_name, 3,
                       (gdouble)mux->format, -1);
    g_free(str);
  }
}
...
static void preset_category_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  for (ii = 0, jj = 0; ii < count; ii++) {
    ...
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, name, 1, TRUE, 2, name, 3, (gdouble)ii,
                       -1);
  }
  g_strfreev(categories);

  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter, 0, "Add New Category", 1, TRUE, 2, "new", 3,
                     -1.0, -1);
}
...
static void srt_codeset_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  for (ii = 0; ii < SRT_TABLE_SIZE; ii++) {
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, srt_codeset_table[ii], 1, TRUE, 2,
                       srt_codeset_table[ii], 3, (gdouble)ii, -1);
  }
}
...
static void language_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  for (iso639 = lang_get_next(NULL); iso639 != NULL;
    ...
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, lang, 1, TRUE, 2, iso639->iso639_2, 3,
                       (gdouble)index, -1);
    g_free(lang);
  }
...
static void ghb_dvd_sanitize_volname(gchar *name) {
  gchar *a, *b;
...
gchar *ghb_create_source_label(const hb_title_t *title) {
  ...
  if (title != NULL && title->name != NULL && title->name[0] != 0) {
    ...
    if (title->type == HB_BD_TYPE) {
      source = g_strdup_printf(_("%s - (%05d.MPLS)"), volname, title->playlist);
      g_free(volname);
    } else {
      source = volname;
    }
...
static gboolean uppers_and_unders(gchar *str) {
  if (str == NULL)
    return FALSE;
...
enum { CAMEL_FIRST_UPPER, CAMEL_OTHER };

static void camel_convert(gchar *str) {
...
static gchar *get_file_label(const gchar *filename) {
  gchar *base, *pos, *end;
...
gchar *ghb_create_title_label(const hb_title_t *title) {
  ...
  if (title->type == HB_STREAM_TYPE || title->type == HB_FF_STREAM_TYPE) {
    if (title->duration != 0) {
      char *tmp;
      tmp = g_strdup_printf(_("%3d - %02dh%02dm%02ds - %s"), title->index,
      ...
      char *tmp;
      tmp = g_strdup_printf("%3d - %s", title->index, title->name);
      label = g_markup_escape_text(tmp, -1);
      g_free(tmp);
    }
    ...
    if (title->duration != 0) {
      ...
      label = g_strdup_printf(_("%3d - (%05d.MPLS)"), title->index,
                              title->playlist);
    }
  } else {
    label = g_strdup_printf(_("%3d - %02dh%02dm%02ds"), title->index,
                            title->hours, title->minutes, title->seconds);
  }
  return label;
}
...
static void title_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  if (count <= 0) {
    ...
    gtk_list_store_set(store, &iter, 0, opt, 1, TRUE, 2, "none", 3, -1.0, -1);
    g_free(opt);
    return;
  }
  for (ii = 0; ii < count; ii++) {
    ...
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, opt, 1, TRUE, 2, title_index, 3,
                       (gdouble)title->index, -1);
    g_free(opt);
    g_free(title_opt);
    g_free(title_index);
  }
...
static int lookup_title_index(hb_handle_t *h, int title_id) {
  if (h == NULL)
    return -1;
...
const hb_title_t *lookup_title(hb_handle_t *h, int title_id, int *index) {
...
static void video_tune_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter, 0, _("None"), 1, TRUE, 2, "none", 3,
                     (gdouble)0, -1);

  for (ii = 0; ii < count; ii++) {
    if (((encoder & HB_VCODEC_X264_MASK) && !strcmp(tunes[ii], "fastdecode")) ||
        ((encoder & (HB_VCODEC_X264_MASK | HB_VCODEC_X265_MASK)) &&
         !strcmp(tunes[ii], "zerolatency"))) {
      continue;
    }
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, tunes[ii], 1, TRUE, 2, tunes[ii], 3,
                       (gdouble)ii + 1, -1);
  }
}
...
static void video_profile_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  for (ii = 0; ii < count; ii++) {
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, profiles[ii], 1, TRUE, 2, profiles[ii],
                       3, (gdouble)ii, -1);
  }
}
...
static void video_level_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  for (ii = 0; ii < count; ii++) {
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, levels[ii], 1, TRUE, 2, levels[ii], 3,
                       (gdouble)ii, -1);
  }
}
...
void audio_track_opts_set(signal_user_data_t *ud, const gchar *name, void *opts,
                          const void *data) {
  (void)opts; // Silence "unused variable" warning
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    opt = g_strdup_printf("<small>%d - %s</small>", ii + 1,
                          audio->lang.description);
    snprintf(idx, 4, "%d", ii);

    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, opt, 1, TRUE, 2, idx, 3, (gdouble)ii,
                       -1);
    g_free(opt);
  }
...
static void subtitle_track_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  if (count <= 0) {
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, _("None"), 1, TRUE, 2, "0", 3, 0.0, -1);
  }
  gtk_combo_box_set_active(combo, 0);
}
...
gint ghb_find_audio_track(const hb_title_t *title, const gchar *lang,
                          int start) {
  hb_audio_config_t *audio;
  gint ii, count = 0;
...
gint ghb_find_subtitle_track(const hb_title_t *title, const gchar *lang,
  ...
  for (ii = start; ii < count; ii++) {
    ...
    if (strcmp(lang, subtitle->iso639_2) == 0 || strcmp(lang, "und") == 0) {
      return ii;
    }
  }
  return -1;
...
static void small_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  for (ii = 0; ii < opts->count; ii++) {
    gtk_list_store_append(store, &iter);
    str = g_strdup_printf("<small>%s</small>", gettext(opts->map[ii].option));
    gtk_list_store_set(store, &iter, 0, str, 1, TRUE, 2, opts->map[ii].shortOpt,
                       3, opts->map[ii].ivalue, -1);
    g_free(str);
  }
}

static void filter_opts_set2(signal_user_data_t *ud, const gchar *name,
  ...
  for (ii = 0; param != NULL && param[ii].name != NULL; ii++) {
    gtk_list_store_append(store, &iter);
    str = g_strdup_printf("<small>%s</small>", gettext(param[ii].name));
    gtk_list_store_set(store, &iter, 0, str, 1, TRUE, 2, param[ii].short_name,
                       3, (double)param[ii].index, -1);
    g_free(str);
  }
}
...
static void deint_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  filter_opts_t *opts = (filter_opts_t *)vopts;
  opts->filter_id =
      ghb_settings_combo_int(ud->settings, "PictureDeinterlaceFilter");
  filter_opts_set2(ud, name, opts->filter_id, opts->preset);

  ghb_set_custom_filter_tooltip(ud, "PictureDeinterlaceCustom", "deinterlace",
                                opts->filter_id);
}
...
static void denoise_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  filter_opts_t *opts = (filter_opts_t *)vopts;
  opts->filter_id =
      ghb_settings_combo_int(ud->settings, "PictureDenoiseFilter");
  filter_opts_set2(ud, name, opts->filter_id, opts->preset);

  ghb_set_custom_filter_tooltip(ud, "PictureDenoiseCustom", "denoise",
                                opts->filter_id);
}
...
static void sharpen_opts_set(signal_user_data_t *ud, const gchar *name,
  ...
  filter_opts_t *opts = (filter_opts_t *)vopts;
  opts->filter_id =
      ghb_settings_combo_int(ud->settings, "PictureSharpenFilter");
  filter_opts_set2(ud, name, opts->filter_id, opts->preset);

  ghb_set_custom_filter_tooltip(ud, "PictureSharpenCustom", "sharpen",
                                opts->filter_id);
}

combo_name_map_t *find_combo_map(const gchar *name) {
  gint ii;
...
combo_opts_t *find_combo_opts(const gchar *name) {
...
static GhbValue *generic_opt_get(const char *name, const void *vopts,
                                 const GhbValue *gval, GhbType type) {
  combo_opts_t *opts = (combo_opts_t *)vopts;
...
static GhbValue *filter_opt_get2(const char *name, const GhbValue *gval,
                                 GhbType type, int filter_id, int preset) {
  GhbValue *result = NULL;
  hb_filter_param_t *param;
...
static GhbValue *filter_opt_get(const char *name, const void *vopts,
                                const GhbValue *gval, GhbType type) {
  filter_opts_t *opts = (filter_opts_t *)vopts;
  return filter_opt_get2(name, gval, type, opts->filter_id, opts->preset);
}

static GhbValue *lookup_combo_value(const gchar *name, const GhbValue *gval,
                                    GhbType type) {
  combo_name_map_t *entry = find_combo_map(name);
  if (entry != NULL) {
...
char *ghb_lookup_combo_option(const gchar *name, const GhbValue *gval) {
  if (gval == NULL)
    return NULL;
...
void ghb_update_ui_combo_box(signal_user_data_t *ud, const gchar *name,
...
static void init_ui_combo_boxes(GtkBuilder *builder) {
  gint ii;
...
gint64 ghb_chapter_range_get_duration(const hb_title_t *title, gint sc,
                                      gint ec) {
...
void ghb_audio_bitrate_opts_filter(GtkComboBox *combo, gint first_rate,
                                   gint last_rate) {
  GtkTreeIter iter;
  GtkListStore *store;
  gdouble ivalue;
...
int ghb_settings_audio_bitrate_rate(const GhbValue *settings,
                                    const char *name) {
  int result;
  char *rate_id = ghb_dict_get_string_xform(settings, name);
...
const hb_rate_t *ghb_settings_audio_bitrate(const GhbValue *settings,
                                            const char *name) {
  const hb_rate_t *result;
  char *rate_id = ghb_dict_get_string_xform(settings, name);
...
void ghb_combo_init(signal_user_data_t *ud) {
  ...
  g_signal_connect(econ, "key-pressed", G_CALLBACK(combo_search_key_press_cb),
                   ud);
#endif
}
...
void ghb_backend_scan_stop() { hb_scan_stop(h_scan); }
...
void ghb_backend_queue_scan(const gchar *path, gint titlenum) {
  g_debug("ghb_backend_queue_scan()");
  hb_scan(h_queue, path, titlenum, -1, 0, 0);
  hb_status.queue.state |= GHB_STATE_SCANNING;
}

gint ghb_get_scan_state() { return hb_status.scan.state; }

gint ghb_get_queue_state() { return hb_status.queue.state; }

void ghb_clear_scan_state(gint state) { hb_status.scan.state &= ~state; }

void ghb_clear_live_state(gint state) { hb_status.live.state &= ~state; }

void ghb_clear_queue_state(gint state) { hb_status.queue.state &= ~state; }

void ghb_set_scan_state(gint state) { hb_status.scan.state |= state; }

void ghb_set_queue_state(gint state) { hb_status.queue.state |= state; }

void ghb_get_status(ghb_status_t *status) {
  memcpy(status, &hb_status, sizeof(ghb_status_t));
}

static void update_status(hb_state_t *state, ghb_instance_status_t *status) {
...
#define p state->param.working
  if (state->state &
      (HB_STATE_WORKING | HB_STATE_WORKDONE | HB_STATE_SEARCHING)) {
    status->pass = p.pass;
...
gint ghb_get_default_acodec() { return HB_ACODEC_FFAAC; }
...
void ghb_picture_settings_deps(signal_user_data_t *ud) {
  ...
  enable_keep_aspect =
      (pic_par != HB_ANAMORPHIC_STRICT && pic_par != HB_ANAMORPHIC_AUTO &&
...
void ghb_limit_rational(gint *num, gint *den, gint limit) {
...
void ghb_set_scale_settings(GhbValue *settings, gint mode) {
  ...
  if (pic_par == HB_ANAMORPHIC_STRICT || pic_par == HB_ANAMORPHIC_AUTO ||
      pic_par == HB_ANAMORPHIC_LOOSE) {
  ...
  if (autoscale) {
    ...
    if (mode & GHB_PIC_USE_MAX) {
      max_width = MOD_DOWN(ghb_dict_get_int(settings, "PictureWidth"), mod);
      max_height = MOD_DOWN(ghb_dict_get_int(settings, "PictureHeight"), mod);
    }
  }
    ...
    if (pic_par == HB_ANAMORPHIC_CUSTOM && !keep_aspect) {
      if (mode & GHB_PIC_KEEP_PAR) {
        ...
        uiGeo.geometry.par.den = ghb_dict_get_int(settings, "PicturePARHeight");
                 ...
                 (GHB_PIC_KEEP_DISPLAY_HEIGHT | GHB_PIC_KEEP_DISPLAY_WIDTH)) {
  ...
  disp_width =
      ((gdouble)resultGeo.par.num / resultGeo.par.den) * resultGeo.width + 0.5;
...
static void get_preview_geometry(signal_user_data_t *ud,
                                 const hb_title_t *title, hb_geometry_t *srcGeo,
                                 hb_geometry_settings_t *uiGeo) {
  srcGeo->width = title->geometry.width;
  srcGeo->height = title->geometry.height;
  srcGeo->par = title->geometry.par;
...
const char *ghb_lookup_filter_name(int filter_id, const char *short_name,
                                   int preset) {
...
void ghb_set_custom_filter_tooltip(signal_user_data_t *ud, const char *name,
                                   const char *desc, int filter_id) {
  ...
  for (ii = 0; keys[ii] != NULL && pos < 1024; ii++) {
    ...
    pos += snprintf(tooltip + pos, 1024 - pos, "%s%s%s=%c", colon, newline,
                    keys[ii], c);
    linelen += len;
    colon = ":";
  }
...
gboolean ghb_validate_filters(GhbValue *settings, GtkWindow *parent) {
  ...
  if (strcasecmp(detel_preset, "off")) {
    ...
    if (hb_validate_filter_preset(filter_id, detel_preset, NULL,
      ...
      ghb_message_dialog(parent, GTK_MESSAGE_ERROR, message, _("Cancel"), NULL);
      g_free(message);
      return FALSE;
    }
  }
  ...
  if (strcasecmp(comb_preset, "off")) {
    ...
    if (hb_validate_filter_preset(filter_id, comb_preset, NULL, comb_custom)) {
      if (comb_custom != NULL && comb_custom[0] != 0) {
        message = g_strdup_printf(_("Invalid Comb Detect Settings:\n\n"
                                    "Preset:\t%s\n"
                                    "Custom:\t%s\n"),
  ...
  if (filter_id != HB_FILTER_INVALID) {
    ...
    deint_filter = ghb_dict_get_string(settings, "PictureDeinterlaceFilter");
    deint_preset = ghb_dict_get_string(settings, "PictureDeinterlacePreset");
    deint_custom = ghb_dict_get_string(settings, "PictureDeinterlaceCustom");
    if (hb_validate_filter_preset(filter_id, deint_preset, NULL,
      ...
      if (deint_custom != NULL) {
        message = g_strdup_printf(_("Invalid Deinterlace Settings:\n\n"
                                    ...
                                    "Preset:\t%s\n"
                                    "Custom:\t%s\n"),
                                  deint_filter, deint_preset, deint_custom);
      } else {
        message = g_strdup_printf(_("Invalid Deinterlace Settings:\n\n"
                                    "Filter:\t%s\n"
                                    "Preset:\t%s\n"),
  ...
  if (filter_id != HB_FILTER_INVALID) {
    ...
    if (hb_validate_filter_preset(filter_id, denoise_preset, denoise_tune,
                                  denoise_custom)) {
      message = g_strdup_printf(_("Invalid Denoise Settings:\n\n"
                                  "Filter:\t%s\n"
                                  "Preset:\t%s\n"
                                  "Tune:\t%s\n"
                                  "Custom:\t%s\n"),
                                denoise_filter, denoise_preset, denoise_tune,
                                denoise_custom);
      ghb_message_dialog(parent, GTK_MESSAGE_ERROR, message, _("Cancel"), NULL);
      g_free(message);
      return FALSE;
  ...
  if (filter_id != HB_FILTER_INVALID) {
    ...
    if (hb_validate_filter_preset(filter_id, sharpen_preset, sharpen_tune,
                                  sharpen_custom)) {
      message = g_strdup_printf(_("Invalid Sharpen Settings:\n\n"
                                  "Filter:\t%s\n"
                                  "Preset:\t%s\n"
                                  "Tune:\t%s\n"
                                  "Custom:\t%s\n"),
                                sharpen_filter, sharpen_preset, sharpen_tune,
                                sharpen_custom);
      ghb_message_dialog(parent, GTK_MESSAGE_ERROR, message, _("Cancel"), NULL);
      g_free(message);
      return FALSE;
...
gboolean ghb_validate_video(GhbValue *settings, GtkWindow *parent) {
  ...
  if ((mux->format & HB_MUX_MASK_MP4) && (vcodec == HB_VCODEC_THEORA)) {
    ...
    v_unsup = TRUE;
             ...
             (vcodec != HB_VCODEC_FFMPEG_VP8 &&
              vcodec != HB_VCODEC_FFMPEG_VP9)) {
  ...
  if (v_unsup) {
    if (!ghb_message_dialog(parent, GTK_MESSAGE_WARNING, message, _("Cancel"),
                            _("Continue"))) {
      g_free(message);
      return FALSE;
    }
...
gboolean ghb_validate_audio(GhbValue *settings, GtkWindow *parent) {
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    if (ghb_audio_is_passthru(codec) &&
      ...
      message =
          g_strdup_printf(_("The source does not support Pass-Thru.\n\n"
    ...
    if (a_unsup) {
      message =
          g_strdup_printf(_("%s is not supported in the %s container.\n\n"
                            "You should choose a different audio codec.\n"
                            "If you continue, one will be chosen for you."),
                          a_unsup, mux_s);
      if (!ghb_message_dialog(parent, GTK_MESSAGE_WARNING, message, _("Cancel"),
...
int ghb_add_job(hb_handle_t *h, GhbValue *job_dict) {
  char *json_job;
  int sequence_id;
...
void ghb_start_queue() { hb_start(h_queue); }

void ghb_stop_queue() { hb_stop(h_queue); }

void ghb_start_live_encode() { hb_start(h_live); }

void ghb_stop_live_encode() { hb_stop(h_live); }
...
void ghb_resume_queue() {
  hb_status.queue.state &= ~GHB_STATE_PAUSED;
  hb_resume(h_queue);
}

void ghb_pause_resume_queue() {
  hb_state_t s;
...
static void vert_line(GdkPixbuf *pb, guint8 r, guint8 g, guint8 b, gint x,
                      gint y, gint len, gint width) {
...
static void horz_line(GdkPixbuf *pb, guint8 r, guint8 g, guint8 b, gint x,
                      gint y, gint len, gint width) {
  guint8 *pixels = gdk_pixbuf_get_pixels(pb);
  guint8 *dst;
  gint ii, jj;
...
static void hash_pixbuf(GdkPixbuf *pb, gint x, gint y, gint w, gint h,
                        gint step, gint orientation) {
  gint ii, jj;
  gint line_width = 8;
  struct {
    guint8 r;
  ...
  if (!orientation) {
    ...
    for (ii = x, jj = 0; ii + line_width < x + w; ii += step, jj++) {
      vert_line(pb, c[jj & 3].r, c[jj & 3].g, c[jj & 3].b, ii, y, h,
                line_width);
    }
  } else {
    // horizontal lines
    for (ii = y, jj = 0; ii + line_width < y + h; ii += step, jj++) {
      horz_line(pb, c[jj & 3].r, c[jj & 3].g, c[jj & 3].b, x, ii, w,
                line_width);
    }
  }
}

GdkPixbuf *ghb_get_preview_image(const hb_title_t *title, gint index,
                                 signal_user_data_t *ud, gint *out_width,
                                 gint *out_height) {
  hb_geometry_t srcGeo, resultGeo;
  hb_geometry_settings_t uiGeo;
  ...
  preview =
      gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, image->width, image->height);
  ...
  *out_height = ghb_dict_get_int(ud->settings, "scale_height");
  ghb_par_scale(ud, out_width, out_height, resultGeo.par.num,
                resultGeo.par.den);
  ...
  if (ghb_dict_get_bool(ud->prefs, "reduce_hd_preview")) {
    ...
    if (s_w > 0 && s_h > 0 &&
      ...
      scaled_preview = gdk_pixbuf_scale_simple(preview, previewWidth,
                                               previewHeight, GDK_INTERP_HYPER);
