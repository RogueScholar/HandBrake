...
static void subtitle_refresh_list_row_ui(GtkTreeModel *tm, GtkTreeIter *ti,
                                         GhbValue *settings,
                                         GhbValue *subsettings) {
  GtkTreeIter cti;
  ...
  g_string_append_printf(str, "%s ",
                         burned ? "Burned Into Video" : "Passthrough");
  if (forced) {
...
static void subtitle_refresh_list_ui_from_settings(signal_user_data_t *ud,
                                                   GhbValue *settings) {
  GtkTreeView *tv;
  GtkTreeModel *tm;
  GtkTreeIter ti;
...
static void subtitle_refresh_list_ui(signal_user_data_t *ud) {
  subtitle_refresh_list_ui_from_settings(ud, ud->settings);
}

static void subtitle_exclusive_burn_settings(GhbValue *settings, gint index) {
...
static void subtitle_exclusive_burn(signal_user_data_t *ud, gint index) {
  subtitle_exclusive_burn_settings(ud->settings, index);
  subtitle_refresh_list_ui(ud);
}

static void subtitle_exclusive_default_settings(GhbValue *settings,
                                                gint index) {
  GhbValue *subtitle_list;
  GhbValue *subtitle;
  gint ii, count;
...
static void subtitle_exclusive_default(signal_user_data_t *ud, gint index) {
...
static void subtitle_add_to_settings(GhbValue *settings,
                                     GhbValue *subsettings) {
  // Add the current subtitle settings to the list.
  GhbValue *subtitle_list;
  gint count;
...
static char *subtitle_get_track_description(GhbValue *settings,
  ...
  if (import != NULL) {
    ...
    if (g_file_test(filename, G_FILE_TEST_IS_REGULAR)) {
      ...
      if (source == IMPORTSRT) {
        desc = g_strdup_printf("%s (%s)(%s)(%s)", lang, code, format, basename);
      } else {
...
static GhbValue *subtitle_add_track(signal_user_data_t *ud, GhbValue *settings,
                                    int track, int mux, gboolean default_track,
                                    gboolean import, int source, gboolean burn,
                                    gboolean *burned) {
...
void ghb_set_pref_subtitle(signal_user_data_t *ud) {
...
static GhbValue *subtitle_get_selected_settings(signal_user_data_t *ud,
                                                int *index) {
  GtkTreeView *tv;
  GtkTreeSelection *ts;
  GtkTreeModel *tm;
...
static void subtitle_update_dialog_widgets(signal_user_data_t *ud,
                                           GhbValue *subsettings) {
  GtkWidget *widget;
  ...
  if (subsettings != NULL) {
    ...
    gtk_widget_set_sensitive(widget, hb_subtitle_can_pass(source, mux->format));
...
static GhbValue *subtitle_update_setting(GhbValue *val, const char *name,
...
G_MODULE_EXPORT void subtitle_track_changed_cb(GtkWidget *widget,
  ...
  subsettings = subtitle_update_setting(ghb_int_value_new(track), "Track", ud);
  if (subsettings != NULL) {
    subtitle_update_dialog_widgets(ud, subsettings);
  }
}

G_MODULE_EXPORT void subtitle_name_changed_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
...
G_MODULE_EXPORT void subtitle_forced_toggled_cb(GtkWidget *widget,
                                                signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
...
G_MODULE_EXPORT void subtitle_burned_toggled_cb(GtkWidget *widget,
                                                signal_user_data_t *ud) {
  GhbValue *subsettings;
  int index;
...
G_MODULE_EXPORT void subtitle_default_toggled_cb(GtkWidget *widget,
                                                 signal_user_data_t *ud) {
  GhbValue *subsettings;
  int index;
...
G_MODULE_EXPORT void subtitle_import_radio_toggled_cb(GtkWidget *widget,
                                                      signal_user_data_t *ud) {
  GhbValue *subsettings;
  ...
  if (subsettings != NULL) {
    if (ghb_dict_get_bool(ud->settings, "SubtitleSrtEnable") ||
      ...
      ghb_dict_set_string(
          import, "Format",
          hb_dict_get_bool(ud->settings, "SubtitleSrtEnable") ? "SRT" : "SSA");
    } else {
...
G_MODULE_EXPORT void import_setting_update(GhbValue *val, const char *name,
                                           signal_user_data_t *ud) {
  GhbValue *subsettings;
  subsettings = subtitle_get_selected_settings(ud, NULL);
  if (subsettings != NULL) {
...
G_MODULE_EXPORT void import_offset_changed_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  GhbValue *val = ghb_widget_value(widget);
...
G_MODULE_EXPORT void srt_codeset_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
...
G_MODULE_EXPORT void import_file_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
...
G_MODULE_EXPORT void import_lang_changed_cb(GtkWidget *widget,
                                            signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_check_dependency(ud, widget, NULL);
...
static void add_to_subtitle_list_ui(signal_user_data_t *ud,
                                    GhbValue *subsettings) {
  GtkTreeView *tv;
  GtkTreeIter ti;
  GtkTreeModel *tm;
...
subtitle_list_selection_changed_cb(GtkTreeSelection *ts,
                                   signal_user_data_t *ud) {
  GtkTreeModel *tm;
  GtkTreeIter iter;
  GhbValue *subsettings;
...
G_MODULE_EXPORT void subtitle_add_clicked_cb(GtkWidget *xwidget,
  ...
  for (subsettings = NULL, track = 0; subsettings == NULL && track < count;
       track++) {
    subsettings = subtitle_add_track(ud, ud->settings, track, mux->format,
                                     FALSE, FALSE, VOBSUB, FALSE, &one_burned);
  }
  if (subsettings == NULL) {
    subsettings = subtitle_add_track(ud, ud->settings, 0, mux->format, FALSE,
                                     TRUE, IMPORTSRT, FALSE, &one_burned);
  }
  if (subsettings != NULL) {
    ...
    if (response != GTK_RESPONSE_OK) {
      ...
      ghb_dict_set(ghb_get_job_settings(ud->settings), "Subtitle", backup);
      subtitle_refresh_list_ui(ud);
    } else {
...
G_MODULE_EXPORT void subtitle_add_fas_clicked_cb(GtkWidget *xwidget,
                                                 signal_user_data_t *ud) {
...
G_MODULE_EXPORT void subtitle_add_all_clicked_cb(GtkWidget *xwidget,
                                                 signal_user_data_t *ud) {
  // Add the current subtitle settings to the list.
  ...
  for (track = 0; track < count; track++) {
    subtitle_add_track(ud, ud->settings, track, mux->format, FALSE, FALSE,
                       VOBSUB, FALSE, &one_burned);
  }
  subtitle_refresh_list_ui(ud);
  ghb_update_summary_info(ud);
  ghb_live_reset(ud);
...
G_MODULE_EXPORT void subtitle_reset_clicked_cb(GtkWidget *xwidget,
                                               signal_user_data_t *ud) {
...
G_MODULE_EXPORT void subtitle_def_widget_changed_cb(GtkWidget *widget,
                                                    signal_user_data_t *ud) {
  ghb_widget_to_setting(ud->settings, widget);
  ghb_clear_presets_selection(ud);
}

static void subtitle_update_pref_lang(signal_user_data_t *ud,
  ...
  if (lang != NULL) {
    ...
    if (strncmp(code, "und", 4)) {
      name = lang->native_name && lang->native_name[0] ? lang->native_name
                                                       : lang->eng_name;
    }
  }
  ...
  button =
      GTK_BUTTON(GHB_WIDGET(ud->builder, "SubtitleAddForeignAudioSubtitle"));
  if (sensitive) {
    str = g_strdup_printf(_("Add %s subtitle track if default audio is not %s"),
                          name, name);
  } else {
    str = g_strdup_printf(_(
        "Add subtitle track if default audio is not your preferred language"));
  }
  gtk_button_set_label(button, str);
  g_free(str);

  gtk_widget_set_sensitive(GTK_WIDGET(button), sensitive);
  button = GTK_BUTTON(GHB_WIDGET(ud->builder, "SubtitleAddForeignAudioSearch"));
  gtk_widget_set_sensitive(GTK_WIDGET(button), sensitive);
}
...
void subtitle_add_lang_iter(GtkTreeModel *tm, GtkTreeIter *iter,
                            signal_user_data_t *ud) {
...
G_MODULE_EXPORT void subtitle_avail_lang_activated_cb(GtkTreeView *tv,
                                                      GtkTreePath *tp,
...
G_MODULE_EXPORT void subtitle_add_lang_clicked_cb(GtkWidget *widget,
                                                  signal_user_data_t *ud) {
  GtkTreeView *avail;
...
void subtitle_remove_lang_iter(GtkTreeModel *tm, GtkTreeIter *iter,
                               signal_user_data_t *ud) {
  GtkTreeView *avail;
...
G_MODULE_EXPORT void subtitle_remove_lang_clicked_cb(GtkWidget *widget,
                                                     signal_user_data_t *ud) {
  GtkTreeView *selected;
...
static void subtitle_edit(GtkTreeView *tv, GtkTreePath *tp,
                          signal_user_data_t *ud) {
  GtkTreeModel *tm;
  GtkTreeSelection *ts;
  GtkTreeIter ti;
...
G_MODULE_EXPORT void subtitle_edit_clicked_cb(GtkWidget *widget, gchar *path,
                                              signal_user_data_t *ud) {
  GtkTreeView *tv;
  GtkTreePath *tp;
...
G_MODULE_EXPORT void subtitle_row_activated_cb(GtkTreeView *tv, GtkTreePath *tp,
                                               GtkTreeViewColumn *col,
                                               signal_user_data_t *ud) {
  subtitle_edit(tv, tp, ud);
}

G_MODULE_EXPORT void subtitle_remove_clicked_cb(GtkWidget *widget, gchar *path,
                                                signal_user_data_t *ud) {
  GtkTreeView *tv;
  GtkTreePath *tp;
  GtkTreeModel *tm;
...
G_MODULE_EXPORT void subtitle_list_toggled_cb(GtkWidget *widget,
                                              signal_user_data_t *ud) {
  gboolean active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  GtkToggleButton *selection =
      GTK_TOGGLE_BUTTON(GHB_WIDGET(ud->builder, "subtitle_selection_toggle"));
...
G_MODULE_EXPORT void subtitle_selection_toggled_cb(GtkWidget *widget,
                                                   signal_user_data_t *ud) {
  gboolean active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  GtkToggleButton *list =
      GTK_TOGGLE_BUTTON(GHB_WIDGET(ud->builder, "subtitle_list_toggle"));
