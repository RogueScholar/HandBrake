...
- (void)applyPreset:(HBPreset *)preset {
  NSAssert(self.title,
           @"HBJob: calling applyPreset: without a valid title loaded");

  self.presetName = preset.name;
  NSDictionary *jobSettings = [self.title jobSettingsWithPreset:preset];

  self.container =
      hb_container_get_from_name([preset[@"FileFormat"] UTF8String]);
...
- (void)writeToPreset:(HBMutablePreset *)preset {
  ...
  [@[ self.video, self.filters, self.picture, self.audio, self.subtitles ]
      makeObjectsPerformSelector:@selector(writeToPreset:)
                      withObject:preset];
}
...
- (void)setUndo:(NSUndoManager *)undo {
  _undo = undo;
  [@[
    self.video, self.range, self.filters, self.picture, self.audio,
    self.subtitles
  ] makeObjectsPerformSelector:@selector(setUndo:)
                    withObject:_undo];
  [self.chapterTitles makeObjectsPerformSelector:@selector(setUndo:)
                                      withObject:_undo];
}
...
- (BOOL)validateOutputFileName:(id *)ioValue
  ...
  if (nil != *ioValue) {
    ...
    if ([value rangeOfString:@"/"].location != NSNotFound) {
      if (outError) {
        *outError = [NSError
            errorWithDomain:@"HBError"
                       code:0
                   userInfo:@{
                     NSLocalizedDescriptionKey : HBKitLocalizedString(
                         @"Invalid name",
                         @"HBJob -> invalid name error description"),
                     NSLocalizedRecoverySuggestionErrorKey :
                         HBKitLocalizedString(
                             @"The file name can't contain the / character.",
                             @"HBJob -> invalid name error recovery "
                             @"suggestion")
                   }];
      }
      return NO;
            ...
            errorWithDomain:@"HBError"
                   ...
                   userInfo:@{
                     NSLocalizedDescriptionKey : HBKitLocalizedString(
                         ...
                         @"HBJob -> invalid name error description"),
                     NSLocalizedRecoverySuggestionErrorKey :
                         HBKitLocalizedString(@"The file name can't be empty.",
                                              @"HBJob -> invalid name error "
                                              @"recovery suggestion")
                   }];
      }
      return NO;
          ...
          errorWithDomain:@"HBError"
                 ...
                 userInfo:@{
                   NSLocalizedDescriptionKey : HBKitLocalizedString(
                       @"Invalid name",
                       @"HBJob -> invalid name error description"),
                   NSLocalizedRecoverySuggestionErrorKey : HBKitLocalizedString(
...
- (void)setContainer:(int)container {
  ...
  // post a notification for any interested observers to indicate that our video
  // container has changed
  [[NSNotificationCenter defaultCenter]
      postNotificationName:HBContainerChangedNotification
                    object:self];
}
...
- (void)setMp4HttpOptimize:(BOOL)mp4HttpOptimize {
  if (mp4HttpOptimize != _mp4HttpOptimize) {
    [[self.undo prepareWithInvocationTarget:self]
        setMp4HttpOptimize:_mp4HttpOptimize];
  }
...
- (void)setMp4iPodCompatible:(BOOL)mp4iPodCompatible {
  if (mp4iPodCompatible != _mp4iPodCompatible) {
    [[self.undo prepareWithInvocationTarget:self]
        setMp4iPodCompatible:_mp4iPodCompatible];
  }
...
- (void)setChaptersEnabled:(BOOL)chaptersEnabled {
  if (chaptersEnabled != _chaptersEnabled) {
    [[self.undo prepareWithInvocationTarget:self]
        setChaptersEnabled:_chaptersEnabled];
  }
  _chaptersEnabled = chaptersEnabled;
  [[NSNotificationCenter defaultCenter]
      postNotificationName:HBChaptersChangedNotification
                    object:self];
}
...
- (BOOL)startAccessingSecurityScopedResource {
  ...
  if (self.accessCount == 0) {
    ...
    self.outputURLToken =
        [HBSecurityAccessToken tokenWithObject:self.outputURL];
    self.subtitlesToken =
        [HBSecurityAccessToken tokenWithObject:self.subtitles];
  }
  self.accessCount += 1;
  return YES;
#else
...
#ifdef __SANDBOX_ENABLED__
  self.accessCount -= 1;
  NSAssert(self.accessCount >= 0,
           @"[HBJob stopAccessingSecurityScopedResource:] unbalanced call");
  if (self.accessCount == 0) {
...
- (instancetype)copyWithZone:(NSZone *)zone {
  ...
  if (copy) {
    ...
    copy->_chapterTitles = [[NSArray alloc] initWithArray:_chapterTitles
                                                copyItems:YES];
  }
...
- (void)encodeWithCoder:(NSCoder *)coder {
  ...
  if (!_outputURLFolderBookmark) {
    __attribute__((unused)) HBSecurityAccessToken *token =
        [HBSecurityAccessToken tokenWithObject:_outputURL];
