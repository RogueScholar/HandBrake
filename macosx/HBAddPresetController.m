...
@property(nonatomic, unsafe_unretained)
    IBOutlet NSPopUpButton *picSettingsPopUp;
...
- (instancetype)initWithPreset:(HBPreset *)preset
                 presetManager:(HBPresetsManager *)manager
                   customWidth:(int)customWidth
                  customHeight:(int)customHeight
               defaultToCustom:(BOOL)defaultToCustom {
...
- (void)windowDidLoad {
  ...
  if ([self.categories selectItemWithTag:2] == NO) {
    HBPreset *category = [[HBPreset alloc] initWithCategoryName:@"My Presets"
                                                        builtIn:NO];
  ...
  [[self.picSettingsPopUp lastItem]
      setTag:HBAddPresetControllerModeSourceMaximum];
...
- (IBAction)showNewCategoryWindow:(id)sender {
  HBAddCategoryController *addCategoryController =
      [[HBAddCategoryController alloc] initWithPresetManager:self.manager];

  [self.window beginSheet:addCategoryController.window
        completionHandler:^(NSModalResponse returnCode) {
          if (returnCode == NSModalResponseOK) {
            NSMenuItem *item =
                [self buildMenuItemWithCategory:addCategoryController.category];
            [self.categories.menu insertItem:item atIndex:2];
          }
...
- (IBAction)addPresetPicDropdownChanged:(id)sender {
  if (self.picSettingsPopUp.selectedItem.tag ==
      HBAddPresetControllerModeCustom) {
...
- (IBAction)showAudioSettingsSheet:(id)sender {
  ...
  self.defaultsController =
      [[HBAudioDefaultsController alloc] initWithSettings:defaults];

  [self.window beginSheet:self.defaultsController.window
        completionHandler:^(NSModalResponse returnCode) {
          if (returnCode == NSModalResponseOK) {
...
- (IBAction)showSubtitlesSettingsSheet:(id)sender {
  ...
  self.defaultsController =
      [[HBSubtitlesDefaultsController alloc] initWithSettings:defaults];
...
- (IBAction)add:(id)sender {
  if (self.name.stringValue.length == 0) {
    ...
    if (self.picSettingsPopUp.selectedTag ==
        HBAddPresetControllerModeSourceMaximum) {
    ...
    // Get the whether or not to apply pic Size and Cropping (includes
    // Anamorphic)
    ...
    [self.selectedCategory insertObject:self.preset
                      inChildrenAtIndex:self.selectedCategory.countOfChildren];
...
- (IBAction)openUserGuide:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[HBUtilities.documentationURL
                  URLByAppendingPathComponent:@"advanced/custom-presets.html"]];
}

@end

@interface HBAddPresetController (TouchBar) <NSTouchBarProvider,
                                             NSTouchBarDelegate>
@end
...
- (NSTouchBar *)makeTouchBar {
  ...
  bar.defaultItemIdentifiers =
      @[ NSTouchBarItemIdentifierOtherItemsProxy, HBTouchBarGroup ];
...
- (NSTouchBarItem *)touchBar:(NSTouchBar *)touchBar
  ...
  if ([identifier isEqualTo:HBTouchBarGroup]) {
    NSCustomTouchBarItem *cancelItem =
        [[NSCustomTouchBarItem alloc] initWithIdentifier:HBTouchBarAdd];
    cancelItem.customizationLabel = NSLocalizedString(@"Cancel", @"Touch bar");
    NSButton *cancelButton =
        [NSButton buttonWithTitle:NSLocalizedString(@"Cancel", @"Touch bar")
                           target:self
                           action:@selector(cancel:)];
    [cancelButton.widthAnchor constraintGreaterThanOrEqualToConstant:160]
        .active = YES;
    cancelItem.view = cancelButton;

    NSCustomTouchBarItem *addItem =
    ...
    NSButton *addButton =
        [NSButton buttonWithTitle:NSLocalizedString(@"Add Preset", @"Touch bar")
                           target:self
                           action:@selector(add:)];
    [addButton.widthAnchor constraintGreaterThanOrEqualToConstant:160].active =
        YES;
    addButton.keyEquivalent = @"\r";
    addItem.view = addButton;

    NSGroupTouchBarItem *item =
        [NSGroupTouchBarItem groupItemWithIdentifier:identifier
                                               items:@[ cancelItem, addItem ]];
    return item;
  }
