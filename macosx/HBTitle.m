...
- (instancetype)initWithAudioTrack:(hb_audio_config_t *)audio index:(int)index {
  ...
  if (self) {
    _displayName = [NSString
        stringWithFormat:@"%d: %@", index, @(audio->lang.description)];
...
- (instancetype)initWithSubtitlesTrack:(hb_subtitle_t *)subtitle
                                 index:(int)index {
  self = [super init];
  if (self) {
    _displayName =
        [NSString stringWithFormat:@"%d: %@", index, @(subtitle->lang)];
...
- (void)stopAccessingSecurityScopedResource {
  ...
  NSAssert(
      self.accessCount >= 0,
      @"[HBSubtitles stopAccessingSecurityScopedResource:] unbalanced call");
  if (self.accessCount == 0) {
...
- (void)encodeWithCoder:(nonnull NSCoder *)coder {
  ...
  if (_fileURL) {
    if (!_bookmark) {
      _bookmark = [HBUtilities
          bookmarkFromURL:_fileURL
                  options:
...
- (instancetype)initWithTitle:(hb_title_t *)title
                       handle:(hb_handle_t *)handle
...
- (NSString *)name {
  if (!_name) {
    ...
    if ([parentURL.pathExtension caseInsensitiveCompare:@"eyetv"] ==
        NSOrderedSame) {
...
- (BOOL)isStream {
  return (self.hb_title->type == HB_STREAM_TYPE ||
          self.hb_title->type == HB_FF_STREAM_TYPE);
}

- (NSString *)description {
  if (self.hb_title->type == HB_BD_TYPE) {
    return
        [NSString stringWithFormat:@"%d - %@ - %05d.MPLS", self.hb_title->index,
                                   self.timeCode, self.hb_title->playlist];
  } else if (self.hb_title->type == HB_DVD_TYPE) {
    return [NSString
        stringWithFormat:@"%d - %@", self.hb_title->index, self.timeCode];
  } else {
    return [NSString stringWithFormat:@"%d - %@ - %@", self.hb_title->index,
                                      self.timeCode, @(self.hb_title->name)];
  }
}
...
- (NSString *)shortFormatDescription {
  ...
  [format appendFormat:@"%dx%d", _hb_title->geometry.width,
                       _hb_title->geometry.height];

  if (_hb_title->geometry.par.num != 1 || _hb_title->geometry.par.den != 1) {
    [format appendFormat:@" (%dx%d)",
                         _hb_title->geometry.width *
                             _hb_title->geometry.par.num /
                             _hb_title->geometry.par.den,
                         _hb_title->geometry.height];
  }

  [format appendString:@", "];

  NSString *fps = [NSString
      localizedStringWithFormat:HBKitLocalizedString(
                                    @"%.6g FPS",
                                    @"Title short description -> video format"),
                                _hb_title->vrate.num /
                                    (double)_hb_title->vrate.den];
  ...
  if (audioCount > 1) {
    [format appendFormat:HBKitLocalizedString(
                             @", %d audio tracks",
                             @"Title short description -> audio format"),
                         audioCount];
  } else if (audioCount == 1) {
    [format appendString:HBKitLocalizedString(
                             @", 1 audio track",
                             @"Title short description -> audio format")];
  }
...
- (int)duration {
  return (self.hb_title->hours * 3600) + (self.hb_title->minutes * 60) +
         (self.hb_title->seconds);
}

- (int)frames {
  double frames = (double)self.hb_title->duration / 90000.f *
                  self.hb_title->vrate.num / self.hb_title->vrate.den;
  return (int)ceil(frames);
}

- (NSString *)timeCode {
  return [NSString stringWithFormat:@"%02d:%02d:%02d", self.hb_title->hours,
                                    self.hb_title->minutes,
                                    self.hb_title->seconds];
}
...
- (NSArray<HBTitleAudioTrack *> *)audioTracks {
  if (!_audioTracks) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      [tracks addObject:[[HBTitleAudioTrack alloc] initWithAudioTrack:audio
                                                                index:i]];
    }
...
- (NSArray<HBTitleSubtitlesTrack *> *)subtitlesTracks {
  if (!_subtitlesTracks) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      [tracks addObject:[[HBTitleSubtitlesTrack alloc]
                            initWithSubtitlesTrack:subtitle
                                             index:i]];
    }
    ...
    for (int i = 0; i < hb_list_count(self.hb_title->list_chapter); i++) {
      ...
      if (chapter != NULL) {
        ...
        if (chapter->title != NULL) {
          ...
          title = [NSString
              stringWithFormat:HBKitLocalizedString(@"Chapter %d",
                                                    "Title -> chapter name"),
                               i + 1];
        }

        [chapters
            addObject:[[HBChapter alloc] initWithTitle:title
                                                 index:i + 1
...
- (NSDictionary *)jobSettingsWithPreset:(HBPreset *)preset {
  ...
  hb_dict_t *job =
      hb_preset_job_init(self.hb_handle, self.hb_title->index, hb_preset);
