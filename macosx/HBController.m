...
- (instancetype)initWithDelegate:(HBAppDelegate *)delegate
  ...
  if (self) {
    ...
    if (!_currentDestination || [[NSFileManager defaultManager]
                                    fileExistsAtPath:_currentDestination.path
                                         isDirectory:nil] == NO) {
      _currentDestination =
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
  ...
  if (action == @selector(selectDefaultPreset:) || action == @selector
                                                       (insertCategory:)) {
    return self.job != nil;
  }
  ...
  [self.window beginSheet:addPresetController.window
        completionHandler:^(NSModalResponse returnCode) {
          if (returnCode == NSModalResponseOK) {
            ...
            [[NSNotificationCenter defaultCenter]
                postNotificationName:HBPresetsChangedNotification
                              object:nil];
          }
        }];
}
...
- (IBAction)showRenamePresetPanel:(id)sender {
  ...
  __block HBRenamePresetController *renamePresetController =
      [[HBRenamePresetController alloc] initWithPreset:self.currentPreset
                                         presetManager:presetManager];
