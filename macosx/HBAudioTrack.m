...
- (void)setSourceTrackIdx:(NSUInteger)sourceTrackIdx {
  if (sourceTrackIdx != _sourceTrackIdx) {
    [[self.undo prepareWithInvocationTarget:self]
        setSourceTrackIdx:_sourceTrackIdx];
  }
...
- (void)setTitle:(NSString *)title {
  if ([title isEqualToString:@"Mono"] || [title isEqualToString:@"Stereo"] ||
...
- (int)sanatizeEncoderValue:(int)proposedEncoder {
  if (proposedEncoder) {
    HBTitleAudioTrack *sourceTrack =
        [_dataSource sourceTrackAtIndex:_sourceTrackIdx];
    int inputCodec = sourceTrack.codec;

    hb_encoder_t *proposedEncoderInfo =
        hb_audio_encoder_get_from_codec(proposedEncoder);
...
- (int)sanatizeBitrateValue:(int)proposedBitrate {
  ...
  {
    return hb_audio_bitrate_get_default(
        self.encoder, self.sampleRate ? self.sampleRate : DEFAULT_SAMPLERATE,
        self.mixdown);
  } else {
    return hb_audio_bitrate_get_best(self.encoder, proposedBitrate,
                                     self.sampleRate, self.mixdown);
  }
}
...
- (NSArray<NSString *> *)bitRates {
  ...
  hb_audio_bitrate_get_limits(self.encoder, self.sampleRate, self.mixdown,
...
- (BOOL)bitrateEnabled {
  ...
  if (retval) {
    int myCodecDefaultBitrate =
        hb_audio_bitrate_get_default(self.encoder, 0, 0);
