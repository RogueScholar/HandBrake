...
/// Pointer to a hb_state_s struct containing the detailed state information of
/// libhb.
...
+ (void)closeGlobal {
  NSAssert(globalInitialized,
           @"[HBCore closeGlobal] global closed but not initialized");
  hb_global_close();
}
...
- (instancetype)initWithLogLevel:(NSInteger)level
...
- (void)preventSleep {
  NSAssert(
      !self.automaticallyPreventSleep,
      @"[HBCore preventSleep:] called with automaticallyPreventSleep enabled.");
...
- (void)allowSleep {
  NSAssert(
      !self.automaticallyPreventSleep,
      @"[HBCore allowSleep:] called with automaticallyPreventSleep enabled.");
...
- (BOOL)canScan:(NSURL *)url error:(NSError *__autoreleasing *)error {
  ...
  __unused HBSecurityAccessToken *token =
      [HBSecurityAccessToken tokenWithObject:url];
#endif

  if (![[NSFileManager defaultManager] fileExistsAtPath:url.path]) {
    if (error) {
      *error =
          [NSError errorWithDomain:@"HBErrorDomain"
                              code:100
                          userInfo:@{
                            NSLocalizedDescriptionKey :
                                @"Unable to find the file at the specified URL"
                          }];
    }
  ...
  if (detector.isVideoDVD || detector.isVideoBluRay) {
    [HBUtilities writeToActivityLog:"%s trying to open a physical disc at: %s",
                                    self.name.UTF8String, url.path.UTF8String];
    void *lib = NULL;
    ...
    if (lib) {
      dlclose(lib);
      [HBUtilities
          writeToActivityLog:"%s library found for decrypting physical disc",
      ...
      [HBUtilities writeToActivityLog:
                       "%s, library not found for decrypting physical disc",
                       self.name.UTF8String];

      if (error) {
        *error = [NSError
            errorWithDomain:@"HBErrorDomain"
                       code:101
                   userInfo:@{
                     NSLocalizedDescriptionKey :
                         @"library not found for decrypting physical disc"
                   }];
      }
    }
  }
...
- (void)scanURL:(NSURL *)url
  ...
  NSAssert(self.state == HBStateIdle, @"[HBCore scanURL:] called while another "
                                      @"scan or encode already in progress");
  NSAssert(url, @"[HBCore scanURL:] called with nil url.");

#ifdef __SANDBOX_ENABLED__
  __block HBSecurityAccessToken *token =
      [HBSecurityAccessToken tokenWithObject:url];
  self.cleanupHandler = ^{
    token = nil;
  };
  ...
  if (index > 0) {
    [HBUtilities writeToActivityLog:"%s scanning specifically for title: %d",
                                    self.name.UTF8String, index];
  } else {
    // minimum title duration doesn't apply to title-specific scan
    // it doesn't apply to batch scan either, but we can't tell it apart from
    // DVD & BD folders here
    [HBUtilities writeToActivityLog:
                     "%s scanning titles with a duration of %d seconds or more",
                     self.name.UTF8String, seconds];
  }

  [self preventAutoSleep];

  hb_scan(_hb_handle, url.fileSystemRepresentation, (int)index,
...
- (HBCoreResult)scanDone {
  ...
  for (int i = 0; i < hb_list_count(title_set->list_title); i++) {
    ...
    [titles addObject:[[HBTitle alloc]
                          initWithTitle:title
                                 handle:self.hb_handle
                               featured:(title->index == title_set->feature)]];
  }
...
- (CGImageRef)copyImageAtIndex:(NSUInteger)index
  ...
  int crop[4] = {frame.cropTop, frame.cropBottom, frame.cropLeft,
                 frame.cropRight};
  memcpy(geo.crop, crop, sizeof(int[4]));

  hb_image_t *image =
      hb_get_preview2(_hb_handle, title.index, (int)index, &geo, deinterlace);

  if (image) {
    // Create an CGImageRef and copy the libhb image into it.
    // The image data returned by hb_get_preview2 is 4 bytes per pixel, BGRA
    ...
    img = CGImageCreate(image->width, image->height, 8, 24, image->width * 3,
...
- (void)encodeJob:(HBJob *)job
      progressHandler:(HBCoreProgressHandler)progressHandler
    completionHandler:(HBCoreCompletionHandler)completionHandler {
  NSAssert(self.state == HBStateIdle,
           @"[HBCore encodeJob:] called while another scan or encode already "
           @"in progress");
  ...
  __block HBSecurityAccessToken *token =
      [HBSecurityAccessToken tokenWithObject:jobCopy];
  self.cleanupHandler = ^{
    token = nil;
  };
#endif
  ...
  [HBUtilities writeToActivityLog:"%s started encoding %s",
                                  self.name.UTF8String,
                                  job.outputFileName.UTF8String];
  [HBUtilities writeToActivityLog:"%s with preset %s", self.name.UTF8String,
                                  job.presetName.UTF8String];
}
...
- (void)updateState {
  ...
  if (_hb_state->state == HB_STATE_WORKDONE ||
      _hb_state->state == HB_STATE_SCANDONE) {
...
- (void)handleCompletion {
  ...
  HBCoreResult result = (_hb_state->state == HB_STATE_WORKDONE)
                            ? [self workDone]
                            : [self scanDone];
