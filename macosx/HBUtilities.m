...
+ (NSString *)handBrakeVersion {
  ...
  return
      [NSString stringWithFormat:@"Handbrake Version: %@ (%@)",
...
+ (NSURL *)appSupportURL {
  ...
  NSURL *appSupportURL =
      [[[fileManager URLsForDirectory:NSApplicationSupportDirectory
          ...
          URLByAppendingPathComponent:@"HandBrake"];

  if (appSupportURL && ![fileManager fileExistsAtPath:appSupportURL.path]) {
    [fileManager createDirectoryAtPath:appSupportURL.path
...
+ (void)writeErrorToActivityLog:(NSError *)error {
  ...
  if (error.localizedDescription) {
    [self writeToActivityLog:"Error description: %s",
                             error.localizedDescription.UTF8String];
  }
  if (error.debugDescription) {
    [self writeToActivityLog:"Error debug description: %s",
...
+ (nullable NSData *)bookmarkFromURL:(NSURL *)url
  ...
  if (error) {
    NSString *error_message =
        [NSString stringWithFormat:@"Failed to create bookmark: %@", error];
...
+ (nullable NSData *)bookmarkFromURL:(NSURL *)url {
  return [HBUtilities bookmarkFromURL:url
...
+ (NSURL *)mediaURLFromURL:(NSURL *)URL {
  ...
  if ([NSWorkspace.sharedWorkspace isFilePackageAtPath:URL.path]) {
    [HBUtilities writeToActivityLog:"trying to open a package at: %s",
                                    URL.path.UTF8String];
    ...
    if ([URL.pathExtension isEqualToString:@"eyetv"]) {
      ...
      // We're looking at an EyeTV package - try to open its enclosed .mpg media
      // file
      NSString *mpgname;
      NSUInteger n = [[URL.path stringByAppendingString:@"/"]
          completePathIntoString:&mpgname
                   caseSensitive:YES
      ...
      if (n > 0) {
        ...
        // We did not find an mpeg file in our package, so we do not call
        // performScan
    ...
    // We check to see if this is a .dvdmedia package
      ...
      // path IS a package - but dvdmedia packages can be treaded like normal
      // directories
    ...
    if ([URL.lastPathComponent isEqualToString:@"VIDEO_TS"]) {
      [HBUtilities
          writeToActivityLog:
              "trying to open video_ts folder (video_ts folder chosen)"];
      // If VIDEO_TS Folder is chosen, choose its parent folder for the source
      // display name
...
+ (HBPrivacyConsentState)
  ...
  if (@available(macOS 10.14, *)) {
    ...
    if ([bundleIdentifier isEqualToString:@"com.apple.systemevents"]) {
      // Mare sure system events is running, if not the consent alert will not
      // be shown.
      ...
      if (result == NO) {
        [HBUtilities writeToActivityLog:"Automation: couldn't launch %s.",
                                        bundleIdentifier.UTF8String];
      }
    }

    OSErr descResult = AECreateDesc(typeApplicationBundleID, identifierCString,
                                    strlen(identifierCString), &addressDesc);

    if (descResult == noErr) {
      OSStatus permission = AEDeterminePermissionToAutomateTarget(
          &addressDesc, typeWildCard, typeWildCard, promptIfNeeded);
      ...
      case errAEEventWouldRequireUserConsent:
        [HBUtilities
            writeToActivityLog:"Automation: request user consent for %s.",
                               bundleIdentifier.UTF8String];
        result = HBPrivacyConsentStateUnknown;
        break;
      case noErr:
        [HBUtilities
            writeToActivityLog:"Automation: permission granted for %s.",
                               bundleIdentifier.UTF8String];
        result = HBPrivacyConsentStateGranted;
        break;
      case errAEEventNotPermitted:
        [HBUtilities
            writeToActivityLog:"Automation: permission not granted for %s.",
                               bundleIdentifier.UTF8String];
