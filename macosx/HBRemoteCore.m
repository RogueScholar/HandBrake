...
- (void)connect {
  _connection = [[NSXPCConnection alloc]
      initWithServiceName:@"fr.handbrake.HandBrakeXPCService"];
  _connection.remoteObjectInterface =
      [NSXPCInterface interfaceWithProtocol:@protocol(HBRemoteCoreProtocol)];

  _connection.exportedInterface = [NSXPCInterface
      interfaceWithProtocol:@protocol(HBRemoteProgressProtocol)];
...
- (void)invalidate {
  [[_connection
      synchronousRemoteObjectProxyWithErrorHandler:^(NSError *_Nonnull error){
      }] tearDown];
...
- (void)handleInterruption {
  [_proxy
      setDVDNav:[NSUserDefaults.standardUserDefaults boolForKey:HBUseDvdNav]];
...
- (void)preventSleep {
  ...
  IOReturn success = IOPMAssertionCreateWithName(
      ...
      reasonForActivity, &_assertionID);

  if (success != kIOReturnSuccess) {
    [HBUtilities
        writeToActivityLog:"HBRemoteCore: failed to prevent system sleep"];
  }
}
...
- (void)scanURL:(NSURL *)url
      ...
      progressHandler:(nonnull HBCoreProgressHandler)progressHandler
...
- (void)encodeJob:(HBJob *)job
...
#ifdef __SANDBOX_ENABLED__
  __block HBSecurityAccessToken *token =
      [HBSecurityAccessToken tokenWithObject:job];

  NSMutableArray<NSData *> *bookmarks = [NSMutableArray array];

  for (HBSubtitlesTrack *track in job.subtitles.tracks) {
    if (track.fileURL) {
      NSData *subtitlesBookmark = [track.fileURL bookmarkDataWithOptions:0
                                          includingResourceValuesForKeys:nil
                                                           relativeToURL:nil
                                                                   error:NULL];
  ...
  NSData *bookmark = [job.outputURL bookmarkDataWithOptions:0
                             includingResourceValuesForKeys:nil
                                              relativeToURL:nil
                                                      error:NULL];
  if (bookmark) {
