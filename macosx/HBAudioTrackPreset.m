...
- (void)setFallbackEncoder:(int)fallbackEncoder {
  if (fallbackEncoder != _fallbackEncoder) {
    [[self.undo prepareWithInvocationTarget:self]
        setFallbackEncoder:_fallbackEncoder];
  }
...
- (void)validateFallbackEncoder {
  if (_encoder & HB_ACODEC_PASS_FLAG) {
    ...
    self.selectedEncoder = (fallbackEncoder != HB_ACODEC_INVALID)
                               ? fallbackEncoder
                               : self.fallbackEncoder;
  } else {
...
- (void)validateSamplerate {
  if (self.selectedEncoder & HB_ACODEC_PASS_FLAG) {
    ...
    self.sampleRate =
        hb_audio_samplerate_find_closest(self.sampleRate, self.selectedEncoder);
  }
}
...
- (void)validateBitrate {
  ...
  {
    self.bitRate = hb_audio_bitrate_get_default(
        self.selectedEncoder,
        self.sampleRate ? self.sampleRate : DEFAULT_SAMPLERATE, self.mixdown);
  } else {
    self.bitRate = hb_audio_bitrate_get_best(
        self.selectedEncoder, self.bitRate,
        self.sampleRate ? self.sampleRate : DEFAULT_SAMPLERATE, self.mixdown);
  }
}

- (BOOL)isAutoPassthruEnabledWithNoFallback {
  return (self.encoder == HB_ACODEC_AUTO_PASS &&
          self.fallbackEncoder == HB_ACODEC_NONE);
}
...
- (BOOL)mixdownEnabled {
  ...
  if (self.mixdown == HB_AMIXDOWN_NONE ||
      self.isAutoPassthruEnabledWithNoFallback) {
...
- (BOOL)bitrateEnabled {
  ...
  int myCodecDefaultBitrate =
      hb_audio_bitrate_get_default(self.selectedEncoder, 0, 0);
...
// Because we have indicated that the binding for the gain validates immediately
// we can implement the key value binding method to ensure the gain stays in our
// accepted range.
...
- (NSArray<NSString *> *)mixdowns {
  ...
  for (const hb_mixdown_t *mixdown = hb_mixdown_get_next(NULL); mixdown != NULL;
...
- (NSArray<NSString *> *)bitRates {
  ...
  hb_audio_bitrate_get_limits(self.selectedEncoder, self.sampleRate,
                              self.mixdown, &minBitRate, &maxBitRate);
  ...
  for (const hb_rate_t *audio_bitrate = hb_audio_bitrate_get_next(NULL);
    ...
    if (audio_bitrate->rate >= minBitRate &&
        audio_bitrate->rate <= maxBitRate) {
...
+ (NSSet *)keyPathsForValuesAffectingValueForKey:(NSString *)key {
  ...
  if ([key isEqualToString:@"bitrateEnabled"] ||
    ...
    retval = [NSSet setWithObjects:@"selectedEncoder", @"encoder",
                                   @"fallbackEncoder", @"mixdown",
                                   @"sampleRate", nil];
  } else if ([key isEqualToString:@"mixdowns"]) {
    retval = [NSSet
        setWithObjects:@"selectedEncoder", @"encoder", @"fallbackEncoder", nil];
  } else if ([key isEqualToString:@"sampleRates"]) {
    retval = [NSSet setWithObjects:@"selectedEncoder", @"encoder",
                                   @"fallbackEncoder", @"mixdown", nil];
