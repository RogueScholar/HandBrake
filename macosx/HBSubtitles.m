...
- (instancetype)initWithJob:(HBJob *)job {
  ...
  if (self) {
    ...
    NSMutableArray<HBTitleSubtitlesTrack *> *sourceTracks =
        [job.title.subtitlesTracks mutableCopy];

    int foreignAudioType = VOBSUB;

    // now set the name of the Foreign Audio Search track
    NSMutableString *foreignAudioSearchTrackName =
        [HBKitLocalizedString(@"Foreign Audio Search",
                              "HBSubtitles -> search pass name") mutableCopy];

    // Add the none and foreign track to the source array
    HBTitleSubtitlesTrack *none = [[HBTitleSubtitlesTrack alloc]
        initWithDisplayName:HBKitLocalizedString(
                                @"None", @"HBSubtitles -> none track name")
                       type:0
                    fileURL:nil];
    [sourceTracks insertObject:none atIndex:0];

    HBTitleSubtitlesTrack *foreign = [[HBTitleSubtitlesTrack alloc]
        initWithDisplayName:foreignAudioSearchTrackName
                       type:foreignAudioType
                    fileURL:nil];
...
- (void)track:(HBSubtitlesTrack *)track
    didChangeSourceFrom:(NSUInteger)oldSourceIdx {
...
- (void)addAllTracks {
  [self
      removeTracksAtIndexes:[NSIndexSet
...
- (void)addExternalTrackFromURL:(NSURL *)fileURL {
  int type = [fileURL.pathExtension.lowercaseString isEqualToString:@"srt"]
                 ? IMPORTSRT
                 : IMPORTSSA;

  // Create a new entry for the subtitle source array so it shows up in our
  // subtitle source list
  NSMutableArray<HBTitleSubtitlesTrack *> *sourceTracks =
      [self.sourceTracks mutableCopy];
  [sourceTracks addObject:[[HBTitleSubtitlesTrack alloc]
                              initWithDisplayName:fileURL.lastPathComponent
                                             type:type
                                          fileURL:fileURL]];

  self.sourceTracks = [sourceTracks copy];
  HBSubtitlesTrack *track =
      [self trackFromSourceTrackIndex:self.sourceTracksArray.count - 1];
...
- (HBSubtitlesTrack *)trackFromSourceTrackIndex:(NSInteger)index {
  HBSubtitlesTrack *track =
      [[HBSubtitlesTrack alloc] initWithTrackIdx:index
...
- (void)addDefaultTracksFromJobSettings:(NSDictionary *)settings {
  ...
  NSArray<NSDictionary<NSString *, id> *> *settingsTracks =
      settings[@"Subtitle"][@"SubtitleList"];
  ...
  if ([search[@"Enable"] boolValue]) {
    HBSubtitlesTrack *track =
        [self trackFromSourceTrackIndex:FOREIGN_TRACK_INDEX];
  ...
  for (NSDictionary *trackDict in settingsTracks) {
    HBSubtitlesTrack *track = [self
        trackFromSourceTrackIndex:[trackDict[@"Track"] unsignedIntegerValue] +
                                  2];
...
- (void)validatePassthru {
  ...
  NSUInteger idx = 0;
  for (HBSubtitlesTrack *track in self.tracks) {
    if (track.isEnabled && track.sourceTrackIdx > FOREIGN_TRACK_INDEX &&
        !track.canPassthru) {
  ...
  // if we converted a track to burned-in, unset it for tracks that support
  // passthru
...
- (void)validateBurned:(NSUInteger)index {
  [self.tracks enumerateObjectsUsingBlock:^(HBSubtitlesTrack *track,
...
- (void)validateDefault:(NSUInteger)index {
  [self.tracks enumerateObjectsUsingBlock:^(HBSubtitlesTrack *obj,
                                            NSUInteger idx, BOOL *stop) {
...
- (instancetype)initWithCoder:(NSCoder *)decoder {
  ...
  if (_container != HB_MUX_MP4 && _container != HB_MUX_MKV &&
      _container != HB_MUX_WEBM) {
    goto fail;
  }
  decodeCollectionOfObjectsOrFail(_sourceTracks, NSArray,
...
- (void)insertObject:(HBSubtitlesTrack *)track
     inTracksAtIndex:(NSUInteger)index {
  [[self.undo prepareWithInvocationTarget:self]
      removeObjectFromTracksAtIndex:index];
  [self.tracks insertObject:track atIndex:index];
}

- (void)insertTracks:(NSArray<HBSubtitlesTrack *> *)array
...
- (void)removeObjectFromTracksAtIndex:(NSUInteger)index {
  ...
  [[self.undo prepareWithInvocationTarget:self] insertObject:track
                                             inTracksAtIndex:index];
  [self.tracks removeObjectAtIndex:index];
}

- (void)removeTracksAtIndexes:(NSIndexSet *)indexes {
  NSArray<HBSubtitlesTrack *> *tracks = [self.tracks objectsAtIndexes:indexes];
  [[self.undo prepareWithInvocationTarget:self] insertTracks:tracks
                                                   atIndexes:indexes];
