...
- (instancetype)initWithDelegate:
    (id<HBQueueDetailsViewControllerDelegate>)delegate {
  self = [super init];
  if (self) {
    _delegate = delegate;
  }
...
- (void)setUpObservers {
  ...
  [center addObserverForName:HBQueueDidStartItemNotification
                  ...
                  usingBlock:^(NSNotification *_Nonnull note) {
                    HBQueueItem *startedItem =
                        note.userInfo[HBQueueItemNotificationItemKey];
...
- (void)updateReset {
  self.canReset = self.item && (self.item.state != HBQueueItemStateWorking &&
                                self.item.state != HBQueueItemStateReady);
}
