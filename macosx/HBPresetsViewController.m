...
- (instancetype)initWithPresetManager:(HBPresetsManager *)presetManager {
  ...
  if (self) {
    ...
    _expandedNodes = [[NSArray
        arrayWithArray:[NSUserDefaults.standardUserDefaults
                           objectForKey:@"HBPreviewViewExpandedStatus"]]
        mutableCopy];
  }
  return self;
}
...
- (void)viewDidLoad {
  ...
  [self.treeController
      setSelectionIndexPath:[self.presets
                                indexPathOfPreset:self.selectedPreset]];
  [self.treeController addObserver:self
                        forKeyPath:@"selectedObjects"
                           options:NSKeyValueObservingOptionNew
                           context:HBPresetsViewControllerContext];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if (context == HBPresetsViewControllerContext) {
    ...
    if (selectedNode && selectedNode.isLeaf &&
        selectedNode != self.selectedPresetInternal) {
...
- (nonnull NSString *)fileNameForPreset:(HBPreset *)preset {
  NSString *name = preset.name == nil || preset.name.length == 0
                       ? @"Unnamed preset"
                       : preset.name;
...
- (nonnull NSURL *)lastPresetExportDirectoryURL {
  NSURL *defaultExportDirectory = [[NSURL fileURLWithPath:NSHomeDirectory()]
      URLByAppendingPathComponent:@"Desktop"
                      isDirectory:YES];
  NSURL *lastPresetExportDirectoryURL = [NSUserDefaults.standardUserDefaults
      URLForKey:@"LastPresetExportDirectoryURL"];
  return lastPresetExportDirectoryURL ? lastPresetExportDirectoryURL
...
- (void)doExportPresets:(NSArray<HBPreset *> *)presets {
  ...
  panel.title =
      NSLocalizedString(@"Export presets", @"Export presets save panel title");
  ...
  [panel beginWithCompletionHandler:^(NSInteger result) {
    if (result == NSModalResponseOK) {
      [NSUserDefaults.standardUserDefaults
          setURL:panel.URL
          forKey:@"LastPresetExportDirectoryURL"];
      ...
      for (HBPreset *preset in presets) {
        ...
        BOOL success = [preset writeToURL:url
                               atomically:YES
                               removeRoot:NO
                                    error:&error];
        if (success == NO) {
...
- (void)doExportPreset:(HBPreset *)preset {
  ...
  panel.title =
      NSLocalizedString(@"Export preset", @"Export presets save panel title");
  panel.directoryURL = self.lastPresetExportDirectoryURL;
  ...
  [panel beginWithCompletionHandler:^(NSInteger result) {
    if (result == NSModalResponseOK) {
      ...
      [NSUserDefaults.standardUserDefaults
          setURL:presetExportDirectory
          forKey:@"LastPresetExportDirectoryURL"];

      NSError *error = NULL;
      BOOL success = [preset writeToURL:panel.URL
                             atomically:YES
                             removeRoot:NO
                                  error:&error];
      if (success == NO) {
...
- (void)doImportPreset:(NSArray<NSURL *> *)URLs
           atIndexPath:(nullable NSIndexPath *)indexPath {
  if (indexPath == nil) {
    for (HBPreset *preset in self.presets.root.children) {
      if (preset.isBuiltIn == NO && preset.isLeaf == NO) {
        indexPath =
            [[self.presets indexPathOfPreset:preset] indexPathByAddingIndex:0];
      }
    }

    if (indexPath == nil) {
      indexPath =
          [NSIndexPath indexPathWithIndex:self.presets.root.countOfChildren];
    }
  }

  for (NSURL *url in URLs) {
    NSError *error;
    HBPreset *preset = [[HBPreset alloc] initWithContentsOfURL:url
                                                         error:&error];

    if (preset) {
      for (HBPreset *child in preset.children) {
        [self.treeController insertObject:child
...
- (IBAction)importPreset:(id)sender {
  NSOpenPanel *panel = [NSOpenPanel openPanel];
  panel.title =
      NSLocalizedString(@"Import presets", @"Import preset open panel title");
  ...
  if ([NSUserDefaults.standardUserDefaults
          URLForKey:@"LastPresetImportDirectoryURL"]) {
    panel.directoryURL = [NSUserDefaults.standardUserDefaults
        URLForKey:@"LastPresetImportDirectoryURL"];
  } else {
  ...
  [panel beginWithCompletionHandler:^(NSInteger result) {
    [NSUserDefaults.standardUserDefaults
        setURL:panel.directoryURL
        forKey:@"LastPresetImportDirectoryURL"];
...
- (IBAction)clicked:(id)sender {
  if (self.delegate &&
      [self.treeController.selectedObjects.firstObject isLeaf]) {
    [self.delegate selectionDidChange];
  }
}
  ...
  if (self.treeController.canRemove) {
    ...
    if (self.treeController.selectedObjects.count > 1) {
      alert.messageText = NSLocalizedString(
          @"Are you sure you want to permanently delete the selected presets?",
      ...
      alert.messageText = NSLocalizedString(
          @"Are you sure you want to permanently delete the selected preset?",
          @"Delete preset alert -> message");
      [alert addButtonWithTitle:NSLocalizedString(
                                    @"Delete Preset",
                                    @"Delete preset alert -> first button")];
    ...
    if (status == NSAlertFirstButtonReturn) {
      for (NSIndexPath *indexPath in self.treeController.selectionIndexPaths
...
- (IBAction)insertCategory:(id)sender {
  HBAddCategoryController *addCategoryController =
      [[HBAddCategoryController alloc] initWithPresetManager:self.presets];

  NSModalResponse returnCode =
      [NSApp runModalForWindow:addCategoryController.window];
  if (returnCode == NSModalResponseOK) {
    NSIndexPath *indexPath =
        [self.presets indexPathOfPreset:addCategoryController.category];
...
- (void)outlineViewItemDidExpand:(NSNotification *)notification {
  ...
  if (![self.expandedNodes containsObject:@(node.hash)]) {
    ...
    [NSUserDefaults.standardUserDefaults
        setObject:self.expandedNodes
           forKey:@"HBPreviewViewExpandedStatus"];
  }
}
...
- (id<NSPasteboardWriting>)outlineView:(NSOutlineView *)outlineView
  ...
  if (@available(macOS 10.12, *)) {
    HBFilePromiseProvider *filePromise =
        [[HBFilePromiseProvider alloc] initWithFileType:@"public.text"
                                               delegate:self];
    filePromise.userInfo = [item representedObject];
    return filePromise;
  } else {
    return [[NSPasteboardItem alloc]
        initWithPasteboardPropertyList:@1
                                ofType:kHandBrakeInternalPBoardType];
  }
}

- (nonnull NSString *)filePromiseProvider:
                          (nonnull NSFilePromiseProvider *)filePromiseProvider
                          fileNameForType:(nonnull NSString *)fileType {
  return [self fileNameForPreset:filePromiseProvider.userInfo];
}

- (void)filePromiseProvider:(nonnull NSFilePromiseProvider *)filePromiseProvider
          writePromiseToURL:(nonnull NSURL *)url
          completionHandler:
              (nonnull void (^)(NSError *_Nullable))completionHandler {
  NSError *error = NULL;
  [filePromiseProvider.userInfo writeToURL:url
                                atomically:YES
                                removeRoot:NO
                                     error:&error];
...
- (NSDragOperation)outlineView:(NSOutlineView *)ov
  ...
  } else if ([self.dragNodesArray allSatisfy:^BOOL(id _Nonnull object) {
               return [[object representedObject] isBuiltIn] == NO;
             }]) {
    if ([[item representedObject] isBuiltIn] ||
        ([[item representedObject] isLeaf] && index == -1) ||
...
- (void)handleInternalDrops:(NSPasteboard *)pboard
              withIndexPath:(NSIndexPath *)indexPath {
  // user is doing an intra app drag within the outline view:
  NSArray *newNodes = self.dragNodesArray;

  // move the items to their new place (we do this backwards, otherwise they
...
- (void)handleExternalDrops:(NSPasteboard *)pboard
              withIndexPath:(NSIndexPath *)indexPath {
...
- (BOOL)outlineView:(NSOutlineView *)ov
  ...
  NSIndexPath *indexPath;
  if (targetItem) {
    // drop down inside the tree node: fetch the index path to insert our
    // dropped node
    indexPath = index == -1
                    ? [[targetItem indexPath] indexPathByAddingIndex:0]
                    : [[targetItem indexPath] indexPathByAddingIndex:index];
  } else {
    // drop at the top root level
    if (index ==
        -1) // drop area might be ambiguous (not at a particular location)
    {
      indexPath = [NSIndexPath
          indexPathWithIndex:self.presets.root.children
                                 .count]; // drop at the end of the top level
    } else {
      indexPath =
          [NSIndexPath indexPathWithIndex:index]; // drop at a particular place
                                                  // at the top level
    }
  }
