...
- (BOOL)isNewer:(NSDictionary *)dict {
  ...
  if ([dict[@"VersionMajor"] intValue] > major ||
      ([dict[@"VersionMajor"] intValue] == major &&
       [dict[@"VersionMinor"] intValue] > minor) ||
      ([dict[@"VersionMajor"] intValue] == major &&
       [dict[@"VersionMinor"] intValue] == minor &&
       [dict[@"VersionMicro"] intValue] > micro)) {
    return YES;
  }
...
- (NSURL *)backupURLfromURL:(NSURL *)url
               versionMajor:(int)major
                      minor:(int)minor
                      micro:(int)micro {
  NSString *backupName = [NSString
      stringWithFormat:@"%@.%d.%d.%d.json",
                       url.lastPathComponent.stringByDeletingPathExtension,
                       major, minor, micro];

  return [url.URLByDeletingLastPathComponent
      URLByAppendingPathComponent:backupName];
}
...
- (NSDictionary *)dictionaryWithPresetsAtURL:(NSURL *)url
                                      backup:(BOOL)backup
                                      result:(HBPresetLoadingResult *)result {
  NSData *presetData = [[NSData alloc] initWithContentsOfURL:url];

  if (presetData) {
    const char *json = [[NSString alloc] initWithData:presetData
                                             encoding:NSUTF8StringEncoding]
                           .UTF8String;
    NSDictionary *presetsDict =
        [NSJSONSerialization HB_JSONObjectWithUTF8String:json
                                                 options:0
                                                   error:NULL];
    ...
    if ([presetsDict isKindOfClass:[NSDictionary class]]) {
      if ([self isNewer:presetsDict]) {
        *result = HBPresetLoadingResultFailedNewer;
        ...
        if (backup) {
          NSURL *backupURL =
              [self backupURLfromURL:self.fileURL
        ...
        presetsDict =
            [NSJSONSerialization HB_JSONObjectWithUTF8String:updatedJson
                                                     options:0
                                                       error:NULL];
        free(updatedJson);
        ...
        *result = HBPresetLoadingResultOKUpgraded;
        ...
        presetsDict =
            [NSJSONSerialization HB_JSONObjectWithUTF8String:cleanedJson
                                                     options:0
                                                       error:NULL];
        free(cleanedJson);
...
- (void)loadPresetsFromURL:(NSURL *)url {
  ...
  if (result == HBPresetLoadingResultFailedNewer) {
    ...
    NSURL *backupURL = [self backupURLfromURL:url
                                 versionMajor:major
                                        minor:minor
                                        micro:micro];

    HBPresetLoadingResult backupResult;
    presetsDict = [self dictionaryWithPresetsAtURL:backupURL
                                            backup:NO
                                            result:&backupResult];
  ...
  if (presetsDict) {
    for (NSDictionary *child in presetsDict[@"PresetList"]) {
      [self.root insertObject:[[HBPreset alloc] initWithDictionary:child]
            inChildrenAtIndex:self.root.countOfChildren];
    }
  ...
  if (result == HBPresetLoadingResultFailed && presetsDict == nil) {
    ...
    [self loadOldPresetsFromURL:[url.URLByDeletingPathExtension
  ...
  [self.root
      enumerateObjectsUsingBlock:^(id obj, NSIndexPath *idx, BOOL *stop) {
        if ([obj isLeaf]) {
          leafFound = YES;
          *stop = YES;
        }
...
- (void)generateBuiltInPresets {
  ...
  NSArray *presetsArray =
      [NSJSONSerialization JSONObjectWithData:presetsData
