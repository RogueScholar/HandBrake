...
- (instancetype)initWithLanguage:(NSString *)value
                    iso639_2code:(NSString *)code {
  self = [super init];
  if (self) {
    _language = value;
    _iso639_2 = code;
  ...
  if (self) {
    ...
    for (lang = lang_get_any(); lang != NULL; lang = lang_get_next(lang)) {
      ...
      if (!strncmp(lang->iso639_2, "any", 3)) {
        nativeLanguage =
            NSLocalizedString(@"Any", @"HBLanguage -> Any language");
      } else if (!strncmp(lang->iso639_2, "und", 3)) {
        nativeLanguage =
            NSLocalizedString(@"Unknown", @"HBLanguage -> Unknown language");
      } else {
        nativeLanguage = strlen(lang->native_name) ? @(lang->native_name)
                                                   : @(lang->eng_name);
      }
    ...
    [selected sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
      return [languages indexOfObject:[obj1 iso639_2]] -
             [languages indexOfObject:[obj2 iso639_2]];
    }];

    [internal
        insertObjects:selected
  ...
  if (self.isDragginEnabled) {
    ...
    if (self.showSelectedOnly) {
      ...
      while (currentIndex != NSNotFound) {
        NSUInteger newIndex =
            [unfilteredArray indexOfObject:filteredArray[currentIndex]];
...
- (BOOL)tableView:(NSTableView *)view
  ...
  if ([info draggingSource] == view) {
    ...
    NSData *rowData =
        [[info draggingPasteboard] dataForType:kHBLanguagesDragRowsType];
    ...
    row -= i;
    NSIndexSet *selectionSet = [NSIndexSet
        indexSetWithIndexesInRange:NSMakeRange(row, [indexSet count])];
