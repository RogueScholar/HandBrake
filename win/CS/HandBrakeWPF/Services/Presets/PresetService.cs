// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PresetService.cs" company="HandBrake Project
// (http://handbrake.fr)">
...
namespace HandBrakeWPF.Services.Presets {
  ...
  using GeneralApplicationException =
      HandBrakeWPF.Exceptions.GeneralApplicationException;
  ...
  public class PresetService : IPresetService {
    ...
    private readonly string presetFile = Path.Combine(
        DirectoryUtilities.GetUserStoragePath(VersionHelper.IsNightly()),
        "presets.json");
    private readonly ObservableCollection<IPresetObject>presets =
        new ObservableCollection<IPresetObject>(); // Can store Presets and
                                                   // PresetDisplayCategory
                                                   // objects.
    private readonly Dictionary<string, Preset>flatPresetDict =
    ...
    public Preset DefaultPreset {
      get { return this.flatPresetList.FirstOrDefault(p => p.IsDefault); }
    }
    ...
    public bool Add(Preset preset) { return this.Add(preset, false); }
    ...
    public bool Add(Preset preset, bool isLoading) {
      if (!this.CheckIfPresetExists(preset.Name)) {
        ...
        PresetDisplayCategory category =
            this.presets.FirstOrDefault(a => a.Category == preset.Category)
                as PresetDisplayCategory; // TODO build Dict for this.
        if (category != null) {
          ...
          this.presets.Add(
              new PresetDisplayCategory(preset.Category, preset.IsBuildIn,
                                        new BindingList<Preset>{preset}));
        } else {
    ...
    public void Import(string filename) {
      if (!string.IsNullOrEmpty(filename)) {
        ...
        try {
          ...
          this.errorService.ShowError(Resources.Main_PresetImportFailed,
                                      Resources.Main_PresetImportFailedSolution,
                                      exc);
          return;
        }
        ...
        if (container?.PresetList == null || container.PresetList.Count == 0) {
          this.errorService.ShowError(Resources.Main_PresetImportFailed,
                                      Resources.Main_PresetImportFailedSolution,
                                      Resources.NoAdditionalInformation);
          return;
        }
        ...
        if (container.PresetList != null) {
          ...
          foreach(var objectPreset in container.PresetList) {
            PresetCategory category =
                JsonConvert.DeserializeObject<PresetCategory>(
                    objectPreset.ToString());
            if (category != null && category.ChildrenArray != null &&
                category.ChildrenArray.Count > 0) {
              ...
              HBPreset hbPreset = JsonConvert.DeserializeObject<HBPreset>(
                  objectPreset.ToString());
              if (hbPreset != null) {
                Preset preset = this.ConvertHbPreset(hbPreset);
                if (preset != null && !preset.IsBuildIn) {
          ...
          if (containsBuildInPreset) {
            this.errorService.ShowMessageBox(
                ...
                Properties.Resources.Warning, MessageBoxButton.OK,
    ...
    /// The configuration.
    /// </param>
    public void Export(string filename, Preset preset,
                       HBConfiguration configuration) {
      // TODO Add support for multiple export
      PresetTransportContainer container =
          JsonPresetFactory.ExportPreset(preset, configuration);
    ...
    /// <summary>
    /// Remove a preset with a given name from either the built in or user
    /// preset list.
    /// </summary>
    /// <param name="preset">
    ...
    public bool Remove(Preset preset) {
      ...
      PresetDisplayCategory cateogry = this.presets.FirstOrDefault(
          p => p.Category == preset.Category) as PresetDisplayCategory;
      if (cateogry != null) {
        // Remove the preset, and cleanup the category if it's not got any
        // presets in it.
    ...
    public void RemoveGroup(string categoryName) {
      PresetDisplayCategory category = this.presets.FirstOrDefault(
          p => p.Category == categoryName) as PresetDisplayCategory;
      if (category != null) {
    ...
    public void SetDefault(Preset preset) {
      ...
      foreach(Preset item in this.flatPresetList) { item.IsDefault = false; }
    ...
    /// <summary>
    /// Reads the CLI's CLI output format and load's them into the preset List
    /// Preset
    /// </summary>
    public void UpdateBuiltInPresets() {
      // Clear the current built in Presets and now parse the temporary Presets
      // file.
      this.ClearBuiltIn();

      IList<PresetCategory>presetCategories =
          HandBrakePresetService.GetBuiltInPresets();
      ...
      foreach(var category in presetCategories) {
        foreach(var hbpreset in category.ChildrenArray) {
          ...
          preset.Task.AllowedPassthruOptions = new AllowedPassthru(
              true); // We don't want to override the built-in preset
      ...
      if (this.presets.Count == 0) {
        ...
        throw new GeneralApplicationException(
            "Failed to load built-in presets.",
            "Restarting HandBrake may resolve this issue", null);
      }
    ...
    /// <summary>
    /// Check if the preset "name" exists in either Presets or UserPresets
    /// lists.
    /// </summary>
    /// <param name="name">
    /// Name of the preset
    ...
    public void SetSelected(Preset selectedPreset) {
      foreach(var item in this.flatPresetList) { item.IsSelected = false; }
    ...
    public void SaveCategoryStates() {
      ...
      this.userSettingService.SetUserSetting(
          UserSettingConstants.PresetExpandedStateList, expandedPresets);
    }

    public void LoadCategoryStates() {
      StringCollection expandedPresets =
          this.userSettingService.GetUserSetting<StringCollection>(
              UserSettingConstants.PresetExpandedStateList);
    ...
    public IList<PresetDisplayCategory>
    GetPresetCategories(bool userCategoriesOnly) {
      List<PresetDisplayCategory>categoriesList =
          new List<PresetDisplayCategory>();
    ...
    private string ArchivePresetFile(string file, bool delete) {
      try {
        ...
        PresetVersion version =
            HandBrakePresetService.GetCurrentPresetVersion();
        string archiveFile =
            string.Format("{0}.{1}_{2}_{3}.archive", file, version.Major,
        ...
        if (File.Exists(file)) {
          ...
          while (File.Exists(archiveFile)) {
            ...
            archiveFile = string.Format("{0}.{1}_{2}_{3} {4}.archive", file,
                                        version.Major, version.Minor,
                                        version.Micro, appendedNumber);
          }
    ...
    private void LoadPresets() {
      ...
      try {
        ...
        try {
          container =
              HandBrakePresetService.GetPresetsFromFile(this.presetFile);
        } catch (Exception exc) {
          this.ServiceLogMessage(
              "Corrupted Presets File Detected: " + Environment.NewLine + exc);
        }
        ...
        if (container?.PresetList == null) {
          ...
          this.errorService.ShowMessageBox(
              ...
              Resources.PresetService_UnableToLoad, MessageBoxButton.OK,
              MessageBoxImage.Exclamation);

          this.UpdateBuiltInPresets();
          this.ServiceLogMessage("Recovery Completed!");
          return; // Update built-in presets stores the presets locally, so just
                  // return.
        }

        // Force Upgrade of presets
        if (this.userSettingService.GetUserSetting<int>(
                UserSettingConstants.ForcePresetReset) < ForcePresetReset) {
          this.userSettingService.SetUserSetting(
              UserSettingConstants.ForcePresetReset, ForcePresetReset);

          string fileName = this.ArchivePresetFile(this.presetFile, true);
          this.errorService.ShowMessageBox(
              Resources.Presets_PresetForceReset + Environment.NewLine +
                  Environment.NewLine + Resources.PresetService_ArchiveFile +
                  fileName,
              Resources.PresetService_UnableToLoad, MessageBoxButton.OK,
              MessageBoxImage.Information);
          this.UpdateBuiltInPresets(); // Update built-in presets stores the
                                       // presets locally, so just return.
          return;
        }
        ...
        PresetVersion presetVersion =
            HandBrakePresetService.GetCurrentPresetVersion();
        if (container.VersionMajor != presetVersion.Major ||
            container.VersionMinor != presetVersion.Minor ||
            container.VersionMicro != presetVersion.Micro) {
          this.UpdateBuiltInPresets();
        }
      } catch (Exception ex) {
    ...
    private void ProcessPresetList(PresetTransportContainer container) {
      ...
      foreach(var item in container.PresetList) {
        object deserialisedItem =
            JsonConvert.DeserializeObject<PresetCategory>(item.ToString());
        ...
        if (category != null && category.Folder) {
          foreach(HBPreset hbpreset in category.ChildrenArray) {
            ...
            // Migration
            preset.Category = category.PresetName == "User Presets"
                                  ? UserPresetCatgoryName
                                  : category.PresetName;
            preset.IsBuildIn = hbpreset.Type == 0;

            // IF we are using Source Max, Set the Max Width / Height values.
            if (preset.PictureSettingsMode ==
                PresetPictureSettingsMode.SourceMaximum) {
        ...
        deserialisedItem =
            JsonConvert.DeserializeObject<HBPreset>(item.ToString());
    ...
    private void SavePresetFiles() {
      try {
        ...
        Dictionary<string, PresetCategory>presetCategories =
            new Dictionary<string, PresetCategory>();
        List<HBPreset>uncategorisedPresets = new List<HBPreset>();

        // Handle User Presets.
        this.HandlePresetListsForSave(
            this.flatPresetList.Where(o => !o.IsBuildIn).ToList(),
            presetCategories, uncategorisedPresets);
        ...
        PresetTransportContainer container = new PresetTransportContainer(
            presetVersion.Major, presetVersion.Minor,
            presetVersion.Micro){PresetList = new List<object>()};
        ...
        using(FileStream strm = new FileStream(this.presetFile, FileMode.Create,
                                               FileAccess.Write)) {
          string presetsJson = JsonConvert.SerializeObject(
              container, Formatting.Indented, settings);
        ...
        throw new GeneralApplicationException(
            ...
            "The details section below may indicate why this error has occurred.",
            exc);
      }
    }

    private void
    HandlePresetListsForSave(List<Preset>processList,
                             Dictionary<string, PresetCategory>presetCategories,
                             List<HBPreset>uncategorisedPresets) {
      foreach(Preset item in processList) {
        if (string.IsNullOrEmpty(item.Category)) {
          uncategorisedPresets.Add(JsonPresetFactory.CreateHbPreset(
              item, HBConfigurationFactory.Create()));
        } else {
          HBPreset preset = JsonPresetFactory.CreateHbPreset(
              item, HBConfigurationFactory.Create());
    ...
    private Preset ConvertHbPreset(HBPreset hbPreset) {
      ...
      preset.Category =
          UserPresetCatgoryName; // TODO can we get this from the preset?
    ...
    private void AddOrUpdateImportedPreset(Preset preset) {
      ...
      if (this.CheckIfPresetExists(preset.Name)) {
        if (!this.CanUpdatePreset(preset.Name)) {
          this.errorService.ShowMessageBox(
              Resources.Main_PresetErrorBuiltInName, Resources.Error,
              MessageBoxButton.OK, MessageBoxImage.Error);
          return;
        }

        MessageBoxResult result = this.errorService.ShowMessageBox(
            string.Format(Resources.Main_PresetOverwriteWarning, preset.Name),
            Resources.Overwrite, MessageBoxButton.YesNo,
            MessageBoxImage.Warning);
    ...
    protected void ServiceLogMessage(string message) {
      this.log.LogMessage(string.Format("Preset Service: {0}{1}{0}",
                                        Environment.NewLine, message),
                          LogMessageType.Application, LogLevel.Info);
    }
  }
}