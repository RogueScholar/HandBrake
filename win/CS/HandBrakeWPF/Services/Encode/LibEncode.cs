...
#region Private Variables
    ...
    public LibEncode(IHbFunctionsProvider hbFunctionsProvider, ILog logService)
        : base(logService) {
      this.log = logService;
      ...
      try {
        ...
        if (this.IsEncoding) {
          throw new GeneralApplicationException(
              Resources.Queue_AlreadyEncoding,
              Resources.Queue_AlreadyEncodingSolution, null);
        }
        ...
        this.log.Reset(); // Reset so we have a clean log for the start of the
                          // encode.
        ...
        if (this.instance != null) {
          ...
          try {
            ...
            this.ServiceLogMessage("Failed to cleanup previous instance: " +
                                   exc);
          }
        }
        ...
        if (!string.IsNullOrEmpty(basePresetName)) {
          this.TimedLogMessage(
              string.Format("base preset: {0}", basePresetName));
        }

        this.instance =
            task.IsPreviewEncode ? HandBrakeInstanceManager
                .GetPreviewInstance(configuration.Verbosity, configuration)
            : HandBrakeInstanceManager.GetEncodeInstance(
        ...
        this.instance.StartEncode(EncodeTaskFactory.Create(
            task, configuration, hbFunctionsProvider.GetHbFunctionsWrapper()));
        ...
        this.ServiceLogMessage("Failed to start encoding ..." +
                               Environment.NewLine + exc);
        this.InvokeEncodeCompleted(new EventArgs.EncodeCompletedEventArgs(
            false, exc, "Unable to start encoding", this.currentTask.Source,
            this.currentTask.Destination, null, 0));
      }
    }
    ...
    public EncodeTask GetActiveJob() {
      if (this.currentTask != null) {
        EncodeTask task =
            new EncodeTask(this.currentTask); // Decouple our current copy.
        return task;
      }
...
#region HandBrakeInstance Event Handlers.
    ...
    protected void TimedLogMessage(string message) {
      this.log.LogMessage(string.Format(
    ...
    /// </param>
    private void InstanceEncodeProgress(object sender,
                                        EncodeProgressEventArgs e) {
      EventArgs.EncodeProgressEventArgs args =
          new EventArgs.EncodeProgressEventArgs{
    ...
    /// The e.
    /// </param>
    private void InstanceEncodeCompleted(object sender,
      ...
      // Handling Log Data
      string hbLog =
          this.ProcessLogs(this.currentTask.Destination, this.isPreviewInstance,
      ...
      this.InvokeEncodeCompleted(
          e.Error
            ? new EventArgs.EncodeCompletedEventArgs(
              false, null, string.Empty, this.currentTask.Source,
              this.currentTask.Destination, hbLog, filesize)
          : new EventArgs.EncodeCompletedEventArgs(
              true, null, string.Empty, this.currentTask.Source,
              this.currentTask.Destination, hbLog, filesize));
    }
    ...
    private long GetFilesize(string destination) {
      try {
        ...
        this.ServiceLogMessage("Unable to get final filesize ..." +
