// --------------------------------------------------------------------------------------------------------------------
// <copyright file="UserSettingService.cs" company="HandBrake Project
// (http://handbrake.fr)">
...
namespace HandBrakeWPF.Services {
  ...
  public class UserSettingService : IUserSettingService {
    private readonly string settingsFile = Path.Combine(
        ...
        "settings.json");
    private readonly string releaseSettingsFile = Path.Combine(
        DirectoryUtilities.GetUserStoragePath(false), "settings.json");
    private readonly string nightlySettingsFile = Path.Combine(
        DirectoryUtilities.GetUserStoragePath(true), "settings.json");
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="UserSettingService"/>
    /// class.
    /// </summary>
    public UserSettingService() { this.Load(); }
    ...
    public void SetUserSetting(string name, object value) {
      ...
      this.OnSettingChanged(
          new SettingChangedEventArgs{Key = name, Value = value});
    }
    ...
    public T GetUserSetting<T>(string name) {
      if (this.userSettings.ContainsKey(name)) {
        ...
        // Treat String Arrays as StringCollections.  TODO refactor upstream
        // code to more traditional string arrays.
        ...
        if (settingValue != null && settingValue.GetType() == typeof (JArray)) {
          ...
          foreach(var item in stringArr) { stringCollection.Add(item); }
    ...
    private void Save() {
      try {
        ...
        using(StreamWriter file = new StreamWriter(new FileStream(
                  this.settingsFile, FileMode.Create, FileAccess.Write))) {
          string appSettings = JsonConvert.SerializeObject(
              this.userSettings, Formatting.Indented, this.settings);
          file.Write(appSettings);
        }
      } catch (Exception exc) {
        throw new GeneralApplicationException(
            Resources.UserSettings_AnErrorOccured,
            Resources.SettingService_SaveErrorReset, exc);
      }
    }
    ...
    private void Load() {
      try {
        ...
        foreach(var item in defaults.Where(
            item => !this.userSettings.Keys.Contains(item.Key))) {
          this.userSettings.Add(item.Key, item.Value);
          this.Save();
        }

        // Legacy Settings forced Reset.
        this.userSettings[UserSettingConstants.ScalingMode] =
            VideoScaler.Lanczos;
      } catch (Exception exc) {
        try {
          ...
          throw new GeneralApplicationException(
              Resources.UserSettings_YourSettingsHaveBeenReset,
              Resources.UserSettings_YourSettingsAreCorrupt, exc);
        } catch (Exception) {
          throw new GeneralApplicationException(
              string.Format(Resources.UserSettings_UnableToLoad,
                            this.settingsFile),
              Resources.UserSettings_UnableToLoadSolution, exc);
        }
      }
    }
    ...
    private Dictionary<string, object>GetDefaults() {
      ...
      defaults.Add(UserSettingConstants.LastUpdateCheckDate,
      ...
      defaults.Add(UserSettingConstants.VLCPath,
      ...
      defaults.Add(UserSettingConstants.WhenDonePerformActionImmediately,
                   false);
      ...
      defaults.Add(UserSettingConstants.PauseQueueOnLowDiskspaceLevel,
                   2000000000L);
      defaults.Add(UserSettingConstants.PauseEncodeOnLowDiskspaceLevel,
                   2000000000L);
