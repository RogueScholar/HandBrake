// --------------------------------------------------------------------------------------------------------------------
// <copyright file="QueueTask.cs" company="HandBrake Project
// (http://handbrake.fr)">
...
namespace HandBrakeWPF.Services.Queue.Model {
  ...
  public class QueueTask : PropertyChangedBase {
    ...
    [JsonProperty]
    public string ScannedSourcePath {
      get;
      set;
    }

    [JsonProperty]
    public QueueItemStatus Status{get{return this.status;
  }
...
[JsonProperty]
public HBConfiguration Configuration {
  get;
  set;
}

[JsonProperty]
public QueueStats Statistics {
  get;
  set;
}

[JsonIgnore]
public string SelectedPresetKey{get{return this.presetKey;
}
}

[JsonIgnore]
public bool ShowEncodeProgress =>
    this.Status == QueueItemStatus.InProgress && SystemInfo.IsWindows10();
...
public override string ToString() {
  return string.Format(
      "Encode Task.  Title: {0}, Source: {1}, Destination: {2}",
      this.Task.Title, this.Task.Source, this.Task.Destination);
}
