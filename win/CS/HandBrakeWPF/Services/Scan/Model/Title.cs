...
#region Properties
    ...
    /// 3: R
    /// </summary>
    public Cropping AutoCropDimensions {
      get;
      set;
    }
    ...
    public string ItemDisplayText {
      get {
        return string.Format("{0} {1} ({2:00}:{3:00}:{4:00}) {5}",
                             this.TitleNumber, this.Playlist,
                             this.Duration.Hours, this.Duration.Minutes,
                             this.Duration.Seconds, this.SourceDisplayName);
      }
    }

    public string ItemDisplayTextClosed {
      get {
        return string.Format("{0} {1} ({2:00}:{3:00}:{4:00})", this.TitleNumber,
                             this.Playlist, this.Duration.Hours,
    ...
    public TimeSpan CalculateDuration(long startPoint, long endPoint) {
      IEnumerable<Chapter>chapers = this.Chapters.Where(
          c => c.ChapterNumber >= startPoint && c.ChapterNumber <= endPoint);

      TimeSpan duration = TimeSpan.FromSeconds(0.0);
      duration = chapers.Aggregate(duration, (current, chapter) =>
    ...
    /// <summary>
    /// Override of the ToString method to provide an easy way to use this
    /// object in the UI
    /// </summary>
    /// <returns>A string representing this track in the format: {title #}
    /// (00:00:00)</returns>
    public override string ToString() {
      if (!string.IsNullOrEmpty(this.Playlist) &&
          !this.Playlist.StartsWith(" ")) {
