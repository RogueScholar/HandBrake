// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SubtitlesViewModel.cs" company="HandBrake Project
// (http://handbrake.fr)">
...
#region Constructors and Destructors
    ...
    /// cref="HandBrakeWPF.ViewModels.SubtitlesViewModel"/> class.
    /// </summary>
    ...
    /// The window Manager.
    /// </param>
    public SubtitlesViewModel(IErrorService errorService,
      ...
      this.foreignAudioSearchTrack = new Subtitle{
          SubtitleType = SubtitleType.ForeignAudioSearch,
...
#region Properties
    ...
    /// </summary>
    public ISubtitlesDefaultsViewModel SubtitleDefaultsViewModel {
      get;
      set;
    }
    ...
    /// </summary>
    public IEnumerable<string>CharacterCodes {
      get;
      set;
    }
    ...
    public string PanelTitle {
      get { return Resources.SubtitlesViewModel_SubTracks; }
    }
    ...
    public string SwitchDisplayTitle {
      get { return Resources.SubtitlesViewModel_ConfigureDefaults; }
    }
    ...
    public SubtitleBehaviours SubtitleBehaviours {
      get { return this.SubtitleDefaultsViewModel.SubtitleBehaviours; }
    }

    public bool IsBurnableOnly {
      get { return this.Task.OutputFormat == OutputFormat.WebM; }
    }
...
#region Public Methods
    ...
    /// Add a new Track
    /// </summary>
    public void Add() { this.Add(null); }

    /// <summary>
    /// Add all closed captions not already on the list.
    /// </summary>
    public void AddAllClosedCaptions() {
      foreach(Subtitle subtitle in this.SourceTitlesSubset(null).Where(
          s => s.SubtitleType == SubtitleType.CC)) {
        this.Add(subtitle);
      }
    }
    ...
    public void AddFirstForSelectedLanguages() {
      bool anyLanguageSelected =
          this.SubtitleBehaviours.SelectedLangauges.Contains(Constants.Any);
      foreach(Subtitle sourceTrack in this.GetSelectedLanguagesTracks()) {
        ...
        bool found = this.Task.SubtitleTracks.Any(
            track => Equals(track.SourceTrack, sourceTrack));
        if (!found) {
          ...
          foreach(var item in this.Task.SubtitleTracks) {
            if (item.SourceTrack != null &&
                item.SourceTrack.LanguageCode != null &&
                track.LanguageCode.Contains(item.SourceTrack.LanguageCode)) {
              foundLanguage = true;
            }
          }
    ...
    /// Clear all Tracks
    /// </summary>
    public void Clear() { this.Task.SubtitleTracks.Clear(); }
      ...
      if (this.Task.SubtitleTracks.Count >= 1) {
        ...
        case SubtitleBurnInBehaviourModes.None:
          foreach(var track in this.Task.SubtitleTracks) {
            if (track.SourceTrack.SubtitleType ==
    ...
    /// </summary>
    public void ReloadDefaults() { this.AutomaticSubtitleSelection(); }
...
#region Implemented Interfaces
    ...
    public bool MatchesPreset(Preset preset) {
      ...
      if (preset.SubtitleTrackBehaviours.AddClosedCaptions !=
          this.SubtitleBehaviours.AddClosedCaptions) {
        return false;
      }

      if (preset.SubtitleTrackBehaviours.AddForeignAudioScanTrack !=
          this.SubtitleBehaviours.AddForeignAudioScanTrack) {
        return false;
      }

      if (preset.SubtitleTrackBehaviours.SelectedBehaviour !=
          this.SubtitleBehaviours.SelectedBehaviour) {
        return false;
      }

      if (preset.SubtitleTrackBehaviours.SelectedBurnInBehaviour !=
          this.SubtitleBehaviours.SelectedBurnInBehaviour) {
        return false;
      }
    ...
    /// <summary>
    /// Checks the configuration of the subtitles and warns the user about any
    /// potential issues.
    /// </summary>
    public bool ValidateSubtitles() {
      var nonBurnedSubtitles =
          this.Task.SubtitleTracks.Where(subtitleTrack => !subtitleTrack.Burned)
              .ToList();
...
#region Methods
    ...
    /// The Source track is set based on the following order. If null, it will
    ...
    /// </summary>
    /// <param name="subtitle">
    /// The subtitle. Use null to add preferred, or first from source (based on
    /// user preference)
    /// </param>
    private void Add(Subtitle subtitle) {
        ...
        // Burn-in Behaviours
        if (this.SubtitleBehaviours.SelectedBurnInBehaviour ==
                SubtitleBurnInBehaviourModes.ForeignAudio ||
            this.SubtitleBehaviours.SelectedBurnInBehaviour ==
                SubtitleBurnInBehaviourModes.ForeignAudioPreferred) {
          if (subtitle != null &&
              subtitle.SubtitleType == SubtitleType.ForeignAudioSearch) {
            track.Burned = true;
            this.SetBurnedToFalseForAllExcept(track);
          }
        }
        ...
        if (!track.Burned && (source.SubtitleType == SubtitleType.PGS) &&
            this.Task != null && this.Task.OutputFormat == OutputFormat.Mp4) {
    ...
    private List<Subtitle>GetSelectedLanguagesTracks() {
      ...
      if (this.SubtitleBehaviours.SelectedLangauges.Contains(Constants.Any)) {
        ...
        iso6392Codes = LanguageUtilities.OrderIsoCodes(
            iso6392Codes, this.SubtitleBehaviours.SelectedLangauges);
      } else {
      ...
      foreach(string code in iso6392Codes) {
        orderedSubtitles.AddRange(this.SourceTracks.Where(
            subtitle => subtitle.LanguageCodeClean == code));
      }
    ...
    private string GetPreferredSubtitleTrackLanguage() {
      string langName =
          this.SubtitleBehaviours.SelectedLangauges.FirstOrDefault(
              w => w != Constants.Any);
    ...
    /// The subtitles. (Optional).  If null, works on the full source subtitle
    /// collection
    /// </param>
    /// <returns>
    ...
    SourceTitlesSubset(IEnumerable<Subtitle>subtitles) {
      return subtitles !=
             null
               ? subtitles
                 .Where(subtitle => !this.Task.SubtitleTracks.Any(
                            track => Equals(track.SourceTrack, subtitle)))
                 .ToList()
          : this.SourceTracks
                 .Where(subtitle => !this.Task.SubtitleTracks.Any(
                            track => Equals(track.SourceTrack, subtitle)))
