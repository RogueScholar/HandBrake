// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RemoteInstance.cs" company="HandBrake Project
// (http://handbrake.fr)">
//   This file is part of the HandBrake source code - It may be used under the
//   terms of the GNU General Public License.
// </copyright>
// <summary>
//   An Implementation of IEncodeInstance that works with a remote process
//   rather than locally in-process. This class is effectively just a shim.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
...
namespace HandBrakeWPF.Instance {
  ...
  public class RemoteInstance : HttpRequestBase, IEncodeInstance, IDisposable {
    ...
    public async void StartEncode(JsonEncodeObject jobToStart) {
      ...
      string job =
          JsonConvert.SerializeObject(jobToStart, Formatting.None, settings);
    ...
    public JsonState GetEncodeProgress() {
      Task<ServerResponse>response =
          this.MakeHttpGetRequest("PollEncodeProgress");
      response.Wait();
    ...
    private void StartServer(int verbosityLvl, bool noHardwareMode) {
      if (this.workerProcess == null || this.workerProcess.HasExited) {
        ...
        this.workerProcess.StartInfo = new ProcessStartInfo(
            "HandBrake.Worker.exe", string.Format("--port={0} --verbosity={1}",
                                                  this.port, verbosityLvl)){
            WindowStyle = ProcessWindowStyle.Normal};

        this.workerProcess.Start();
        this.workerProcess.Exited += this.WorkerProcess_Exited;
        Debug.WriteLine("Worker Process Started. PID = {0}",
                        this.workerProcess.Id);
      }
    }
      ...
      if (taskState != null &&
        ...
        if (this.EncodeProgress != null) {
          var progressEventArgs = new EncodeProgressEventArgs(
              fractionComplete
              : state.Working.Progress, currentFrameRate
              : state.Working.Rate, averageFrameRate
              : state.Working.RateAvg, estimatedTimeLeft
              : TimeSpan.FromSeconds(state.Working.ETASeconds), passId
              : state.Working.PassID, pass
              : state.Working.Pass, passCount
              : state.Working.PassCount, stateCode
              : taskState.Code);
        ...
        this.EncodeCompleted?.Invoke(
            sender
            : this, e
            : new EncodeCompletedEventArgs(state.WorkDone.Error != 0));
      }
    }
  }
}
