...
class Configure(object):
    ...
    def doc_ready(self):
        ...
        if build_tuple.match("*-*-darwin*"):
            self.xcode_prefix_final = self._final_dir(self.build_dir,
                                                      self.xcode_prefix_dir)
        ...
        if self.build_dir != os.curdir:
            if os.path.exists(self.build_dir):
                if not options.force:
                    self.errln(
                        "build directory already exists: %s (use --force to overwrite)",
                        self.build_dir,
                    )
            else:
...
class ShellProbe(Action):
    ...
    def _action(self):
        ...
        pipe = subprocess.Popen(self.command,
                                shell=True,
...
class CCProbe(Action):
    ...
    def _action(self):
        ...
        pipe = subprocess.Popen(
            "%s -c -o conftest.o conftest.c" % self.command,
            shell=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
        )
...
def PkgConfigTest(args, lib):
    ...
    # pipe and redirect stderr to stdout; effects communicate result
    pipe = subprocess.Popen(
        "%s %s %s" % (Tools.pkgconfig.pathname, args, lib),
        shell=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    )
...
def LDTest(command, lib, test_file):
    ...
    # pipe and redirect stderr to stdout; effects communicate result
    pipe = subprocess.Popen(
        "%s -o conftest conftest.c %s" % (command, lib),
        shell=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    )
...
class LDProbe(Action):
    ...
    def _action(self):
        self.fail, self.msg_end, self.session = LDTest(self.command, self.lib,
                                                       self.test_file)
...
class ChkLib(Action):
    ...
    def _action(self):
        ...
        self.fail, self.msg_end, session = LDTest(self.command,
                                                  "-l%s" % self.lib,
                                                  self.test_file)
...
class HostTupleAction(Action, list):
    def __init__(self, cross=None):
        super(HostTupleAction, self).__init__("compute",
                                              "host tuple",
                                              abort=True)
...
class ArchAction(Action):
    def __init__(self):
        super(ArchAction, self).__init__("compute",
                                         "available architectures",
                                         abort=True)
        self.mode = SelectMode("architecture",
                               (host_tuple.machine, host_tuple.spec))
...
class StoreCallbackAction(argparse.Action):
    def __init__(self, option_strings, dest, nargs=None, **kwargs):
        ...
        super(StoreCallbackAction, self).__init__(option_strings, dest, nargs,
                                                  **kwargs)
...
class RepoProbe(ShellProbe):
    def __init__(self):
        ...
        super(RepoProbe, self).__init__("repo info",
                                        "%s %s" % (repo_info, cfg.src_dir))
...
class Project(Action):
    ...
    def _action(self):
        ...
        if repo.type != "release" or options.snapshot:
            ...
            if not m:
                # Regular release
                self.version = "%d.%d.%d" % (self.vmajor, self.vminor,
                                             self.vpoint)
                self.debversion = "%d.%d.%d" % (self.vmajor, self.vminor,
                                                self.vpoint)
                url_ctype = ""
                url_ntype = "stable"
            else:
...
class ToolProbe(Action):
    ...
    def __init__(self, var, option, *names, **kwargs):
        super(ToolProbe, self).__init__("find",
    ...
    def _action(self):
        ...
        if self.fail:
            ...
            self.version = VersionProbe(
                self.name,
                [self.pathname, self.versionopt],
                abort=self.abort,
...
class VersionProbe(Action):
    def __init__(self, name, command, minversion=None, rexpr=None,
                 abort=False):
        super(VersionProbe, self).__init__(
            "version probe",
            "%s %s" % (os.path.basename(command[0]), ".".join(
                [str(i) for i in minversion])),
            abort,
        )
        self.name = name
        self.command = command
        ...
        self.rexprs = [
            "(?P<name>[^.]+)\s+(?P<svers>(?P<i0>\d+)(\.(?P<i1>\d+))?(\.(?P<i2>\d+))?)",
    ...
    def _action(self):
        ...
        if self.inadequate():
            ...
            if self.abort is True:
                ...
                raise AbortError(
                    "minimum required %s version is %s and %s is %s\n" % (
                        self.name,
                        ".".join([str(i) for i in self.minversion]),
                        self.command[0],
                        self.svers,
                    ))
    ...
    def _parse(self):
        for expression in self.rexprs:
            mo = re.match(expression, self.session[0].decode("utf-8"),
                          re.IGNORECASE)
            if mo is None:
                continue
            md = mo.groupdict()
...
def createCLI(cross=None):
    cli = argparse.ArgumentParser(
        usage="%s [OPTIONS...] [TARGETS...]" % os.path.basename(__file__),
        description="Configure %s build system" % project.name,
    )

    # add hidden options
    cli.add_argument("--xcode-driver",
                     default="bootstrap",
                     action="store",
    ...
    grp.add_argument(
        ...
        default=False,
        action="store_true",
        help="disable distfile downloads",
    )
    grp.add_argument(
        "--disable-df-verify",
        default=False,
        action="store_true",
        help="disable distfile data verification",
    )
    grp.add_argument(
        "--df-jobs",
        action="store",
        metavar="N",
        type=int,
        help="allow N distfile downloads at once",
    )
    grp.add_argument(
        "--df-verbose",
        action="count",
        dest="df_verbosity",
        help="increase distfile tools verbosity",
    )
    grp.add_argument(
        "--df-accept-url",
        default=[],
        action="append",
        metavar="SPEC",
        help="accept URLs matching regex pattern",
    )
    grp.add_argument(
        "--df-deny-url",
        default=[],
        action="append",
        metavar="SPEC",
        help="deny URLs matching regex pattern",
    )
    ...
    h = ("specify Xcode SDK sysroot (macOS only)" if
         (build_tuple.match("*-*-darwin*")
          and cross is None) else argparse.SUPPRESS)
    grp.add_argument("--sysroot",
                     default=None,
                     action="store",
                     metavar="DIR",
                     help=h)
    ...
    grp.add_argument("--snapshot",
                     default=False,
                     action="store_true",
                     help="Force a snapshot build")
    h = IfHost(
        "Build extra contribs for flatpak packaging",
        "*-*-linux*",
        "*-*-freebsd*",
    ...
    grp.add_argument(
        ...
        default=False,
        action="store_true",
        help="harden app to protect against buffer overflows",
    )
    h = IfHost(
        "sandbox app to limit host system access (macOS only)",
        "*-*-darwin*",
        none=argparse.SUPPRESS,
    ).value
    grp.add_argument(
        ...
        default=False,
        action="store_true",
        help=(("%s" % h) if h != argparse.SUPPRESS else h),
    )
    cli.add_argument_group(grp)
    ...
    if build_tuple.match("*-*-darwin*") and cross is None:
        ...
        grp.add_argument("--disable-xcode",
                         default=False,
                         action="store_true",
                         help="disable Xcode")
        grp.add_argument(
            "--xcode-prefix",
            default=cfg.xcode_prefix_dir,
            action="store",
            metavar="DIR",
            help="specify install dir for Xcode products [%s]" %
            (cfg.xcode_prefix_dir),
        )
        grp.add_argument(
            "--xcode-symroot",
            default="xroot",
            ...
            metavar="DIR",
            help=
            "specify root of the directory hierarchy that contains product files and intermediate build files",
        )
    ...
    h = IfHost(
        "enable assembly code in non-contrib modules",
        "NOMATCH*-*-darwin*",
        "NOMATCH*-*-linux*",
        none=argparse.SUPPRESS,
    ).value
    grp.add_argument("--enable-asm",
                     default=False,
                     action="store_true",
                     help=h)

    h = IfHost("disable GTK GUI",
               "*-*-linux*",
               "*-*-freebsd*",
               none=argparse.SUPPRESS).value
    grp.add_argument("--disable-gtk",
                     default=False,
                     action="store_true",
                     help=h)

    h = IfHost(
        "disable GTK GUI update checks",
        "*-*-linux*",
        "*-*-freebsd*",
        none=argparse.SUPPRESS,
    ).value
    ...
    grp.add_argument("--enable-gtk-mingw",
                     default=False,
                     action="store_true",
                     help=h)

    h = IfHost("Build GUI with GTK4",
               "*-*-linux*",
               "*-*-freebsd*",
               none=argparse.SUPPRESS).value
    grp.add_argument("--enable-gtk4",
                     default=False,
                     action="store_true",
                     help=h)

    h = IfHost(
        "disable GStreamer (live preview)",
        "*-*-linux*",
        "*-*-freebsd*",
        none=argparse.SUPPRESS,
    ).value
    grp.add_argument("--disable-gst",
                     default=False,
                     action="store_true",
                     help=h)
    ...
    grp.add_argument(
        ...
        default=True,
        action="store_true",
        help=(("enable %s" % h) if h != argparse.SUPPRESS else h),
    )
    grp.add_argument(
        "--disable-x265",
        dest="enable_x265",
    ...
    grp.add_argument(
        "--enable-numa",
        dest="enable_numa",
        default=IfHost(True, "*-*-linux*", none=False).value,
        action="store_true",
        help=(("enable %s" % h) if h != argparse.SUPPRESS else h),
    )
    grp.add_argument(
    ...
    grp.add_argument(
        "--enable-ffmpeg-aac",
        dest="enable_ffmpeg_aac",
        default=not host_tuple.match("*-*-darwin*"),
        action="store_true",
        help=(("enable %s" % h) if h != argparse.SUPPRESS else h),
    )
    grp.add_argument(
    ...
    grp.add_argument(
        "--enable-nvenc",
        dest="enable_nvenc",
        default=IfHost(True, "*-*-linux*", "*-*-mingw*", none=False).value,
        action="store_true",
        help=(("enable %s" % h) if h != argparse.SUPPRESS else h),
    )
    grp.add_argument(
    ...
    h = IfHost(
        "Intel QSV video encoder/decoder",
        "*-*-linux*",
        "*-*-freebsd*",
        "*-*-mingw*",
        none=argparse.SUPPRESS,
    ...
    grp.add_argument(
        "--enable-qsv",
        dest="enable_qsv",
        default=IfHost(True, "*-*-mingw*", none=False).value,
        action="store_true",
        help=(("enable %s" % h) if h != argparse.SUPPRESS else h),
    )
    grp.add_argument(
    ...
    h = IfHost("AMD VCE video encoder",
               "*-*-linux*",
               "*-*-mingw*",
               none=argparse.SUPPRESS).value
    grp.add_argument(
...
try:
    ...
    class Tools:
        ...
        autoconf = ToolProbe("AUTOCONF.exe",
                             "autoconf",
                             "autoconf",
                             abort=True,
                             minversion=[2, 69, 0])
        automake = ToolProbe("AUTOMAKE.exe",
                             "automake",
                             "automake",
                             abort=True,
                             minversion=[1, 15, 0])
        ...
        meson = ToolProbe("MESON.exe",
                          "meson",
                          "meson",
                          abort=True,
                          minversion=[0, 47, 0])
        nasm = ToolProbe("NASM.exe",
                         "asm",
                         "nasm",
                         abort=True,
                         minversion=[2, 13, 0])
        ninja = ToolProbe("NINJA.exe",
                          "ninja",
                          "ninja-build",
                          "ninja",
                          abort=True)

        xcodebuild = ToolProbe(
            "XCODEBUILD.exe",
            "xcodebuild",
            "xcodebuild",
            abort=(True if (build_tuple.match("*-*-darwin*") and cross is None)
                   else False),
            versionopt="-version",
            minversion=[10, 3, 0],
        )
    ...
    if build_tuple.match("*-*-darwin*"):
        for xc in glob.glob(
                os.path.join(cfg.dir, "../macosx/xcconfig/*.xcconfig")):
    ...
    if cross:
        for tool in (Tools.ar, Tools.gcc, Tools.ranlib, Tools.strip):
            tool.__init__(tool.var, tool.option, "%s-%s" % (cross, tool.name),
                          **tool.kwargs)
            tool.run()

    debugMode = SelectMode("debug", ("none", "none"), ("min", "min"),
                           ("std", "std"), ("max", "max"))

    Oz_check_command = (
        "%s -Oz -S -o /dev/null -xc /dev/null > /dev/null 2>&1" %
        Tools.gcc.pathname)
    ...
    if Oz_check.fail is False:
        optimizeMode = SelectMode(
            ...
            ("speed", "speed"),
            ("size", "size"),
            ("size-aggressive", "size-aggressive"),
            default="speed",
        )
    else:
        optimizeMode = SelectMode(
    ...
    options.enable_sandbox = IfHost(options.enable_sandbox,
                                    "*-*-darwin*",
                                    none=False).value
    # Require FFmpeg AAC on Linux and Windows
    options.enable_ffmpeg_aac = IfHost(options.enable_ffmpeg_aac,
                                       "*-*-darwin*",
                                       none=True).value
    # Allow GTK mingw only on mingw
    options.enable_gtk_mingw = IfHost(options.enable_gtk_mingw,
                                      "*-*-mingw*",
                                      none=False).value
    # Disable NVENC on unsupported platforms
    options.enable_nvenc = IfHost(options.enable_nvenc,
                                  "*-*-linux*",
                                  "*-*-mingw*",
                                  none=False).value
    # NUMA is linux only and only needed with x265
    options.enable_numa = (IfHost(
        options.enable_numa, "*-*-linux*", none=False).value
                           and options.enable_x265)
    # Disable QSV on unsupported platforms
    options.enable_qsv = IfHost(options.enable_qsv,
                                "*-*-linux*",
                                "*-*-freebsd*",
                                "*-*-mingw*",
                                none=False).value
    # Disable VCE on unsupported platforms
    options.enable_vce = IfHost(options.enable_vce,
                                "*-*-linux*",
                                "*-*-mingw*",
                                none=False).value
    ...
    if options.flatpak or host_tuple.match("*-*-darwin*", "*-*-mingw"):
        ...
        Tools.cmake = ToolProbe("CMAKE.exe",
                                "cmake",
                                "cmake",
                                abort=True,
                                minversion=[3, 1, 0])
        ...
        Tools.cmake = ToolProbe("CMAKE.exe",
                                ...
                                abort=True,
                                minversion=[2, 8, 12])
    Tools.cmake.__init__(Tools.cmake.var, Tools.cmake.option, Tools.cmake.name,
                         **Tools.cmake.kwargs)
    Tools.cmake.run()
    for action in Action.actions:
        action.run()
...
"""
        dlfcn = LDProbe("static dlfcn", "%s -static" % Tools.gcc.pathname,
                        "-ldl", dlfcn_test)
        dlfcn.run()
...
"""
        bz2 = LDProbe("static bz2", "%s -static" % Tools.gcc.pathname, "-lbz2",
                      bz2_test)
        bz2.run()
...
"""
        libz = LDProbe("static zlib", "%s -static" % Tools.gcc.pathname, "-lz",
                       libz_test)
        libz.run()
...
"""
        xz = LDProbe("static xz", "%s -static" % Tools.gcc.pathname, "-llzma",
                     xz_test)
        xz.run()
...
"""
        iconv = LDProbe("static iconv", "%s -static" % Tools.gcc.pathname,
                        "-liconv", iconv_test)
        iconv.run()
...
"""
        regex = LDProbe("static regex", "%s -static" % Tools.gcc.pathname,
                        "-lregex", regex_test)
        regex.run()
...
"""
        strtok_r = LDProbe("static strtok_r",
                           "%s -static" % Tools.gcc.pathname, "",
                           strtok_r_test)
        strtok_r.run()
...
"""
    ...
    strerror_r = LDProbe("strerror_r", "%s" % Tools.gcc.pathname, "",
                         strerror_r_test)
    strerror_r.run()
    ...
    Wno_format_truncation_command = (
        "%s -O2 -Werror -Wno-format-truncation -S -o /dev/null -xc /dev/null > /dev/null 2>&1"
        % Tools.gcc.pathname)
    Wno_format_truncation = ShellProbe("checking for -Wno-format-truncation",
                                       "%s" % Wno_format_truncation_command)
    ...
    doc.add("HOST.cross",
            int(options.cross != None or arch.mode.mode != arch.mode.default))
    if options.cross:
    ...
    doc.add("FEATURE.gtk.update.checks",
            int(not options.disable_gtk_update_checks))
    ...
    if build_tuple.match("*-*-darwin*") and options.cross is None:
        doc.add("FEATURE.xcode",
                int(not (Tools.xcodebuild.fail or options.disable_xcode)))
    ...
    stdout.write(" (%s)\n" % note_unsupported
                 ) if not host_tuple.system == "darwin" else stdout.write("\n")
    ...
    stdout.write(" (%s)\n" % note_unsupported) if not (
        host_tuple.system == "linux"
        or host_tuple.system == "mingw") else stdout.write("\n")
    stdout.write("Enable QSV:         %s" % options.enable_qsv)
    stdout.write(" (%s)\n" % note_unsupported) if not (
        host_tuple.system == "linux" or host_tuple.system == "freebsd"
        or host_tuple.system == "mingw") else stdout.write("\n")
