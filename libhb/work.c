 ...
 */
hb_thread_t *hb_work_init(hb_list_t *jobs, volatile int *die,
                          hb_error_code *error, hb_job_t **job) {
...
void hb_display_job_info(hb_job_t *job) {
  ...
  if (job->pts_to_start || job->pts_to_stop) {
    ...
    if (job->pts_to_stop) {
      hb_log("   + title %d, start %02d:%02d:%02.2f stop %02d:%02d:%02.2f",
             title->index, hr_start, min_start, sec_start, hr_stop, min_stop,
             sec_stop);
    } else {
      hb_log("   + title %d, start %02d:%02d:%02.2f", title->index, hr_start,
             min_start, sec_start);
    }
  } else if (job->frame_to_start || job->frame_to_stop) {
    hb_log("   + title %d, frames %d to %d", title->index, job->frame_to_start,
           job->frame_to_start + job->frame_to_stop - 1);
  } else {
  ...
  if (hb_list_count(job->list_filter)) {
    hb_log("   + %s",
           hb_list_count(job->list_filter) > 1 ? "filters" : "filter");
    for (i = 0; i < hb_list_count(job->list_filter); i++) {
      hb_filter_object_t *filter = hb_list_item(job->list_filter, i);
      if (filter->aliased && global_verbosity_level < 2) {
        continue;
      }
      char *settings = hb_filter_settings_string(filter->id, filter->settings);
      if (settings != NULL)
      ...
      if (filter->info) {
        ...
        if (info != NULL && info->human_readable_desc != NULL &&
  ...
  if (!job->indepth_scan) {
    /* Video encoder */
    hb_log("   + encoder: %s", hb_video_encoder_get_long_name(job->vcodec));
    ...
    if (job->vquality > HB_INVALID_VIDEO_QUALITY) {
      ...
      if (job->pass_id == HB_PASS_ENCODE_1ST && job->fastfirstpass == 1 &&
        ...
        if (job->vcodec & HB_VCODEC_X264_MASK) {
          ...
          hb_log("                analyse=i4x4 (if originally enabled, else "
                 "analyse=none)");
          hb_log("                subq=2 (if originally greater than 2, else "
                 "subq unchanged)");
        }
      }
    }

    hb_log("     + color profile: %d-%d-%d", job->color_prim,
           job->color_transfer, job->color_matrix);
  }

  if (job->indepth_scan) {
    hb_log(" * Foreign Audio Search: %s%s%s",
           job->select_subtitle_config.dest == RENDERSUB ? "Render/Burn-in"
                                                         : "Passthrough",
  ...
  for (i = 0; i < hb_list_count(job->list_subtitle); i++) {
    ...
    if (subtitle) {
      if (job->indepth_scan) {
        hb_log("   + subtitle, %s (track %d, id 0x%x, %s)", subtitle->lang,
  ...
  if (!job->indepth_scan) {
    for (i = 0; i < hb_list_count(job->list_audio); i++) {
      ...
      hb_log("   + decoder: %s (track %d, id 0x%x)",
             audio->config.lang.description, audio->config.in.track + 1,
             audio->id);

      if (audio->config.in.bitrate >= 1000)
        hb_log("     + bitrate: %d kbps, samplerate: %d Hz",
               audio->config.in.bitrate / 1000, audio->config.in.samplerate);
      else
        hb_log("     + samplerate: %d Hz", audio->config.in.samplerate);

      if (audio->config.out.codec & HB_ACODEC_PASS_FLAG) {
        hb_log("   + %s", hb_audio_encoder_get_name(audio->config.out.codec));
      } else {
        hb_log("   + mixdown: %s",
...
void correct_framerate(hb_interjob_t *interjob, hb_job_t *job) {
  ...
  if (ABS(((double)job->orig_vrate.num / job->orig_vrate.den) -
    ...
    hb_log("work: correcting framerate, %d/%d -> %d/%d", job->orig_vrate.num,
           job->orig_vrate.den, job->vrate.num, job->vrate.den);
  }
}
...
static void analyze_subtitle_scan(hb_job_t *job) {
  ...
  for (i = 0; i < hb_list_count(job->list_subtitle); i++) {
    ...
    hb_log("Subtitle track %d (id 0x%x) '%s': %d hits (%d forced)",
           subtitle->track, subtitle->id, subtitle->lang, subtitle->hits,
    ...
    if (subtitle_lowest == 0 || subtitle_lowest > subtitle->hits) {
...
static int sanitize_subtitles(hb_job_t *job) {
  ...
  if (interjob->select_subtitle != NULL) {
    ...
    for (i = 0; i < hb_list_count(job->list_subtitle);) {
      ...
      if ((interjob->select_subtitle->id == subtitle->id) &&
          ((subtitle->config.force &&
            interjob->select_subtitle->forced_hits == 0) ||
           (subtitle->config.force ==
            interjob->select_subtitle->config.force) ||
           (!subtitle->config.force &&
            interjob->select_subtitle->hits ==
                interjob->select_subtitle->forced_hits))) {
        hb_list_rem(job->list_subtitle, subtitle);
        free(subtitle);
        continue;
      }
    ...
    if (job->pass_id == HB_PASS_ENCODE || job->pass_id == HB_PASS_ENCODE_2ND) {
      ...
      hb_list_insert(job->list_subtitle, 0,
                     hb_subtitle_copy(interjob->select_subtitle));
    }
  }
  ...
  for (i = 0; i < hb_list_count(job->list_subtitle);) {
    ...
    if (subtitle->config.dest == RENDERSUB) {
      if (one_burned) {
        if (!hb_subtitle_can_pass(subtitle->source, job->mux)) {
          hb_log("More than one subtitle burn-in requested, dropping track %d.",
                 i);
          hb_list_rem(job->list_subtitle, subtitle);
          free(subtitle);
          continue;
        } else {
          hb_log("More than one subtitle burn-in requested.  Changing track %d "
                 "to soft subtitle.",
                 i);
          subtitle->config.dest = PASSTHRUSUB;
        }
      } else if (!hb_subtitle_can_burn(subtitle->source)) {
        hb_log("Subtitle burn-in requested and input track can not be "
               "rendered.  Changing track %d to soft subtitle.",
               i);
        subtitle->config.dest = PASSTHRUSUB;
      } else {
        one_burned = 1;
    ...
    if (subtitle->config.dest == PASSTHRUSUB &&
      ...
      if (!one_burned) {
        hb_log("Subtitle pass-thru requested and input track is not compatible "
               "with container.  Changing track %d to burned-in subtitle.",
               i);
        ...
        hb_log("Subtitle pass-thru requested and input track is not compatible "
               "with container.  One track already burned, dropping track %d.",
               i);
        hb_list_rem(job->list_subtitle, subtitle);
        free(subtitle);
        continue;
...
static int sanitize_audio(hb_job_t *job) {
  ...
  for (i = 0; i < hb_list_count(job->list_audio);) {
    ...
    if (audio->config.out.codec == HB_ACODEC_AUTO_PASS) {
      ...
      hb_log("Auto Passthru error, dropping track %d", audio->config.out.track);
  ...
  for (i = 0; i < hb_list_count(job->list_audio); i++) {
    ...
    if (audio->config.out.codec & HB_ACODEC_PASS_FLAG) {
      ...
      audio->config.out.samples_per_frame = audio->config.in.samples_per_frame;
    ...
    best_samplerate = hb_audio_samplerate_find_closest(
    ...
    if (audio->config.out.mixdown <= HB_AMIXDOWN_NONE) {
      ...
      best_mixdown = hb_mixdown_get_best(audio->config.out.codec,
    ...
    if (audio->config.out.compression_level < 0) {
      ...
      if (audio->config.out.compression_level >= 0) {
        hb_log("work: compression level not specified, track %d setting "
               "compression level %.2f",
               audio->config.out.track, audio->config.out.compression_level);
      }
    } else {
      float best_compression = hb_audio_compression_get_best(
    ...
    if (audio->config.out.quality != HB_INVALID_AUDIO_QUALITY) {
      float best_quality = hb_audio_quality_get_best(audio->config.out.codec,
      ...
      if (best_quality != audio->config.out.quality) {
        if (best_quality == HB_INVALID_AUDIO_QUALITY) {
          ...
          hb_log("work: sanitizing track %d quality %.2f to %.2f",
                 audio->config.out.track, audio->config.out.quality,
                 best_quality);
        }
    ...
    if (audio->config.out.quality == HB_INVALID_AUDIO_QUALITY) {
      if (audio->config.out.bitrate <= 0) {
        ...
        if (audio->config.out.bitrate > 0) {
          hb_log(
              "work: bitrate not specified, track %d setting bitrate %d Kbps",
              audio->config.out.track, audio->config.out.bitrate);
        }
      } else {
        best_bitrate = hb_audio_bitrate_get_best(
            audio->config.out.codec, audio->config.out.bitrate,
            audio->config.out.samplerate, audio->config.out.mixdown);
        if (best_bitrate > 0 && best_bitrate != audio->config.out.bitrate) {
    ...
    if (hb_audio_dither_is_supported(audio->config.out.codec)) {
      if (audio->config.out.dither_method == hb_audio_dither_get_default()) {
        /* "auto", enable with default settings */
        audio->config.out.dither_method = hb_audio_dither_get_default_method();
      }
...
static void do_job(hb_job_t *job) {
  ...
  if (job->list_filter && hb_list_count(job->list_filter)) {
    ...
    for (i = 0; i < hb_list_count(job->list_filter);) {
      ...
      if (filter->init != NULL && filter->init(filter, &init)) {
        hb_log("Failure to initialise filter '%s', disabling", filter->name);
        hb_list_rem(job->list_filter, filter);
        hb_filter_close(&filter);
        continue;
      }
   ...
   */
  hb_reduce(&job->orig_vrate.num, &job->orig_vrate.den, job->orig_vrate.num,
            job->orig_vrate.den);
  hb_reduce(&job->vrate.num, &job->vrate.den, job->vrate.num, job->vrate.den);
  ...
  if (job->list_filter && !job->indepth_scan) {
    for (i = 0; i < hb_list_count(job->list_filter); i++) {
      ...
      if (!filter->skip) {
        ...
        filter->thread =
            hb_thread_init(filter->name, filter_loop, filter, HB_LOW_PRIORITY);
      }
    }
  }
 ...
 * Loops calling work function for associated work object. Sleeps when fifo is
 * full. Monitors work done indicator. Exits loop when work indicator is set.
...
void hb_work_loop(void *_w) {
  ...
  while ((w->die == NULL || !*w->die) && !*w->done && w->fifo_in != NULL) {
    buf_in = hb_fifo_get_wait(w->fifo_in);
    if (buf_in != NULL)
      hb_buffer_close(&buf_in);
  }
...
static void filter_loop(void *_f) {
  ...
  while (!*f->done && f->status != HB_FILTER_DONE) {
    ...
    if (f->status == HB_FILTER_DELAY &&
      ...
      hb_filter_private_t_qsv *qsv_user =
          buf_in ? buf_in->qsv_details.filter_details
                 : last_buf_in->qsv_details.filter_details;
