...
static hb_buffer_t *FilterAudioFrame(sync_stream_t *stream, hb_buffer_t *buf);
static void SortedQueueBuffer(sync_stream_t *stream, hb_buffer_t *buf);
static hb_buffer_t *sanitizeSubtitle(sync_stream_t *stream, hb_buffer_t *sub);
...
static hb_buffer_t *CreateSilenceBuf(sync_stream_t *stream, int64_t dur,
  ...
  if (duration > 0) {
    ...
    size = (int)(duration * stream->audio.audio->config.in.samplerate / 90000) *
           sizeof(float) *
           hb_mixdown_get_discrete_channel_count(
               stream->audio.audio->config.out.mixdown);
    if (size > 0) {
...
static hb_buffer_t *CreateBlackBuf(sync_stream_t *stream, int64_t dur,
                                   int64_t pts) {
...
static void fixVideoOverlap(sync_stream_t *stream) {
  ...
  while ((buf = hb_list_item(stream->in_queue, 0)) != NULL) {
    ...
    if (overlap >= 0) {
      ...
      stream->drop_duration += 90000. * stream->common->job->title->vrate.den /
                               stream->common->job->title->vrate.num;
      stream->drop++;
      drop++;
      saveChap(stream, buf);
...
static void fixSubtitleOverlap(sync_stream_t *stream) {
  ...
  if (stream->subtitle.subtitle->source != SSASUB &&
    ...
    hb_log("sync: subtitle 0x%x time went backwards %d ms, PTS %" PRId64 "",
           stream->subtitle.subtitle->id, (int)overlap / 90, buf->s.start);
    hb_list_rem(stream->in_queue, buf);
    hb_buffer_close(&buf);
  }
}
...
static void streamFlush(sync_stream_t *stream) {
  while (hb_list_count(stream->in_queue) > 0) {
    ...
    if (buf != NULL) {
      ...
      if (!stream->first_frame) {
        if (buf->s.start >= 0) {
          ...
          case SYNC_TYPE_VIDEO:
            hb_log("sync: first pts video is %" PRId64, buf->s.start);
            break;
      ...
      if (stream->type == SYNC_TYPE_AUDIO || stream->type == SYNC_TYPE_VIDEO) {
        buf->s.start = stream->next_pts;
        buf->s.stop = stream->next_pts + buf->s.duration;
        stream->last_pts = stream->next_pts;
        stream->next_pts += buf->s.duration;
      } else {
        stream->next_pts = stream->last_pts = subtitle_last_pts;
      }
      ...
      stream->frame_count++;
      if (buf->s.duration > 0 && stream->min_frame_duration > buf->s.duration) {
        stream->min_frame_duration = buf->s.duration;
      }
...
static void log_chapter(sync_common_t *common, int chap_num, int nframes,
                        int64_t pts) {
  hb_chapter_t *c;
  ...
  if (c && c->title) {
    hb_log("sync: \"%s\" (%d) at frame %d time %" PRId64, c->title, chap_num,
           nframes, pts);
  } else {
    hb_log("sync: Chapter %d at frame %d time %" PRId64, chap_num, nframes,
           pts);
  }
}
  ...
  if (!(buf->s.flags & PROGRESSIVE) && !(buf->s.flags & TOP_FIRST)) {
    ...
    */
    ...
    cadence[0] = TB;
             ...
             !(buf->s.flags & REPEAT_FIRST)) {
    /* Progressive, but noting else.
       That means Bottom first,
       2 fields displayed.
    */
    // hb_log("MPEG2 Flag: Progressive. Bottom field first, 2 fields
    // displayed.");
    cadence[0] = BT_PROG;
  } else if ((buf->s.flags & PROGRESSIVE) && !(buf->s.flags & TOP_FIRST) &&
             (buf->s.flags & REPEAT_FIRST)) {
    /* Progressive, and repeat. .
       ...
       3 fields displayed.
    */
    // hb_log("MPEG2 Flag: Progressive repeat. Bottom field first, 3 fields
    // displayed.");
    cadence[0] = BTB_PROG;
  } else if ((buf->s.flags & PROGRESSIVE) && (buf->s.flags & TOP_FIRST) &&
             !(buf->s.flags & REPEAT_FIRST)) {
    ...
    */
    ...
    /* Progressive, top, repeat.
       ...
       3 fields displayed.
    */
    // hb_log("MPEG2 Flag: Progressive repeat. Top field first, 3 fields
    // displayed.");
    cadence[0] = TBT_PROG;
  }

  if ((cadence[2] <= TB) && (cadence[1] <= TB) && (cadence[0] > TB) &&
      (cadence[11])) {
...
static void OutputBuffer(sync_common_t *common) {
  ...
  do {
    ...
    if (!common->start_found) {
      ...
      if (!common->start_found) {
        if (out_stream->type == SYNC_TYPE_VIDEO) {
          UpdateSearchState(common, buf->s.start, out_stream->frame_count);
    ...
    if (common->stop_pts && buf->s.start >= common->stop_pts) {
      ...
      case SYNC_TYPE_VIDEO:
        ...
        break;
      case SYNC_TYPE_AUDIO:
        hb_log("sync: reached audio 0x%x pts %" PRId64 ", exiting early",
               out_stream->audio.audio->id, buf->s.start);
        break;
      case SYNC_TYPE_SUBTITLE:
        hb_log("sync: reached subtitle 0x%x pts %" PRId64 ", exiting early",
    ...
    if (out_stream->type == SYNC_TYPE_VIDEO && common->job->frame_to_stop &&
    ...
    if (out_stream->type == SYNC_TYPE_AUDIO ||
      ...
      out_stream->next_pts = out_stream->last_pts = subtitle_last_pts;
    }

    if (buf->s.stop > 0) {
      out_stream->current_duration = buf->s.stop - out_stream->first_pts;
    }
    out_stream->frame_count++;
    if (buf->s.duration > 0 &&
        out_stream->min_frame_duration > buf->s.duration) {
...
static void QueueBuffer(sync_stream_t *stream, hb_buffer_t *buf) {
  ...
  while (hb_list_count(stream->in_queue) > stream->max_len && !stream->done &&
         !stream->common->job->done && !*stream->common->job->die) {
  ...
  if (stream->common->pts_to_start == AV_NOPTS_VALUE) {
    ...
    if (job->pts_to_start > 0) {
      stream->common->start_pts = stream->common->pts_to_start =
  ...
  hb_deep_log(
      11, "type %8s id %x scr seq %d start %" PRId64 " stop %" PRId64 " dur %f",
...
static int InitAudio(sync_common_t *common, int index) {
  ...
  if (pv->stream->in_queue == NULL)
    goto fail;
  pv->stream->delta_list = hb_list_init();
  if (pv->stream->delta_list == NULL)
    goto fail;
  ...
  if (!(audio->config.out.codec & HB_ACODEC_PASS_FLAG) &&
    ...
    pv->stream->audio.resample = hb_audio_resample_init(
        AV_SAMPLE_FMT_FLT, audio->config.out.samplerate,
...
static int InitSubtitle(sync_common_t *common, int index) {
  ...
  // previous subtitle
  if (subtitle->config.dest == PASSTHRUSUB && subtitle->source != PGSSUB) {
...
static int syncVideoInit(hb_work_object_t *w, hb_job_t *job) {
  ...
  pv->common->streams = calloc(pv->common->stream_count, sizeof(sync_stream_t));

  // Allocate streams
  if (pv->common->streams == NULL)
    goto fail;

  // create audio and subtitle work list
  pv->common->list_work = hb_list_init();
  if (pv->common->list_work == NULL)
    goto fail;

  // mutex to mediate access to pv->common
  pv->common->mutex = hb_lock_init();
  if (pv->common->mutex == NULL)
    goto fail;
  ...
  if (job->pass_id == HB_PASS_ENCODE_2ND) {
    ...
    if (job->frame_to_stop) {
      ...
      pv->common->est_frame_count =
          duration * job->title->vrate.num / job->title->vrate.den / 90000;
    }
  }
  ...
  for (ii = 0; ii < hb_list_count(job->list_audio); ii++) {
    if (InitAudio(pv->common, ii))
      goto fail;
  }

  // Initialize subtitle sync work objects
  for (ii = 0; ii < hb_list_count(job->list_subtitle); ii++) {
    if (InitSubtitle(pv->common, ii))
      goto fail;
  }
  ...
  for (ii = 0; ii < hb_list_count(pv->common->list_work); ii++) {
    ...
    work->done = w->done;
    work->thread =
        hb_thread_init(work->name, hb_work_loop, work, HB_LOW_PRIORITY);
  }
...
static void syncVideoClose(hb_work_object_t *w) {
  ...
  hb_log("sync: got %d frames, %d expected", pv->stream->frame_count,
         pv->common->est_frame_count);
  if (pv->stream->min_frame_duration > 0 &&
      pv->stream->max_frame_duration > 0 && pv->stream->current_duration > 0) {
    hb_log("sync: framerate min %.3f fps, max %.3f fps, avg %.3f fps",
           ...
           (pv->stream->frame_count * 90000.) / pv->stream->current_duration);
  }
...
// buffers.
static hb_buffer_t *findOverlap(subtitle_sanitizer_t *sanitizer, int64_t start,
                                int64_t *stop_out) {
  hb_buffer_list_t list;
  hb_buffer_t *buf;
  int64_t stop;
...
static int mergeSubtitleOverlaps(subtitle_sanitizer_t *sanitizer) {
  ...
  if (a == NULL || a->s.start == AV_NOPTS_VALUE ||
      a->s.stop == AV_NOPTS_VALUE) {
  ...
  // completely merged.
  while (b != NULL && b->s.start < a->s.stop &&
         !(b->s.flags & HB_BUF_FLAG_EOF)) {
...
static hb_buffer_t *mergeSubtitles(sync_stream_t *stream) {
  ...
  if (!sanitizer->merge) {
    ...
    while (hb_buffer_list_count(&sanitizer->list_current) > 0) {
      buf = hb_buffer_list_head(&sanitizer->list_current);
      if (!(buf->s.flags & HB_BUF_FLAG_EOF) && buf->s.stop != AV_NOPTS_VALUE) {
...
hb_work_object_t hb_sync_video = {WORK_SYNC_VIDEO, "Video Synchronization",
                                  syncVideoInit, syncVideoWork, syncVideoClose};
...
static int syncAudioInit(hb_work_object_t *w, hb_job_t *job) { return 0; }
...
hb_work_object_t hb_sync_audio = {WORK_SYNC_AUDIO, "Audio Synchronization",
                                  syncAudioInit, syncAudioWork, syncAudioClose};
  ...
  if (!(audio->config.out.codec & HB_ACODEC_PASS_FLAG)) {
    ...
    if (stream->audio.resample != NULL) {
      ...
      out = hb_audio_resample(stream->audio.resample,
                              (const uint8_t **)&buf->data, nsamples);
      hb_buffer_close(&buf);
      if (out == NULL) {
        return NULL;
      }
...
static void UpdateState(sync_common_t *common, int frame_count) {
  ...
  if (hb_get_date() > common->st_dates[3] + 1000) {
    memmove(&common->st_dates[0], &common->st_dates[1], 3 * sizeof(uint64_t));
    memmove(&common->st_counts[0], &common->st_counts[1], 3 * sizeof(uint64_t));
...
static int syncSubtitleInit(hb_work_object_t *w, hb_job_t *job) { return 0; }
...
hb_work_object_t hb_sync_subtitle = {
    WORK_SYNC_SUBTITLE, "Subtitle Synchronization", syncSubtitleInit,
    syncSubtitleWork, syncSubtitleClose};
