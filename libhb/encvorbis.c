...
hb_work_object_t hb_encvorbis = {WORK_ENCVORBIS, "Vorbis encoder (libvorbis)",
                                 encvorbisInit, encvorbisWork, encvorbisClose};
...
int encvorbisInit(hb_work_object_t *w, hb_job_t *job) {
  ...
  /* get the 3 headers */
  vorbis_analysis_headerout(&pv->vd, &pv->vc, &header[0], &header[1],
                            &header[2]);
  ogg_packet *pheader;
  for (i = 0; i < 3; i++) {
  ...
  hb_audio_remap_build_table(&hb_vorbis_chan_map, audio->config.in.channel_map,
                             layout, pv->remap_table);
...
static hb_buffer_t *Flush(hb_work_object_t *w) {
  ...
  if (vorbis_analysis_blockout(&pv->vd, &pv->vb) == 1) {
    ...
    if (vorbis_bitrate_flushpacket(&pv->vd, &op)) {
      ...
      buf->s.start =
          (int64_t)(vorbis_granule_time(&pv->vd, op.granulepos) * 90000);
      buf->s.stop = (int64_t)(
          vorbis_granule_time(&pv->vd, (pv->prev_blocksize + blocksize) / 4 +
                                           op.granulepos) *
          90000);
      buf->s.duration = buf->s.stop - buf->s.start;
      /* The stop time isn't accurate for the first ~3 packets, as the actual
       * blocksize depends on the previous _and_ current packets. */
...
static hb_buffer_t *Encode(hb_work_object_t *w) {
  ...
  for (i = 0; i < OGGVORBIS_FRAME_SIZE; i++) {
    for (j = 0; j < pv->out_discrete_channels; j++) {
      buffer[j][i] =
          ((float *)
               pv->buf)[(pv->out_discrete_channels * i + pv->remap_table[j])];
    }
  }
