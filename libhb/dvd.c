...
hb_dvd_func_t hb_dvdread_func = {
    hb_dvdread_init,        hb_dvdread_close,       hb_dvdread_name,
    hb_dvdread_title_count, hb_dvdread_title_scan,  hb_dvdread_start,
    hb_dvdread_stop,        hb_dvdread_seek,        hb_dvdread_read,
    hb_dvdread_chapter,     hb_dvdread_angle_count, hb_dvdread_set_angle,
...
hb_dvd_func_t *hb_dvdread_methods(void) { return &hb_dvdread_func; }
...
static char *hb_dvdread_name(char *path) {
  ...
  if (DVDUDFVolumeInfo(reader, name, sizeof(name), unused, sizeof(unused))) {
    DVDClose(reader);
    return NULL;
  }
...
hb_dvd_t *hb_dvdread_init(hb_handle_t *h, const char *path) {
  ...
  d->path = strdup(path); /* hb_dvdread_title_scan assumes UTF-8 path, so not
                             path_ccp here */
  free(path_ccp);
...
static void add_subtitle(hb_list_t *list_subtitle, int position,
  ...
  case 6:
    ...
    break;
  case 7:
    subtitle->attributes = HB_SUBTITLE_ATTR_CC | HB_SUBTITLE_ATTR_CHILDREN;
  ...
  case 13:
    ...
    break;
  case 14:
    subtitle->attributes = HB_SUBTITLE_ATTR_COMMENTARY | HB_SUBTITLE_ATTR_LARGE;
    strcat(subtitle->lang, " Director's Commentary, Large Type");
    name = "Commentary, Large Type";
    break;
  case 15:
    subtitle->attributes =
        HB_SUBTITLE_ATTR_COMMENTARY | HB_SUBTITLE_ATTR_CHILDREN;
  ...
  hb_log("scan: id=0x%x, lang=%s, 3cc=%s ext=%i", subtitle->id, subtitle->lang,
         subtitle->iso639_2, lang_extension);
...
static hb_title_t *hb_dvdread_title_scan(hb_dvd_t *e, int t,
  ...
  for (i = 0; i < vts->vts_c_adt->nr_of_vobs; ++i) {
    if ((vts->vts_c_adt->cell_adr_table[i].start_sector & 0xffffff) ==
      ...
      hb_log("scan: cell_adr_table[%d].start_sector invalid (0x%x) "
             "- skipping title",
             i, vts->vts_c_adt->cell_adr_table[i].start_sector);
      goto fail;
    }
    if ((vts->vts_c_adt->cell_adr_table[i].last_sector & 0xffffff) ==
      ...
      hb_log("scan: cell_adr_table[%d].last_sector invalid (0x%x) "
             "- skipping title",
             i, vts->vts_c_adt->cell_adr_table[i].last_sector);
      goto fail;
    }
  ...
  hb_log("scan: duration is %02d:%02d:%02d (%" PRId64 " ms)", title->hours,
         title->minutes, title->seconds, title->duration / 90);
  ...
  for (i = 0; i < vts->vtsi_mat->nr_of_vts_audio_streams; i++) {
    ...
    audio_control = vts->vts_pgcit->pgci_srp[pgc_id - 1].pgc->audio_control[i];
    ...
    default:
      ...
      codec_name = "Unknown";
      hb_log("scan: unknown audio codec (%x)", audio_format);
      break;
    }
    if (!audio->id) {
      continue;
  ...
  for (i = 0; i < vts->vtsi_mat->nr_of_vts_subp_streams; i++) {
    ...
    spu_control = vts->vts_pgcit->pgci_srp[pgc_id - 1].pgc->subp_control[i];
    ...
    if (vts->vtsi_mat->vts_video_attr.display_aspect_ratio) {
      ...
      if (!(vts->vtsi_mat->vts_video_attr.permitted_df & 1)) {
        ...
        add_subtitle(
            title->list_subtitle, pos, lang, lang_ext,
            (uint8_t *)vts->vts_pgcit->pgci_srp[pgc_id - 1].pgc->palette,
  ...
  for (i = 0; i < vts->vts_ptt_srpt->title[title_ttn - 1].nr_of_ptts; i++) {
...
#undef cp
  ...
  for (i = 0; i < hb_list_count(title->list_chapter); i++) {
    ...
    hb_log("scan: chap %d, %" PRId64 " ms", chapter->index,
           chapter->duration / 90);
  }
  ...
  hb_log("scan: aspect = %d:%d", title->container_dar.num,
         title->container_dar.den);
...
static int hb_dvdread_start(hb_dvd_t *e, hb_title_t *title, int chapter) {
  ...
  if (!(d->file = DVDOpenFile(d->reader, d->vts, DVD_READ_TITLE_VOBS))) {
...
int is_nav_pack(unsigned char *buf) {
  ...
  if ((buf[0x26] == 0x00 && // PCI
       buf[0x27] == 0x00 && buf[0x28] == 0x01 && buf[0x29] == 0xbf &&
       buf[0x2c] == 0x00) &&
      (buf[0x400] == 0x00 && // DSI
       buf[0x401] == 0x00 && buf[0x402] == 0x01 && buf[0x403] == 0xbf &&
       buf[0x406] == 0x01)) {
    return (1);
  } else {
    return (0);
  ...
  if (!d->pack_len) {
    ...
    for (;;) {
      ...
      if (d->in_sync == 0 && d->cur_cell_id &&
        ...
        hb_log("dvd: left cell %d (%u,%u) for (%u,%u) at block %u", d->cell_cur,
               d->cur_vob_id, d->cur_cell_id, dsi_pack.dsi_gi.vobu_vob_idn,
               dsi_pack.dsi_gi.vobu_c_idn, d->next_vobu);
      ...
      if ((next_ptr & (1 << 31)) == 0 || (next_ptr & (1 << 30)) != 0 ||
      ...
      if (pack_len > 0 && pack_len < 1024 && block >= d->next_vobu &&
    ...
    if ((dsi_pack.vobu_sri.prev_vobu & (1 << 31)) == 0 ||
      ...
      if (d->pgc->cell_playback[d->cell_cur].first_sector <
              dsi_pack.dsi_gi.nv_pck_lbn &&
          d->pgc->cell_playback[d->cell_cur].last_sector >=
              dsi_pack.dsi_gi.nv_pck_lbn) {
        ...
        if (d->in_cell) {
          hb_error("dvd: assuming missed end of cell %d at block %d",
                   d->cell_cur, d->block);
          d->cell_cur = d->cell_next;
          d->next_vobu = d->pgc->cell_playback[d->cell_cur].first_sector;
          FindNextCell(d);
    ...
    if (DVDReadBlocks(d->file, d->block, 1, b->data) != 1) {
      ...
      hb_error("dvd: DVDReadBlocks failed (%d), skipping to vobu %u", d->block,
               d->next_vobu);
      d->pack_len = 0;
 ...
 * with dvdread
 **********************************************************************/
static int hb_dvdread_angle_count(hb_dvd_t *d) { return 1; }
...
static void FindNextCell(hb_dvdread_t *d) {
  ...
  if (d->pgc->cell_playback[d->cell_cur].block_type == BLOCK_TYPE_ANGLE_BLOCK) {
    ...
    hb_log("dvd: Skipping multi-angle cells %d-%d", d->cell_cur,
           d->cell_next - 1);
  } else {
...
char *hb_dvd_name(char *path) { return dvd_methods->name(path); }

hb_dvd_t *hb_dvd_init(hb_handle_t *h, const char *path) {
  return dvd_methods->init(h, path);
}

int hb_dvd_title_count(hb_dvd_t *d) { return dvd_methods->title_count(d); }
...
void hb_dvd_stop(hb_dvd_t *d) { dvd_methods->stop(d); }

int hb_dvd_seek(hb_dvd_t *d, float f) { return dvd_methods->seek(d, f); }

hb_buffer_t *hb_dvd_read(hb_dvd_t *d) { return dvd_methods->read(d); }

int hb_dvd_chapter(hb_dvd_t *d) { return dvd_methods->chapter(d); }

void hb_dvd_close(hb_dvd_t **_d) { dvd_methods->close(_d); }

int hb_dvd_angle_count(hb_dvd_t *d) { return dvd_methods->angle_count(d); }
