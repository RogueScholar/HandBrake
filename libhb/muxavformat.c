...
struct hb_mux_data_s {
  enum { MUX_TYPE_VIDEO, MUX_TYPE_AUDIO, MUX_TYPE_SUBTITLE } type;
...
enum { META_MUX_MP4, META_MUX_MKV, META_MUX_WEBM, META_MUX_LAST };
...
static int avformatInit(hb_mux_object_t *m) {
  ...
  max_tracks =
      1 + hb_list_count(job->list_audio) + hb_list_count(job->list_subtitle);
  ...
  case HB_VCODEC_QSV_H264:
    ...
    priv_data[10 + job->config.h264.sps_length] = job->config.h264.pps_length;

    memcpy(priv_data + 11 + job->config.h264.sps_length, job->config.h264.pps,
           job->config.h264.pps_length);
    break;
  ...
  case HB_VCODEC_FFMPEG_VT_H264:
    ...
    if (job->config.extradata.length > 0) {
      ...
      memcpy(priv_data, job->config.extradata.bytes,
  ...
  case HB_VCODEC_THEORA: {
    ...
    for (ii = 0; ii < 3; ii++) {
      ...
      size += 2;
      memcpy(priv_data + size, ogg_headers[ii]->packet, ogg_headers[ii]->bytes);
  ...
  for (ii = 0; ii < hb_list_count(job->list_audio); ii++) {
    ...
    track->st->codecpar->initial_padding =
        audio->priv.config.init_delay * audio->config.out.samplerate / 90000;
    ...
    case HB_ACODEC_OPUS:
      ...
      if (audio->priv.config.extradata.length) {
        ...
        memcpy(priv_data, audio->priv.config.extradata.bytes,
    ...
    case HB_ACODEC_FDK_HAAC:
      ...
      break;
    default:
      hb_error("muxavformat: Unknown audio codec: %x", audio->config.out.codec);
      goto error;
    }
    track->st->codecpar->extradata = priv_data;
    track->st->codecpar->extradata_size = priv_size;
    ...
    default: {
      ...
      for (jj = 0; jj < hb_list_count(job->list_audio); jj++) {
        ...
        if (fallback->config.in.track == audio->config.in.track &&
            (codec == HB_ACODEC_FFAAC || codec == HB_ACODEC_CA_AAC ||
             codec == HB_ACODEC_CA_HAAC || codec == HB_ACODEC_FDK_AAC ||
          ...
          sd = (int *)av_stream_new_side_data(
              track->st, AV_PKT_DATA_FALLBACK_TRACK, sizeof(int));
          if (sd != NULL) {
  ...
  for (ii = 0; ii < hb_list_count(job->list_subtitle); ii++) {
    ...
    case VOBSUB: {
      ...
      len = snprintf(subidx, 2048, subidx_fmt, subtitle->width,
                     subtitle->height, 0, 0, "OFF", rgb[0], rgb[1], rgb[2],
                     rgb[3], rgb[4], rgb[5], rgb[6], rgb[7], rgb[8], rgb[9],
                     rgb[10], rgb[11], rgb[12], rgb[13], rgb[14], rgb[15]);
    ...
    case IMPORTSSA: {
      if (job->mux == HB_MUX_AV_MP4) {
        ...
        track->tx3g =
            hb_tx3g_style_init(job->height, (char *)subtitle->extradata);
      } else {
    ...
    if (subtitle->config.name != NULL && subtitle->config.name[0] != 0) {
      ...
      av_dict_set(&track->st->metadata, "title", subtitle->config.name, 0);
      if (job->mux == HB_MUX_AV_MP4) {
        // Some software (MPC, mediainfo) use hdlr description
        // for track title
        av_dict_set(&track->st->metadata, "handler_name", subtitle->config.name,
                    0);
      }
    }
  }
    ...
    for (i = 0; i < hb_list_count(list_attachment); i++) {
      ...
      if ((attachment->type == FONT_TTF_ATTACH ||
        ...
        if (attachment->type == FONT_TTF_ATTACH) {
          ...
          av_dict_set(&st->metadata, "mimetype", "application/vnd.ms-opentype",
                      0);
        }
  ...
  if (job->metadata) {
    ...
    if (md->name && metadata_keys[META_TITLE][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_TITLE][meta_mux],
                  md->name, 0);
    }
    if (md->artist && metadata_keys[META_ARTIST][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_ARTIST][meta_mux],
                  md->artist, 0);
    }
    if (md->album_artist && metadata_keys[META_DIRECTOR][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_DIRECTOR][meta_mux],
                  md->album_artist, 0);
    }
    if (md->composer && metadata_keys[META_COMPOSER][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_COMPOSER][meta_mux],
                  md->composer, 0);
    }
    if (md->release_date &&
        metadata_keys[META_RELEASE_DATE][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_RELEASE_DATE][meta_mux],
                  md->release_date, 0);
    }
    if (md->comment && metadata_keys[META_COMMENT][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_COMMENT][meta_mux],
                  md->comment, 0);
    }
    if (!md->name && md->album && metadata_keys[META_ALBUM][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_ALBUM][meta_mux],
                  md->album, 0);
    }
    if (md->genre && metadata_keys[META_GENRE][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_GENRE][meta_mux],
                  md->genre, 0);
    }
    if (md->description && metadata_keys[META_DESCRIPTION][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_DESCRIPTION][meta_mux],
                  md->description, 0);
    }
    if (md->long_description &&
        metadata_keys[META_SYNOPSIS][meta_mux] != NULL) {
      av_dict_set(&m->oc->metadata, metadata_keys[META_SYNOPSIS][meta_mux],
...
static int add_chapter(hb_mux_object_t *m, int64_t start, int64_t end,
                       char *title) {
  AVChapter *chap;
  AVChapter **chapters;
...
static int avformatMux(hb_mux_object_t *m, hb_mux_data_t *track,
                       hb_buffer_t *buf) {
  AVPacket pkt;
  ...
  pts =
      av_rescale_q(buf->s.start, (AVRational){1, 90000}, track->st->time_base);
  ...
  if (duration < 0) {
    ...
    if (track->type == MUX_TYPE_SUBTITLE &&
        track->st->codecpar->codec_id != AV_CODEC_ID_HDMV_PGS_SUBTITLE) {
      duration = av_rescale_q(10, (AVRational){1, 1}, track->st->time_base);
    } else if (track->type == MUX_TYPE_VIDEO) {
      duration = av_rescale_q((int64_t)job->vrate.den * 90000 / job->vrate.num,
                              (AVRational){1, 90000}, track->st->time_base);
    } else {
      duration = 0;
    }
  ...
  case MUX_TYPE_VIDEO: {
    if (job->chapter_markers && buf->s.new_chap) {
      if (track->current_chapter > 0) {
        ...
        if (chapter != NULL && track->prev_chapter_tc != AV_NOPTS_VALUE &&
    ...
    if (job->mux == HB_MUX_AV_MP4) {
      ...
      if (track->duration < pts) {
        ...
        if (ret < 0) {
          ...
          hb_error("avformatMux: track %d, av_interleaved_write_frame failed "
                   "with error '%s' (empty_pkt)",
      ...
      if (track->st->codecpar->codec_id == AV_CODEC_ID_MOV_TEXT) {
         ...
         */
        hb_muxmp4_process_subtitle_style(track->tx3g, buf->data, &buffer,
                                         &styleatom, &stylesize);
  ...
  if (ret < 0 || m->oc->pb->error != 0) {
    ...
    hb_error("avformatMux: track %d, av_interleaved_write_frame failed with "
             "error '%s'",
             track->st->index, errstr);
    *job->done_error = HB_ERROR_UNKNOWN;
    *job->die = 1;
  ...
  for (ii = 0; ii < hb_list_count(job->list_audio); ii++) {
    ...
    case HB_ACODEC_FFFLAC24:
      if (audio->priv.config.extradata.length) {
        ...
        priv_size = audio->priv.config.extradata.length;
        priv_data = av_realloc(st->codecpar->extradata,
                               priv_size + AV_INPUT_BUFFER_PADDING_SIZE);
        if (priv_data == NULL) {
          break;
        }
        memcpy(priv_data, audio->priv.config.extradata.bytes,
