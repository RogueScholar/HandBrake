...
static inline int check_mpeg_scr(hb_psdemux_t *state, int64_t scr, int tol) {
  ...
  if (state->last_scr == AV_NOPTS_VALUE || scr_delta > 90 * tol ||
      scr_delta < -90 * 10) {
    ++state->scr_changes;
...
#define d (buf->data)
    /* pack_header */
    if (d[pos] != 0 || d[pos + 1] != 0 || d[pos + 2] != 0x1 ||
        d[pos + 3] != 0xBA) {
      hb_log("demux_dvd_ps: not a PS packet (%02x%02x%02x%02x)", d[pos],
    ...
    /* pes */
    while (pos + 6 < buf->size && d[pos] == 0 && d[pos + 1] == 0 &&
           d[pos + 2] == 0x1) {
      int id;
      ...
      if (id != 0xE0 && id != 0xBD && (id & 0xC0) != 0xC0) {
        /* Not interesting */
        pos = pes_packet_end;
        continue;
      }
      ...
      if (has_pts) {
        pts = ((uint64_t)(d[pos] & 0xe) << 29) + (d[pos + 1] << 22) +
              ((d[pos + 2] >> 1) << 15) + (d[pos + 3] << 7) + (d[pos + 4] >> 1);
        if (has_pts & 1) {
          dts = ((uint64_t)(d[pos + 5] & 0xe) << 29) + (d[pos + 6] << 22) +
                ((d[pos + 7] >> 1) << 15) + (d[pos + 8] << 7) +
                (d[pos + 9] >> 1);
        } else {
          dts = pts;
        }
...
static void demux_null(hb_buffer_t *buf, hb_buffer_list_t *list_es,
  ...
  while (buf) {
    ...
    if (state) {
      ...
      if (state->scr_changes == 0 && (buf->s.start != AV_NOPTS_VALUE ||
                                      buf->s.renderOffset != AV_NOPTS_VALUE)) {
        ++state->scr_changes;
        state->last_scr =
            buf->s.start >= 0 ? buf->s.start : buf->s.renderOffset;
      }
