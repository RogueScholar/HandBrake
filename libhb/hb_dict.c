...
hb_value_t *hb_value_incref(hb_value_t *value) { return json_incref(value); }
...
hb_value_t *hb_value_null() { return json_null(); }
...
hb_value_t *hb_value_int(json_int_t value) { return json_integer(value); }

hb_value_t *hb_value_double(double value) { return json_real(value); }

hb_value_t *hb_value_bool(int value) { return json_boolean(value); }
...
static hb_value_t *xform_dict(const hb_value_t *dict, hb_value_type_t type) {
  ...
  case HB_VALUE_TYPE_STRING: {
    char *r = strdup("");
    hb_dict_iter_t iter;
    for (iter = hb_dict_iter_init(dict); iter != HB_DICT_ITER_DONE;
      ...
      char *tmp = r;
      r = hb_strdup_printf("%s%s%s%s:", r, k, s ? "=" : "", s ? s : "");
      free(tmp);
      hb_value_free(&x);
    }
    int len = strlen(r);
...
void hb_dict_free(hb_dict_t **_dict) { hb_value_free(_dict); }

hb_dict_t *hb_dict_init() { return json_object(); }

int hb_dict_elements(hb_dict_t *dict) { return json_object_size(dict); }
...
int hb_dict_extract_rational(hb_rational_t *dst, const hb_dict_t *dict,
  ...
  if (hb_value_type(val) == HB_VALUE_TYPE_DICT) {
    ...
    if (rational[0] != NULL && rational[1] != NULL && isdigit(rational[0][0]) &&
        isdigit(rational[1][0])) {
...
int hb_dict_extract_int_array(int *dst, int count, const hb_dict_t *dict,
                              const char *key) {
  if (dict == NULL || key == NULL || dst == NULL) {
    return 0;
  }
...
int hb_dict_iter_next_ex(const hb_dict_t *dict, hb_dict_iter_t *iter,
...
hb_value_array_t *hb_value_array_init() { return json_array(); }

void hb_value_array_clear(hb_value_array_t *array) { json_array_clear(array); }

hb_value_t *hb_value_array_get(const hb_value_array_t *array, int index) {
  return json_array_get(array, index);
}

void hb_value_array_set(hb_value_array_t *array, int index, hb_value_t *value) {
  if (index < 0 || index >= json_array_size(array)) {
    hb_error("hb_value_array_set: invalid index %d size %zu", index,
...
void hb_value_array_append(hb_value_array_t *array, hb_value_t *value) {
  json_array_append_new(array, value);
}

void hb_value_array_concat(hb_value_array_t *array, hb_value_t *value) {
...
void hb_value_array_copy(hb_value_array_t *dst, const hb_value_array_t *src,
                         int count) {
  size_t len;
  int ii;
