...
static int hb_bitvec_and_cmp(hb_bitvec_t *bv1, hb_bitvec_t *bv2,
                             hb_bitvec_t *bv3) {
  if (bv1->size != bv2->size)
    return 0;
...
// human time scales (seconds) rather than near the video frame rate
// (milliseconds). If intermittent sources were treated like continuous sources
// huge sections of audio and video would get buffered waiting for the next
// subtitle to show up. To keep this from happening the muxer doesn't wait for
// intermittent tracks (essentially it assumes that they will always go through
// the HB processing pipeline faster than the associated video). They are still
// time aligned and interleaved at the appropriate point in the output file.
...
static hb_buffer_t *mf_peek(hb_track_t *track) {
  return track->mf.out == track->mf.in
             ? NULL
             : track->mf.fifo[track->mf.out & (track->mf.flen - 1)];
}
...
static void muxClose(hb_work_object_t *muxer) {
  ...
  if (job->pass_id == HB_PASS_ENCODE || job->pass_id == HB_PASS_ENCODE_2ND) {
    ...
    if (!hb_stat(job->file, &sb)) {
      ...
      for (i = 0; i < mux->ntracks; ++i) {
        track = mux->track[i];
        hb_log("mux: track %d, %" PRId64 " frames, %" PRId64
               " bytes, %.2f kbps, fifo %d",
               i, track->frames, track->bytes,
               90000.0 * track->bytes / mux->pts / 125, track->mf.flen);
        if (!i && job->vquality <= HB_INVALID_VIDEO_QUALITY) {
          /* Video */
          hb_deep_log(
              2, "mux: video bitrate error, %+" PRId64 " bytes",
      ...
      if (bytes_total && frames_total) {
        hb_deep_log(2, "mux: overhead, %.2f bytes per frame",
                    (float)(sb.st_size - bytes_total) / frames_total);
      }
    }
  }
...
hb_work_object_t hb_muxer = {WORK_MUX, "Muxer", muxInit, muxWork, muxClose};
