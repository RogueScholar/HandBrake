...
hb_work_object_t hb_encavcodec = {WORK_ENCAVCODEC,
                                  "FFMPEG encoder (libavcodec)", encavcodecInit,
                                  encavcodecWork, encavcodecClose};
...
static const char *const h26x_nvenc_preset_names[] = {
    "hq",   "hp",      "fast", "medium",
    "slow", "default", NULL // No Lossless "losslesshp", "lossless", "llhp",
                            // "llhq", "ll", "bd". We don't need them.
};
...
static const char *const h265_nvenc_profile_names[] = {
    "auto", "main", NULL // "main10", "rext"  We do not currently support 10bit
                         // encodes with this encoder.
};

static const char *const h26x_vt_preset_name[] = {"default", NULL};

static const char *const h264_vt_profile_name[] = {"auto", "baseline", "main",
                                                   "high", NULL};
...
int encavcodecInit(hb_work_object_t *w, hb_job_t *job) {
  ...
  if (codec->supported_framerates) {
    ...
    supported_fps = codec->supported_framerates[av_find_nearest_q_idx(
    ...
    hb_log("encavcodec: truncating framerate %d / %d", fps.num, fps.den);
  ...
  context->gop_size =
      ((double)job->orig_vrate.num / job->orig_vrate.den + 0.5) * 10;
  if ((job->vcodec == HB_VCODEC_FFMPEG_VCE_H264) ||
    ...
    if (job->encoder_preset != NULL && *job->encoder_preset) {
      if ((!strcasecmp(job->encoder_preset, "balanced")) ||
        ...
        av_opt_set(context, "quality", job->encoder_preset,
  ...
  hb_dict_iter_t iter;
  for (iter = hb_dict_iter_init(lavc_opts); iter != HB_DICT_ITER_DONE;
  ...
  if (job->vquality <= HB_INVALID_VIDEO_QUALITY) {
    ...
    if (w->codec_param == AV_CODEC_ID_VP8 ||
      ...
      context->bit_rate = (int64_t)job->width * job->height * fps.num / fps.den;
  ...
  hb_log("encavcodec: encoding with stored aspect %d/%d", job->par.num,
         job->par.den);
  ...
  if (job->pass_id == HB_PASS_ENCODE_1ST ||
    ...
    if (job->pass_id == HB_PASS_ENCODE_1ST) {
      ...
      if (!pv->file) {
        ...
        hb_error("encavcodecInit: Failed to open %s (reason: %s)", filename,
                 reason);
        free(filename);
        ret = 1;
        goto done;
      ...
      log = malloc(size + 1);
      log[size] = '\0';
      if (size > 0 && fread(log, size, 1, pv->file) < size) {
        if (ferror(pv->file)) {
          if (strerror_r(errno, reason, 79) != 0)
            strcpy(reason, "unknown -- strerror_r() failed");

          hb_error("encavcodecInit: Failed to read %s (reason: %s)", filename,
                   reason);
          free(filename);
          ret = 1;
          fclose(pv->file);
  ...
  if (job->pass_id == HB_PASS_ENCODE_1ST && context->stats_out != NULL) {
...
static hb_buffer_t *process_delay_list(hb_work_private_t *pv,
                                       hb_buffer_t *buf) {
  if (pv->job->areBframes) {
