...
char *hb_utf8_to_cp(const char *src) {
  ...
  int len =
      WideCharToMultiByte(GetACP(), 0, tmp, num_chars, NULL, 0, NULL, NULL);
  if (len <= 0)
    return NULL;
...
void hb_snooze(int delay) {
  ...
  snooze(1000 * delay);
    ...
    defined(SYS_NETBSD) || defined(SYS_SunOS)
  usleep(1000 * delay);
...
int hb_get_cpu_count() { return hb_cpu_info.count; }

int hb_get_cpu_platform() { return hb_cpu_info.platform; }

const char *hb_get_cpu_name() { return hb_cpu_info.name; }
...
static void init_cpu_info() {
  ...
  if (av_get_cpu_flags() & AV_CPU_FLAG_SSE) {
    ...
    // 4/January 2019 Table 2-1. CPUID Signature Values of
    ...
    if ((eax & 0x80000004) < 0x80000004) {
      cpuid(0x80000002, &hb_cpu_info.buf4[0], &hb_cpu_info.buf4[1],
            &hb_cpu_info.buf4[2], &hb_cpu_info.buf4[3]);
      cpuid(0x80000003, &hb_cpu_info.buf4[4], &hb_cpu_info.buf4[5],
            &hb_cpu_info.buf4[6], &hb_cpu_info.buf4[7]);
      cpuid(0x80000004, &hb_cpu_info.buf4[8], &hb_cpu_info.buf4[9],
            &hb_cpu_info.buf4[10], &hb_cpu_info.buf4[11]);
...
#elif defined(SYS_DARWIN) || defined(SYS_FREEBSD) || defined(SYS_NETBSD) ||    \
    defined(SYS_OPENBSD)
...
void hb_get_user_config_directory(char path[512]) {
  ...
  if (SHGetKnownFolderPath(&FOLDERID_RoamingAppData, 0, NULL, &wide_path) ==
          S_OK &&
      WideCharToMultiByte(CP_UTF8, 0, wide_path, -1, path, 512, NULL, NULL) !=
          0) {
...
char *hb_get_temporary_directory() {
  ...
  while ((p = strchr(base, '\\')))
    *p = '/';
#else
  if ((p = getenv("TMPDIR")) != NULL || (p = getenv("TEMP")) != NULL)
    base = strdup(p);
  else
    base = strdup("/tmp");
#endif
...
struct dirent *hb_readdir(HB_DIR *dir) {
  ...
  int len =
      WideCharToMultiByte(CP_UTF8, 0, entry->d_name, -1, dir->entry.d_name,
                          sizeof(dir->entry.d_name), NULL, NULL);
...
static void attribute_align_thread hb_thread_func(void *_t) {
  ...
  hb_deep_log(2, "thread %" PRIx64 " exited (\"%s\")", hb_thread_to_integer(t),
              t->name);
  hb_lock(t->lock);
  t->exited = 1;
  hb_unlock(t->lock);
...
#if defined(SYS_BEOS)
  t->thread = spawn_thread((thread_func)hb_thread_func, name, priority, t);
...
//        SetThreadPriority( GetCurrentThread(), THREAD_PRIORITY_BELOW_NORMAL );
  ...
  hb_deep_log(2, "thread %" PRIx64 " started (\"%s\")", hb_thread_to_integer(t),
              t->name);
  return t;
}
...
//    WaitForSingleObject( t->thread, INFINITE );
  ...
  hb_deep_log(2, "thread %" PRIx64 " joined (\"%s\")", hb_thread_to_integer(t),
              t->name);
...
void hb_yield(void) { sched_yield(); }
...
hb_net_t *hb_net_open(char *address, int port) {
  ...
  if (connect(n->socket, (struct sockaddr *)&sock, sizeof(struct sockaddr_in)) <
      0) {
...
void hb_system_sleep_private_enable(void *opaque) {
  ...
  if (success == kIOReturnSuccess) {
    hb_deep_log(3, "hb_system_sleep: assertion %d released, sleep allowed",
                *assertionID);
    *assertionID = -1;
  } else {
...
void hb_system_sleep_private_disable(void *opaque) {
  ...
  if (success == kIOReturnSuccess) {
    hb_deep_log(3, "hb_system_sleep: assertion %d created, sleep prevented",
                *assertionID);
  } else {
...
static int try_adapter(const char *name, const char *dir, const char *prefix,
                       int node_start, int node_last) {
  int node;
  int len = strlen(name);
  ...
  for (node = node_start; node <= node_last; node++) {
    ...
    if (!ioctl(fd, DRM_IOCTL_VERSION, &version) && version.name_len == len &&
        !strncmp(driverName, name, len)) {
      free(driverName);
      return fd;
    }
    close(fd);
...
static int open_adapter(const char *name) {
  int fd = try_adapter(name, DRI_PATH, DRI_NODE_RENDER, DRI_RENDER_NODE_START,
                       DRI_RENDER_NODE_LAST);
  if (fd < 0) {
    fd = try_adapter(name, DRI_PATH, DRI_NODE_CARD, DRI_CARD_NODE_START,
                     DRI_CARD_NODE_LAST);
  }
  return fd;
}
...
hb_display_t *hb_display_init(const char *driver_name,
  ...
  if ((env = getenv("LIBVA_DRIVER_NAME")) != NULL) {
    ...
    for (ii = 0; interface_names[ii] != NULL; ii++) {
      hb_log("hb_display_init: attempting VA driver '%s'", interface_names[ii]);
...
void hb_display_close(hb_display_t **_d) { (void)_d; }
