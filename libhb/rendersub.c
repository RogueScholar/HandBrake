...
static int vobsub_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                       hb_buffer_t **buf_out);
...
static int ssa_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                    hb_buffer_t **buf_out);
...
static int textsub_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                        hb_buffer_t **buf_out);
...
static int pgssub_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                       hb_buffer_t **buf_out);
...
static int hb_rendersub_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                             hb_buffer_t **buf_out);
...
static void blend(hb_buffer_t *dst, hb_buffer_t *src, int left, int top) {
  ...
  for (yy = y0; yy < hh; yy++) {
    ...
    for (xx = x0; xx < ww; xx++) {
       ...
       */
      y_out[left + xx] = ((uint16_t)y_out[left + xx] * (255 - alpha) +
                          (uint16_t)y_in[xx] * alpha) /
                         255;
    }
  }
...
static void ApplySub(hb_filter_private_t *pv, hb_buffer_t *buf,
                     hb_buffer_t *sub) {
  blend(buf, sub, sub->f.x, sub->f.y);
}

static hb_buffer_t *ScaleSubtitle(hb_filter_private_t *pv, hb_buffer_t *sub,
                                  hb_buffer_t *buf) {
  hb_buffer_t *scaled;
  ...
  if (ABS(xfactor - 1) > 0.01 || ABS(yfactor - 1) > 0.01) {
    ...
    if (pv->sws == NULL || pv->sws_width != width || pv->sws_height != height) {
      if (pv->sws != NULL)
        sws_freeContext(pv->sws);
      pv->sws =
          hb_sws_get_context(sub->f.width, sub->f.height, sub->f.fmt,
                             scaled->f.width, scaled->f.height, sub->f.fmt,
                             SWS_LANCZOS | SWS_ACCURATE_RND, SWS_CS_DEFAULT);
      pv->sws_width = width;
      pv->sws_height = height;
    }
    sws_scale(pv->sws, (const uint8_t *const *)in_data, in_stride, 0,
              sub->f.height, out_data, out_stride);
  } else {
   ...
   */
  margin_top =
      ((buf->f.height - pv->crop[0] - pv->crop[1]) * margin_percent) / 100;
  ...
  if (scaled->f.height >
    ...
    top = pv->crop[0] + margin_top;
             ...
             buf->f.height - pv->crop[1] - margin_top - scaled->f.height) {
    /*
  ...
  if (scaled->f.width > buf->f.width - pv->crop[2] - pv->crop[3] - 40)
    left = pv->crop[2] +
           (buf->f.width - pv->crop[2] - pv->crop[3] - scaled->f.width) / 2;
...
static void ApplyVOBSubs(hb_filter_private_t *pv, hb_buffer_t *buf) {
  ...
  for (ii = 0; ii < hb_list_count(pv->sub_list);) {
    ...
    if ((sub->s.stop != AV_NOPTS_VALUE && sub->s.stop <= buf->s.start) ||
        (next != NULL && sub->s.stop == AV_NOPTS_VALUE &&
         next->s.start <= buf->s.start)) {
...
static void ApplySSASubs(hb_filter_private_t *pv, hb_buffer_t *buf) {
  ...
  frameList =
      ass_render_frame(pv->renderer, pv->ssaTrack, buf->s.start / 90, NULL);
  if (!frameList)
    return;
...
static int ssa_post_init(hb_filter_object_t *filter, hb_job_t *job) {
  ...
  for (i = 0; i < hb_list_count(list_attachment); i++) {
    ...
    if (attachment->type == FONT_TTF_ATTACH ||
      ...
      ass_add_font(pv->ssa, attachment->name, attachment->data,
                   attachment->size);
    }
  }
...
static int ssa_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
  ...
  if (!pv->script_initialized) {
    ...
    ass_process_codec_private(pv->ssaTrack, (char *)filter->subtitle->extradata,
  ...
  while ((sub = hb_fifo_get(filter->subtitle->fifo_out))) {
    ...
    ass_process_chunk(pv->ssaTrack, (char *)sub->data, sub->size,
                      sub->s.start / 90, (sub->s.stop - sub->s.start) / 90);
    hb_buffer_close(&sub);
  }
...
static int cc608sub_post_init(hb_filter_object_t *filter, hb_job_t *job) {
  ...
  hb_subtitle_add_ssa_header(filter->subtitle, HB_FONT_MONO, .08 * safe_height,
                             width, height);
  ...
  while ((sub = hb_fifo_get(filter->subtitle->fifo_out))) {
    if (sub->s.flags & HB_BUF_FLAG_EOF) {
      ...
      if (pv->current_sub != NULL) {
        ...
        pv->current_sub->s.stop = pv->current_sub->s.start + 90000LL * 10;
