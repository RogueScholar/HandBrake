...
static int comb_detect_init(hb_filter_object_t *filter, hb_filter_init_t *init);

static int comb_detect_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                            hb_buffer_t **buf_out);
...
static void apply_mask_line(uint8_t *srcp, uint8_t *mskp, int width) {
  int x;
...
static void check_combing_mask(hb_filter_private_t *pv, int segment, int start,
                               int stop) {
  /* Go through the mask in X*Y blocks. If any of these windows
     have threshold or more combed pixels, consider the whole
     frame to be combed and send it on to be deinterlaced.     */
    ...
    for (y = segment_start; y < segment_stop; y++) {
      ...
      for (x = 0; x < width; x++) {
        ...
        if ((up_diff > athresh && down_diff > athresh) ||
          ...
          int motion = 0;
          if (mthresh > 0) {
            /* Make sure there's sufficient motion between frame t-1 to frame
             * t+1. */
            if (fabs(pv->gamma_lut[prev[0]] - pv->gamma_lut[cur[0]]) >
                    mthresh &&
                fabs(pv->gamma_lut[cur[up_1]] - pv->gamma_lut[next[up_1]]) >
                    mthresh &&
                fabs(pv->gamma_lut[cur[down_1]] - pv->gamma_lut[next[down_1]]) >
                    mthresh)
              motion++;
            if (fabs(pv->gamma_lut[next[0]] - pv->gamma_lut[cur[0]]) >
                    mthresh &&
                fabs(pv->gamma_lut[prev[up_1]] - pv->gamma_lut[cur[up_1]]) >
                    mthresh &&
                fabs(pv->gamma_lut[prev[down_1]] - pv->gamma_lut[cur[down_1]]) >
                    mthresh)
              motion++;
          ...
          if (motion || pv->frames == 0) {
            ...
            combing = fabs(
                pv->gamma_lut[cur[up_2]] + (4 * pv->gamma_lut[cur[0]]) +
                pv->gamma_lut[cur[down_2]] -
                (3 * (pv->gamma_lut[cur[up_1]] + pv->gamma_lut[cur[down_1]])));
...
static void detect_combed_segment(hb_filter_private_t *pv, int segment_start,
                                  int segment_stop) {
  /* A mish-mash of various comb detection tricks
     picked up from neuron2's Decomb plugin for
     AviSynth and tritical's IsCombedT and
        ...
        if ((up_diff > athresh && down_diff > athresh) ||
          ...
          if (motion || pv->frames == 0) {
            ...
            if (spatial_metric == 0) {
              ...
              int combing = (cur[up_1] - cur[0]) * (cur[down_1] - cur[0]);
              ...
              /* Tritical's noise-resistant combing scorer.
                 The check is done on a bob+blur convolution. */
              int combing = abs(cur[up_2] + (4 * cur[0]) + cur[down_2] -
                                (3 * (cur[up_1] + cur[down_1])));
    ...
    for (pp = 0; pp < 1; pp++) {
      ...
      for (yy = start; yy < stop; yy++) {
        for (xx = 1; xx < width - 1; xx++) {
          ...
          count = curp[xx - 1] + curp[xx] + curp[xx + 1] + cur[xx - 1] +
                  cur[xx + 1] + curn[xx - 1] + curn[xx] + curn[xx + 1];
...
static int comb_detect_init(hb_filter_object_t *filter,
  ...
  pv->mask = hb_frame_buffer_init(init->pix_fmt, init->geometry.width,
                                  init->geometry.height);
  pv->mask_filtered = hb_frame_buffer_init(init->pix_fmt, init->geometry.width,
                                           init->geometry.height);
  pv->mask_temp = hb_frame_buffer_init(init->pix_fmt, init->geometry.width,
                                       init->geometry.height);
  ...
  for (ii = 0; ii < pv->cpu_count; ii++) {
    ...
    if (taskset_thread_spawn(&pv->decomb_filter_taskset, ii,
                             "decomb_filter_segment", decomb_filter_thread,
  ...
  for (ii = 0; ii < pv->comb_check_nthreads; ii++) {
    ...
    for (pp = 0; pp < 3; pp++) {
      ...
      int h = hb_image_height(init->pix_fmt, init->geometry.height, pp) /
    ...
    if (taskset_thread_spawn(&pv->decomb_check_taskset, ii,
                             "decomb_check_segment", decomb_check_thread,
                             HB_NORMAL_PRIORITY) == 0) {
  ...
  if (pv->mode & MODE_FILTER) {
    ...
    for (ii = 0; ii < pv->cpu_count; ii++) {
      ...
      if (taskset_thread_spawn(&pv->mask_filter_taskset, ii,
                               "mask_filter_segment", mask_filter_thread,
                               HB_NORMAL_PRIORITY) == 0) {
    ...
    if (pv->filter_mode == FILTER_ERODE_DILATE) {
      ...
      for (ii = 0; ii < pv->cpu_count; ii++) {
        ...
        if (taskset_thread_spawn(&pv->mask_erode_taskset, ii,
                                 "mask_erode_segment", mask_erode_thread,
                                 HB_NORMAL_PRIORITY) == 0) {
      ...
      for (ii = 0; ii < pv->cpu_count; ii++) {
        ...
        if (taskset_thread_spawn(&pv->mask_dilate_taskset, ii,
                                 "mask_dilate_segment", mask_dilate_thread,
                                 HB_NORMAL_PRIORITY) == 0) {
