...
// AVC implementations
static hb_qsv_info_t qsv_software_info_avc = {
    .available = 0,
    .codec_id = MFX_CODEC_AVC,
    .implementation = MFX_IMPL_SOFTWARE,
};
static hb_qsv_info_t qsv_hardware_info_avc = {
    .available = 0,
    .codec_id = MFX_CODEC_AVC,
    .implementation = MFX_IMPL_HARDWARE_ANY | MFX_IMPL_VIA_ANY,
};
// HEVC implementations
static hb_qsv_info_t qsv_software_info_hevc = {
    .available = 0,
    .codec_id = MFX_CODEC_HEVC,
    .implementation = MFX_IMPL_SOFTWARE,
};
static hb_qsv_info_t qsv_hardware_info_hevc = {
    .available = 0,
    .codec_id = MFX_CODEC_HEVC,
    .implementation = MFX_IMPL_HARDWARE_ANY | MFX_IMPL_VIA_ANY,
};
...
/*
 * Determine the "generation" of QSV hardware based on the CPU
 * microarchitecture. Anything unknown is assumed to be more recent than the
 * latest known generation. This avoids having to order the hb_cpu_platform enum
 * depending on QSV hardware.
 */
enum {
...
int hb_qsv_available() {
  ...
  return (
      (hb_qsv_video_encoder_is_enabled(HB_VCODEC_QSV_H264) ? HB_VCODEC_QSV_H264
                                                           : 0) |
      (hb_qsv_video_encoder_is_enabled(HB_VCODEC_QSV_H265) ? HB_VCODEC_QSV_H265
                                                           : 0) |
      (hb_qsv_video_encoder_is_enabled(HB_VCODEC_QSV_H265_10BIT)
           ? HB_VCODEC_QSV_H265_10BIT
           : 0));
}
...
static void
init_ext_video_signal_info(mfxExtVideoSignalInfo *extVideoSignalInfo) {
...
static int query_capabilities(mfxSession session, mfxVersion version,
                              hb_qsv_info_t *info) {
  /*
   ...
   */
  if (HB_CHECK_MFX_VERSION(version, HB_QSV_MINVERSION_MAJOR,
    ...
    if (info->implementation & MFX_IMPL_AUDIO) {
      ...
      if (mfxRes >= MFX_ERR_NONE && videoParam.mfx.CodecId == info->codec_id) {
        /*
         ...
         */
        if ((status = MFXVideoENCODE_Init(session, &videoParam)) >=
            MFX_ERR_NONE) {
          info->available = 1;
  ...
  if (info->implementation & MFX_IMPL_AUDIO) {
    ...
    if (HB_CHECK_MFX_VERSION(version, 1, 7)) {
      ...
      if (MFXVideoENCODE_Query(session, &inputParam, &videoParam) >=
              MFX_ERR_NONE &&
    ...
    if (HB_CHECK_MFX_VERSION(version, 1, 6) &&
      ...
      if (status >= MFX_ERR_NONE) {
        ...
        fprintf(stderr,
                "hb_qsv_info_init: mfxExtCodingOption2 check failed (0x%" PRIX32
                ", 0x%" PRIX32 ", %d)\n",
                info->codec_id, info->implementation, status);
      }
    }
...
int hb_qsv_info_init() {
  ...
  if (MFXInit(MFX_IMPL_SOFTWARE, &version, &session) == MFX_ERR_NONE) {
    ...
    if (HB_CHECK_MFX_VERSION(qsv_software_version, HB_QSV_MINVERSION_MAJOR,
                             HB_QSV_MINVERSION_MINOR)) {
      query_capabilities(session, qsv_software_version, &qsv_software_info_avc);
      query_capabilities(session, qsv_software_version,
  ...
  do {
    if (MFXInit(MFX_IMPL_HARDWARE_ANY | hw_preference, &version, &session) ==
        MFX_ERR_NONE) {
      ...
      if (qsv_hardware_generation(hb_get_cpu_platform()) >= QSV_G1 &&
          HB_CHECK_MFX_VERSION(qsv_hardware_version, HB_QSV_MINVERSION_MAJOR,
                               HB_QSV_MINVERSION_MINOR)) {
        query_capabilities(session, qsv_hardware_version,
                           &qsv_hardware_info_avc);
        qsv_hardware_info_avc.implementation =
            MFX_IMPL_HARDWARE_ANY | hw_preference;
        query_capabilities(session, qsv_hardware_version,
                           &qsv_hardware_info_hevc);
        qsv_hardware_info_hevc.implementation =
            MFX_IMPL_HARDWARE_ANY | hw_preference;
        // now that we know which hardware encoders are
        // available, we can set the preferred implementation
...
void hb_qsv_info_print() {
  ...
  if (hb_qsv_available()) {
    ...
    if (qsv_hardware_version.Version) {
      hb_log(" - Intel Media SDK hardware: API %" PRIu16 ".%" PRIu16
             " (minimum: %" PRIu16 ".%" PRIu16 ")",
    ...
    if (qsv_software_version.Version) {
      hb_log(" - Intel Media SDK software: API %" PRIu16 ".%" PRIu16
             " (minimum: %" PRIu16 ".%" PRIu16 ")",
    ...
    if (hb_qsv_info_hevc != NULL && hb_qsv_info_hevc->available) {
      hb_log(" - H.265 encoder: yes (8bit: yes, 10bit: %s)",
             (qsv_hardware_generation(hb_get_cpu_platform()) < QSV_G6) ? "no"
                                                                       : "yes");
      hb_log("    - preferred implementation: %s %s",
...
hb_list_t *hb_qsv_load_plugins(hb_qsv_info_t *info, mfxSession session,
                               mfxVersion version) {
...
void hb_qsv_unload_plugins(hb_list_t **_l, mfxSession session,
                           mfxVersion version) {
...
int hb_qsv_atobool(const char *str, int *err) {
  if (!strcasecmp(str, "1") || !strcasecmp(str, "yes") ||
      !strcasecmp(str, "true")) {
    return 1;
  }
  if (!strcasecmp(str, "0") || !strcasecmp(str, "no") ||
...
int hb_qsv_param_parse(hb_qsv_param_t *param, hb_qsv_info_t *info,
  ...
  if (!strcasecmp(key, "target-usage") || !strcasecmp(key, "tu")) {
    ivalue = hb_qsv_atoi(value, &error);
    if (!error) {
      param->videoParam->mfx.TargetUsage =
          HB_QSV_CLIP3(MFX_TARGETUSAGE_1, MFX_TARGETUSAGE_7, ivalue);
    }
  } else if (!strcasecmp(key, "num-ref-frame") || !strcasecmp(key, "ref")) {
    ivalue = hb_qsv_atoi(value, &error);
    if (!error) {
  ...
  } else if (!strcasecmp(key, "gop-pic-size") || !strcasecmp(key, "keyint")) {
    ivalue = hb_qsv_atoi(value, &error);
    if (!error) {
  ...
  } else if (!strcasecmp(key, "adaptive-i") || !strcasecmp(key, "i-adapt")) {
  ...
  } else if (!strcasecmp(key, "adaptive-b") || !strcasecmp(key, "b-adapt")) {
    if (info->capabilities & HB_QSV_CAP_OPTION2_IB_ADAPT) {
      ivalue = hb_qsv_atobool(value, &error);
      if (!error) {
  ...
  } else if (!strcasecmp(key, "tff") || !strcasecmp(key, "interlaced")) {
    switch (info->codec_id) {
    case MFX_CODEC_AVC:
      ivalue = hb_qsv_atobool(value, &error);
      break;
    ...
    if (!error) {
      param->videoParam->mfx.FrameInfo.PicStruct =
          (ivalue ? MFX_PICSTRUCT_FIELD_TFF : MFX_PICSTRUCT_PROGRESSIVE);
    }
    ...
    if (!error) {
      param->videoParam->mfx.FrameInfo.PicStruct =
          (ivalue ? MFX_PICSTRUCT_FIELD_BFF : MFX_PICSTRUCT_PROGRESSIVE);
    }
  ...
  } else if (!strcasecmp(key, "lookahead") || !strcasecmp(key, "la")) {
    if (info->capabilities & HB_QSV_CAP_RATECONTROL_LA) {
      ivalue = hb_qsv_atobool(value, &error);
      if (!error) {
  ...
  } else if (!strcasecmp(key, "lookahead-ds") || !strcasecmp(key, "la-ds")) {
    if (info->capabilities & HB_QSV_CAP_OPTION2_LA_DOWNS) {
      ivalue = hb_qsv_atoi(value, &error);
      if (!error) {
        param->codingOption2.LookAheadDS =
            HB_QSV_CLIP3(MFX_LOOKAHEAD_DS_UNKNOWN, MFX_LOOKAHEAD_DS_4x, ivalue);
      }
    } else {
      return HB_QSV_PARAM_UNSUPPORTED;
    }
    ...
    if (info->capabilities & HB_QSV_CAP_LOWPOWER_ENCODE) {
      ivalue = hb_qsv_atobool(value, &error);
      if (!error) {
        param->videoParam->mfx.LowPower =
            ivalue ? MFX_CODINGOPTION_ON : MFX_CODINGOPTION_OFF;
      }
    } else {
      return HB_QSV_PARAM_UNSUPPORTED;
    }
...
int hb_qsv_profile_parse(hb_qsv_param_t *param, hb_qsv_info_t *info,
                         const char *profile_key, const int codec) {
  ...
  else if (((profile_key != NULL && !strcasecmp(profile_key, "auto")) ||
            profile_key == NULL) &&
  ...
  if (level_key != NULL && *level_key && strcasecmp(level_key, "auto")) {
    ...
    if (param->videoParam->mfx.CodecId == MFX_CODEC_AVC) {
      if (info->capabilities & HB_QSV_CAP_MSDK_API_1_6) {
        param->videoParam->mfx.CodecLevel =
            FFMIN(MFX_LEVEL_AVC_52, level->value);
      } else {
        // Media SDK API < 1.6, MFX_LEVEL_AVC_52 unsupported
        param->videoParam->mfx.CodecLevel =
            FFMIN(MFX_LEVEL_AVC_51, level->value);
      }
    } else {
...
const char *hb_qsv_video_quality_get_name(uint32_t codec) {
  ...
  switch (codec) {
  case HB_VCODEC_QSV_H264:
    if (hb_qsv_info_avc != NULL)
      caps = hb_qsv_info_avc->capabilities;
    break;

  case HB_VCODEC_QSV_H265_10BIT:
  case HB_VCODEC_QSV_H265:
    if (hb_qsv_info_hevc != NULL)
      caps = hb_qsv_info_hevc->capabilities;
    break;
...
int hb_qsv_param_default_preset(hb_qsv_param_t *param,
                                mfxVideoParam *videoParam, hb_qsv_info_t *info,
                                const char *preset) {
...
int hb_qsv_param_default(hb_qsv_param_t *param, mfxVideoParam *videoParam,
  ...
  if (param != NULL && videoParam != NULL && info != NULL) {
    ...
    param->videoSignalInfo.ColourDescriptionPresent =
    ...
    param->codingOption2.BRefType =
        MFX_B_REF_UNKNOWN; // controlled via gop.b_pyramid
    ...
    if (info->capabilities & HB_QSV_CAP_VUI_VSINFO) {
      param->videoParam->ExtParam[param->videoParam->NumExtParam++] =
          (mfxExtBuffer *)&param->videoSignalInfo;
    }
    if (info->capabilities & HB_QSV_CAP_OPTION1) {
      param->videoParam->ExtParam[param->videoParam->NumExtParam++] =
          (mfxExtBuffer *)&param->codingOption;
    }
    if (info->capabilities & HB_QSV_CAP_OPTION2) {
      param->videoParam->ExtParam[param->videoParam->NumExtParam++] =
          (mfxExtBuffer *)&param->codingOption2;
    }
  } else {
...
typedef HRESULT(WINAPI *HB_PFN_CREATE_DXGI_FACTORY)(REFIID riid,
                                                    void **ppFactory);

static int hb_dxva2_device_create9(HMODULE d3dlib, UINT adapter,
                                   IDirect3D9 **d3d9_out) {
  pDirect3DCreate9 *createD3D =
      (pDirect3DCreate9 *)hb_dlsym(d3dlib, "Direct3DCreate9");
  if (!createD3D) {
...
static int hb_dxva2_device_create9ex(HMODULE d3dlib, UINT adapter,
                                     IDirect3D9 **d3d9_out) {
  IDirect3D9Ex *d3d9ex = NULL;
  HRESULT hr;
  pDirect3DCreate9Ex *createD3DEx =
      (pDirect3DCreate9Ex *)hb_dlsym(d3dlib, "Direct3DCreate9Ex");
  if (!createD3DEx) {
...
static int hb_d3d11va_device_check() {
  ...
  mD3D11CreateDevice =
      (PFN_D3D11_CREATE_DEVICE)hb_dlsym(d3dlib, "D3D11CreateDevice");
  mCreateDXGIFactory =
      (HB_PFN_CREATE_DXGI_FACTORY)hb_dlsym(dxgilib, "CreateDXGIFactory1");

  if (!mD3D11CreateDevice || !mCreateDXGIFactory) {
    hb_error("hb_d3d11va_device_check: failed to locate D3D11CreateDevice and "
             "CreateDXGIFactory1 functions");
    return -1;
  }
  ...
  while (IDXGIFactory2_EnumAdapters(pDXGIFactory, adapter_id++, &pAdapter) !=
         DXGI_ERROR_NOT_FOUND) {
    ID3D11Device *g_pd3dDevice = NULL;
    DXGI_ADAPTER_DESC adapterDesc;

    hr = mD3D11CreateDevice(pAdapter, D3D_DRIVER_TYPE_UNKNOWN, NULL,
                            D3D11_CREATE_DEVICE_VIDEO_SUPPORT, NULL, 0,
                            D3D11_SDK_VERSION, &g_pd3dDevice, NULL, NULL);
    if (FAILED(hr)) {
    ...
    if (FAILED(hr)) {
      hb_error("hb_d3d11va_device_check: IDXGIAdapter2_GetDesc returned %d",
               hr);
      continue;
    }
    ...
    if (adapterDesc.VendorId == 0x8086) {
      ...
      hb_log("hb_d3d11va_device_check: QSV adapter with id %d has been found",
             adapter_id - 1);
      return adapter_id - 1;
    }
  }
...
static int hb_dxva2_device_check() {
  ...
  if (d3dai) {
    if (d3dai->VendorId != intel_id) {
      hb_error("hb_dxva2_device_check: adapter that was found does not support "
               "QSV. It is required for zero-copy QSV path");
      hr = -1;
      goto clean_up;
    }
  }
...
static HRESULT
lock_device(IDirect3DDeviceManager9 *pDeviceManager, BOOL fBlock,
  ...
  *ppDevice = NULL;
  ...
  HRESULT hr =
      pDeviceManager->lpVtbl->OpenDeviceHandle(pDeviceManager, &hDevice);

  if (SUCCEEDED(hr)) {
    hr = pDeviceManager->lpVtbl->LockDevice(pDeviceManager, hDevice, ppDevice,
                                            fBlock);
  }
...
static HRESULT unlock_device(IDirect3DDeviceManager9 *pDeviceManager,
...
static int hb_qsv_find_surface_idx(const QSVMid *mids, const int nb_mids,
                                   const QSVMid *mid) {
  if (mids) {
...
int hb_qsv_replace_surface_mid(const QSVMid *mid, mfxFrameSurface1 *surface) {
  int ret = hb_qsv_find_surface_idx(hb_enc_qsv_frames_ctx.mids,
                                    hb_enc_qsv_frames_ctx.nb_mids, mid);
  if (ret < 0) {
    ret = hb_qsv_find_surface_idx(hb_enc_qsv_frames_ctx.mids2,
                                  hb_enc_qsv_frames_ctx.nb_mids, mid);
    if (ret < 0) {
...
void hb_qsv_get_free_surface_from_pool(const int start_index,
                                       const int end_index, QSVMid **out_mid,
                                       mfxFrameSurface1 **out_surface) {
  QSVMid *mid = NULL;
  mfxFrameSurface1 *output_surface = NULL;

  AVHWFramesContext *frames_ctx =
      (AVHWFramesContext *)hb_enc_qsv_frames_ctx.hw_frames_ctx->data;
  AVQSVFramesContext *frames_hwctx = frames_ctx->hwctx;

  AVHWFramesContext *frames_ctx2 =
      (AVHWFramesContext *)hb_enc_qsv_frames_ctx.hw_frames_ctx2->data;
...
hb_buffer_t *hb_qsv_copy_frame(AVFrame *frame, hb_qsv_context *qsv_ctx) {
  ...
  hb_qsv_get_free_surface_from_pool(
      0, HB_POOL_SURFACE_SIZE - HB_POOL_ENCODER_SIZE, &mid, &output_surface);

  static const mfxHandleType handle_types[] = {MFX_HANDLE_VA_DISPLAY,
  ...
  if (device_manager_handle == NULL) {
    for (i = 0; i < 3; i++) {
      int err = MFXVideoCORE_GetHandle(parent_session, handle_types[i],
  ...
  if (device_handle_type == MFX_HANDLE_D3D9_DEVICE_MANAGER) {
    ...
    HRESULT result = lock_device(
        (IDirect3DDeviceManager9 *)device_manager_handle, 0, &pDevice, &handle);
    if (FAILED(result)) {
    ...
    *output_surface = *input_surface;
    ...
    result = IDirect3DDevice9_StretchRect(pDevice, input_surface->Data.MemId, 0,
                                          mid->handle, 0, D3DTEXF_LINEAR);
    if (FAILED(result)) {
      hb_error("hb_qsv_copy_frame: IDirect3DDevice9_StretchRect failded %d",
               result);
      return out;
    }
    result =
        unlock_device((IDirect3DDeviceManager9 *)device_manager_handle, handle);
    if (FAILED(result)) {
    ...
    *output_surface = *input_surface;
    ...
    ID3D11DeviceContext_CopySubresourceRegion(
        device_context, mid->texture, (uint64_t)mid->handle, 0, 0, 0,
        hb_enc_qsv_frames_ctx.input_texture,
        (uint64_t)input_surface->Data.MemId, NULL);
  }
...
static int qsv_device_init(AVCodecContext *s) {
  ...
  err = av_hwdevice_ctx_create(&hb_hw_device_ctx, AV_HWDEVICE_TYPE_QSV, 0, dict,
                               0);
  if (err < 0) {
...
static int qsv_init(AVCodecContext *s) {
  ...
  hb_enc_qsv_frames_ctx.mids_buf =
      hb_qsv_create_mids(hb_enc_qsv_frames_ctx.hw_frames_ctx);
  if (!hb_enc_qsv_frames_ctx.mids_buf)
    return AVERROR(ENOMEM);
  av_buffer_unref(&hb_enc_qsv_frames_ctx.mids_buf2);
  hb_enc_qsv_frames_ctx.mids_buf2 =
      hb_qsv_create_mids(hb_enc_qsv_frames_ctx.hw_frames_ctx2);
  ...
  memset(hb_enc_qsv_frames_ctx.pool, 0,
         hb_enc_qsv_frames_ctx.nb_mids * sizeof(hb_enc_qsv_frames_ctx.pool[0]));
  memset(hb_enc_qsv_frames_ctx.pool2, 0,
         hb_enc_qsv_frames_ctx.nb_mids *
             sizeof(hb_enc_qsv_frames_ctx.pool2[0]));
  return 0;
}
...
enum AVPixelFormat hb_qsv_get_format(AVCodecContext *s,
  ...
  hb_error(
      "hb_qsv_get_format: the QSV pixel format not offered in get_format()");
  return AV_PIX_FMT_NONE;
}
...
int hb_qsv_release_surface_from_pool(const QSVMid *mid) { return -1; }
...
int hb_qsv_preset_is_zero_copy_enabled(const hb_dict_t *job_dict) { return 0; }

static int hb_dxva2_device_check() { return -1; }

static int hb_d3d11va_device_check() { return -1; }
#endif
