...
static int hb_dxva2_device_check() {
  ...
  if (FAILED(IDirect3D9_GetAdapterIdentifier(d3d9, D3DADAPTER_DEFAULT, 0,
                                             d3dai))) {
  ...
  if (d3dai) {
    if (d3dai->VendorId != intel_id) {
      hb_error("D3D9: adapter that was found does not support QSV. It is "
               "required for zero-copy QSV path");
      err = -1;
      goto clean_up;
    }
  }
...
hb_buffer_t *hb_qsv_copy_frame(AVFrame *frame, hb_qsv_context *qsv_ctx) {
  ...
  if (handle_type == MFX_HANDLE_D3D9_DEVICE_MANAGER) {
    ...
    *output_surface = *input_surface;
    ...
    if (FAILED(result)) {
      hb_error("copy_frame qsv: IDirect3DDevice9_StretchRect failded=%d",
               result);
      return out;
    }
    result =
    ...
    *output_surface = *input_surface;
    ...
    ID3D11DeviceContext_CopySubresourceRegion(
        device_context, mid->texture, mid->handle, 0, 0, 0,
        hb_enc_qsv_frames_ctx.input_texture, input_surface->Data.MemId, NULL);
  }
    ...
    if (*p == AV_PIX_FMT_QSV) {
      ...
      if (ret < 0) {
        av_log(NULL, AV_LOG_FATAL,
               "QSV hwaccel requested for input stream but cannot be "
               "initialized.\n");
