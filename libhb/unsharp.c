...
static int unsharp_init(hb_filter_object_t *filter, hb_filter_init_t *init);

static int unsharp_init_thread(hb_filter_object_t *filter, int threads);

static int unsharp_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                        hb_buffer_t **buf_out);
static int unsharp_work_thread(hb_filter_object_t *filter, hb_buffer_t **buf_in,
...
static void unsharp(const uint8_t *src, uint8_t *dst, const int width,
                    const int height, const int stride,
                    unsharp_plane_context_t *ctx,
                    unsharp_thread_context_t *tctx) {
  uint32_t **SC = tctx->SC;
  uint32_t SR[UNSHARP_SIZE_MAX - 1], Tmp1, Tmp2;
  ...
  for (y = -steps; y < height + steps; y++) {
    ...
    for (x = -steps; x < width + steps; x++) {
      ...
      if (x >= steps && y >= steps) {
        ...
        res = (int32_t)*srx +
              ((((int32_t)*srx - (int32_t)((Tmp1 + halfscale) >> scalebits)) *
                amount) >>
               16);
...
static int unsharp_init(hb_filter_object_t *filter, hb_filter_init_t *init) {
  ...
  for (int c = 0; c < 3; c++) {
    ...
    if (ctx->strength == -1) {
      ctx->strength =
          c ? UNSHARP_STRENGTH_CHROMA_DEFAULT : UNSHARP_STRENGTH_LUMA_DEFAULT;
    }
    if (ctx->size == -1) {
      ctx->size = c ? UNSHARP_SIZE_CHROMA_DEFAULT : UNSHARP_SIZE_LUMA_DEFAULT;
    }
    ...
    if (ctx->size % 2 == 0)
      ctx->size--;
    if (ctx->size < UNSHARP_SIZE_MIN)
      ctx->size = UNSHARP_SIZE_MIN;
    if (ctx->size > UNSHARP_SIZE_MAX)
      ctx->size = UNSHARP_SIZE_MAX;
...
static int unsharp_init_thread(hb_filter_object_t *filter, int threads) {
  ...
  for (int c = 0; c < 3; c++) {
    ...
    for (int t = 0; t < threads; t++) {
      ...
      int z;
      for (z = 0; z < 2 * ctx->steps; z++) {
        tctx->SC[z] = malloc(sizeof(*(tctx->SC[z])) * (w + 2 * ctx->steps));
        if (tctx->SC[z] == NULL) {
          hb_error("Unsharp calloc failed");
          unsharp_close(filter);
          return -1;
...
static int unsharp_work_thread(hb_filter_object_t *filter, hb_buffer_t **buf_in,
  ...
  for (c = 0; c < 3; c++) {
    ...
    unsharp(in->plane[c].data, out->plane[c].data, in->plane[c].width,
            in->plane[c].height, in->plane[c].stride, ctx, tctx);
  }
