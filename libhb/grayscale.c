...
static int hb_grayscale_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                             hb_buffer_t **buf_out);
...
hb_filter_object_t hb_filter_grayscale = {.id = HB_FILTER_GRAYSCALE,
                                          .enforce_order = 1,
                                          .name = "Grayscale",
                                          .settings = NULL,
...
void grayscale_filter_thread(void *thread_args_v) {
  ...
  while (run) {
    ...
    for (plane = 1; plane < 3; plane++) {
      ...
      memset(&src_buf->plane[plane].data[segment_start * src_stride], 0x80,
             (segment_stop - segment_start) * src_stride);
    }
 ...
 */
static void grayscale_filter(hb_filter_private_t *pv, hb_buffer_t *in) {
...
static int hb_grayscale_init(hb_filter_object_t *filter,
   ...
   * Create gray taskset.
   */
  pv->grayscale_arguments =
      malloc(sizeof(grayscale_arguments_t) * pv->cpu_count);
  ...
  for (ii = 0; ii < pv->cpu_count; ii++) {
    ...
    if (taskset_thread_spawn(
            &pv->grayscale_taskset, ii, "grayscale_filter_segment",
            grayscale_filter_thread, HB_NORMAL_PRIORITY) == 0) {
