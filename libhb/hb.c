...
int hb_picture_fill(uint8_t *data[], int stride[], hb_buffer_t *buf) {
  ...
  ret = av_image_fill_pointers(data, buf->f.fmt, buf->plane[0].height_stride,
                               buf->data, stride);
  if (ret != buf->size) {
    hb_error("Internal error hb_picture_fill expected %d, got %d", buf->size,
             ret);
  }
  return ret;
}

int hb_picture_crop(uint8_t *data[], int stride[], hb_buffer_t *buf, int top,
                    int left) {
...
hb_handle_t *hb_init(int verbose) {
  ...
  h->die = 0;
  h->main_thread = hb_thread_init("libhb", thread_func, h, HB_NORMAL_PRIORITY);
 ...
 */
int hb_get_build(hb_handle_t *h) { return hb_build; }
...
void hb_scan(hb_handle_t *h, const char *path, int title_index,
  ...
  h->scan_thread =
      hb_scan_init(h, &h->scan_die, path, title_index, &h->title_set,
                   preview_count, store_previews, min_duration);
}
 ...
 */
hb_list_t *hb_get_titles(hb_handle_t *h) { return h->title_set.list_title; }

hb_title_set_t *hb_get_title_set(hb_handle_t *h) { return &h->title_set; }
...
int hb_save_preview(hb_handle_t *h, int title, int preview, hb_buffer_t *buf) {
  ...
  filename = hb_get_temporary_filename("%d_%d_%d", hb_get_instance_id(h), title,
                                       preview);
  ...
  if (file == NULL) {
    ...
    hb_error("hb_save_preview: Failed to open %s (reason: %s)", filename,
             reason);
    free(filename);
    return -1;
  }
...
hb_buffer_t *hb_read_preview(hb_handle_t *h, hb_title_t *title, int preview) {
  ...
  if (file == NULL) {
    ...
    hb_error("hb_read_preview: Failed to open %s (reason: %s)", filename,
             reason);
    free(filename);
    return NULL;
  }
  ...
  hb_buffer_t *buf;
  buf = hb_frame_buffer_init(AV_PIX_FMT_YUV420P, title->geometry.width,
...
hb_image_t *hb_get_preview2(hb_handle_t *h, int title_idx, int picture,
  ...
  int width =
      geo->geometry.width * geo->geometry.par.num / geo->geometry.par.den;
  ...
  if (deinterlace) {
    ...
    deint_buf = hb_frame_buffer_init(AV_PIX_FMT_YUV420P, title->geometry.width,
                                     title->geometry.height);
    hb_deinterlace(deint_buf, in_buf);
    hb_picture_crop(crop_data, crop_stride, deint_buf, geo->crop[0],
                    geo->crop[2]);
  } else {
    // Crop
    hb_picture_crop(crop_data, crop_stride, in_buf, geo->crop[0], geo->crop[2]);
  }
  ...
  // Get scaling context
  context =
      hb_sws_get_context(title->geometry.width - (geo->crop[2] + geo->crop[3]),
  ...
  // Scale
  sws_scale(context, (const uint8_t *const *)crop_data, crop_stride, 0,
            title->geometry.height - (geo->crop[0] + geo->crop[1]),
 ...
 * @param prog_equal  Sensitivity for detecting similar colors on progressive
 * frames
 * @param prog_diff   Sensitivity for detecting different colors on progressive
 * frames
...
int hb_detect_comb(hb_buffer_t *buf, int color_equal, int color_diff,
                   int threshold, int prog_equal, int prog_diff,
                   int prog_threshold) {
  ...
  for (k = 0; k <= buf->f.max_plane; k++) {
    ...
    // compare results
    /*  The final cc score for a plane is the percentage of combed pixels it
       contains. Because sensitivity goes down to hundreths of a percent,
       multiply by 1000 so it will be easy to compare against the threshold
       value which is an integer. */
    cc[k] = (int)((cc_1 + cc_2) * 1000.0 / (width * height));
  }

  /* HandBrake is all yuv420, so weight the average percentage of all 3 planes
   * accordingly.*/
  int average_cc = (2 * cc[0] + (cc[1] / 2) + (cc[2] / 2)) / 3;

  /* Now see if that average percentage of combed pixels surpasses the threshold
   * percentage given by the user.*/
...
void hb_set_anamorphic_size2(hb_geometry_t *src_geo,
  ...
  hb_reduce(&in_par.num, &in_par.den, geo->geometry.par.num,
  ...
  if (src_geo->width == 720 && geo->itu_par) {
    ...
    int iaspect =
        src_geo->width * src_par.num * 9. / (src_geo->height * src_par.den);
  ...
  */
  ...
  if (geo->maxWidth > 0) {
    maxWidth = MIN(MAX(MULTIPLE_MOD_DOWN(geo->maxWidth, mod), HB_MIN_WIDTH),
                   HB_MAX_WIDTH);
  } else {
    maxWidth = HB_MAX_WIDTH;
  }
  if (geo->maxHeight > 0) {
    maxHeight = MIN(MAX(MULTIPLE_MOD_DOWN(geo->maxHeight, mod), HB_MIN_HEIGHT),
                    HB_MAX_HEIGHT);
  } else {
  ...
  if (width < HB_MIN_WIDTH || height < HB_MIN_HEIGHT || width > maxWidth ||
      height > maxHeight) {
  ...
  hb_limit_rational64(&dst_par_num, &dst_par_den, dst_par_num, dst_par_den,
                      65535);
 ...
 */
int hb_count(hb_handle_t *h) { return hb_list_count(h->jobs); }
 ...
 */
hb_job_t *hb_job(hb_handle_t *h, int i) { return hb_list_item(h->jobs, i); }

hb_job_t *hb_current_job(hb_handle_t *h) { return (h->current_job); }
  ...
  if (job->indepth_scan) {
    ...
    for (i = 0; i < hb_list_count(job->title->list_subtitle); i++) {
      ...
      if (strcmp(subtitle->iso639_2, audio_lang) == 0 &&
        ...
        hb_list_add(job_copy->list_subtitle, hb_subtitle_copy(subtitle));
      }
    }
    int count = hb_list_count(job_copy->list_subtitle);
    if (count == 0 || (count == 1 && !job_copy->select_subtitle_config.force)) {
 ...
 * @param job Handle to hb_job_t.
 */
void hb_rem(hb_handle_t *h, hb_job_t *job) { hb_list_rem(h->jobs, job); }
...
#undef p
  ...
  h->work_thread =
      hb_work_init(h->jobs, &h->work_die, &h->work_error, &h->current_job);
}
...
void hb_get_state(hb_handle_t *h, hb_state_t *s) {
  ...
  if (h->state.state == HB_STATE_SCANDONE ||
...
/**
 * Cleans up libhb at a process level. Call before the app closes. Removes
 * preview directory.
 */
...
static void thread_func(void *_h) {
  ...
  while (!h->die) {
    ...
    if (h->scan_thread && hb_thread_has_exited(h->scan_thread)) {
    ...
    if (h->work_thread && hb_thread_has_exited(h->work_thread)) {
 ...
 * @param h Handle to hb_handle_t
 */
int hb_get_pid(hb_handle_t *h) { return h->pid; }
 ...
 * @returns The ID for the given instance
 */
int hb_get_instance_id(hb_handle_t *h) { return h->id; }
...
hb_interjob_t *hb_interjob_get(hb_handle_t *h) { return h->interjob; }

int is_hardware_disabled(void) { return disable_hardware; }
