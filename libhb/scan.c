...
static void ScanFunc(void *_data) {
  ...
  if ((data->bd = hb_bd_init(data->h, data->path))) {
    hb_log("scan: BD has %d title(s)", hb_bd_title_count(data->bd));
    if (data->title_index) {
      /* Scan this title only */
      hb_list_add(data->title_set->list_title,
                  hb_bd_title_scan(data->bd, data->title_index, 0));
    } else {
      /* Scan all titles */
      for (i = 0; i < hb_bd_title_count(data->bd); i++) {
        ...
        hb_list_add(
            ...
            hb_bd_title_scan(data->bd, i + 1, data->min_title_duration));
      }
      feature = hb_bd_main_feature(data->bd, data->title_set->list_title);
    }
  } else if ((data->dvd = hb_dvd_init(data->h, data->path))) {
    hb_log("scan: DVD has %d title(s)", hb_dvd_title_count(data->dvd));
    if (data->title_index) {
      /* Scan this title only */
      hb_list_add(data->title_set->list_title,
                  hb_dvd_title_scan(data->dvd, data->title_index, 0));
    } else {
      /* Scan all titles */
      for (i = 0; i < hb_dvd_title_count(data->dvd); i++) {
        ...
        hb_list_add(
            data->title_set->list_title,
            hb_dvd_title_scan(data->dvd, i + 1, data->min_title_duration));
      }
  ...
  for (i = 0; i < hb_list_count(data->title_set->list_title);) {
    ...
    for (j = 0; j < hb_list_count(title->list_audio);) {
      ...
      if (!audio->config.in.bitrate) {
        hb_log("scan: removing audio 0x%x because no bitrate found", audio->id);
...
static inline int absdiff(int x, int y) { return x < y ? y - x : x - y; }
...
static int DecodePreviews(hb_scan_t *data, hb_title_t *title, int flush) {
  ...
  if (data->batch) {
    hb_log("scan: decoding previews for title %d (%s)", title->index,
           title->path);
  } else {
  ...
  for (i = 0; i < data->preview_count; i++) {
    ...
    if (data->dvd) {
      if (!hb_dvd_seek(data->dvd,
                       (float)(i + 1) / (data->preview_count + 1.0))) {
        continue;
      }
    } else if (stream) {
      ...
      if (i != 0) {
        if (!hb_stream_seek(stream, (float)i / (data->preview_count + 1.0))) {
          continue;
        }
      } else {
    ...
    for (right = 0; right < w4; ++right) {
      if (!column_all_dark(vid_buf, top, bottom,
                           vid_info.geometry.width - 1 - right))
        break;
    }
  ...
  if (npreviews) {
    ...
    hb_log("scan: %d previews, %dx%d, %.3f fps, autocrop = %d/%d/%d/%d, "
           ...
           (float)title->vrate.num / title->vrate.den, title->crop[0],
           title->crop[1], title->crop[2], title->crop[3],
           aspect_to_string(&title->dar), title->geometry.par.num,
           title->geometry.par.den);

    if (title->video_decode_support != HB_DECODE_SUPPORT_SW) {
      hb_log("scan: supported video decoders:%s%s",
             !(title->video_decode_support & HB_DECODE_SUPPORT_SW) ? ""
                                                                   : " avcodec",
             !(title->video_decode_support & HB_DECODE_SUPPORT_QSV) ? ""
                                                                    : " qsv");
    }

    if (interlaced_preview_count >= (npreviews / 2)) {
      hb_log("Title is likely interlaced or telecined (%i out of %i previews). "
...
static void LookForAudio(hb_scan_t *scan, hb_title_t *title, hb_buffer_t *b) {
  ...
  if (ret < 0) {
    hb_log("no info on audio type %d/0x%x for id 0x%x", audio->config.in.codec,
           audio->config.in.codec_param, audio->id);
    goto drop_audio;
  }
  if (!info.bitrate) {
  ...
  if (audio->config.in.codec & HB_ACODEC_FF_MASK) {
    ...
    if (codec != NULL) {
      ...
      case AV_CODEC_ID_DTS:
        if (profile_name == NULL && audio->config.in.codec == HB_ACODEC_DCA_HD)
          codec_name = "DTS-HD";
        else
          codec_name = "DTS";
        break;
  ...
  if (codec_name != NULL && profile_name != NULL) {
    snprintf(audio->config.lang.description,
             sizeof(audio->config.lang.description), "%s (%s %s)",
  ...
  if (audio->config.in.channel_layout) {
    ...
    char *desc =
        audio->config.lang.description + strlen(audio->config.lang.description);
  ...
  // Append input bitrate in kbps to the end of the description if greater than
  // 1 ffmpeg may report some audio bitrates as 1, not an issue
  ...
  hb_log("scan: audio 0x%x: %s, rate=%dHz, bitrate=%d %s", audio->id, info.name,
         audio->config.in.samplerate, audio->config.in.bitrate,
...
#define p state.param.scanning
  ...
  p.preview_count = 1;
  p.progress = 0.5 *
               ((float)p.title_cur + ((float)p.preview_cur / p.preview_count)) /
               p.title_count;
#undef p
...
#define p state.param.scanning
  ...
  if (scan->title_index)
    p.progress =
        ((float)p.title_cur - 1 + ((float)p.preview_cur / p.preview_count)) /
        p.title_count;
  else
    p.progress = 0.5 + 0.5 *
                           ((float)p.title_cur - 1 +
                            ((float)p.preview_cur / p.preview_count)) /
                           p.title_count;
#undef p
