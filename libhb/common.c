...
int hb_video_rate_min =
    HB_VIDEO_CLOCK / HB_VIDEO_FPS_MAX; // Min clock rate from *max* frame rate
int hb_video_rate_max =
    HB_VIDEO_CLOCK / HB_VIDEO_FPS_MIN; // Max clock rate from *min* frame rate
...
int hb_audio_bitrates_count =
    sizeof(hb_audio_bitrates) / sizeof(hb_audio_bitrates[0]);
...
int hb_audio_dithers_count =
    sizeof(hb_audio_dithers) / sizeof(hb_audio_dithers[0]);
...
int hb_audio_mixdowns_count =
    sizeof(hb_audio_mixdowns) / sizeof(hb_audio_mixdowns[0]);
    ...
    {
        {
            "H.264 10-bit (x264)",
            "x264_10bit",
            "H.264 10-bit (libx264)",
            HB_VCODEC_X264_10BIT,
            HB_MUX_MASK_MP4 | HB_MUX_MASK_MKV,
        },
        NULL,
        1,
    ...
    {
        {
            "H.265 10-bit (x265)",
            "x265_10bit",
            "H.265 10-bit (libx265)",
            HB_VCODEC_X265_10BIT,
            HB_MUX_AV_MP4 | HB_MUX_AV_MKV,
        },
        NULL,
        1,
    ...
    {
        {
            "H.265 12-bit (x265)",
            "x265_12bit",
            "H.265 12-bit (libx265)",
            HB_VCODEC_X265_12BIT,
            HB_MUX_AV_MP4 | HB_MUX_AV_MKV,
        },
        NULL,
        1,
    ...
    {
        {
            "H.265 16-bit (x265)",
            "x265_16bit",
            "H.265 16-bit (libx265)",
            HB_VCODEC_X265_16BIT,
            HB_MUX_AV_MP4 | HB_MUX_AV_MKV,
        },
        NULL,
        1,
...
int hb_video_encoders_count =
    sizeof(hb_video_encoders) / sizeof(hb_video_encoders[0]);
...
int hb_audio_encoders_count =
    sizeof(hb_audio_encoders) / sizeof(hb_audio_encoders[0]);
...
void hb_common_global_init(int disable_hardware) {
  ...
  for (i = 0; i < hb_video_encoders_count; i++) {
    if (hb_video_encoders[i].enabled) {
      ...
      hb_video_encoders[i].enabled = hb_video_encoder_is_enabled(
          hb_video_encoders[i].item.codec, disable_hardware);
    }
    if (hb_video_encoders[i].enabled) {
...
const char *hb_video_framerate_get_name(int framerate) {
  ...
  while ((video_framerate = hb_video_framerate_get_next(video_framerate)) !=
         NULL) {
...
const char *hb_audio_samplerate_get_name(int samplerate) {
  ...
  while ((audio_samplerate = hb_audio_samplerate_get_next(audio_samplerate)) !=
         NULL) {
 ...
 *
 * Encoder    1.0 channel    2.0 channels    5.1 channels    6.1 channels    7.1
 * channels
 * --------------------------------------------------------------------------------------
 *
 * ffaac
 ...
 * Also, ffaac isn't a great encoder, so you don't want to allow too low a
 * bitrate. Limits: minimum of  32 Kbps per channel maximum of 192 Kbps per
 * channel at 32 kHz, adjusted for sr_shift maximum of 256 Kbps per channel
 * at 44.1-48 kHz, adjusted for sr_shift
 *
 * vorbis
 * ------
 * supported samplerates: 8 - 48 kHz
 * lib/modes/setup_*.h provides a range of allowed bitrates for various
 * configurations. for each samplerate, the highest minimums and lowest maximums
 * are: 8 kHz        Minimum  8 Kbps, maximum  32 Kbps (per channel, incl. LFE).
 ...
 *         maximum of 32/86/190/240 Kbps per channel (8-12, 16-24, 32, 44.1-48
 * kHz)
 *
 * lame
 * ----
 ...
 * ffac3
 * -----
 * supported samplerates: 32 - 48 kHz (< 32 kHz disabled for compatibility
 * reasons) Dolby's encoder has a min. of 224 Kbps for 5 full-bandwidth channels
 * (5.0, 5.1) The maximum AC3 bitrate is 640 Kbps Limits: minimum of 224/5 Kbps
 * per full-bandwidth channel, maximum of 640 Kbps
 *
 * ffeac3
 * ------
 * supported samplerates: 32 - 48 kHz (< 32 kHz not supported by libav encoder)
 * Dolby's encoder has a min. of 224 Kbps for 5 full-bandwidth channels
 * (5.0, 5.1) The maximum bitrate is 128 bits per sample per second Limits:
 * minimum of 224/5 Kbps per full-bandwidth channel maximum of 128/1000 *
 * samplerate Kbps
 *
 * ca_aac
 * ------
 ...
 *  8 kHz         8 -  24        16 -  48        40 - 112        48 - 144 56 -
 * 160 12 kHz        12 -  32        24 -  64        64 - 160        72 - 192 96
 * - 224 16 kHz        12 -  48        24 -  96        64 - 224        72 - 288
 * 96 - 320 24 kHz        16 -  64        32 - 128        80 - 320        96 -
 * 384       112 - 448 32 kHz        24 -  96        48 - 192       128 - 448
 * 144 - 576       192 - 640 48 kHz        32 - 256        64 - 320       160 -
 * 768       192 - 960       224 - 960 Limits: 8 kHz -> minimum of  8 Kbps and
 * maximum of  24 Kbps per full-bandwidth channel 12 kHz -> minimum of 12 Kbps
 * and maximum of  32 Kbps per full-bandwidth channel 16 kHz -> minimum of 12
 * Kbps and maximum of  48 Kbps per full-bandwidth channel 24 kHz -> minimum of
 * 16 Kbps and maximum of  64 Kbps per full-bandwidth channel 32 kHz -> minimum
 * of 24 Kbps and maximum of  96 Kbps per full-bandwidth channel 48 kHz ->
 * minimum of 32 Kbps and maximum of 160 Kbps per full-bandwidth channel 48 kHz
 * ->                        maximum of +96 Kbps for Mono Note:
 * encCoreAudioInit() will sanitize any mistake made here.
 *
 * ca_haac
 * -------
 ...
 * 32 kHz             192             384             960
 * 48 kHz             288             576            1440
 * Limits: minimum of samplerate * 2/3 Kbps per full-bandwidth channel (see
 * ca_aac) maximum of samplerate * 6.0 Kbps per full-bandwidth channel
 *
 * fdk_haac
 * --------
 ...
 * Limits: minimum of 12 Kbps per full-bandwidth channel  (<= 32 kHz) (see
 * ca_haac) minimum of 16 Kbps per full-bandwidth channel  ( > 32 kHz) (see
 * ca_haac) maximum of 48,  96 or 192 Kbps (1.0, 2.0, 5.1) (<= 16 kHz) maximum
 * of 64, 128 or 256 Kbps (1.0, 2.0, 5.1) ( > 16 kHz)
 */
void hb_audio_bitrate_get_limits(uint32_t codec, int samplerate, int mixdown,
  ...
  if (samplerate == 0) {
    int dummy;
    hb_audio_bitrate_get_limits(codec, hb_audio_rates_first_item->rate, mixdown,
                                low, &dummy);
    hb_audio_bitrate_get_limits(codec, hb_audio_rates_last_item->rate, mixdown,
                                &dummy, high);
    return;
  }
  ...
  case HB_ACODEC_FDK_HAAC:
    *low = (nchannels * (12 + (4 * (samplerate >= 44100))));
    *high = (nchannels - (nchannels > 2)) * (48 + (16 * (samplerate >= 22050)));
    break;

  case HB_ACODEC_FFAAC:
    *low = ((nchannels + lfe_count) * 32);
    *high = ((nchannels + lfe_count) *
             ((192 + (64 * ((samplerate << sr_shift) >= 44100))) >> sr_shift));
    break;
  ...
  case HB_ACODEC_VORBIS:
    *low = (nchannels + lfe_count) *
           (14 + (8 * (sr_shift < 2)) + (6 * (sr_shift < 1)));
    *high = (nchannels + lfe_count) *
            (32 + (54 * (sr_shift < 2)) + (104 * (sr_shift < 1)) +
             (50 * (samplerate >= 44100)));
    break;
...
int hb_mixdown_get_best(uint32_t codec, uint64_t layout, int mixdown) {
  ...
  while ((audio_mixdown = hb_mixdown_get_next(audio_mixdown)) != NULL) {
    if ((mixdown == HB_INVALID_AMIXDOWN ||
         audio_mixdown->amixdown <= mixdown) &&
...
int hb_layout_get_low_freq_channel_count(int64_t layout) {
  return !!(layout & AV_CH_LOW_FREQUENCY) + !!(layout & AV_CH_LOW_FREQUENCY_2);
}
...
void hb_autopassthru_apply_settings(hb_job_t *job) {
  ...
  for (i = already_printed = 0; i < hb_list_count(job->list_audio);) {
    ...
    if (audio->config.out.codec == HB_ACODEC_AUTO_PASS) {
      ...
      already_printed = 1;
      audio->config.out.codec = hb_autopassthru_get_encoder(
          audio->config.in.codec, job->acodec_copy_mask, job->acodec_fallback,
          job->mux);
      if (audio->config.out.codec == HB_ACODEC_NONE ||
          audio->config.out.codec == HB_ACODEC_INVALID) {
        hb_log("Auto Passthru: passthru not possible and no valid fallback "
               "specified, dropping track %d",
      ...
      if (!(audio->config.out.codec & HB_ACODEC_PASS_FLAG)) {
        hb_log(
            "Auto Passthru: passthru not possible for track %d, using fallback",
            audio->config.out.track);
        if (audio->config.out.mixdown <= 0) {
          audio->config.out.mixdown = hb_mixdown_get_default(
              audio->config.out.codec, audio->config.in.channel_layout);
        } else {
          audio->config.out.mixdown = hb_mixdown_get_best(
              audio->config.out.codec, audio->config.in.channel_layout,
              audio->config.out.mixdown);
        }
        if (audio->config.out.samplerate <= 0)
          audio->config.out.samplerate = audio->config.in.samplerate;
        audio->config.out.samplerate = hb_audio_samplerate_find_closest(
        ...
        if (audio->config.out.bitrate > 0) {
          // Use best bitrate
          audio->config.out.bitrate = hb_audio_bitrate_get_best(
              audio->config.out.codec, audio->config.out.bitrate,
              audio->config.out.samplerate, audio->config.out.mixdown);
        } else if (quality_not_allowed ||
                   audio->config.out.quality != HB_INVALID_AUDIO_QUALITY) {
          // Use default bitrate
          audio->config.out.bitrate = hb_audio_bitrate_get_default(
              audio->config.out.codec, audio->config.out.samplerate,
              audio->config.out.mixdown);
        } else {
          // Use best quality
          audio->config.out.quality = hb_audio_quality_get_best(
        ...
        if (audio->config.out.compression_level < 0) {
          ...
          audio->config.out.compression_level = hb_audio_compression_get_best(
              audio->config.out.codec, audio->config.out.compression_level);
        }
      } else {
        const hb_encoder_t *audio_encoder = NULL;
        while ((audio_encoder = hb_audio_encoder_get_next(audio_encoder)) !=
               NULL) {
          if (audio_encoder->codec == audio->config.out.codec) {
            hb_log("Auto Passthru: using %s for track %d", audio_encoder->name,
                   audio->config.out.track);
            break;
          }
        }
...
void hb_limit_rational64(int64_t *x, int64_t *y, int64_t num, int64_t den,
                         int64_t limit) {
...
int hb_buffer_list_size(hb_buffer_list_t *list) { return list->size; }
...
void hb_list_add(hb_list_t *l, void *p) {
  ...
  if (l->items_count == l->items_alloc) {
    ...
    l->items_alloc += HB_LIST_DEFAULT_SIZE;
    l->items = realloc(l->items, l->items_alloc * sizeof(void *));
  }
 ...
 * in that buffer.
 *********************************************************************/
void hb_list_getbytes(hb_list_t *l, uint8_t *dst, int size, uint64_t *pts,
                      uint64_t *pos) {
  hb_buffer_t *buf;
  int copied;
  int copying;
 ...
 *********************************************************************/
void hb_valog(hb_debug_level_t level, const char *prefix, const char *log,
              va_list args) {
  char *string;
  time_t _now;
  struct tm *now;
  ...
  if (prefix && *prefix) {
    ...
    snprintf(preamble, 361, "[%02d:%02d:%02d] %s %s\n", now->tm_hour,
             now->tm_min, now->tm_sec, prefix, log);
  } else {
...
static void job_setup(hb_job_t *job, hb_title_t *title) {
  ...
  job->qsv.decode = !!(title->video_decode_support & HB_DECODE_SUPPORT_QSV);
#endif
}
...
hb_dict_t *hb_parse_filter_settings(const char *settings_str) {
  ...
  for (ii = 0; settings_list[ii] != NULL; ii++) {
    ...
    if (settings_pair[0] == NULL || settings_pair[1] == NULL) {
      ...
      hb_value_free(&dict);
      hb_log("hb_parse_filter_settings: Error parsing (%s)", settings_str);
      return NULL;
    }
    key = settings_pair[0];
 ...
 *********************************************************************/
void hb_chapter_set_title_by_index(hb_job_t *job, int chapter_index,
                                   const char *title) {
...
void hb_audio_config_init(hb_audio_config_t *audiocfg) {
  ...
  audiocfg->out.codec =
      hb_audio_encoder_get_default(HB_MUX_MP4); // default container
...
int hb_subtitle_add_ssa_header(hb_subtitle_t *subtitle, const char *font,
  ...
  const char *ssa_header =
      ...
      "Style: "
      "Default,%s,%d,&H00FFFFFF,&H00FFFFFF,&H000F0F0F,&H000F0F0F,0,0,0,0,100,"
      "100,0,0.00,1,2,3,2,20,20,20,0\r\n";
...
int hb_subtitle_add(const hb_job_t *job,
                    const hb_subtitle_config_t *subtitlecfg, int track) {
  hb_title_t *title = job->title;
  hb_subtitle_t *subtitle;
...
int hb_import_subtitle_add(const hb_job_t *job,
  ...
  subtitle->id =
      (hb_list_count(job->list_subtitle) << 8) | HB_SUBTITLE_IMPORT_TAG;
...
int hb_audio_can_apply_drc(uint32_t codec, uint32_t codec_param, int encoder) {
  if (encoder & HB_ACODEC_PASS_FLAG) {
    ...
    return (codec_param == AV_CODEC_ID_AC3 || codec_param == AV_CODEC_ID_EAC3);
...
int hb_audio_can_apply_drc2(hb_handle_t *h, int title_idx, int audio_idx,
                            int encoder) {
  ...
  if (src) {
    ...
    if (src->list_coverart) {
      ...
      for (ii = 0; ii < hb_list_count(src->list_coverart); ii++) {
        ...
        hb_metadata_add_coverart(metadata, art->data, art->size, art->type);
      }
    }
  }
  return metadata;
...
void hb_metadata_set_release_date(hb_metadata_t *metadata,
...
void hb_metadata_set_album_artist(hb_metadata_t *metadata,
...
void hb_metadata_set_description(hb_metadata_t *metadata,
...
void hb_metadata_set_long_description(hb_metadata_t *metadata,
...
void hb_metadata_add_coverart(hb_metadata_t *metadata, const uint8_t *data,
                              int size, int type) {
  if (metadata) {
...
void hb_hexdump(hb_debug_level_t level, const char *label, const uint8_t *data,
                int len) {
  int ii;
