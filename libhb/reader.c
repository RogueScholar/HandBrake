...
hb_work_object_t hb_reader = {.id = WORK_READER,
                              .name = "Reader",
                              .init = reader_init,
                              .work = reader_work,
                              .close = reader_close,
...
static int hb_reader_open(hb_work_private_t *r) {
  if (r->title->type == HB_BD_TYPE) {
    ...
    if (r->job->start_at_preview) {
      ...
      r->duration -= chapter_end_pts(r->job->title, r->job->chapter_start - 1);
    }
...
static int reader_init(hb_work_object_t *w, hb_job_t *job) {
  ...
  if (job->pts_to_stop > 0) {
    ...
    r->duration =
        (int64_t)frames * job->title->vrate.den * 90000 / job->title->vrate.num;
  } else {
...
static hb_buffer_t *splice_discontinuity(hb_work_private_t *r,
                                         hb_buffer_t *buf) {
