...
hb_work_object_t hb_encx264 = {WORK_ENCX264, "H.264/AVC encoder (libx264)",
                               encx264Init, encx264Work, encx264Close};
...
const char *libx264_10bit_names[] = {"libx26410b", "libx264_main10", NULL};

const char *libx264_8bit_names[] = {"libx264", "libx2648b", "libx264_main",
                                    NULL};
...
void hb_x264_global_init(void) {
  ...
  if (dll_bitdepth > 0 && dll_bitdepth != x264_apis[0].bit_depth &&
      ...
      x264_apis[1].param_parse != NULL && x264_apis[1].encoder_open != NULL &&
      x264_apis[1].encoder_headers != NULL &&
      x264_apis[1].encoder_encode != NULL &&
      x264_apis[1].encoder_delayed_frames != NULL &&
      x264_apis[1].encoder_close != NULL && x264_apis[1].picture_init != NULL) {
...
const x264_api_t *hb_x264_api_get(int bit_depth) {
  ...
  for (ii = 0; ii < HB_X264_API_COUNT; ii++) {
    if (-1 != x264_apis[ii].bit_depth && bit_depth == x264_apis[ii].bit_depth) {
      return &x264_apis[ii];
    }
  }
  return NULL;
...
int encx264Init(hb_work_object_t *w, hb_job_t *job) {
  ...
  param.i_keyint_min = (double)job->orig_vrate.num / job->orig_vrate.den + 0.5;
  ...
  int ret;
  hb_dict_iter_t iter;
  for (iter = hb_dict_iter_init(x264_opts); iter != HB_DICT_ITER_DONE;
    ...
    if (ret == X264_PARAM_BAD_VALUE)
      hb_log("x264 options: Bad argument %s=%s", key, str ? str : "(null)");
    free(str);
  }
  ...
  /* For 25 fps sources, HandBrake's explicit keyints will match the x264
   * defaults: min-keyint 25 (same as auto), keyint 250. */
  ...
  char *x264_opts_unparsed = hb_x264_param_unparse(
      pv->api->bit_depth, job->encoder_preset, job->encoder_tune,
      job->encoder_options, job->encoder_profile, job->encoder_level,
...
static hb_buffer_t *expand_buf(int bit_depth, hb_buffer_t *in) {
  ...
  buf = hb_frame_buffer_init(AV_PIX_FMT_YUV420P16BE, in->f.width, in->f.height);
  for (pp = 0; pp < 3; pp++) {
    int xx, yy;
...
static hb_buffer_t *x264_encode(hb_work_object_t *w, hb_buffer_t *in) {
  ...
  if (pv->last_stop != AV_NOPTS_VALUE && pv->last_stop != in->s.start) {
    hb_log("encx264 input continuity err: last stop %" PRId64
           "  start %" PRId64,
...
static int apply_h264_profile(const x264_api_t *api, x264_param_t *param,
  ...
  if (h264_profile != NULL && strcasecmp(h264_profile, profile_names[0]) != 0) {
    /*
    ...
    if ((param->b_interlaced || param->b_fake_interlaced) &&
      ...
      if (verbose) {
        hb_log("apply_h264_profile [warning]: baseline profile doesn't support "
               "interlacing, disabling");
      }
    ...
    if (strcasecmp(h264_profile, "high444") != 0 &&
      ...
      if (verbose) {
        hb_log("apply_h264_profile [warning]: lossless requires high444 "
               "profile, disabling");
      }
...
int apply_h264_level(const x264_api_t *api, x264_param_t *param,
...
#endif
  {
    ...
    if (param->rc.i_rc_method == X264_RC_CRF && param->rc.f_rf_constant < 1.0) {
      hb_encx264_profile = HB_ENCX264_PROFILE_HIGH444;
   ...
   */
  if (x264_level->frame_only &&
      (param->b_interlaced || param->b_fake_interlaced)) {
    if (verbose) {
      hb_log("apply_h264_level [warning]: interlaced flag not supported for "
             "level %s, disabling",
             h264_level);
    }
    ret = 1;
  ...
  if (hb_encx264_profile != HB_ENCX264_PROFILE_HIGH444) {
    ...
    int cbp_factor;
    cbp_factor = hb_encx264_profile == HB_ENCX264_PROFILE_HIGH10
                     ? 12
    ...
    if (!param->rc.i_vbv_max_bitrate) {
      ...
      param->rc.i_vbv_max_bitrate = MIN(param->rc.i_vbv_max_bitrate,
    ...
    if (!param->rc.i_vbv_buffer_size) {
      ...
      param->rc.i_vbv_buffer_size =
          MIN(param->rc.i_vbv_buffer_size, (x264_level->cpb * cbp_factor) / 4);
    }
  }
   ...
   */
  param->analyse.i_mv_range = MIN(
      param->analyse.i_mv_range, x264_level->mv_range >> !!param->b_interlaced);
  ...
  if (x264_level->frame_size < i_mb_size) {
    if (verbose) {
      hb_log("apply_h264_level [warning]: frame size (%dx%d, %d macroblocks) "
             "too high for level %s (max. %d macroblocks)",
    ...
    if (verbose) {
      hb_log("apply_h264_level [warning]: framerate (%.3f) too high for level "
             "%s at %dx%d (max. %.3f)",
  ...
  if (i_mb_width > max_mb_side) {
    if (verbose) {
      hb_log("apply_h264_level [warning]: frame too wide (%d) for level %s "
             "(max. %d)",
  ...
  if (i_mb_height > max_mb_side) {
    if (verbose) {
      hb_log("apply_h264_level [warning]: frame too tall (%d) for level %s "
             "(max. %d)",
...
char *hb_x264_param_unparse(int bit_depth, const char *x264_preset,
  ...
  if (param.i_bframe > 0) {
    if (param.i_bframe_adaptive != defaults.i_bframe_adaptive) {
      ...
      hb_dict_set(x264_opts, "b-adapt", hb_value_int(param.i_bframe_adaptive));
    } else {
  ...
  if (param.analyse.i_me_method != defaults.analyse.i_me_method) {
    ...
    for (i = 0; x264_motion_est_names[i] != NULL; i++)
      if (param.analyse.i_me_method == i)
        hb_dict_set(x264_opts, "me", hb_value_string(x264_motion_est_names[i]));
  } else {
    hb_dict_remove(x264_opts, "me");
  }
  if (param.analyse.i_me_range != defaults.analyse.i_me_range) {
    // can be modified by: presets
    hb_dict_set(x264_opts, "merange", hb_value_int(param.analyse.i_me_range));
  } else {
    hb_dict_remove(x264_opts, "merange");
  }
  if (param.analyse.i_mv_range != defaults.analyse.i_mv_range) {
    // can be modified by: level
    hb_dict_set(x264_opts, "mvrange", hb_value_int(param.analyse.i_mv_range));
  } else {
    hb_dict_remove(x264_opts, "mvrange");
  }
  if (param.analyse.i_subpel_refine > 9 &&
      (param.rc.i_aq_mode == 0 || param.analyse.i_trellis < 2)) {
  ...
  if (param.analyse.i_trellis != defaults.analyse.i_trellis) {
    // can be modified by: presets
    hb_dict_set(x264_opts, "trellis", hb_value_int(param.analyse.i_trellis));
  } else {
  ...
  if (param.rc.f_ip_factor != defaults.rc.f_ip_factor) {
    // can be modified by: tune grain
    hb_dict_set(x264_opts, "ipratio", hb_value_double(param.rc.f_ip_factor));
  } else {
  ...
  if (param.i_bframe > 0 && !param.rc.b_mb_tree &&
    ...
    // can be modified by: tune grain
    hb_dict_set(x264_opts, "pbratio", hb_value_double(param.rc.f_pb_factor));
  } else {
  ...
  if (param.rc.i_lookahead != defaults.rc.i_lookahead) {
    // can be modified by: presets, tune zerolatency
    hb_dict_set(x264_opts, "rc-lookahead", hb_value_int(param.rc.i_lookahead));
  } else {
