...
static int ssa_update_style(const char *ssa, hb_subtitle_style_context_t *ctx) {
  ...
  while (ssa[pos] != '}' && ssa[pos] != '\0') {
    ...
    if (strchr("ibu", ssa[pos]) != NULL && isdigit(ssa[pos + 1])) {
      ...
      case 'b':
        style->flags =
            (style->flags & ~HB_STYLE_FLAG_BOLD) | !!val * HB_STYLE_FLAG_BOLD;
        break;
      case 'u':
    ...
    if ((ssa[pos] == 'a' && ssa[pos + 1] == '&' && ssa[pos + 2] == 'H') ||
        (!strcmp(ssa + pos, "alpha") && ssa[pos + 5] == '&' &&
         ssa[pos + 6] == 'H')) {
      // Font alpha markup
      char *endptr;
      uint8_t alpha;
...
static int add_style(hb_subtitle_style_context_t *ctx, char **style,
  ...
  if (ctx->styles_count + 1 > ctx->styles_size) {
    ...
    tmp = realloc(ctx->styles, ctx->styles_size * sizeof(hb_subtitle_style_t));
    if (tmp == NULL) {
      return 1;
    }
    ctx->styles = tmp;
...
#define TX3G_STYLES                                                            \
  (HB_STYLE_FLAG_BOLD | HB_STYLE_FLAG_ITALIC | HB_STYLE_FLAG_UNDERLINE)
...
static int tx3g_update_style(hb_tx3g_style_context_t *ctx, int utf8_end_pos) {
  ...
  if (((ctx->out_style.flags ^ style->flags) & TX3G_STYLES) ||
      ...
      ctx->out_style.fg_alpha != style->fg_alpha || ctx->flush) {
...
hb_tx3g_style_context_t *hb_tx3g_style_init(int height,
 ...
 */
void hb_muxmp4_process_subtitle_style(hb_tx3g_style_context_t *ctx,
                                      uint8_t *input, uint8_t **out_buf,
