...
queue_t *queue_new() { return calloc(1, sizeof(queue_t)); }
...
int queue_is_empty(queue_t *q) { return q->head == NULL; }
...
static tag_map_t tag_map[] = {
    {"plist", P_PLIST},   {"key", P_KEY},         {"array", P_ARRAY},
    {"dict", P_DICT},     {"integer", P_INTEGER}, {"real", P_REAL},
    {"string", P_STRING}, {"date", P_DATE},       {"true", P_TRUE},
    {"false", P_FALSE},   {"data", P_DATA},
};
...
static void start_element(void *ud, const xmlChar *xname,
  ...
  if (gval) { // There's an element to add
    if (current == NULL) {
      pd->plist = gval;
      return;
    }
...
static void end_element(void *ud, const xmlChar *xname) {
  char *name = (char *)xname;
  parse_data_t *pd = (parse_data_t *)ud;
  int id;
  union {
...
static void text_data(void *ud, const xmlChar *xtext, int len) {
...
static void parse_warning(void *ud, const char *msg, ...) {
  va_list args;
...
static void parse_error(void *ud, const char *msg, ...) {
  va_list args;
...
static void indent_fprintf(FILE *file, int indent, const char *fmt, ...) {
  va_list ap;
...
static void gval_write(FILE *file, hb_value_t *gval) {
  static int indent = 0;
  int ii;
  ...
  if (gtype == HB_VALUE_TYPE_ARRAY) {
    ...
    for (iter = hb_dict_iter_init(gval); iter != HB_DICT_ITER_DONE;
...
void hb_plist_write(FILE *file, hb_value_t *gval) {
  fprintf(file, "%s", preamble);
  gval_write(file, gval);
  fprintf(file, "%s", postfix);
}

void hb_plist_write_file(const char *filename, hb_value_t *gval) {
  FILE *file;
