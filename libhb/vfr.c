...
static int hb_vfr_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                       hb_buffer_t **buf_out);
...
// count less.
static inline unsigned sse_block16(unsigned *gamma_lut, uint8_t *a, uint8_t *b,
                                   int stride) {
  int x, y;
  unsigned sum = 0;
  int diff;
...
static float motion_metric(unsigned *gamma_lut, hb_buffer_t *a,
                           hb_buffer_t *b) {
  int bw = a->f.width / 16;
...
static hb_buffer_t *adjust_frame_rate(hb_filter_private_t *pv,
  ...
  if (in != NULL) {
    ...
    pv->frame_metric[count - 1] =
        motion_metric(pv->gamma_lut, penultimate, ultimate);
...
#if defined(HB_DEBUG_CFR_DROPS)
    hb_log("CFR Drop: %ld metric %d", out->s.pcr,
           (int)pv->frame_metric[drop_frame]);
    int jj;
  ...
  hb_log("CFR Pass: %ld ~ %ld metric %d", out->s.pcr, out->s.pcr - lastpass,
         (int)pv->frame_metric[0]);
  lastpass = out->s.pcr;
#endif
...
static hb_filter_info_t *hb_vfr_info(hb_filter_object_t *filter) {
  ...
  if (pv->cfr == 0) {
    ...
    snprintf(info->human_readable_desc, 128,
             "frame rate: %.3f fps -> constant %.3f fps", source_fps,
             constant_fps);
  }
...
static void hb_vfr_close(hb_filter_object_t *filter) {
  ...
  hb_log("vfr: lost time: %" PRId64 " (%i frames)", pv->total_lost_time,
         pv->dropped_frames);
  hb_log("vfr: gained time: %" PRId64 " (%i frames) (%" PRId64
         " not accounted for)",
