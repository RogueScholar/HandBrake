...
#if defined(HB_FIFO_DEBUG)
static hb_fifo_t fifo_list = {.next = NULL};
#endif
...
void hb_buffer_pool_init(void) {
  ...
  // 2^10 come from here.
  buffers.pool[BUFFER_POOL_FIRST] =
      hb_fifo_init(BUFFER_POOL_MAX_ELEMENTS * 10, 1);
...
void hb_buffer_pool_free(void) {
  ...
  for (i = 0; i < hb_list_count(buffers.alloc_list); i++) {
    ...
    hb_deep_log(2, "leaked buffer %p type %d size %d alloc %d", b, b->s.type,
                b->size, b->alloc);
  }
#endif
  ...
  hb_deep_log(2,
              "Allocated %" PRId64
              " bytes of buffers on this pass and Freed %" PRId64 " bytes, "
              "%" PRId64 " bytes leaked",
...
hb_buffer_t *hb_buffer_init(int size) { return hb_buffer_init_internal(size); }
  ...
  for (pp = 0; pp <= buf->f.max_plane; pp++) {
    ...
    if (data != NULL) {
      ...
      for (ii = 0; ii < height_stride - height; ii++) {
        memcpy(data + pos + ii * stride, data + pos - ((ii + 1) * stride),
               stride);
      }
    }
  }
  ...
  while (b) {
    ...
    if (b->qsv_details.frame) {
      mfxFrameSurface1 *surface =
          (mfxFrameSurface1 *)b->qsv_details.frame->data[3];
      if (surface) {
...
hb_image_t *hb_image_init(int pix_fmt, int width, int height) {
  ...
  for (pp = 0; pp <= image->max_plane; pp++) {
    ...
    image->plane[pp].size =
        image->plane[pp].stride * image->plane[pp].height_stride;
...
// Pulls the first packet out of this FIFO, blocking until such a packet is
// available. Returns NULL if this FIFO has been closed or flushed.
...
// Waits until the specified FIFO is no longer full or until FIFO_TIMEOUT
// milliseconds have elapsed. Returns whether the FIFO is non-full upon return.
...
void hb_fifo_push(hb_fifo_t *f, hb_buffer_t *b) {
  ...
  hb_lock(f->lock);
  if (f->size >= f->capacity && f->cond_alert_full != NULL) {
    hb_cond_broadcast(f->cond_alert_full);
  }
  if (f->size > 0) {
    f->last->next = b;
