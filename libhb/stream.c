...
static const stream2codec_t st2codec[256] = {
    ...
    st(0x80, U, HB_ACODEC_FFMPEG, AV_CODEC_ID_PCM_BLURAY,
       "Digicipher II Video"),
...
static int update_ps_streams(hb_stream_t *stream, int stream_id,
                             int stream_id_ext, int stream_type, int in_kind);
static int update_ts_streams(hb_stream_t *stream, int pid, int stream_id_ext,
                             int stream_type, int in_kind, int *pes_idx);
...
static inline void bits_init(bitbuf_t *bb, uint8_t *buf, int bufsize,
                             int clear);
static inline unsigned int bits_peek(bitbuf_t *bb, int bits);
static void pes_add_audio_to_title(hb_stream_t *s, int i, hb_title_t *t,
                                   int sort);
static int hb_parse_ps(hb_stream_t *stream, uint8_t *buf, int len,
                       hb_pes_info_t *pes_info);
...
static void ts_warn_helper(hb_stream_t *stream, char *log, va_list args) {
  ...
  if (stream->frames - stream->last_error_frame >= 30 * 60) {
    ...
    if (stream->errors - stream->last_error_count < 10) {
      ...
      double Epcnt =
          (double)Edelta * 100. / (stream->frames - stream->last_error_frame);
      hb_log("stream: %d new errors (%.0f%%) up to frame %d: %s", Edelta, Epcnt,
             stream->frames, msg);
    }
...
static int check_ps_sc(const uint8_t *buf) {
  ...
  if (mark == 0x02) {
    ...
    if ((buf[4] & 0xf1) != 0x21 || (buf[6] & 0x01) != 0x01 ||
        (buf[8] & 0x01) != 0x01 || (buf[9] & 0x80) != 0x80 ||
        (buf[11] & 0x01) != 0x01) {
      return 0;
    }
    // mpeg-1 pack header
    ...
    // Check other marker bits to make it less likely
    // that we are being spoofed.
    if ((buf[4] & 0xC4) != 0x44 || (buf[6] & 0x04) != 0x04 ||
        (buf[8] & 0x04) != 0x04 || (buf[9] & 0x01) != 0x01 ||
        (buf[12] & 0x03) != 0x03) {
      return 0;
    }
    // mpeg-2 pack header
      ...
      if (check_ps_sync(&buf[offset]) && check_ps_sc(&buf[offset])) {
        ...
        if (data_len && sid > 0xba && sid < 0xf9) {
          ...
          if (fread(sc_buf, 1, 4, stream->file_handle) != 4)
            return 0;
          if (sc_buf[0] == 0x00 && sc_buf[1] == 0x00 && sc_buf[2] == 0x01) {
            return 1;
          }
...
static void prune_streams(hb_stream_t *d) {
  if (d->hb_stream_type == transport) {
    ...
    for (ii = 0; ii < d->ts.count; ii++) {
      ...
      if (ts_stream_kind(d, ii) == U && !d->ts.list[ii].is_pcr) {
      ...
      if (ts_stream_kind(d, ii) == A) {
        ...
        if (!d->ts.list[ii].is_pcr && hb_stream_try_delete_ts_entry(d, ii)) {
          continue;
        }
      }
    }
 ...
 *
 **********************************************************************/
hb_stream_t *hb_stream_open(hb_handle_t *h, const char *path, hb_title_t *title,
                            int scan) {
  if (title == NULL) {
...
static int new_pid(hb_stream_t *stream) {
  ...
  if (stream->ts.count == stream->ts.alloc) {
    ...
    stream->ts.list = realloc(stream->ts.list, sizeof(hb_ts_stream_t) * num);
  }
  int ii;
...
static int new_pes(hb_stream_t *stream) {
  ...
  if (stream->pes.count == stream->pes.alloc) {
    ...
    stream->pes.list = realloc(stream->pes.list, sizeof(hb_pes_stream_t) * num);
  }
  int ii;
...
void hb_stream_close(hb_stream_t **_d) {
  ...
  if (stream->frames) {
    hb_log("stream: %d good frames, %d errors (%.0f%%)", stream->frames,
           stream->errors,
           (double)stream->errors * 100. / (double)stream->frames);
  }
...
static const uint8_t *next_packet(hb_stream_t *stream) {
  ...
  while (1) {
    ...
    ts_warn(stream,
            "next_packet: sync lost @ %" PRId64 ", regained after %" PRId64
            " bytes",
            pos, pos2);
  }
}
...
static void CreateDecodedNAL(uint8_t **dst, int *dst_len, const uint8_t *src,
                             int src_len) {
  ...
  *dst = d;
  ...
  if (d) {
    while (src < end) {
      if (src < end - 3 && src[0] == 0x00 && src[1] == 0x00 && src[2] == 0x01) {
...
static int isIframe(hb_stream_t *stream, const uint8_t *buf, int len) {
  ...
  if (pes->stream_type <= 2 || pes->codec_param == AV_CODEC_ID_MPEG1VIDEO ||
...
static const uint8_t *hb_ts_stream_getPEStype(hb_stream_t *stream, uint32_t pid,
  ...
  while (--npack >= 0) {
    ...
    case 0x30: // adaptation
      adapt_len = buf[4] + 1;
      if (adapt_len > 184) {
        hb_log("hb_ts_stream_getPEStype: invalid adaptation field length %d "
               "for PID 0x%x",
               buf[4], pid);
        continue;
      }
    ...
    if (buf[adapt_len + 4] == 0x00 && buf[adapt_len + 5] == 0x00 &&
        buf[adapt_len + 6] == 0x01) {
...
static hb_buffer_t *hb_ps_stream_getVideo(hb_stream_t *stream,
                                          hb_pes_info_t *pi) {
...
static struct pts_pos hb_sample_pts(hb_stream_t *stream, uint64_t fpos) {
  ...
  if (stream->hb_stream_type == transport) {
    ...
    pp.pts = ((((uint64_t)pes[9] >> 1) & 7) << 30) | ((uint64_t)pes[10] << 22) |
             (((uint64_t)pes[11] >> 1) << 15) | ((uint64_t)pes[12] << 7) |
             ((uint64_t)pes[13] >> 1);
...
int hb_stream_seek_chapter(hb_stream_t *stream, int chapter_num) {
  ...
  if (chapter != NULL && chapter_num > 1) {
    int64_t pos =
        ((sum_dur * AV_TIME_BASE) / 90000) + ffmpeg_initial_timestamp(stream);
    ...
    if (pos > 0) {
      ...
      avformat_seek_file(stream->ffmpeg_ic, stream->ffmpeg_video_id, 0, pos,
 ...
 **********************************************************************/
int hb_stream_chapter(hb_stream_t *src_stream) { return (src_stream->chapter); }
...
static const char *stream_type_name2(hb_stream_t *stream,
                                     hb_pes_stream_t *pes) {
...
static void pes_add_subtitle_to_title(hb_stream_t *stream, int idx,
                                      hb_title_t *title, int sort) {
  ...
  if (sort >= 0) {
    ...
    for (i = sort; i < count; i++) {
      ...
      if (pes->stream_id < sid)
        break;
      else if (pes->stream_id <= sid && pes->stream_id_ext <= ssid) {
        break;
      }
    }
...
// Sort specifies the index in the audio list where you would
// like sorted items to begin.
static void pes_add_audio_to_title(hb_stream_t *stream, int idx,
                                   hb_title_t *title, int sort) {
  hb_pes_stream_t *pes = &stream->pes.list[idx];
  ...
  hb_log("stream id 0x%x (type 0x%x substream 0x%x) audio 0x%x", pes->stream_id,
         pes->stream_type, pes->stream_id_ext, audio->id);
  ...
  if (stream->scan) {
    ...
    for (i = 0; i < stream->ts.count; i++) {
      if (ts_stream_kind(stream, i) == V) {
        hb_log("      0x%x type %s (0x%x)%s", stream->ts.list[i].pid,
               stream_type_name2(
                   stream, &stream->pes.list[stream->ts.list[i].pes_list]),
    ...
    hb_log("    Other PIDS : ");
    for (i = 0; i < stream->ts.count; i++) {
      if (ts_stream_kind(stream, i) == N || ts_stream_kind(stream, i) == P) {
        hb_log("      0x%x type %s (0x%x)%s", stream->ts.list[i].pid,
               stream_type_name2(
                   stream, &stream->pes.list[stream->ts.list[i].pes_list]),
               ts_stream_type(stream, i),
  ...
  if (stream->scan) {
    ...
    for (i = 0; i < stream->pes.count; i++) {
      if (stream->pes.list[i].stream_kind == V) {
        hb_log("      0x%x-0x%x type %s (0x%x)", stream->pes.list[i].stream_id,
               stream->pes.list[i].stream_id_ext,
               stream_type_name2(stream, &stream->pes.list[i]),
...
static inline void bits_init(bitbuf_t *bb, uint8_t *buf, int bufsize,
  ...
  bb->size = bufsize;
  bb->val =
      (bb->buf[0] << 24) | (bb->buf[1] << 16) | (bb->buf[2] << 8) | bb->buf[3];
  if (clear)
...
static inline unsigned int bits_peek(bitbuf_t *bb, int bits) {
  ...
  if (bits < left) {
    ...
    if (bits > 0) {
      int pos = bpos >> 3;
      int bval = (bb->buf[pos] << 24) | (bb->buf[pos + 1] << 16) |
                 (bb->buf[pos + 2] << 8) | bb->buf[pos + 3];
...
static inline unsigned int bits_get(bitbuf_t *bb, int bits) {
  ...
  if (bits < left) {
    ...
    int pos = bb->pos >> 3;
    bb->val = (bb->buf[pos] << 24) | (bb->buf[pos + 1] << 16) |
              (bb->buf[pos + 2] << 8) | bb->buf[pos + 3];
...
static void decode_element_descriptors(hb_stream_t *stream, int pes_idx,
                                       bitbuf_t *bb) {
  int ii;
  ...
  while (bits_bytes_left(bb) > 2) {
    ...
    case 10: // ISO_639_language descriptor
    {
      ...
      stream->pes.list[pes_idx].lang_code = lang_to_code(lang_for_code2(code));
    ...
    case 0x56: // DVB Teletext descriptor
    {
      ...
      strncpy(stream->pes.list[pes_idx].codec_name, "DVB Teletext", 80);
      bits_skip(bb, 8 * len);
    } break;
    ...
    case 0x59: // DVB Subtitleing descriptor
    {
      ...
      stream->pes.list[pes_idx].stream_type = 0x00;
      stream->pes.list[pes_idx].stream_kind = N;
      strncpy(stream->pes.list[pes_idx].codec_name, "DVB Subtitling", 80);
      bits_skip(bb, 8 * len);
    } break;
...
int decode_program_map(hb_stream_t *stream) {
  ...
  while (!done_reading_stream_types) {
    ...
    // the PES headers.
    int pes_idx;
    update_ts_streams(stream, elementary_PID, 0, stream_type, -1, &pes_idx);
    if (pes_idx >= 0)
...
static int build_program_map(const uint8_t *buf, hb_stream_t *stream) {
  ...
  if (!start &&
      (stream->pmt_info.current_continuity_counter + 1 != continuity_counter)) {
    hb_log("build_program_map - Continuity Counter %d out of sequence - "
           "expected %d",
           continuity_counter, stream->pmt_info.current_continuity_counter + 1);
    return 0;
  }
  ...
  if (stream->pmt_info.reading && (amount_to_copy > 0)) {
    stream->pmt_info.tablebuf = realloc(
        stream->pmt_info.tablebuf, stream->pmt_info.tablepos + amount_to_copy);

    memcpy(stream->pmt_info.tablebuf + stream->pmt_info.tablepos,
           buf + 4 + adapt_len + pointer_len, amount_to_copy);
  ...
  if (stream->pmt_info.tablepos > 3) {
    ...
    int length;
    length =
        ((stream->pmt_info.tablebuf[1] << 8) + stream->pmt_info.tablebuf[2]) &
        0xFFF;
...
static int decode_PAT(const uint8_t *buf, hb_stream_t *stream) {
  ...
  if (reading && (184 - adapt_len) > 0) {
    ...
    memcpy(tablebuf + tablepos, buf + 4 + adapt_len + pointer_len,
           184 - adapt_len - pointer_len);
  ...
  if (start && reading) {
    ...
    // while (pos < tablepos)
    {
      ...
      case 0x00: {
        ...
        section_len -= 5; // Already read transport stream ID, version num,
                          // section num, and last section num
        section_len -= 4; // Ignore the CRC
        int curr_pos = 0;
        stream->ts_number_pat_entries = 0;
        while ((curr_pos < section_len) &&
               (stream->ts_number_pat_entries < kMaxNumberPMTStreams)) {
          unsigned int pkt_program_num = bits_get(&bb, 16);
          stream->pat_info[stream->ts_number_pat_entries].program_number =
              pkt_program_num;
          ...
          if (pkt_program_num == 0) {
            ...
            stream->pat_info[stream->ts_number_pat_entries].program_map_PID =
                pkt_program_map_PID;
          }
          curr_pos += 4;
...
static int64_t parse_pes_timestamp(bitbuf_t *bb) {
  ...
  ts = ((uint64_t)bits_get(bb, 3) << 30) + bits_skip(bb, 1) +
       (bits_get(bb, 15) << 15) + bits_skip(bb, 1) + bits_get(bb, 15);
  bits_skip(bb, 1);
  return ts;
}

static int parse_pes_header(hb_stream_t *stream, bitbuf_t *bb,
                            hb_pes_info_t *pes_info) {
  ...
  */
  ...
  if (mark == 0x02) {
    ...
    */
    ...
    if (has_ext) {
      ...
      expect = (has_private)*16 + has_pack + has_counter * 2 + has_pstd * 2 +
               has_ext2 * 2;
  ...
  if (pes_info->stream_id == 0xbd && stream->hb_stream_type == program) {
    ...
    if ((ssid >= 0xa0 && ssid <= 0xaf) || (ssid >= 0x20 && ssid <= 0x2f)) {
...
static int parse_pack_header(hb_stream_t *stream, bitbuf_t *bb,
                             hb_pes_info_t *pes_info) {
...
static int hb_parse_ps(hb_stream_t *stream, uint8_t *buf, int len,
  ...
  } else if (pes_info->stream_id >= 0xbd && pes_info->stream_id != 0xbe &&
             pes_info->stream_id != 0xbf && pes_info->stream_id != 0xf0 &&
             pes_info->stream_id != 0xf1 && pes_info->stream_id != 0xf2 &&
             pes_info->stream_id != 0xf8 && pes_info->stream_id != 0xff) {
  ...
  if (stream_id == 0xba) {
    ...
    if (mark != 0x02) {
      ...
      if (fread(cp + pos, 1, 2, stream->file_handle) == 2) {
        ...
        pos += 2;
        if (len > 0 && fread(cp + pos, 1, len, stream->file_handle) == len)
          pos += len;
        else
          goto done;
      }
  ...
  // sections to avoid mis-detection of the next pack or pes start code
    ...
    if (len) {
      ...
      while ((c = getc_unlocked(stream->file_handle)) != EOF) {
        ...
        cp[pos++] = c;
        if ((start_code >> 8) == 0x000001 && (start_code & 0xff) >= 0xb9) {
          // we found the start of the next start
          break;
        }
      }
...
static hb_buffer_t *hb_ps_stream_decode(hb_stream_t *stream) {
  ...
  while (1) {
    ...
    if (stream->need_keyframe) {
      ...
      if (buf->s.type != VIDEO_BUF || !isIframe(stream, buf->data, buf->size)) {
...
static int update_ts_streams(hb_stream_t *stream, int pid, int stream_id_ext,
                             int stream_type, int in_kind, int *out_pes_idx) {
  int ii;
  int pes_idx =
      update_ps_streams(stream, pid, stream_id_ext, stream_type, in_kind);
...
static int decode_ps_map(hb_stream_t *stream, uint8_t *buf, int len) {
  ...
  while (bits_bytes_left(&bb) >= 8) {
    ...
    pes_idx =
        update_ps_streams(stream, stream_id, substream_id, stream_type, -1);
    if (pes_idx >= 0)
      stream->pes.list[pes_idx].map_idx = ii;
  ...
  for (ii = 0; ii < 2; ii++) {
    for (jj = 0; jj < MAX_PS_PROBE_SIZE; jj += buf->size) {
      ...
      if (pes_info.stream_id == 0xba) {
        ...
        // MPeg audio (c0 - df)
        stream_type = 0x04;
        update_ps_streams(stream, pes_info.stream_id, pes_info.stream_id_ext,
                          stream_type, -1);
        ...
        if (ssid >= 0x20 && ssid <= 0x37) {
          ...
          strncpy(stream->pes.list[idx].codec_name, "DVD Subtitle", 80);
          continue;
        }
        ...
        update_ps_streams(stream, pes_info.stream_id, pes_info.bd_substream_id,
                          stream_type, -1);
        ...
        if (pes_info.stream_id_ext == 0x55 || pes_info.stream_id_ext == 0x56) {
...
static int probe_dts_profile(hb_stream_t *stream, hb_pes_stream_t *pes) {
  ...
  if (ret < 0) {
    hb_log("probe_dts_profile: no info type %d/0x%x for id 0x%x", pes->codec,
           pes->codec_param, pes->stream_id);
  }
  switch (info.profile) {
  case FF_PROFILE_DTS:
...
static int do_deep_probe(hb_stream_t *stream, hb_pes_stream_t *pes) {
  int result = 0;
  AVCodec *codec = avcodec_find_decoder(pes->codec_param);
  ...
  while (pos < pes->probe_buf->size) {
    ...
    default:
      hb_error("do_deep_probe: unexpected codec_id (%d)", context->codec_id);
      result = -1;
      break;
    }
  }
...
static int do_probe(hb_stream_t *stream, hb_pes_stream_t *pes,
                    hb_buffer_t *buf) {
  int result = 0;
...
static void hb_ts_resolve_pid_types(hb_stream_t *stream) {
  ...
  for (ii = 0; ii < stream->ts.count; ii++) {
    ...
    if (stype == 0x80 && stream->reg_desc == STR4_TO_UINT32("HDMV")) {
    ...
    if (stype == 0x83 && stream->reg_desc == STR4_TO_UINT32("HDMV")) {
      // This is an interleaved TrueHD/AC-3 stream and the esid of
      // the AC-3 is 0x76
      update_ts_streams(stream, pid, HB_SUBSTREAM_BD_AC3, stype, A, &pes_idx);
      stream->pes.list[pes_idx].codec = HB_ACODEC_AC3;
      stream->pes.list[pes_idx].codec_param = AV_CODEC_ID_AC3;

      update_ts_streams(stream, pid, HB_SUBSTREAM_BD_TRUEHD, stype, A,
                        &pes_idx);
    ...
    if (stype == 0xa2 && stream->reg_desc == STR4_TO_UINT32("HDMV")) {
    ...
    if (stype == 0x85 && stream->reg_desc == STR4_TO_UINT32("HDMV")) {
      ...
      update_ts_streams(stream, pid, HB_SUBSTREAM_BD_DTS, stype, A, &pes_idx);
    ...
    if (stype == 0x86 && stream->reg_desc == STR4_TO_UINT32("HDMV")) {
    ...
    if (stype != 0 &&
        (ts_stream_kind(stream, ii) == A || ts_stream_kind(stream, ii) == S ||
  ...
  while (probe && (buf = hb_ts_stream_decode(stream)) != NULL) {
    ...
    if (result < 0) {
      ...
      hb_log("    Probe: Found stream %s. stream id 0x%x-0x%x", pes->codec_name,
             pes->stream_id, pes->stream_id_ext);
      probe--;
    }
    hb_buffer_close(&buf);
  }
...
static void hb_ps_resolve_stream_types(hb_stream_t *stream) {
  ...
  for (ii = 0; ii < stream->pes.count; ii++) {
    ...
    // stype == 0 indicates a type not in st2codec table
    if (stype != 0 && (stream->pes.list[ii].stream_kind == A ||
...
static int hb_ts_stream_find_pids(hb_stream_t *stream) {
  ...
  // Read the Transport Stream Packets (188 bytes each) looking at first for PID
  // 0 (the PAT PID), then decode that to find the program map PID and then
  // decode that to get the list of audio and video PIDs
  ...
  for (;;) {
    ...
    int pat_index = 0;
    for (pat_index = 0; pat_index < stream->ts_number_pat_entries;
         pat_index++) {
...
static int64_t pes_timestamp(const uint8_t *buf) {
  ...
  ts = ((uint64_t)(buf[0] & 0x0e) << 29) + (buf[1] << 22) +
       ((buf[2] >> 1) << 15) + (buf[3] << 7) + (buf[4] >> 1);
  return ts;
}
...
static void hb_ts_stream_append_pkt(hb_stream_t *stream, int idx,
  ...
  memcpy(stream->ts.list[idx].buf->data + stream->ts.list[idx].buf->size, buf,
         len);
  stream->ts.list[idx].buf->size += len;
...
hb_buffer_t *hb_ts_decode_pkt(hb_stream_t *stream, const uint8_t *pkt,
  ...
  if (adapt_len > 0) {
    ...
    if (adapt_len > 7 && (pkt[5] & 0x10) != 0 &&
      ...
      int64_t pcr;
      pcr = ((uint64_t)pkt[6] << (33 - 8)) | ((uint64_t)pkt[7] << (33 - 16)) |
            ((uint64_t)pkt[8] << (33 - 24)) | ((uint64_t)pkt[9] << (33 - 32)) |
            (pkt[10] >> 7);
  ...
  if ((adaption & 0x01) != 0) {
    ...
    if (!start && (ts_stream->continuity != -1) && !ts_stream->skipbad &&
      ...
      ts_warn(stream, "continuity error: got %d expected %d", (int)continuity,
    ...
    if (adapt_len + 4 + 6 + 9 <= 188) {
      memcpy(&ts_stream->pkt_summary[2], pkt + 4 + adapt_len + 9, 6);
    } else {
  ...
  if (ts_stream->pes_info_valid && ts_stream->pes_info.packet_len > 0 &&
      ts_stream->packet_len >= ts_stream->pes_info.packet_len + 6) {
    buf = generate_output_data(stream, curstream);
    hb_buffer_list_append(&list, buf);
  }
...
static int ffmpeg_parse_vobsub_extradata_mkv(AVCodecParameters *codecpar,
  ...
  for (curLine = strtok_r(lines, "\n", &curLine_parserData); curLine;
    ...
    if (!gotPalette) {
      int numElementsRead =
          sscanf(curLine,
                 ...
                 "%06x, %06x, %06x, %06x, "
                 "%06x, %06x, %06x, %06x",
                 &rgb[0], &rgb[1], &rgb[2], &rgb[3], &rgb[4], &rgb[5], &rgb[6],
                 &rgb[7], &rgb[8], &rgb[9], &rgb[10], &rgb[11], &rgb[12],
                 &rgb[13], &rgb[14], &rgb[15]);
    ...
    if (!gotDimensions) {
      int numElementsRead =
          sscanf(curLine, "size: %dx%d", &subtitle->width, &subtitle->height);
...
static int ffmpeg_parse_vobsub_extradata_mp4(AVCodecParameters *codecpar,
  ...
  for (i = 0, j = 0; i < 16; i++, j += 4) {
    subtitle->palette[i] = codecpar->extradata[j + 1] << 16 | // Y
...
/*
 * Parses the 'subtitle->palette' information from the specific VOB subtitle
 * track's private data. Returns 0 if successful or 1 if parsing failed or was
 * incomplete.
 */
static int ffmpeg_parse_vobsub_extradata(AVCodecParameters *codecpar,
                                         hb_subtitle_t *subtitle) {
  // XXX: Better if we actually chose the correct parser based on the input
  // container
  return ffmpeg_parse_vobsub_extradata_mkv(codecpar, subtitle) &&
...
static void add_ffmpeg_subtitle(hb_title_t *title, hb_stream_t *stream,
  ...
  case AV_CODEC_ID_DVD_SUBTITLE:
    subtitle->format = PICTURESUB;
    subtitle->source = VOBSUB;
    subtitle->config.dest =
        RENDERSUB; // By default render (burn-in) the VOBSUB.
  ...
  if (codecpar->extradata != NULL) {
    ...
    memcpy(subtitle->extradata, codecpar->extradata, codecpar->extradata_size);
...
static void add_ffmpeg_attachment(hb_title_t *title, hb_stream_t *stream,
                                  int id) {
  AVStream *st = stream->ffmpeg_ic->streams[id];
  AVCodecParameters *codecpar = st->codecpar;
...
static hb_title_t *ffmpeg_title_scan(hb_stream_t *stream, hb_title_t *title) {
  ...
  hb_deep_log(2, "Found ffmpeg %d chapters, container=%s", ic->nb_chapters,
              ic->iformat->name);
    ...
    for (i = 0; i < ic->nb_chapters; i++)
      if ((m = ic->chapters[i]) != NULL) {
        ...
        if (i + 1 < ic->nb_chapters) {
          end = ic->chapters[i + 1]->start * 90000 * m->time_base.num /
                m->time_base.den;
        } else {
        ...
        hb_deep_log(
            2, "Added chapter %i, name='%s', dur=%" PRIu64 ", (%02i:%02i:%02i)",
...
static int ffmpeg_is_keyframe(hb_stream_t *stream) {
  ...
  switch (
      stream->ffmpeg_ic->streams[stream->ffmpeg_video_id]->codecpar->codec_id) {
  case AV_CODEC_ID_VC1:
  ...
  case AV_CODEC_ID_WMV3:
    ...
    // header to get that.
    pkt = stream->ffmpeg_pkt.data;
    uint8_t *seqhdr = stream->ffmpeg_ic->streams[stream->ffmpeg_video_id]
                          ->codecpar->extradata;
    int pshift = 2;
    if ((seqhdr[3] & 0x02) == 0)
      // no FINTERPFLAG
...
again:
  ...
  if (stream->ffmpeg_pkt.size <= 0) {
    ...
    int size;
    palette = av_packet_get_side_data(&stream->ffmpeg_pkt, AV_PKT_DATA_PALETTE,
                                      &size);
    if (palette != NULL) {
  ...
  if (stream->ffmpeg_pkt.stream_index == stream->ffmpeg_video_id &&
    ...
    hb_chapter_t *chapter =
        hb_list_item(stream->title->list_chapter, stream->chapter);
    if (chapter != NULL) {
      stream->chapter++;
