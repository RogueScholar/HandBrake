...
static int decsubWork(hb_work_object_t *w, hb_buffer_t **buf_in,
  ...
  if (!pv->job->indepth_scan && w->subtitle->config.dest == PASSTHRUSUB &&
  ...
  do {
    int usedBytes =
        avcodec_decode_subtitle2(pv->context, &subtitle, &has_subtitle, &avp);
    if (usedBytes < 0) {
     ...
     *   3. the sub is non-empty or we've seen one such sub before
     * (!pv->discard_subtitle) For forced-only extraction, usable subtitles also
     * need to: a. be forced (subtitle.rects[0]->flags &
     * AV_SUBTITLE_FLAG_FORCED) OR
    ...
    if (has_subtitle) {
      ...
      if (!pv->job->indepth_scan) {
        ...
        useable_sub =
            (!pv->discard_subtitle &&
             (!w->subtitle->config.force || forced_sub || pv->seen_forced_sub));
        // do we need to create an empty subtitle?
        if (w->subtitle->config.force && useable_sub && !forced_sub &&
            !clear_subtitle) {
    ...
    if (useable_sub) {
      ...
      if (pts < pv->last_pts) {
        ...
        hb_log(
            "decpgssub: track %d, non-monotically increasing PTS, last %" PRId64
            " current %" PRId64 "",
            w->subtitle->out_track, pv->last_pts, pts);
...
hb_work_object_t hb_decpgssub = {WORK_DECPGSSUB, "PGS decoder", decsubInit,
                                 decsubWork, decsubClose};
