...
hb_work_object_t hb_encx265 = {
    WORK_ENCX265, "H.265/HEVC encoder (libx265)", encx265Init, encx265Work,
    encx265Close,
};
...
int apply_h265_level(hb_work_private_t *pv, x265_param *param,
  ...
  if (h265_level == NULL || !strcasecmp(h265_level, hb_h265_level_names[0])) {
    return 0;
  }
  ...
  for (i = 1; hb_h265_level_values[i]; i++) {
    if (!strcmp(hb_h265_level_names[i], h265_level) ||
      ...
      return param_parse(pv, param, "level-idc", hb_h265_level_names2[i]);
    }
  }
...
int encx265Init(hb_work_object_t *w, hb_job_t *job) {
  ...
  if (pv->api->param_default_preset(param, job->encoder_preset,
                                    job->encoder_tune) < 0) {
    hb_error("encx265: x265_param_default_preset failed. Preset (%s) Tune (%s)",
             job->encoder_preset, job->encoder_tune);
    goto fail;
  }
  ...
  param->keyframeMin = (double)job->orig_vrate.num / job->orig_vrate.den + 0.5;
  ...
  hb_dict_iter_t iter;
  for (iter = hb_dict_iter_init(x265_opts); iter != HB_DICT_ITER_DONE;
  ...
  job->areBframes =
      (param->bframes > 0) + (param->bframes > 0 && param->bBPyramid > 0);
...
static hb_buffer_t *nal_encode(hb_work_object_t *w, x265_picture *pic_out,
...
static hb_buffer_t *x265_encode(hb_work_object_t *w, hb_buffer_t *in) {
  ...
  if (pv->last_stop != AV_NOPTS_VALUE && pv->last_stop != in->s.start) {
    hb_log("encx265 input continuity err: last stop %" PRId64
           "  start %" PRId64,
