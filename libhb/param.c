...
static hb_filter_param_t nlmeans_presets[] = {
    {1, "Custom", "custom", NULL}, {5, "Ultralight", "ultralight", NULL},
    {2, "Light", "light", NULL},   {3, "Medium", "medium", NULL},
    {4, "Strong", "strong", NULL}, {0, NULL, NULL, NULL}};

static hb_filter_param_t nlmeans_tunes[] = {
    {0, "None", "none", NULL},           {1, "Film", "film", NULL},
    {2, "Grain", "grain", NULL},         {3, "High Motion", "highmotion", NULL},
    {4, "Animation", "animation", NULL}, {5, "Tape", "tape", NULL},
    {6, "Sprite", "sprite", NULL},       {0, NULL, NULL, NULL}};
...
static hb_filter_param_t chroma_smooth_tunes[] = {
    {0, "None", "none", NULL},   {1, "Tiny", "tiny", NULL},
    {2, "Small", "small", NULL}, {3, "Medium", "medium", NULL},
    {4, "Wide", "wide", NULL},   {5, "Very Wide", "verywide", NULL},
    {0, NULL, NULL, NULL}};
...
static hb_filter_param_t unsharp_tunes[] = {
    {0, "None", "none", NULL},     {1, "Ultrafine", "ultrafine", NULL},
    {2, "Fine", "fine", NULL},     {3, "Medium", "medium", NULL},
    {4, "Coarse", "coarse", NULL}, {5, "Very Coarse", "verycoarse", NULL},
    {0, NULL, NULL, NULL}};
...
static hb_filter_param_t lapsharp_tunes[] = {
    {0, "None", "none", NULL},     {1, "Film", "film", NULL},
    {2, "Grain", "grain", NULL},   {3, "Animation", "animation", NULL},
    {4, "Sprite", "sprite", NULL}, {0, NULL, NULL, NULL}};
...
static hb_dict_t *generate_nlmeans_settings(const char *preset,
  ...
  if (!strcasecmp(preset, "ultralight") || !strcasecmp(preset, "light") ||
      !strcasecmp(preset, "medium") || !strcasecmp(preset, "strong")) {
    double strength[2], origin_tune[2];
    int patch_size[2], range[2], frames[2], prefilter[2];
...
static hb_dict_t *generate_chroma_smooth_settings(const char *preset,
  ...
  if (!strcasecmp(preset, "ultralight") || !strcasecmp(preset, "light") ||
      !strcasecmp(preset, "medium") || !strcasecmp(preset, "strong") ||
      !strcasecmp(preset, "stronger") || !strcasecmp(preset, "verystrong")) {
    double strength;
    int size;

    // Strength
    if ((tune == NULL || !strcasecmp(tune, "none")) ||
        (!strcasecmp(tune, "tiny")) || (!strcasecmp(tune, "small")) ||
        (!strcasecmp(tune, "medium")) || (!strcasecmp(tune, "wide")) ||
    ...
    if (tune != NULL) {
      fprintf(stderr,
              "Custom chroma smooth parameters specified; ignoring chroma "
              "smooth tune (%s).\n",
              tune);
    }
  }
...
static hb_dict_t *generate_unsharp_settings(const char *preset,
  ...
  if (!strcasecmp(preset, "ultralight") || !strcasecmp(preset, "light") ||
    ...
    if (tune != NULL) {
      fprintf(
          stderr,
          "Custom unsharp parameters specified; ignoring unsharp tune (%s).\n",
          tune);
    }
  }
...
static hb_dict_t *generate_lapsharp_settings(const char *preset,
  ...
  if (!strcasecmp(preset, "ultralight") || !strcasecmp(preset, "light") ||
    ...
    if (tune != NULL) {
      fprintf(stderr,
              "Custom lapsharp parameters specified; ignoring lapsharp tune "
              "(%s).\n",
              tune);
    }
  }
...
int hb_validate_filter_settings(int filter_id, const hb_dict_t *settings) {
  ...
  for (iter = hb_dict_iter_init(settings); iter != HB_DICT_ITER_DONE;
    ...
    if (regex_pattern != NULL && regex_pattern[0] != 0) {
      ...
      if (hb_validate_param_string(regex_pattern, param) != 0) {
        hb_log("Invalid filter value (%s) for key %s filter %s", param, key,
               filter->name);
        free(param);
        return 1;
      }
...
static hb_filter_param_t *filter_param_get_presets_internal(int filter_id,
                                                            int *count) {
  int ii;
...
static hb_filter_param_t *filter_param_get_tunes_internal(int filter_id,
                                                          int *count) {
  int ii;
...
static hb_filter_param_t *filter_param_get_entry(hb_filter_param_t *table,
...
static hb_value_t *generate_generic_settings(int filter_id, const char *preset,
                                             const char *tune,
                                             const char *custom) {
...
hb_value_t *hb_generate_filter_settings(int filter_id, const char *preset,
  ...
  case HB_FILTER_DEINTERLACE:
    settings = generate_generic_settings(filter_id, preset, tune, custom);
    break;
  default:
