 ...
 *
 * These values are used to limit the range of edge direction searches and
 * filtering.
 */
 ...
 * @param ptr The aligned pointer, created with eedi2_aligned_malloc, to be
 * freed
 */
 ...
 *
 * When row_size, dst_pitch, and src_pitch are equal, eedi2_bit_blit can work
 * more quickly by copying the whole plane at once instead of individual lines.
 */
void eedi2_bit_blit(uint8_t *dstp, int dst_pitch, const uint8_t *srcp,
...
/**
 * A specialized variant of bit_blit, just for setting up the initial,
 * field-sized bitmap planes that EEDI2 interpolates from.
 ...
 */
void eedi2_fill_half_height_buffer_plane(uint8_t *src, uint8_t *dst, int pitch,
                                         int height) {
...
/**
 * A specialized variant of bit_blit, just for resizing the field-height maps
 * EEDI2 generates to frame-height...a simple line doubler
...
/**
 * Finds places where vertically adjacent pixels abruptly change in intensity,
 * i.e., sharp edges.
 ...
 * @param mtresh Magnitude threshold, ensures it doesn't mark edges on pixels
 * that are too similar (10 is a good default value)
 * @param vthresh Variance threshold, ensures it doesn't look for edges in
 * highly random pixel blocks (20 is a good default value)
 * @param lthresh Laplacian threshold, ensures edges are still prominent in the
 * 2nd spatial derivative of the srcp plane (20 is a good default value)
 * @param height Height of half-height single-field frame
 * @param width Width of srcp bitmap rows, as opposed to the padded stride in
 * src_pitch
 */
void eedi2_build_edge_mask(uint8_t *dstp, int dst_pitch, uint8_t *srcp,
                           int src_pitch, int mthresh, int lthresh, int vthresh,
  ...
  for (y = 1; y < height - 1; ++y) {
    for (x = 1; x < width - 1; ++x) {
      ...
      const int sum = srcpp[x - 1] + srcpp[x] + srcpp[x + 1] + srcp[x - 1] +
                      srcp[x] + srcp[x + 1] + srcpn[x - 1] + srcpn[x] +
                      srcpn[x + 1];

      const int sumsq = srcpp[x - 1] * srcpp[x - 1] + srcpp[x] * srcpp[x] +
                        srcpp[x + 1] * srcpp[x + 1] +
                        srcp[x - 1] * srcp[x - 1] + srcp[x] * srcp[x] +
                        srcp[x + 1] * srcp[x + 1] +
                        srcpn[x - 1] * srcpn[x - 1] + srcpn[x] * srcpn[x] +
                        srcpn[x + 1] * srcpn[x + 1];
      ...
      const int Iy = MAX(MAX(abs(srcpp[x] - srcpn[x]), abs(srcpp[x] - srcp[x])),
                         abs(srcp[x] - srcpn[x]));
 ...
 * @param dst_pitch Stride of dstp
 * @param dstr Dilation threshold, ensures a pixel is only retained as an edge
 * in dstp if this number of adjacent pixels or greater are also edges in mskp
 * (4 is a good default value)
 * @param height Height of half-height field-sized frame
 * @param width Width of mskp bitmap rows, as opposed to the pdded stride in
 * msk_pitch
 */
void eedi2_dilate_edge_mask(uint8_t *mskp, int msk_pitch, uint8_t *dstp,
                            int dst_pitch, int dstr, int height, int width) {
  int x, y;
 ...
 * @param dst_pitch Stride of dstp
 * @param estr Erosion threshold, ensures a pixel isn't retained as an edge in
 * dstp if fewer than this number of adjacent pixels are also edges in mskp (2
 * is a good default value)
 * @param height Height of half-height field-sized frame
 * @param width Width of mskp bitmap rows, as opposed to the pdded stride in
 * msk_pitch
 */
void eedi2_erode_edge_mask(uint8_t *mskp, int msk_pitch, uint8_t *dstp,
                           int dst_pitch, int estr, int height, int width) {
  int x, y;
 ...
 *
 * If none of the 6 horizontally adjacent pixels are edges, mark the current
 * pixel as not edged. If at least 1 of the 3 on either side are edges, mark the
 * current pixel as an edge.
 *
 * @param mskp Pointer to the source edge mask being read from
 * @param msk_pitch Stride of mskp
 ...
 * msk_pitch
 */
void eedi2_remove_small_gaps(uint8_t *mskp, int msk_pitch, uint8_t *dstp,
...
/**
 * Calculates spatial direction vectors for the edges. This is EEDI2's timesink,
 * and can be thought of as YADIF_CHECK on steroids, as both try to discern
 * which angle a given edge follows
 * @param plane The plane of the image being processed, to know to reduce maxd
 * for chroma planes (HandBrake only works with YUV420 video so it is assumed
 * they are half-height)
 * @param mskp Pointer to the source edge mask being read from
 * @param msk_pitch Stride of mskp
 ...
 * @param height Height of half-height field-sized frame
 * @param width Width of srcp bitmap rows, as opposed to the pdded stride in
 * src_pitch
 */
void eedi2_calc_directions(const int plane, uint8_t *mskp, int msk_pitch,
                           uint8_t *srcp, int src_pitch, uint8_t *dstp,
                           int dst_pitch, int maxd, int nt, int height,
                           int width) {
  int x, y, u, i;
  ...
  for (y = 1; y < height - 1; ++y) {
    for (x = 1; x < width - 1; ++x) {
      ...
      int minb =
          MIN(13 * nt, (abs(srcp[x] - srcpn[x]) + abs(srcp[x] - srcpp[x])) * 6);
      int mina =
          MIN(19 * nt, (abs(srcp[x] - srcpn[x]) + abs(srcp[x] - srcpp[x])) * 9);
      int minc = mina;
      int mind = minb;
      int mine = minb;
      int dira = -5000, dirb = -5000, dirc = -5000, dird = -5000, dire = -5000;
      for (u = startu; u <= stopu; ++u) {
        if (y == 1 || mskpp[x - 1 + u] == 0xFF || mskpp[x + u] == 0xFF ||
            mskpp[x + 1 + u] == 0xFF) {
          if (y == height - 2 || mskpn[x - 1 - u] == 0xFF ||
              mskpn[x - u] == 0xFF || mskpn[x + 1 - u] == 0xFF) {
 ...
 * msk_pitch
 */
void eedi2_filter_map(uint8_t *mskp, int msk_pitch, uint8_t *dmskp,
                      int dmsk_pitch, uint8_t *dstp, int dst_pitch, int height,
                      int width) {
  int x, y, j;
 ...
 * @param dmsk_pitch Stride of dmskp
 * @param dstp Pointer to the destination to store the filtered edge direction
 * mask
 * @param dst_pitch Stride of dstp
 * @param height Height of half_height field-sized frame
 * @param width Width of dmskp bitmap rows, as opposed to the pdded stride in
 * dmsk_pitch
 */
void eedi2_filter_dir_map(uint8_t *mskp, int msk_pitch, uint8_t *dmskp,
                          int dmsk_pitch, uint8_t *dstp, int dst_pitch,
                          int height, int width) {
  int x, y, i;
  ...
  for (y = 1; y < height - 1; ++y) {
    for (x = 1; x < width - 1; ++x) {
      ...
      const int mid = (u & 1) ? order[u >> 1]
                              : (order[(u - 1) >> 1] + order[u >> 1] + 1) >> 1;
      int sum = 0, count = 0;
 ...
 * @param dmskp Pointer to the edge direction mask being read from
 * @param dmsk_pitch Stride of dmskp
 * @param dstp Pointer to the destination to store the expanded edge direction
 * mask
 * @param dst_pitch Stride of dstp
 * @param height Height of half-height field-sized frame
 * @param width Width of dmskp bitmap rows, as opposed to the pdded stride in
 * dmsk_pitch
 */
void eedi2_expand_dir_map(uint8_t *mskp, int msk_pitch, uint8_t *dmskp,
                          int dmsk_pitch, uint8_t *dstp, int dst_pitch,
                          int height, int width) {
  int x, y, i;
 ...
 * dmsk_pitch
 */
void eedi2_mark_directions_2x(uint8_t *mskp, int msk_pitch, uint8_t *dmskp,
                              int dmsk_pitch, uint8_t *dstp, int dst_pitch,
                              int tff, int height, int width) {
  int x, y, i;
  memset(dstp, 255, dst_pitch * height);
  ...
  for (y = 2 - tff; y < height - 1; y += 2) {
    for (x = 1; x < width - 1; ++x) {
      ...
      if (v < 3)
        ...
        const int mid = (v & 1)
                            ? order[v >> 1]
                            : (order[(v - 1) >> 1] + order[v >> 1] + 1) >> 1;
        const int lim = eedi2_limlut[abs(mid - 128) >> 2];
        int u = 0;
        if (abs(dmskp[x - 1] - dmskpn[x - 1]) <= lim || dmskp[x - 1] == 0xFF ||
            dmskpn[x - 1] == 0xFF)
          ++u;
        if (abs(dmskp[x] - dmskpn[x]) <= lim || dmskp[x] == 0xFF ||
            dmskpn[x] == 0xFF)
          ++u;
        if (abs(dmskp[x + 1] - dmskpn[x - 1]) <= lim || dmskp[x + 1] == 0xFF ||
            dmskpn[x + 1] == 0xFF)
 ...
 * dmsk_pitch
 */
void eedi2_filter_dir_map_2x(uint8_t *mskp, int msk_pitch, uint8_t *dmskp,
                             int dmsk_pitch, uint8_t *dstp, int dst_pitch,
                             int field, int height, int width) {
  int x, y, i;
  eedi2_bit_blit(dstp, dst_pitch, dmskp, dmsk_pitch, width, height);
 ...
 * dmsk_pitch
 */
void eedi2_expand_dir_map_2x(uint8_t *mskp, int msk_pitch, uint8_t *dmskp,
                             int dmsk_pitch, uint8_t *dstp, int dst_pitch,
                             int field, int height, int width) {
  int x, y, i;
  ...
  for (y = 2 - field; y < height - 1; y += 2) {
    for (x = 1; x < width - 1; ++x) {
      if (dmskp[x] != 0xFF || (mskp[x] != 0xFF && mskpn[x] != 0xFF))
        continue;
      int u = 0, order[9];
      if (y > 1) {
        if (dmskpp[x - 1] != 0xFF)
...
/**
 * Like the name suggests, this function fills in gaps in the frame-height edge
 * direction mask
 * @param mskp Pointer to the edge mask
 * @param msk_pitch Stride of mskp
 * @param dmskp Pointer to the edge direction mask being read from
 ...
 * dmsk_pitch
 */
void eedi2_fill_gaps_2x(uint8_t *mskp, int msk_pitch, uint8_t *dmskp,
                        int dmsk_pitch, uint8_t *dstp, int dst_pitch, int field,
                        int height, int width) {
  int x, y, j;
  ...
  for (y = 2 - field; y < height - 1; y += 2) {
    for (x = 1; x < width - 1; ++x) {
      ...
      for (j = u; j <= v; ++j) {
        if (tc) {
          if (y <= 2 || dmskpp[j] == 0xFF ||
              (mskpp[j] != 0xFF && mskp[j] != 0xFF)) {
            tc = 0;
            mint = maxt = 20;
          } else {
            if (dmskpp[j] < mint)
        ...
        if (bc) {
          if (y >= height - 3 || dmskpn[j] == 0xFF ||
              (mskpn[j] != 0xFF && mskpnn[j] != 0xFF)) {
            bc = 0;
            minb = maxb = 20;
          } else {
            if (dmskpn[j] < minb)
      ...
      if (maxb == -20)
        maxb = minb = 20;
      int thresh = MAX(MAX(MAX(abs(forward - 128), abs(back - 128)) >> 2, 8),
 ...
 * @param plane The plane of the image being processed, to know to reduce a
 * search distance for chroma planes (HandBrake only works with YUV420 video so
 * it is assumed they are half-height)
 * @param dmskp Pointer to the edge direction mask being read from
 * @param dmsk_pitch Stride of dmskp
 * @param dstp Pointer to the line-doubled source field used being filtered in
 * place
 * @param dst_pitch Stride of dstp
 * @param omskp Pointer to the destination to store the output edge mask used
 * for post-processing
 * @param osmk_pitch Stride of omskp
 * @param field Field to filter
 * @nt Noise threshold, (50 is a good default value)
 * @param height Height of the full-frame output
 * @param width Width of dstp bitmap rows, as opposed to the pdded stride in
 * dst_pitch
 */
void eedi2_interpolate_lattice(const int plane, uint8_t *dmskp, int dmsk_pitch,
                               uint8_t *dstp, int dst_pitch, uint8_t *omskp,
                               int omsk_pitch, int field, int nt, int height,
                               int width) {
  int x, y, u;

  if (field == 1) {
    eedi2_bit_blit(dstp + (height - 1) * dst_pitch, dst_pitch,
                   dstp + (height - 2) * dst_pitch, dst_pitch, width, 1);
  } else {
    eedi2_bit_blit(dstp, dst_pitch, dstp + dst_pitch, dst_pitch, width, 1);
  }
  ...
  for (y = 2 - field; y < height - 1; y += 2) {
    for (x = 0; x < width; ++x) {
      ...
      if (dir == 255 || (abs(dmskp[x] - dmskp[x - 1]) > lim &&
      ...
      if (lim < 9) {
        const int sum = dstp[x - 1] + dstp[x] + dstp[x + 1] + dstpnn[x - 1] +
                        dstpnn[x] + dstpnn[x + 1];
        const int sumsq = dstp[x - 1] * dstp[x - 1] + dstp[x] * dstp[x] +
                          dstp[x + 1] * dstp[x + 1] +
                          dstpnn[x - 1] * dstpnn[x - 1] +
                          dstpnn[x] * dstpnn[x] + dstpnn[x + 1] * dstpnn[x + 1];
      ...
      int min = 8 * nt;
      for (u = startu; u <= stopu; ++u) {
        const int diff = abs(dstp[x - 1] - dstpnn[x - u - 1]) +
          ...
          if (diff2 < 4 * nt &&
              (((abs(omskp[x + (u >> 1)] - omskn[x - (u >> 1)]) <= lim ||
               ...
               ((abs(omskp[x + ((u + 1) >> 1)] - omskn[x - (u >> 1)]) <= lim ||
                 abs(omskp[x + ((u + 1) >> 1)] - omskn[x - ((u + 1) >> 1)]) <=
                     lim) &&
                omskp[x + ((u + 1) >> 1)] != 0xFF))) {
      ...
      if (min != 8 * nt) {
        ...
        for (u = startu; u <= stopu; ++u) {
          ...
          const int diff = abs(dstp[x - 1] - dstpnn[x - u - 1]) +
                           ...
                           abs(dstpnn[x - 1] - dstp[x + u - 1]) +
                           abs(dstpnn[x] - dstp[x + u]) +
                           abs(dstpnn[x + 1] - dstp[x + u + 1]) + abs(p1 - p2);
          if (diff < min) {
 ...
 * @param nmskp Pointer to the newly-filtered edge direction mask being read
 * from
 * @param nmsk_pitch Stride of nmskp
 * @param omskp Pointer to the old unfiltered edge direction mask being read
 * from
 ...
 * src_pitch
 */
void eedi2_post_process(uint8_t *nmskp, int nmsk_pitch, uint8_t *omskp,
                        int omsk_pitch, uint8_t *dstp, int src_pitch, int field,
                        int height, int width) {
  int x, y;
 ...
 * @param width Width of dstp bitmap rows, as opposed to the padded stride in
 * dst_pitch
 */
void eedi2_gaussian_blur1(uint8_t *src, int src_pitch, uint8_t *tmp,
                          int tmp_pitch, uint8_t *dst, int dst_pitch,
                          int height, int width) {
  uint8_t *srcp = src;
  uint8_t *dstp = tmp;
  int x, y;
  ...
  for (y = 0; y < height; ++y) {
    dstp[0] = (srcp[3] * 582 + srcp[2] * 7078 + srcp[1] * 31724 +
               srcp[0] * 26152 + 32768) >>
              16;
    dstp[1] = (srcp[4] * 582 + srcp[3] * 7078 + (srcp[0] + srcp[2]) * 15862 +
               srcp[1] * 26152 + 32768) >>
              16;
    dstp[2] = (srcp[5] * 582 + (srcp[0] + srcp[4]) * 3539 +
               (srcp[1] + srcp[3]) * 15862 + srcp[2] * 26152 + 32768) >>
              16;
    for (x = 3; x < width - 3; ++x) {
      dstp[x] =
          ((srcp[x - 3] + srcp[x + 3]) * 291 +
           (srcp[x - 2] + srcp[x + 2]) * 3539 +
           (srcp[x - 1] + srcp[x + 1]) * 15862 + srcp[x] * 26152 + 32768) >>
          16;
    }
  ...
  for (x = 0; x < width; ++x) {
    dstp[x] = (src3n[x] * 582 + src2n[x] * 7078 +
               (srcpp[x] + srcpn[x]) * 15862 + srcp[x] * 26152 + 32768) >>
              16;
  }
  src3p += tmp_pitch;
  src2p += tmp_pitch;
  ...
  for (y = 3; y < height - 3; ++y) {
    for (x = 0; x < width; ++x) {
      dstp[x] = ((src3p[x] + src3n[x]) * 291 + (src2p[x] + src2n[x]) * 3539 +
                 (srcpp[x] + srcpn[x]) * 15862 + srcp[x] * 26152 + 32768) >>
                16;
    }
    src3p += tmp_pitch;
 ...
 * @param tmp Pointer to a temporary storage for the derivative array while it's
 * being filtered
 * @param dst Pointer to the destination to store the filtered output derivative
 * array
 * @param pitch Stride of the bitmap from which the src array is derived
 * @param height Height of the half-height field-sized frame from which the src
 * array derivs were taken
 * @param width Width of the bitmap from which the src array is derived, as
 * opposed to the padded stride in pitch
 */
void eedi2_gaussian_blur_sqrt2(int *src, int *tmp, int *dst, const int pitch,
  ...
  for (y = 0; y < height; ++y) {
    ...
    dstp[x] = (srcp[x + 4] * 678 + srcp[x + 3] * 3902 + srcp[x + 2] * 13618 +
               srcp[x + 1] * 28830 + srcp[x] * 18508 + 32768) >>
              16;
    ++x;
    dstp[x] = (srcp[x + 4] * 678 + srcp[x + 3] * 3902 + srcp[x + 2] * 13618 +
               (srcp[x - 1] + srcp[x + 1]) * 14415 + srcp[x] * 18508 + 32768) >>
              16;
    ++x;
  ...
  for (x = 0; x < width; ++x) {
    dstp[x] = (src4n[x] * 678 + src3n[x] * 3902 + src2n[x] * 13618 +
               srcpn[x] * 28830 + srcp[x] * 18508 + 32768) >>
              18;
  }
  src4p += pitch;
  src3p += pitch;
  ...
  for (x = 0; x < width; ++x) {
    dstp[x] = (src4n[x] * 678 + src3n[x] * 3902 + src2n[x] * 13618 +
               (srcpp[x] + srcpn[x]) * 14415 + srcp[x] * 18508 + 32768) >>
              18;
  }
  src4p += pitch;
  src3p += pitch;
  ...
  dstp += pitch;
  for (x = 0; x < width; ++x) {
    dstp[x] = (src4p[x] * 678 + src3p[x] * 3902 + (src2p[x] + src2n[x]) * 6809 +
               (srcpp[x] + srcpn[x]) * 14415 + srcp[x] * 18508 + 32768) >>
              18;
  }
  src4p += pitch;
  ...
  for (x = 0; x < width; ++x) {
    dstp[x] = (src4p[x] * 678 + src3p[x] * 3902 + src2p[x] * 13618 +
               srcpp[x] * 28830 + srcp[x] * 18508 + 32768) >>
              18;
  }
}
 ...
 * @param pitch Stride of the source field plane from which the derivatives were
 * calculated
 ...
 * @param field Field to filter
 */
void eedi2_post_process_corner(int *x2, int *y2, int *xy, const int pitch,
                               uint8_t *mskp, int msk_pitch, uint8_t *dstp,
                               int dst_pitch, int height, int width,
                               int field) {
  ...
  for (y = 8 - field; y < height - 7; y += 2) {
    for (x = 4; x < width - 4; ++x) {
      if (mskp[x] == 255 || mskp[x] == 128)
        continue;
      const int c1 = (int)(x2[x] * y2[x] - xy[x] * xy[x] -
                           0.09 * (x2[x] + y2[x]) * (x2[x] + y2[x]));
      const int c2 = (int)(x2n[x] * y2n[x] - xyn[x] * xyn[x] -
                           0.09 * (x2n[x] + y2n[x]) * (x2n[x] + y2n[x]));
      if (c1 > 775 || c2 > 775)
