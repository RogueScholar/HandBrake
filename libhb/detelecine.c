...
static int hb_detelecine_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                              hb_buffer_t **buf_out);
...
static void
pullup_compute_metric(struct pullup_context *c, struct pullup_field *fa, int pa,
...
static int pullup_find_first_break(struct pullup_field *f, int max) {
  int i;
  for (i = 0; i < max; i++) {
    if (f->breaks & PULLUP_BREAK_RIGHT || f->next->breaks & PULLUP_BREAK_LEFT) {
      return i + 1;
    }
    f = f->next;
  }
...
static void pullup_compute_breaks(struct pullup_context *c,
  ...
  if (f0->flags & PULLUP_HAVE_BREAKS)
    return;
...
static int pullup_decide_frame_length(struct pullup_context *c) {
  ...
  case 2:
    ...
    if (c->strict_pairs && (f0->prev->breaks & PULLUP_BREAK_RIGHT) &&
...
static void pullup_print_aff_and_breaks(struct pullup_context *c,
  ...
  printf("\nbreaks:   ");
  for (i = 0; i < 4; i++) {
    printf("%c%d%c", f->breaks & PULLUP_BREAK_LEFT ? '|' : '.', i,
...
void pullup_init_context(struct pullup_context *c) {
  ...
  c->metric_offset =
      c->junk_left * c->bpp[mp] + (c->junk_top << 1) * c->stride[mp];
...
struct pullup_buffer *pullup_get_buffer(struct pullup_context *c, int parity) {
  ...
  for (i = 0; i < c->nbuffers; i++) {
    if (c->buffers[i].lock[0])
      continue;
    if (c->buffers[i].lock[1])
      continue;
  ...
  for (i = 0; i < c->nbuffers; i++) {
    if (((parity + 1) & 1) && c->buffers[i].lock[0])
      continue;
    if (((parity + 1) & 2) && c->buffers[i].lock[1])
      continue;
    pullup_alloc_buffer(c, &c->buffers[i]);
    return pullup_lock_buffer(&c->buffers[i], parity);
  }
...
void pullup_release_frame(struct pullup_frame *fr) {
  ...
  if (fr->buffer)
    pullup_release_buffer(fr->buffer, 2);
  fr->lock--;
}
...
void pullup_submit_field(struct pullup_context *c, struct pullup_buffer *b,
  ...
  if (c->last && c->last->parity == parity)
    return;
  ...
  pullup_compute_metric(c, f, parity, f->prev->prev, parity, c->diff, f->diffs);
  pullup_compute_metric(c, parity ? f->prev : f, 0, parity ? f : f->prev, 1,
                        c->comb, f->comb);
  pullup_compute_metric(c, f, parity, f, -1, c->var, f->var);
...
static int hb_detelecine_init(hb_filter_object_t *filter,
  ...
  ctx->w[3] =
      ((init->geometry.width + 15) / 16) * ((init->geometry.height + 15) / 16);
  ctx->h[3] = 2;
