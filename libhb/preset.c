...
void hb_preset_index_append(hb_preset_index_t *dst,
  ...
  int ii;
  for (ii = 0; ii < src->depth && dst->depth < HB_MAX_PRESET_FOLDER_DEPTH;
       ii++, dst->depth++) {
...
static int get_job_mux(hb_dict_t *job_dict) {
  ...
  if (hb_value_type(mux_value) == HB_VALUE_TYPE_STRING) {
    ...
    if (mux == 0)
      mux = hb_container_get_from_extension(hb_value_get_string(mux_value));
  } else {
...
static int find_audio_track(const hb_title_t *title, const char *lang,
                            int start, int behavior) {
  ...
  for (ii = start; ii < count; ii++) {
    ...
    if ((behavior == 2 || audio->lang.attributes == HB_AUDIO_ATTR_NONE ||
...
static int validate_audio_encoders(const hb_dict_t *preset) {
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    if (hb_value_type(value) == HB_VALUE_TYPE_STRING) {
      ...
      if (!strcasecmp(str, "source") || !strcasecmp(str, "auto") ||
...
static int sanitize_audio_codec(int in_codec, int out_codec, int copy_mask,
                                int fallback, int mux) {
  int codec = out_codec;
...
void hb_sanitize_audio_settings(const hb_title_t *title,
  ...
  quality_enable =
      !(bitrate > 0 || val == NULL || quality == HB_INVALID_AUDIO_QUALITY);
  ...
  if (codec & HB_ACODEC_PASS_FLAG) {
    ...
    if (quality_enable) {
      ...
      if (bitrate != -1) {
        bitrate = hb_audio_bitrate_get_best(codec, bitrate, samplerate, mix);
      } else {
...
static void add_audio_for_lang(hb_value_array_t *list, const hb_dict_t *preset,
                               ...
                               int fallback, const char *lang, int behavior,
                               int mode, hb_dict_t *track_dict) {
  hb_value_array_t *encoder_list = hb_dict_get(preset, "AudioList");
  int count = hb_value_array_len(encoder_list);
  ...
  while (track >= 0) {
    ...
    for (ii = 0; ii < count; ii++) {
      ...
      if (hb_value_type(acodec_value) == HB_VALUE_TYPE_STRING) {
        out_codec =
            hb_audio_encoder_get_from_name(hb_value_get_string(acodec_value));
      } else {
      ...
      out_codec = sanitize_audio_codec(aconfig->in.codec, out_codec, copy_mask,
                                       fallback, mux);
      ...
      hb_dict_set(audio_dict, "Encoder",
                  hb_value_string(hb_audio_encoder_get_short_name(out_codec)));
      ...
      if (!(out_codec & HB_ACODEC_PASS_FLAG)) {
        if (hb_dict_get(encoder_dict, "AudioTrackGainSlider") != NULL) {
          hb_dict_set(
              audio_dict, "Gain",
              hb_value_dup(hb_dict_get(encoder_dict, "AudioTrackGainSlider")));
        }
        if (hb_dict_get(encoder_dict, "AudioTrackDRCSlider") != NULL) {
          hb_dict_set(
              audio_dict, "DRC",
              hb_value_dup(hb_dict_get(encoder_dict, "AudioTrackDRCSlider")));
        }
        if (hb_dict_get(encoder_dict, "AudioMixdown") != NULL) {
          hb_dict_set(audio_dict, "Mixdown",
                      hb_value_dup(hb_dict_get(encoder_dict, "AudioMixdown")));
        }
        ...
        if (hb_dict_get(encoder_dict, "AudioDitherMethod") != NULL) {
          hb_dict_set(
              audio_dict, "DitherMethod",
              hb_value_dup(hb_dict_get(encoder_dict, "AudioDitherMethod")));
        }
        if (hb_dict_get(encoder_dict, "AudioSamplerate") != NULL) {
          const char *sr_name;
          int sr;

          sr_name = hb_dict_get_string(encoder_dict, "AudioSamplerate");
        ...
        if (hb_dict_get(encoder_dict, "AudioCompressionLevel") != NULL) {
          hb_dict_set(
              audio_dict, "CompressionLevel",
              hb_value_dup(hb_dict_get(encoder_dict, "AudioCompressionLevel")));
        }
        if (hb_value_get_bool(
                hb_dict_get(encoder_dict, "AudioTrackQualityEnable"))) {
          hb_dict_set(
              audio_dict, "Quality",
              hb_value_xform(hb_dict_get(encoder_dict, "AudioTrackQuality"),
                             HB_VALUE_TYPE_DOUBLE));
        } else {
          hb_dict_set(audio_dict, "Bitrate",
                      hb_value_xform(hb_dict_get(encoder_dict, "AudioBitrate"),
                                     HB_VALUE_TYPE_INT));
        }
      }
...
int hb_preset_job_add_audio(hb_handle_t *h, int title_index,
  ...
  if (fallback_value != NULL) {
    hb_dict_set(audio_dict, "FallbackEncoder", hb_value_dup(fallback_value));
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    add_audio_for_lang(list, preset, title, mux, copy_mask, fallback, lang,
                       behavior, mode, track_dict);
  }
  // If AudioLanguageList is empty, try "any" language option
  if (count <= 0) {
    add_audio_for_lang(list, preset, title, mux, copy_mask, fallback, "any",
                       behavior, mode, track_dict);
  }
...
// Find a source audio track matching given language
static int find_subtitle_track(const hb_title_t *title, const char *lang,
                               int start) {
...
static void add_subtitle(hb_value_array_t *list, int track, int make_default,
                         int force, int burn, const char *name) {
...
static void add_subtitle_for_lang(hb_value_array_t *list, hb_title_t *title,
  ...
  int t;
  for (t = find_subtitle_track(title, lang, 0); t >= 0;
    ...
    // default.
    make_default =
        (!burn && behavior->make_default) ||
        (!has_default_subtitle(list) && subtitle->config.default_track);
...
int hb_preset_job_add_subtitles(hb_handle_t *h, int title_index,
  ...
  const char *s;
  s = hb_value_get_string(
      hb_dict_get(preset, "SubtitleTrackSelectionBehavior"));
  if (s != NULL) {
  ...
  behavior.burn_dvd =
      hb_value_get_int(hb_dict_get(preset, "SubtitleBurnDVDSub"));
  behavior.burn_bd = hb_value_get_int(hb_dict_get(preset, "SubtitleBurnBDSub"));
  ...
  foreign_audio_search =
      hb_value_get_bool(hb_dict_get(preset, "SubtitleAddForeignAudioSearch"));
  foreign_first_audio =
      hb_value_get_bool(hb_dict_get(preset, "SubtitleAddForeignAudioSubtitle"));
  ...
  int track;
  if (first_audio_lang != NULL && foreign_first_audio &&
      strncmp(first_audio_lang, pref_lang, 4)) {
  ...
  if (first_audio_lang != NULL && foreign_audio_search &&
      !strncmp(first_audio_lang, pref_lang, 4)) {
...
static int get_video_framerate(hb_value_t *rate_value) {
  ...
  if (hb_value_type(rate_value) == HB_VALUE_TYPE_STRING) {
    ...
    if (!strcasecmp(rate_name, "source") || !strcasecmp(rate_name, "auto") ||
  ...
  // Arbitrary
  int clock_min, clock_max, clock, frame_min, frame_max;
...
int hb_preset_apply_filters(const hb_dict_t *preset, hb_dict_t *job_dict) {
  ...
  if (detel_val != NULL) {
    const char *custom;
    custom =
        hb_value_get_string(hb_dict_get(preset, "PictureDetelecineCustom"));
  ...
  comb_preset =
      hb_value_get_string(hb_dict_get(preset, "PictureCombDetectPreset"));
  if (comb_preset != NULL) {
    const char *comb_custom;
    comb_custom =
        hb_value_get_string(hb_dict_get(preset, "PictureCombDetectCustom"));
    filter_settings = hb_generate_filter_settings(
        HB_FILTER_COMB_DETECT, comb_preset, NULL, comb_custom);
    if (filter_settings == NULL) {
  ...
  deint_filter =
      hb_value_get_string(hb_dict_get(preset, "PictureDeinterlaceFilter"));
  deint_preset =
      hb_value_get_string(hb_dict_get(preset, "PictureDeinterlacePreset"));
  deint_custom =
      hb_value_get_string(hb_dict_get(preset, "PictureDeinterlaceCustom"));
  if (deint_filter != NULL && deint_preset != NULL &&
    ...
    filter_settings = hb_generate_filter_settings(filter_id, deint_preset, NULL,
                                                  deint_custom);
    if (filter_settings == NULL) {
  ...
  denoise =
      hb_value_type(denoise_value) == HB_VALUE_TYPE_STRING
          ? (!strcasecmp(hb_value_get_string(denoise_value), "off")
                 ? 0
                 : !strcasecmp(hb_value_get_string(denoise_value), "nlmeans")
                       ? 1
                       : 2)
  ...
  if (denoise != 0) {
    ...
    denoise_preset =
        hb_value_get_string(hb_dict_get(preset, "PictureDenoisePreset"));
    if (denoise_preset != NULL) {
      denoise_tune =
          hb_value_get_string(hb_dict_get(preset, "PictureDenoiseTune"));
      denoise_custom =
          hb_value_get_string(hb_dict_get(preset, "PictureDenoiseCustom"));

      filter_settings = hb_generate_filter_settings(
          filter_id, denoise_preset, denoise_tune, denoise_custom);
      if (filter_settings == NULL) {
        hb_error("Invalid denoise filter settings (%s%s%s)", denoise_preset,
                 denoise_tune ? "," : "", denoise_tune ? denoise_tune : "");
        return -1;
      } else if (!hb_dict_get_bool(filter_settings, "disable")) {
        filter_dict = hb_dict_init();
        hb_dict_set(filter_dict, "ID", hb_value_int(filter_id));
  ...
  chroma_smooth_preset =
      hb_value_get_string(hb_dict_get(preset, "PictureChromaSmoothPreset"));
  chroma_smooth_tune =
      hb_value_get_string(hb_dict_get(preset, "PictureChromaSmoothTune"));
  chroma_smooth_custom =
      hb_value_get_string(hb_dict_get(preset, "PictureChromaSmoothCustom"));
  if (chroma_smooth_preset != NULL && strcasecmp(chroma_smooth_preset, "off")) {
    int filter_id = HB_FILTER_CHROMA_SMOOTH;
    filter_settings =
        hb_generate_filter_settings(filter_id, chroma_smooth_preset,
                                    chroma_smooth_tune, chroma_smooth_custom);
    if (filter_settings == NULL) {
      hb_error("Invalid chroma smooth filter settings (%s%s%s)",
               chroma_smooth_preset, chroma_smooth_tune ? "," : "",
  ...
  sharpen_filter =
      hb_value_get_string(hb_dict_get(preset, "PictureSharpenFilter"));
  sharpen_preset =
      hb_value_get_string(hb_dict_get(preset, "PictureSharpenPreset"));
  sharpen_tune = hb_value_get_string(hb_dict_get(preset, "PictureSharpenTune"));
  sharpen_custom =
      hb_value_get_string(hb_dict_get(preset, "PictureSharpenCustom"));
  if (sharpen_filter != NULL && sharpen_preset != NULL &&
    ...
    filter_settings = hb_generate_filter_settings(filter_id, sharpen_preset,
                                                  sharpen_tune, sharpen_custom);
    if (filter_settings == NULL) {
      hb_error("Invalid sharpen filter settings (%s%s%s)", sharpen_preset,
               sharpen_tune ? "," : "", sharpen_tune ? sharpen_tune : "");
      return -1;
    } else if (!hb_dict_get_bool(filter_settings, "disable")) {
      filter_dict = hb_dict_init();
      hb_dict_set(filter_dict, "ID", hb_value_int(filter_id));
  ...
  // Deblock filter
  const char *deblock =
      hb_value_get_string(hb_dict_get(preset, "PictureDeblockPreset"));
  if (deblock != NULL) {
    const char *deblock_tune =
        hb_value_get_string(hb_dict_get(preset, "PictureDeblockTune"));
    const char *deblock_custom =
        hb_value_get_string(hb_dict_get(preset, "PictureDeblockCustom"));
    filter_settings = hb_generate_filter_settings(HB_FILTER_DEBLOCK, deblock,
                                                  deblock_tune, deblock_custom);
    if (filter_settings == NULL) {
  ...
  // Rotate filter
  char *rotate =
      hb_value_get_string_xform(hb_dict_get(preset, "PictureRotate"));
  if (rotate != NULL) {
    filter_settings =
        hb_generate_filter_settings(HB_FILTER_ROTATE, NULL, NULL, rotate);
    if (filter_settings == NULL) {
  ...
  if (pad != NULL) {
    filter_settings =
        hb_generate_filter_settings(HB_FILTER_PAD, NULL, NULL, pad);
    if (filter_settings == NULL) {
  ...
  fr_mode =
      hb_value_type(fr_mode_value) == HB_VALUE_TYPE_STRING
          ? (!strcasecmp(hb_value_get_string(fr_mode_value), "cfr")
                 ? 1
                 : !strcasecmp(hb_value_get_string(fr_mode_value), "pfr") ? 2
                                                                          : 0)
...
int hb_preset_apply_video(const hb_dict_t *preset, hb_dict_t *job_dict) {
  ...
  if (hb_value_type(vcodec_value) == HB_VALUE_TYPE_STRING) {
    vcodec = hb_video_encoder_get_from_name(hb_value_get_string(vcodec_value));
  } else {
  ...
  if (!(encoder->muxers & mux)) {
    hb_error("Incompatible video encoder (%s) for muxer (%s)",
             hb_video_encoder_get_name(vcodec), hb_container_get_name(mux));
    return -1;
  }
  ...
  color_matrix_code =
      hb_value_get_int(hb_dict_get(preset, "VideoColorMatrixCodeOveride"));
  if (color_matrix_code != 0) {
    ...
    hb_dict_set(video_dict, "ColorPrimariesOverride", hb_value_int(color_prim));
    hb_dict_set(video_dict, "ColorTransferOverride",
                hb_value_int(color_transfer));
    hb_dict_set(video_dict, "ColorMatrixOverride", hb_value_int(color_matrix));
  }
  ...
  if ((value = hb_dict_get(preset, "VideoQSVDecode")) != NULL) {
    hb_dict_set(qsv, "Decode", hb_value_xform(value, HB_VALUE_TYPE_BOOL));
  }
  if ((value = hb_dict_get(preset, "VideoQSVLowPower")) != NULL) {
    hb_dict_set(qsv, "LowPower", hb_value_xform(value, HB_VALUE_TYPE_BOOL));
  }
  if ((value = hb_dict_get(preset, "VideoQSVAsyncDepth")) != NULL) {
    hb_dict_set(qsv, "AsyncDepth", hb_value_xform(value, HB_VALUE_TYPE_INT));
  }
...
int hb_preset_apply_mux(const hb_dict_t *preset, hb_dict_t *job_dict) {
  ...
  hb_dict_set(
      dest_dict, "AlignAVStart",
      hb_value_xform(hb_dict_get(preset, "AlignAVStart"), HB_VALUE_TYPE_BOOL));
...
static void dict_clean(hb_value_t *dict, hb_value_t *template) {
  ...
  for (iter = hb_dict_iter_init(tmp); iter != HB_DICT_ITER_DONE;
    ...
    if (template_val == NULL) {
      ...
      if (val_type == HB_VALUE_TYPE_DICT || val_type == HB_VALUE_TYPE_ARRAY ||
        ...
        hb_error("Preset %s: Incompatible value types for key %s. "
                 ...
                 preset_name, key);
        hb_dict_remove(dict, key);
      } else if (hb_value_is_number(val) && hb_value_is_number(template_val)) {
  ...
  if (!hb_value_get_bool(hb_dict_get(dict, "Folder"))) {
    ...
    for (iter = hb_dict_iter_init(template); iter != HB_DICT_ITER_DONE;
...
static void preset_clean(hb_value_t *preset, hb_value_t *template) {
    ...
    if (strcasecmp(s, "auto")) {
      ...
      if (fr < 0) {
        if (strcasecmp(s, "same as source")) {
          hb_error("Preset %s: Invalid video framerate (%s)", preset_name, s);
        }
  ...
  if (val != NULL) {
    ...
    if (acodec == HB_ACODEC_INVALID) {
      ...
      hb_error("Preset %s: Invalid audio fallback encoder (%s)", preset_name,
               s);
    }
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    if (val != NULL) {
      ...
      if (acodec == HB_ACODEC_INVALID) {
        acodec = hb_audio_encoder_get_default(muxer);
        hb_error("Preset %s: Invalid audio encoder (%s)", preset_name, s);
      }
      enc = hb_audio_encoder_get_short_name(acodec);
      val = hb_value_string(enc);
    ...
    if (val != NULL) {
      ...
      if (strcasecmp(s, "auto")) {
        ...
        if (sr < 0) {
          hb_error("Preset %s: Invalid audio samplerate (%s)", preset_name, s);
          val = hb_value_string("auto");
    ...
    if (val != NULL) {
      ...
      if (mixdown == HB_INVALID_AMIXDOWN) {
        ...
        hb_error("Preset %s: Invalid audio mixdown (%s)", preset_name, s);
      }
  ...
  if (val != NULL) {
    ...
    if (str != NULL) {
      if (strcasecmp(str, "hqdn3d")) {
        import_custom_11_1_0(preset, HB_FILTER_HQDN3D, "PictureDenoiseCustom");
      } else if (strcasecmp(str, "nlmeans")) {
        import_custom_11_1_0(preset, HB_FILTER_NLMEANS, "PictureDenoiseCustom");
      }
    }
  }
  import_custom_11_1_0(preset, HB_FILTER_DETELECINE, "PictureDetelecineCustom");
  import_custom_11_1_0(preset, HB_FILTER_ROTATE, "PictureRotate");
}
...
static void import_deint_12_0_0(hb_value_t *preset) {
  ...
  if (val == NULL) {
    hb_dict_set(preset, "PictureDeinterlacePreset", hb_value_string("default"));
    return;
  }
  ...
  if (!strcasecmp(deint, "fast")) {
    ...
    hb_dict_set(preset, "PictureCombDetectPreset", hb_value_string("fast"));
    hb_dict_set(preset, "PictureDeinterlacePreset", hb_value_string("default"));
    return;
  } else if (!strcasecmp(deint, "bob") || !strcasecmp(deint, "default")) {
    hb_dict_set(preset, "PictureCombDetectPreset", hb_value_string("default"));
    return;
  ...
  char *custom = hb_strdup_printf("mode=%d:spatial-metric=%d:"
                                  ...
                                  detect_mode, spatial_metric, motion_threshold,
                                  spatial_threshold, filter_mode,
                                  block_threshold, block_width, block_height);
  hb_dict_set(preset, "PictureCombDetectCustom", hb_value_string(custom));
  free(custom);

  custom = hb_strdup_printf(
      "mode=%d:magnitude-thresh=%d:variance-thresh=%d:"
      "laplacian-thresh=%d:dilation-thresh=%d:"
      "erosion-thresh=%d:noise-thresh=%d:"
      "search-distance=%d:postproc=%d:parity=%d",
      decomb_mode, magnitude_threshold, variance_threshold, laplacian_threshold,
      dilation_threshold, erosion_threshold, noise_threshold,
      maximum_search_distance, post_processing, parity);
...
static void import_deint_10_0_0(hb_value_t *preset) {
  ...
  if (val != NULL) {
    ...
    if (decomb_or_deint) {
      deint_preset = hb_value_get_string(hb_dict_get(preset, "PictureDecomb"));
    } else {
      deint_preset =
          hb_value_get_string(hb_dict_get(preset, "PictureDeinterlace"));
    }
...
static void import_pic_0_0_0(hb_value_t *preset) {
  ...
  if (hb_value_is_number(val)) {
    ...
    if (end != v) {
      ...
      if (strcasecmp(v, "off") && strcasecmp(v, "strict") &&
          strcasecmp(v, "loose") && strcasecmp(v, "custom")) {
        hb_dict_set(preset, "PicturePAR", hb_value_string("loose"));
      }
    }
  }
...
static int cmpVersion(int a_major, int a_minor, int a_micro, int b_major,
                      int b_minor, int b_micro) {
  if (a_major > b_major)
    return 1;
  if (a_major < b_major)
    return -1;
...
void hb_presets_builtin_init(void) {
  ...
  hb_preset_version_major =
      hb_value_get_int(hb_dict_get(template, "VersionMajor"));
  hb_preset_version_minor =
      hb_value_get_int(hb_dict_get(template, "VersionMinor"));
  hb_preset_version_micro =
      hb_value_get_int(hb_dict_get(template, "VersionMicro"));
...
static hb_preset_index_t *preset_lookup_path(const char *name, int recurse,
  ...
  ctx.last_match_idx = -1;
  result =
      presets_do(do_preset_search, hb_presets, (preset_do_context_t *)&ctx);
  if (result != PRESET_DO_SUCCESS)
...
int hb_presets_version_file(const char *filename, int *major, int *minor,
                            int *micro) {
  int result;
...
hb_value_t *hb_presets_get(void) { return hb_presets; }
...
int hb_preset_set(const hb_preset_index_t *path, const hb_value_t *dict) {
  hb_value_t *folder = NULL;
