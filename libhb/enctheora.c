...
hb_work_object_t hb_enctheora = {WORK_ENCTHEORA, "Theora encoder (libtheora)",
                                 enctheoraInit, enctheoraWork, enctheoraClose};
...
int enctheoraInit(hb_work_object_t *w, hb_job_t *job) {
  ...
  if (job->pass_id == HB_PASS_ENCODE_1ST) {
    ...
    int bytes;
    bytes =
        th_encode_ctl(pv->ctx, TH_ENCCTL_2PASS_OUT, &buffer, sizeof(buffer));
    if (bytes < 0) {
