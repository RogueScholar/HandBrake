...
hb_bd_t *hb_bd_init(hb_handle_t *h, const char *path) {
  ...
  qsort(d->title_info, d->title_count, sizeof(BLURAY_TITLE_INFO *),
 ...
 * hb_bd_title_count
 **********************************************************************/
int hb_bd_title_count(hb_bd_t *d) { return d->title_count; }

static void add_subtitle(int track, hb_list_t *list_subtitle,
                         BLURAY_STREAM_INFO *bdsub, uint32_t codec) {
  ...
  hb_log("bd: subtitle id=0x%x, lang=%s, 3cc=%s", subtitle->id, subtitle->lang,
         subtitle->iso639_2);
...
static void add_audio(int track, hb_list_t *list_audio,
                      BLURAY_STREAM_INFO *bdaudio, int substream_type,
                      uint32_t codec, uint32_t codec_param, int attributes) {
...
static void show_clip_list(BLURAY_TITLE_INFO *ti) {
  ...
  for (ii = 0; ii < ti->clip_count; ii++) {
    ...
    hb_log("bd:\t\t%s.M2TS -- Duration: %02d:%02d:%02d", ti->clips[ii].clip_id,
           hh, mm, ss);
  }
}
...
hb_title_t *hb_bd_title_scan(hb_bd_t *d, int tt, uint64_t min_duration) {
  ...
  hb_log("bd: duration is %02d:%02d:%02d (%" PRIu64 " ms)", title->hours,
  ...
  hb_log(
      "bd: video id=0x%x, stream type=%s, format %s", title->video_id,
      bdvideo->coding_type == BLURAY_STREAM_TYPE_VIDEO_MPEG1
          ? "MPEG1"
          : bdvideo->coding_type == BLURAY_STREAM_TYPE_VIDEO_MPEG2
                ? "MPEG2"
                : bdvideo->coding_type == BLURAY_STREAM_TYPE_VIDEO_VC1
                      ? "VC-1"
                      : bdvideo->coding_type == BLURAY_STREAM_TYPE_VIDEO_H264
                            ? "H.264"
                            : bdvideo->coding_type ==
                                      BLURAY_STREAM_TYPE_VIDEO_HEVC
                                  ? "HEVC"
                                  : "Unknown",
      bdvideo->format == BLURAY_VIDEO_FORMAT_480I
          ...
          : bdvideo->format == BLURAY_VIDEO_FORMAT_576I
                ? "576i"
                : bdvideo->format == BLURAY_VIDEO_FORMAT_480P
                      ? "480p"
                      : bdvideo->format == BLURAY_VIDEO_FORMAT_1080I
                            ? "1080i"
                            : bdvideo->format == BLURAY_VIDEO_FORMAT_720P
                                  ? "720p"
                                  : bdvideo->format == BLURAY_VIDEO_FORMAT_1080P
                                        ? "1080p"
                                        : bdvideo->format ==
  ...
  default:
    hb_log("scan: unknown video codec (0x%x)", bdvideo->coding_type);
    goto fail;
  }
  ...
  hb_log("bd: aspect = %d:%d", title->container_dar.num,
  ...
  for (ii = 0; ii < ti->clips[audio_clip_index].audio_stream_count; ii++) {
    ...
    case BLURAY_STREAM_TYPE_AUDIO_TRUHD:
      ...
      add_audio(ii, title->list_audio, bdaudio, HB_SUBSTREAM_BD_TRUEHD,
                HB_ACODEC_FFTRUEHD, AV_CODEC_ID_TRUEHD, HB_AUDIO_ATTR_NONE);
      break;

    case BLURAY_STREAM_TYPE_AUDIO_DTS:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_DCA,
                AV_CODEC_ID_DTS, HB_AUDIO_ATTR_NONE);
      break;

    case BLURAY_STREAM_TYPE_AUDIO_MPEG2:
    case BLURAY_STREAM_TYPE_AUDIO_MPEG1:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_FFMPEG,
                AV_CODEC_ID_MP2, HB_AUDIO_ATTR_NONE);
      break;

    case BLURAY_STREAM_TYPE_AUDIO_AC3PLUS:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_FFEAC3,
                AV_CODEC_ID_EAC3, HB_AUDIO_ATTR_NONE);
      break;
    ...
    case BLURAY_STREAM_TYPE_AUDIO_LPCM:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_FFMPEG,
                AV_CODEC_ID_PCM_BLURAY, HB_AUDIO_ATTR_NONE);
      break;

    case BLURAY_STREAM_TYPE_AUDIO_AC3:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_AC3,
                AV_CODEC_ID_AC3, HB_AUDIO_ATTR_NONE);
      break;
    ...
    case BLURAY_STREAM_TYPE_AUDIO_DTSHD:
      ...
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_DCA_HD,
                AV_CODEC_ID_DTS, HB_AUDIO_ATTR_NONE);
      break;
  ...
  for (jj = 0; jj < ti->clips[audio_clip_index].sec_audio_stream_count;
       jj++, ii++) {
    BLURAY_STREAM_INFO *bdaudio;
    ...
    case BLURAY_STREAM_TYPE_AUDIO_TRUHD:
      // Add 2 audio tracks.  One for TrueHD and one for AC-3
      add_audio(ii, title->list_audio, bdaudio, HB_SUBSTREAM_BD_AC3,
                HB_ACODEC_AC3, AV_CODEC_ID_AC3, HB_AUDIO_ATTR_SECONDARY);
      add_audio(ii, title->list_audio, bdaudio, HB_SUBSTREAM_BD_TRUEHD,
    ...
    case BLURAY_STREAM_TYPE_AUDIO_DTS:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_DCA,
                AV_CODEC_ID_DTS, HB_AUDIO_ATTR_SECONDARY);
      break;

    case BLURAY_STREAM_TYPE_AUDIO_MPEG2:
    case BLURAY_STREAM_TYPE_AUDIO_MPEG1:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_FFMPEG,
                AV_CODEC_ID_MP2, HB_AUDIO_ATTR_SECONDARY);
      break;

    case BLURAY_STREAM_TYPE_AUDIO_AC3PLUS:
    case BLURAY_STREAM_TYPE_AUDIO_AC3PLUS_SECONDARY:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_FFEAC3,
                AV_CODEC_ID_EAC3, HB_AUDIO_ATTR_SECONDARY);
      break;

    case BLURAY_STREAM_TYPE_AUDIO_LPCM:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_FFMPEG,
                AV_CODEC_ID_PCM_BLURAY, HB_AUDIO_ATTR_SECONDARY);
      break;

    case BLURAY_STREAM_TYPE_AUDIO_AC3:
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_AC3,
                AV_CODEC_ID_AC3, HB_AUDIO_ATTR_SECONDARY);
      break;
    ...
    case BLURAY_STREAM_TYPE_AUDIO_DTSHD:
      // Add 2 audio tracks.  One for DTS-HD and one for DTS
      add_audio(ii, title->list_audio, bdaudio, HB_SUBSTREAM_BD_DTS,
                HB_ACODEC_DCA, AV_CODEC_ID_DTS, HB_AUDIO_ATTR_SECONDARY);
      // DTS-HD is special.  The substreams must be concatinated
      // DTS-core followed by DTS-hd-extensions.  Setting
      // a substream id of 0 says use all substreams.
      add_audio(ii, title->list_audio, bdaudio, 0, HB_ACODEC_DCA_HD,
  ...
  for (ii = 0; ii < ti->clips[audio_clip_index].pg_stream_count; ii++) {
    ...
    case BLURAY_STREAM_TYPE_SUB_PG:
      ...
      break;
    default:
      hb_log("scan: unknown subtitle pid 0x%x codec 0x%x", bdpgs->pid,
             bdpgs->coding_type);
      break;
    }
  }
  ...
  for (ii = 0, jj = 0; ii < ti->chapter_count; ii++) {
    ...
    if (ii < ti->chapter_count - 1) {
      ...
      if (ti->chapters[ii + 1].start < ti->chapters[ii].start) {
        ...
        ti->chapters[ii + 1].start = ti->chapters[ii].start + chapter->duration;
      }
      if (ti->chapters[ii + 1].start - ti->chapters[ii].start !=
          chapter->duration) {
        hb_log("bd: chapter %d invalid duration %" PRIu64 "", ii + 1,
               chapter->duration);
        chapter->duration = ti->chapters[ii + 1].start - ti->chapters[ii].start;
      }
    } else {
    ...
    hb_log("bd: chap %d, %" PRIu64 " ms", chapter->index,
           chapter->duration / 90);
 ...
 **********************************************************************/
int hb_bd_chapter(hb_bd_t *d) { return d->chapter; }
...
static int next_packet(BLURAY *bd, uint8_t *pkt) {
  ...
  while (1) {
    ...
    hb_log("next_packet: sync lost @ %" PRIu64 ", regained after %" PRIu64
           " bytes",
           pos, pos2);
  }
}
