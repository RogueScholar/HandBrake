...
typedef enum { BOLD = 0x1, ITALIC = 0x2, UNDERLINE = 0x4 } FaceStyleFlag;
...
typedef struct {
  uint16_t
      startChar; // NOTE: indices in terms of *character* (not: byte) positions
  uint16_t endChar;
  uint16_t fontID;
...
#define FOURCC(str)                                                            \
  ((((uint32_t)str[0]) << 24) | (((uint32_t)str[1]) << 16) |                   \
   (((uint32_t)str[2]) << 8) | (((uint32_t)str[3]) << 0))
...
static int write_ssa_markup(char *dst, StyleRecord *style) {
  ...
  sprintf(dst, "{\\i%d\\b%d\\u%d\\1c&H%X&\\1a&H%02X&}",
          !!(style->faceStyleFlags & ITALIC), !!(style->faceStyleFlags & BOLD),
          !!(style->faceStyleFlags & UNDERLINE),
          HB_RGB_TO_BGR(style->textColorRGBA >> 8),
          255 -
              (style->textColorRGBA & 0xFF)); // SSA alpha is inverted 0==opaque
...
static hb_buffer_t *tx3g_decode_to_ssa(hb_work_private_t *pv, hb_buffer_t *in) {
   ...
   */
  int maxOutputSize =
      textLength + SSA_PREAMBLE_LEN + (numStyleRecords * MAX_MARKUP_LEN);
...
static void dectx3gClose(hb_work_object_t *w) { free(w->private_data); }

hb_work_object_t hb_dectx3gsub = {WORK_DECTX3GSUB, "TX3G Subtitle Decoder",
                                  dectx3gInit, dectx3gWork, dectx3gClose};
