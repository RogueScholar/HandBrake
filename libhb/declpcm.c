...
hb_work_object_t hb_declpcm = {WORK_DECLPCM, "LPCM decoder", declpcmInit,
                               declpcmWork,  declpcmClose,   0,
                               declpcmBSInfo};
...
static const uint64_t hdr2layout[] = {
    ...
    AV_CH_LAYOUT_QUAD,    AV_CH_LAYOUT_5POINT0_BACK, AV_CH_LAYOUT_6POINT0_FRONT,
...
static void lpcmInfo(hb_work_object_t *w, hb_buffer_t *in) {
   ...
   * byte 1,2 offset to first frame that begins in this packet (not including
   * hdr) byte 3: bits 0-4  continuity counter (increments modulo 20) bit   5
   * reserved bit   6   audio mute on/off bit   7   audio emphasis on/off byte
   * 4: bits 0-2  #channels - 1 (e.g., stereo = 1) bit   3   reserved bits 4-5
   * sample rate (0=48K,1=96K,2=44.1K,3=32K) bits 6-7  bits per sample (0=16
   * bit, 1=20 bit, 2=24 bit) byte 5   Dynamic range control (0x80 = off)
   *
   * The audio is viewed as "frames" of 150 90KHz ticks each (80 samples @
   * 48KHz). The frames are laid down continuously without regard to MPEG packet
  ...
  int frames = (90000 * samples / (pv->samplerate * pv->nchannels) + 149) / 150;

  pv->duration = frames * 150;
  pv->nchunks =
      (pv->duration * pv->nchannels * pv->samplerate + samples_per_chunk - 1) /
      (90000 * samples_per_chunk);
 ...
 * frames line up with the DVD frames. Since audio frame boundaries are
 * unrelated to DVD PES boundaries, this routine has to reconstruct then extract
 * the audio frames. Because of the arbitrary alignment, it can output zero, one
 * or two buf's.
 */
...
static hb_buffer_t *Decode(hb_work_object_t *w) {
    ...
    while (count--) {
      ...
      for (gg = 0; gg < 2; gg++) {
        for (cc = 0; cc < pv->nchannels; cc++) {
          ...
          // us proper signed values. The final division adjusts
          // the range to [-1.0 ... 1.0]
          *odat = (float)(((int)(frm[0] << 24) >> 12) | (frm[1] << 4) |
                          (((lsb[0] >> shift) & 0x0f))) /
                  (16. * 32768.0);
          odat++;
          lsb += !shift;
    ...
    while (count--) {
      ...
      for (gg = 0; gg < 2; gg++) {
        for (cc = 0; cc < pv->nchannels; cc++) {
          ...
          // the range to [-1.0 ... 1.0]
          *odat++ =
              (float)(((int)(frm[0] << 24) >> 8) | (frm[1] << 8) | lsb[0]) /
              (256. * 32768.0);
          frm += 2;
          lsb++;
        }
  ...
  hb_audio_resample_set_sample_rate(pv->resample, pv->samplerate);
