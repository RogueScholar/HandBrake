...
/*
 * Decoder for DVD bitmap subtitles, also known as "VOB subtitles" within the
 * HandBrake source code.
 *
 ...
 * - In the case of an MKV file input, the palette is read from the codec
 * private data of the subtitle track.
 *
...
int decsubWork(hb_work_object_t *w, hb_buffer_t **buf_in,
  ...
  if (!pv->size_sub) {
    ...
    if (size_sub && size_rle && size_sub > size_rle && in->size <= size_sub) {
...
hb_work_object_t hb_decvobsub = {WORK_DECVOBSUB, "VOBSUB decoder", decsubInit,
  ...
  for (i = pv->size_rle;;) {
    ...
    for (;;) {
      ...
      case 0x04: // 0x04 - SET_CONTR - Set Contrast
      {
        ...
        int lastAlpha =
            pv->alpha[3] + pv->alpha[2] + pv->alpha[1] + pv->alpha[0];
...
#define GET_NEXT_NIBBLE                                                        \
  code = (code << 4) |                                                         \
         (((*offset & 1) ? (pv->buf->data[((*offset) >> 1)] & 0xF)             \
                         : (pv->buf->data[((*offset) >> 1)] >> 4)));           \
  (*offset)++
