...
static int decavcodecaBSInfo(hb_work_object_t *, const hb_buffer_t *,
                             hb_work_info_t *);

static int get_color_prim(int color_primaries, hb_geometry_t geometry,
                          hb_rational_t rate);
static int get_color_transfer(int color_trc);
static int get_color_matrix(int colorspace, hb_geometry_t geometry);

hb_work_object_t hb_decavcodeca = {.id = WORK_DECAVCODEC,
...
static int decavcodecaInit(hb_work_object_t *w, hb_job_t *job) {
  ...
  if (!(w->audio->config.out.codec & HB_ACODEC_PASS_FLAG)) {
    ...
    // filter pipleine.
    pv->resample = hb_audio_resample_init(
        AV_SAMPLE_FMT_FLT, w->audio->config.in.samplerate,
  ...
  if (w->audio->config.out.dynamic_range_compression >= 0.0f &&
    ...
    if (w->audio->config.out.dynamic_range_compression > drc_scale_max) {
      hb_log(
          "decavcodecaInit: track %d, sanitizing out-of-range DRC %.2f to %.2f",
...
static void closePrivData(hb_work_private_t **ppv) {
  ...
  if (pv) {
    ...
    if (pv->context && pv->context->codec) {
       ...
       */
      // if (!(pv->qsv.decode && pv->job != NULL && (pv->job->vcodec &
      // HB_VCODEC_QSV_MASK)))
...
static void audioParserFlush(hb_work_object_t *w) {
  ...
  do {
    if (pv->parser) {
      av_parser_parse2(pv->parser, pv->context, &pout, &pout_len, NULL, 0,
  ...
  for (pos = 0; pos < in->size; pos += len) {
    ...
    if (pv->parser != NULL) {
      len = av_parser_parse2(pv->parser, pv->context, &pout, &pout_len,
                             in->data + pos, in->size - pos, pts, pts, 0);
      parser_pts = pv->parser->pts;
      pts = AV_NOPTS_VALUE;
    } else {
      pout = in->data;
...
static int parse_adts_extradata(hb_audio_t *audio, AVCodecContext *context,
  ...
  if (audio->priv.config.extradata.length == 0) {
    ...
    extradata = av_packet_get_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA, &size);
...
static int decavcodecaBSInfo(hb_work_object_t *w, const hb_buffer_t *buf,
  ...
  if (w->title && w->title->opaque_priv != NULL) {
    ...
    avcodec_parameters_to_context(context, ic->streams[audio->id]->codecpar);
    // libav's eac3 parser toggles the codec_id in the context as
    // it reads eac3 data between AV_CODEC_ID_AC3 and AV_CODEC_ID_EAC3.
    // It detects an AC3 sync pattern sometimes in ac3_sync() which
    // causes it to eventually set avctx->codec_id to AV_CODEC_ID_AC3
  ...
  while (buf != NULL && !done) {
    ...
    while (parse_pos < buf->size && !done) {
      ...
      if (parser != NULL) {
        parse_len =
            av_parser_parse2(parser, context, &parse_buffer, &parse_buffer_size,
      ...
      do {
        ...
        if (ret >= 0) {
          ...
          if (frame->sample_rate != 0) {
            ...
            hb_log("decavcodecaBSInfo: warning: invalid frame sample_rate! "
                   "Using context sample_rate.");
          }
          info->rate.den = 1;
          ...
          if (truehd_mono) {
            ...
            if ((side_data = av_frame_get_side_data(
                     frame, AV_FRAME_DATA_MATRIXENCODING)) != NULL) {
...
reordered_data_t *reordered_hash_rem(hb_work_private_t *pv, int64_t sequence) {
...
void reordered_hash_add(hb_work_private_t *pv, reordered_data_t *reordered) {
...
static hb_buffer_t *cc_fill_buffer(hb_work_private_t *pv, uint8_t *cc,
                                   int size) {
  int cc_count[4] = {
      0,
  };
...
// or at least one of its dimensions is odd, use sws_scale to convert/rescale
// it. Otherwise just copy the bits.
...
int reinit_video_filters(hb_work_private_t *pv) {
  ...
  if (pix_fmt == pv->frame->format && orig_width == pv->frame->width &&
...
static int decodeFrame(hb_work_private_t *pv, packet_info_t *packet_info) {
  ...
  if (pv->palette != NULL) {
    ...
    int size;
    palette =
        av_packet_new_side_data(&avp, AV_PKT_DATA_PALETTE, AVPALETTE_SIZE);
  ...
  if (context->extradata == NULL) {
    if (pv->parser == NULL || pv->parser->parser == NULL ||
      ...
      if (size > 0) {
        ...
        context->extradata = av_malloc(size + AV_INPUT_BUFFER_PADDING_SIZE);
        if (context->extradata == NULL)
          return 1;
  ...
  *buf_out = NULL;
  ...
  for (pos = 0; pos < in->size; pos += len) {
    ...
    if (pv->parser) {
      ...
      len = av_parser_parse2(pv->parser, pv->context, &pout, &pout_len,
                             in->data + pos, in->size - pos, pts, dts, 0);
      parser_pts = pv->parser->pts;
      parser_dts = pv->parser->dts;
      pts = AV_NOPTS_VALUE;
      dts = AV_NOPTS_VALUE;
...
static int decavcodecvInfo(hb_work_object_t *w, hb_work_info_t *info) {
  ...
  info->color_prim =
      get_color_prim(pv->context->color_primaries, info->geometry, info->rate);
  info->color_transfer = get_color_transfer(pv->context->color_trc);
  info->color_matrix =
      get_color_matrix(pv->context->colorspace, info->geometry);
  info->color_range = pv->context->color_range;

  info->video_decode_support = HB_DECODE_SUPPORT_SW;

#if HB_PROJECT_FEATURE_QSV
  if (avcodec_find_decoder_by_name(
          hb_qsv_decode_get_codec_name(pv->context->codec_id))) {
...
hb_work_object_t hb_decavcodecv = {.id = WORK_DECAVCODECV,
...
static void decodeAudio(hb_work_private_t *pv, packet_info_t *packet_info) {
  ...
  if (pv->next_pts == (int64_t)AV_NOPTS_VALUE && packet_info != NULL &&
  ...
  do {
    ...
    if (pv->audio->config.out.codec & HB_ACODEC_PASS_FLAG) {
      ...
      if ((side_data = av_frame_get_side_data(
               pv->frame, AV_FRAME_DATA_DOWNMIX_INFO)) != NULL) {
        ...
        hb_audio_resample_set_mix_levels(pv->resample, surround_mix_level,
                                         center_mix_level,
                                         downmix_info->lfe_mix_level);
      }
      channel_layout = pv->frame->channel_layout;
      if (channel_layout == 0) {
        channel_layout = av_get_default_channel_layout(pv->frame->channels);
      }
      hb_audio_resample_set_channel_layout(pv->resample, channel_layout);
      hb_audio_resample_set_sample_fmt(pv->resample, pv->frame->format);
      hb_audio_resample_set_sample_rate(pv->resample, pv->frame->sample_rate);
      ...
      if (out != NULL && pv->drop_samples > 0) {
        ...
        if (samples <= pv->drop_samples) {
          ...
          double drop_duration =
              pv->drop_samples * 90000L / pv->audio->config.out.samplerate;
