...
// σ ~= 1.2
// size = 5, coef = 1.0 / 15
static const int kernel_isolog[] = {0,  -1, -1, -1, 0,  -1, -3, -4, -3,
                                    -1, -1, -4, 55, -4, -1, -1, -3, -4,
                                    -3, -1, 0,  -1, -1, -1, 0};

static kernel_t kernels[] = {{kernel_lap, 3, 1.0},
...
static int hb_lapsharp_init(hb_filter_object_t *filter, hb_filter_init_t *init);

static int hb_lapsharp_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                            hb_buffer_t **buf_out);
...
static void hb_lapsharp(const uint8_t *src, uint8_t *dst, const int width,
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      if ((y < offset_max) || (y > height - offset_max) ||
      ...
      for (int k = offset_min; k < offset_max; k++) {
        for (int j = offset_min; j < offset_max; j++) {
          pixel +=
              kernel->mem[((j - offset_min) * kernel->size) + k - offset_min] *
              *(src + stride * (y + j) + (x + k));
        }
      }
      pixel = (int16_t)(((pixel * kernel->coef) - *(src + stride * y + x)) *
                        ctx->strength) +
              *(src + stride * y + x);
...
static int hb_lapsharp_init(hb_filter_object_t *filter,
  ...
  for (int c = 1; c < 3; c++) {
    ...
    if (ctx->strength == -1)
      ctx->strength = prev_ctx->strength;
    if (ctx->kernel == -1)
      ctx->kernel = prev_ctx->kernel;
  }
  ...
  for (int c = 0; c < 3; c++) {
    ...
    if (ctx->strength == -1) {
      ctx->strength =
          c ? LAPSHARP_STRENGTH_CHROMA_DEFAULT : LAPSHARP_STRENGTH_LUMA_DEFAULT;
    }
    if (ctx->kernel == -1) {
      ctx->kernel =
          c ? LAPSHARP_KERNEL_CHROMA_DEFAULT : LAPSHARP_KERNEL_LUMA_DEFAULT;
    }
...
static int hb_lapsharp_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
  ...
  for (c = 0; c < 3; c++) {
    lapsharp_plane_context_t *ctx = &pv->plane_ctx[c];
    hb_lapsharp(in->plane[c].data, out->plane[c].data, in->plane[c].width,
                in->plane[c].height, in->plane[c].stride, ctx);
  }
