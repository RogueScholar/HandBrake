...
static int read_time_from_string(const char *timeString,
                                 struct start_and_end *result) {
  // for ex. 00:00:15,248 --> 00:00:16,545

  long houres1, minutes1, seconds1, milliseconds1, houres2, minutes2, seconds2,
      milliseconds2;
  int scanned;

  scanned = sscanf(timeString, "%ld:%ld:%ld,%ld --> %ld:%ld:%ld,%ld\n",
                   &houres1, &minutes1, &seconds1, &milliseconds1, &houres2,
  ...
  result->start = milliseconds1 + seconds1 * 1000 + minutes1 * 60 * 1000 +
                  houres1 * 60 * 60 * 1000;
  result->end = milliseconds2 + seconds2 * 1000 + minutes2 * 60 * 1000 +
                houres2 * 60 * 60 * 1000;
  return 1;
}
...
static int utf8_fill(hb_work_private_t *pv) {
  ...
  memmove(pv->utf8_buf, pv->utf8_buf + pv->utf8_pos,
...
static hb_buffer_t *srt_read(hb_work_private_t *pv) {
  ...
  while (reprocess || get_line(pv, line_buffer, sizeof(line_buffer))) {
    ...
    case k_state_potential_new_entry: {
      ...
      if (!resync && (*line_buffer == '\n' || *line_buffer == '\r')) {
         ...
         */
        strncat(pv->current_entry.text, " ",
                sizeof(pv->current_entry.text) -
                    strlen(pv->current_entry.text) - 1);
      ...
      if (*pv->current_entry.text != '\0') {
        ...
        char *p, *q;
        uint64_t start_time =
            (pv->current_entry.start + pv->subtitle->config.offset) * 90;
        uint64_t stop_time =
            (pv->current_entry.stop + pv->subtitle->config.offset) * 90;

        // Drop subtitles that end before the start time
        // or start after the stop time
        if (stop_time <= pv->start_time || start_time >= pv->stop_time) {
          hb_deep_log(3,
                      "Discarding SRT at time start %" PRId64 ", stop %" PRId64,
  ...
  if (*pv->current_entry.text != '\0') {
    ...
    if (!(start_time >= pv->start_time && stop_time < pv->stop_time)) {
      hb_deep_log(3, "Discarding SRT at time start %" PRId64 ", stop %" PRId64,
                  start_time, stop_time);
      memset(&pv->current_entry, 0, sizeof(srt_entry_t));
      return NULL;
    }
...
hb_work_object_t hb_decsrtsub = {WORK_DECSRTSUB, "SRT Subtitle Decoder",
                                 decsrtInit, decsrtWork, decsrtClose};
