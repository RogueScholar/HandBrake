...
hb_work_object_t hb_encavcodeca = {
    WORK_ENCAVCODEC_AUDIO, "AVCodec Audio encoder (libavcodec)",
    encavcodecaInit, encavcodecaWork, encavcodecaClose};
...
static int encavcodecaInit(hb_work_object_t *w, hb_job_t *job) {
  ...
  if (codec_name != NULL) {
    ...
    if (codec == NULL) {
      hb_error("encavcodecaInit: avcodec_find_encoder(%d) failed", codec_id);
      return 1;
    }
  }
  ...
  w->config->init_delay =
      av_rescale(context->initial_padding, 90000, context->sample_rate);
  ...
  pv->max_output_bytes =
      MAX(AV_INPUT_BUFFER_MIN_SIZE,
          (pv->input_samples * av_get_bytes_per_sample(context->sample_fmt)));
  ...
  if (context->sample_fmt != AV_SAMPLE_FMT_FLT) {
    ...
    av_opt_set_int(pv->swresample, "in_sample_fmt", AV_SAMPLE_FMT_FLT, 0);
    av_opt_set_int(pv->swresample, "out_sample_fmt", context->sample_fmt, 0);
    av_opt_set_int(pv->swresample, "in_channel_layout", context->channel_layout,
                   0);
    av_opt_set_int(pv->swresample, "out_channel_layout",
                   context->channel_layout, 0);
    av_opt_set_int(pv->swresample, "in_sample_rate", context->sample_rate, 0);
    av_opt_set_int(pv->swresample, "out_sample_rate", context->sample_rate, 0);
...
static void get_packets(hb_work_object_t *w, hb_buffer_list_t *list) {
  ...
  while (1) {
    ...
    if (pkt.pts > pv->last_pts) {
      ...
      out->s.start =
          av_rescale_q(pkt.pts, pv->context->time_base, (AVRational){1, 90000});
      out->s.duration =
          (double)90000 * pv->samples_per_frame / audio->config.out.samplerate;
...
static void Encode(hb_work_object_t *w, hb_buffer_list_t *list) {
  ...
  while (hb_list_bytes(pv->list) >= pv->input_samples * sizeof(float)) {
    ...
    int out_size;
    AVFrame frame = {
        .nb_samples = pv->samples_per_frame,
    };

    out_size = av_samples_get_buffer_size(NULL, pv->context->channels,
                                          pv->samples_per_frame,
                                          pv->context->sample_fmt, 1);
    avcodec_fill_audio_frame(&frame, pv->context->channels,
                             pv->context->sample_fmt, pv->output_buf, out_size,
                             1);
    if (pv->swresample != NULL) {
      int out_samples;

      out_samples =
          swr_convert(pv->swresample, frame.extended_data, frame.nb_samples,
    ...
    frame.pts = pts + (90000LL * pos /
                       (sizeof(float) * pv->out_discrete_channels *
                        audio->config.out.samplerate));

    frame.pts =
        av_rescale_q(frame.pts, (AVRational){1, 90000}, pv->context->time_base);
