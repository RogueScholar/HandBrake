...
int taskset_init(taskset_t *ts, int thread_count, size_t arg_size) {
  ...
  memset(ts->task_complete_bitmap, 0xFF,
         sizeof(uint32_t) * ts->bitmap_elements);
...
int taskset_thread_spawn(taskset_t *ts, int thr_idx, const char *descr,
                         thread_func_t *func, int priority) {
  ts->task_threads[thr_idx] =
      hb_thread_init(descr, func, taskset_thread_args(ts, thr_idx), priority);
