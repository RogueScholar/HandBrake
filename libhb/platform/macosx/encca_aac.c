...
hb_work_object_t hb_encca_aac = {WORK_ENC_CA_AAC, "AAC encoder (Apple)",
                                 encCoreAudioInitLC, encCoreAudioWork,
                                 encCoreAudioClose};

hb_work_object_t hb_encca_haac = {WORK_ENC_CA_HAAC, "HE-AAC encoder (Apple)",
                                  encCoreAudioInitHE, encCoreAudioWork,
                                  encCoreAudioClose};
...
static long ReadESDSDescExt(void *descExt, UInt8 **buffer, UInt32 *size,
                            int versionFlags) {
...
int encCoreAudioInit(hb_work_object_t *w, hb_job_t *job, enum AAC_MODE mode) {
  ...
  if (err != noErr) {
    ...
    if (err != noErr) {
      hb_log("Error creating an AudioConverter err=%" PRId64
             " output.mBytesPerFrame=%" PRIu64 "",
  ...
  if (audio->config.out.bitrate > 0) {
    ...
    AudioConverterGetPropertyInfo(
        pv->converter, kAudioConverterApplicableEncodeBitRates, &tmpsiz, NULL);
    bitrates = malloc(tmpsiz);
    AudioConverterGetProperty(pv->converter,
                              kAudioConverterApplicableEncodeBitRates, &tmpsiz,
                              bitrates);
    ...
    if (tmp != audio->config.out.bitrate * 1000) {
      hb_log(
          "encCoreAudioInit: sanitizing track %d audio bitrate %d to %" PRIu32
          "",
  ...
  AudioConverterGetProperty(pv->converter,
                            kAudioConverterCompressionMagicCookie, &tmp,
                            w->config->extradata.bytes);
  ...
  AudioConverterGetProperty(pv->converter, kAudioConverterPrimeInfo, &piSize,
                            &primeInfo);
...
static OSStatus inInputDataProc(AudioConverterRef converter, UInt32 *npackets,
  ...
  buffers->mBuffers[0].mDataByteSize =
      MIN(pv->ibytes, pv->isamplesiz * *npackets);
...
static hb_buffer_t *Encode(hb_work_object_t *w) {
  ...
  OSStatus err = AudioConverterFillComplexBuffer(
      pv->converter, inInputDataProc, pv, &npackets, &obuflist, &odesc);

  if (err != noErr && err != 1) {
    hb_log(
        "encCoreAudio: unexpected error in AudioConverterFillComplexBuffer()");
  }
  ...
  obuf->s.start =
      pv->first_pts - pv->delay + 90000LL * pv->samples / pv->osamplerate;
