...
static int hb_decomb_init(hb_filter_object_t *filter, hb_filter_init_t *init);

static int hb_decomb_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                          hb_buffer_t **buf_out);
...
static const uint8_t hb_crop_table[256 + 2 * 1024] = {times1024(0x00),
                                                      0x00,
                                                      0x01,
                                                      0x02,
                                                      0x03,
...
static void cubic_interpolate_line(uint8_t *dst, uint8_t *cur, int width,
                                   int height, int stride, int y) {
  int w = width;
  int x;
...
static inline int blend_filter_pixel(filter_param_t *filter, int up2, int up1,
                                     int current, int down1, int down2) {
...
static void blend_filter_line(filter_param_t *filter, uint8_t *dst,
                              uint8_t *cur, int width, int height, int stride,
                              int y) {
  int w = width;
  int x;
  ...
  for (x = 0; x < w; x++) {
    /* Low-pass 5-tap filter */
    dst[0] = blend_filter_pixel(filter, cur[up2], cur[up1], cur[0], cur[down1],
                                cur[down2]);
    dst++;
    cur++;
  }
...
static void eedi2_interpolate_plane(hb_filter_private_t *pv, int plane) {
  ...
  // edge mask
  eedi2_build_edge_mask(mskp, pitch, srcp, pitch, pv->magnitude_threshold,
                        pv->variance_threshold, pv->laplacian_threshold,
                        half_height, width);
  eedi2_erode_edge_mask(mskp, pitch, tmpp, pitch, pv->erosion_threshold,
                        half_height, width);
  eedi2_dilate_edge_mask(tmpp, pitch, mskp, pitch, pv->dilation_threshold,
                         half_height, width);
  eedi2_erode_edge_mask(mskp, pitch, tmpp, pitch, pv->erosion_threshold,
                        half_height, width);
  ...
  eedi2_calc_directions(plane, mskp, pitch, srcp, pitch, tmpp, pitch,
                        ...
                        half_height, width);
  eedi2_filter_dir_map(mskp, pitch, tmpp, pitch, dstp, pitch, half_height,
                       width);
  eedi2_expand_dir_map(mskp, pitch, dstp, pitch, tmpp, pitch, half_height,
                       width);
  ...
  eedi2_mark_directions_2x(msk2p, pitch, tmp2p2, pitch, tmp2p, pitch, pv->tff,
                           height, width);
  eedi2_filter_dir_map_2x(msk2p, pitch, tmp2p, pitch, dst2mp, pitch, pv->tff,
                          height, width);
  eedi2_expand_dir_map_2x(msk2p, pitch, dst2mp, pitch, tmp2p, pitch, pv->tff,
                          height, width);
  eedi2_fill_gaps_2x(msk2p, pitch, tmp2p, pitch, dst2mp, pitch, pv->tff, height,
                     width);
  eedi2_fill_gaps_2x(msk2p, pitch, dst2mp, pitch, tmp2p, pitch, pv->tff, height,
                     width);

  // interpolate a full-size plane
  eedi2_interpolate_lattice(plane, tmp2p, pitch, dst2p, pitch, tmp2p2, pitch,
                            pv->tff, pv->noise_threshold, height, width);
  ...
  if (pv->post_processing == 1 || pv->post_processing == 3) {
    ...
    eedi2_expand_dir_map_2x(msk2p, pitch, dst2mp, pitch, tmp2p, pitch, pv->tff,
                            height, width);
    eedi2_post_process(tmp2p, pitch, tmp2p2, pitch, dst2p, pitch, pv->tff,
                       height, width);
  }
  if (pv->post_processing == 2 || pv->post_processing == 3) {
    // filter junctions and corners
    eedi2_gaussian_blur1(srcp, pitch, tmpp, pitch, srcp, pitch, half_height,
                         width);
  ...
  {                                                                            \
    ...
    if (score < spatial_score) {                                               \
      ...
      if ((pv->mode & MODE_DECOMB_CUBIC) && !vertical_edge) {                  \
        ...
        case -1:                                                               \
          spatial_pred =                                                       \
              cubic_interpolate_pixel(cur[-3 * stride - 3], cur[-stride - 1],  \
                                      cur[+stride + 1], cur[3 * stride + 3]);  \
          break;                                                               \
        case -2:                                                               \
          spatial_pred = cubic_interpolate_pixel(                              \
              ((cur[-3 * stride - 4] + cur[-stride - 4]) / 2),                 \
              cur[-stride - 2], cur[+stride + 2],                              \
              ((cur[3 * stride + 4] + cur[stride + 4]) / 2));                  \
          break;                                                               \
        case 1:                                                                \
          spatial_pred =                                                       \
...
static void yadif_filter_line(hb_filter_private_t *pv, uint8_t *dst,
                              uint8_t *prev, uint8_t *cur, uint8_t *next,
                              int plane, int width, int height, int stride,
                              int parity, int y) {
  ...
  for (x = 0; x < width; x++) {
    ...
    /* The average of how much the pixels above and below change from the frame
     * before to now. */
    int temporal_diff1 = (ABS(prev[-stride] - cur[-stride]) +
                          ABS(prev[+stride] - cur[+stride])) >>
                         1;
    /* The average of how much the pixels above and below change from now to the
     * next frame. */
    int temporal_diff2 = (ABS(next[-stride] - cur[-stride]) +
                          ABS(next[+stride] - cur[+stride])) >>
                         1;
    ...
    {
      /* SAD of how the pixel-1, the pixel, and the pixel+1 change from the line
       * above to below. */
      int spatial_score = ABS(cur[-stride - 1] - cur[+stride - 1]) +
                          ABS(cur[-stride] - cur[+stride]) +
                          ABS(cur[-stride + 1] - cur[+stride + 1]) - 1;

      /* Spatial pred is either a bilinear or cubic vertical interpolation. */
      if ((pv->mode & MODE_DECOMB_CUBIC) && !vertical_edge) {
        spatial_pred = cubic_interpolate_pixel(cur[-3 * stride], cur[-stride],
      ...
      if (mode == MODE_DECOMB_BLEND) {
        ...
        for (yy = start; yy < segment_stop; yy += 2) {
          if (yy > 1 && yy < penultimate) {
            ...
            yadif_filter_line(pv, dst2, prev, cur, next, pp, width, height,
                              stride, parity ^ tff, yy);
          } else {
            // parity == 0 (TFF), y1 = y0
            // parity == 1 (BFF), y0 = y1
            // parity == 0 (TFF), yu = yp
...
static void yadif_filter(hb_filter_private_t *pv, hb_buffer_t *dst, int parity,
                         int tff) {
...
static int hb_decomb_init(hb_filter_object_t *filter, hb_filter_init_t *init) {
  ...
  if (filter->settings) {
    ...
    if (pv->mode & MODE_DECOMB_EEDI2) {
      hb_dict_extract_int(&pv->magnitude_threshold, dict, "magnitude-thresh");
      hb_dict_extract_int(&pv->variance_threshold, dict, "variance-thresh");
      hb_dict_extract_int(&pv->laplacian_threshold, dict, "laplacian-thresh");
      hb_dict_extract_int(&pv->dilation_threshold, dict, "dilation-thresh");
      hb_dict_extract_int(&pv->erosion_threshold, dict, "erosion-thresh");
      hb_dict_extract_int(&pv->noise_threshold, dict, "noise-thresh");
      hb_dict_extract_int(&pv->maximum_search_distance, dict,
                          "search-distance");
      hb_dict_extract_int(&pv->post_processing, dict, "postproc");
    }
  }
  ...
  pv->segment_height[1] =
      hb_image_height(init->pix_fmt, pv->segment_height[0], 1);
  pv->segment_height[2] =
      hb_image_height(init->pix_fmt, pv->segment_height[0], 2);
  ...
  for (ii = 0; ii < pv->cpu_count; ii++) {
    ...
    for (pp = 0; pp < 3; pp++) {
      ...
      if (ii == pv->cpu_count - 1) {
        ...
        thread_args->segment_height[pp] =
            ((hb_image_height(init->pix_fmt, init->geometry.height, pp) + 3) &
             ~3) -
            thread_args->segment_start[pp];
      } else {
    ...
    if (taskset_thread_spawn(&pv->yadif_taskset, ii, "yadif_filter_segment",
  ...
  if (pv->mode & MODE_DECOMB_EEDI2) {
    ...
    for (ii = 0; ii < 3; ii++) {
      ...
      if (taskset_thread_spawn(&pv->eedi2_taskset, ii, "eedi2_filter_segment",
                               eedi2_filter_thread, HB_NORMAL_PRIORITY) == 0) {
...
static void hb_decomb_close(hb_filter_object_t *filter) {
  ...
  if (pv->post_processing > 1 && (pv->mode & MODE_DECOMB_EEDI2)) {
    if (pv->cx2)
      eedi2_aligned_free(pv->cx2);
    if (pv->cy2)
      eedi2_aligned_free(pv->cy2);
    if (pv->cxy)
      eedi2_aligned_free(pv->cxy);
    if (pv->tmpc)
      eedi2_aligned_free(pv->tmpc);
  }
...
static void fill_stride(hb_buffer_t *buf) {
  ...
  for (pp = 0; pp < 3; pp++) {
    ...
    src = buf->plane[pp].data +
          (buf->plane[pp].height - 1) * buf->plane[pp].stride;
    dst = buf->plane[pp].data + buf->plane[pp].height * buf->plane[pp].stride;
    for (ii = 0; ii < 3; ii++) {
...
static void process_frame(hb_filter_private_t *pv) {
  if ((pv->mode & MODE_DECOMB_SELECTIVE) &&
    ...
    for (frame = 0; frame < num_frames; frame++) {
      ...
      buf = hb_frame_buffer_init(pv->ref[1]->f.fmt, pv->ref[1]->f.width,
                                 pv->ref[1]->f.height);
