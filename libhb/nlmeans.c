 ...
 *
 * Parameters:
 *     lumaY_strength   : lumaY_origin_tune   : lumaY_patch_size   : lumaY_range
 * : lumaY_frames   : lumaY_prefilter   : chromaB_strength : chromaB_origin_tune
 * : chromaB_patch_size : chromaB_range : chromaB_frames : chromaB_prefilter :
 *     chromaR_strength : chromaR_origin_tune : chromaR_patch_size :
 * chromaR_range : chromaR_frames : chromaR_prefilter
 *
 * Defaults:
 *     8:1:7:3:2:0 for each channel (equivalent to
 * 8:1:7:3:2:0:8:1:7:3:2:0:8:1:7:3:2:0)
 *
 ...
 *
 * Strength is relative and must be adjusted; ALL parameters affect overall
 * strength. Lower origin tune improves results for noisier input or animation
 * (film 0.5-1, animation 0.15-0.5). Large patch size (>9) may greatly reduce
 * quality by clobbering detail. Larger search range increases quality; however,
 * computation time increases exponentially. Large number of frames (film >3,
 * animation >6) may cause temporal smearing. Prefiltering can potentially
 * improve weight decisions, yielding better results for difficult sources.
 *
...
#define NLMEANS_SORT(a, b)                                                     \
  {                                                                            \
    if (a > b)                                                                 \
      NLMEANS_SWAP(a, b);                                                      \
  }
#define NLMEANS_SWAP(a, b)                                                     \
  {                                                                            \
    a = (a ^ b);                                                               \
    b = (a ^ b);                                                               \
    a = (b ^ a);                                                               \
...
static int nlmeans_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                        hb_buffer_t **buf_out);
...
static void nlmeans_border(uint8_t *src, const int w, const int h,
                           const int border) {
  ...
  for (int y = 0; y < border; y++) {
    ...
    memcpy(image - border + (y + h) * bw, image - border + (h - y - 1) * bw,
           bw);
  }
}

static void nlmeans_deborder(const BorderedPlane *src, uint8_t *dst,
...
static void nlmeans_alloc(const uint8_t *src, const int src_w, const int src_s,
                          const int src_h, BorderedPlane *dst,
                          const int border) {
...
static void nlmeans_filter_mean(const uint8_t *src, uint8_t *dst, const int w,
                                const int h, const int border, const int size) {
...
static uint8_t nlmeans_filter_median_opt(uint8_t *pixels, const int size) {
...
static void nlmeans_filter_median(const uint8_t *src, uint8_t *dst, const int w,
                                  const int h, const int border,
                                  const int size) {
  // Median filter
  const int bw = w + 2 * border;
  const int offset_min = -((size - 1) / 2);
...
static void nlmeans_filter_csm(const uint8_t *src, uint8_t *dst, const int w,
                               const int h, const int border, const int size) {
  // CSM filter
  const int bw = w + 2 * border;
  const int offset_min = -((size - 1) / 2);
  const int offset_max = (size + 1) / 2;
  uint8_t min, max, min2, max2, min3, max3, median, pixel;
  for (int y = 0; y < h; y++) {
    for (int x = 0; x < w; x++) {
      for (int k = offset_min; k < offset_max; k++) {
        for (int j = offset_min; j < offset_max; j++) {
...
static void nlmeans_filter_edgeboost(const uint8_t *src, uint8_t *dst,
  ...
  const int kernel[3][3] = {{-31, 0, 31}, {-44, 0, 44}, {-31, 0, 31}};
    ...
    for (int x = 0; x < w; x++) {
      if (*(mask + bw * y + x) > 16) {
        ...
        if (*(mask + bw * y + x) > 16) {
          if (*(mask + bw * y + x) == 235) {
            *(dst + bw * y + x) =
                (3 * *(src + bw * y + x) + 1 * *(dst + bw * y + x)) / 4;
          } else {
            *(dst + bw * y + x) =
                (2 * *(src + bw * y + x) + 3 * *(dst + bw * y + x)) / 5;
          }
...
static void nlmeans_prefilter(BorderedPlane *src, const int filter_type) {
  hb_lock(src->mutex);
  ...
  if (filter_type & NLMEANS_PREFILTER_MODE_MEAN3X3 ||
    ...
    if (dry > 0) {
      for (int y = 0; y < bh; y++) {
        for (int x = 0; x < bw; x++) {
          *(mem_pre + bw * y + x) = (uint8_t)(
              (wet * *(mem_pre + bw * y + x) + dry * *(mem + bw * y + x)) /
              (wet + dry));
        }
      }
    }
...
static void build_integral_scalar(uint32_t *integral, int integral_stride,
                                  const uint8_t *src, const uint8_t *src_pre,
                                  const uint8_t *compare,
                                  const uint8_t *compare_pre, int w, int border,
                                  int dst_w, int dst_h, int dx, int dy) {
  const int bw = w + 2 * border;
...
static void nlmeans_plane(NLMeansFunctions *functions, Frame *frame,
                          int prefilter, int plane, int nframes, uint8_t *dst,
                          int dst_w, int dst_s, int dst_h, double h_param,
                          double origin_tune, int n, int r,
                          const float *exptable, const float weight_fact_table,
                          const int diff_max) {
  ...
  uint32_t *const integral_mem =
      calloc(integral_stride * (dst_h + 1), sizeof(uint32_t));
  ...
  for (int f = 0; f < nframes; f++) {
    ...
    for (int dy = -r_half; dy <= r_half; dy++) {
      for (int dx = -r_half; dx <= r_half; dx++) {
        ...
        if (dx == 0 && dy == 0 && f == 0) {
          for (int y = n_half; y < dst_h - n + n_half; y++) {
            for (int x = n_half; x < dst_w - n + n_half; x++) {
              ...
              tmp_data[y * dst_w + x].pixel_sum +=
                  origin_tune * src[y * bw + x];
            }
          }
          continue;
        }
        ...
        // Build integral
        functions->build_integral(integral, integral_stride, src, src_pre,
                                  compare, compare_pre, w, border, dst_w, dst_h,
                                  dx, dy);

        // Average displacement
        for (int y = 0; y <= dst_h - n; y++) {
          const uint32_t *integral_ptr1 =
              integral + (y - 1) * integral_stride - 1;
          const uint32_t *integral_ptr2 =
              integral + (y + n - 1) * integral_stride - 1;
          ...
          for (int x = 0; x <= dst_w - n; x++) {
            ...
            const int diff = (uint32_t)(integral_ptr2[n] - integral_ptr2[0] -
                                        integral_ptr1[n] + integral_ptr1[0]);
            ...
            if (diff < diff_max) {
              ...
              tmp_data[yc * dst_w + xc].pixel_sum +=
                  weight * compare[(yc + dy) * bw + xc + dx];
            }
  ...
  for (int y = n_half; y < dst_h - n_half; y++) {
    for (int x = n_half; x < dst_w - n_half; x++) {
      result = (uint8_t)(tmp_data[y * dst_w + x].pixel_sum /
                         tmp_data[y * dst_w + x].weight_sum);
...
static int nlmeans_init(hb_filter_object_t *filter, hb_filter_init_t *init) {
  ...
  for (int c = 0; c < 3; c++) {
    ...
    if (pv->strength[c] == -1) {
      pv->strength[c] =
          c ? NLMEANS_STRENGTH_LUMA_DEFAULT : NLMEANS_STRENGTH_CHROMA_DEFAULT;
    }
    if (pv->origin_tune[c] == -1) {
      pv->origin_tune[c] = c ? NLMEANS_ORIGIN_TUNE_LUMA_DEFAULT
                             : NLMEANS_ORIGIN_TUNE_CHROMA_DEFAULT;
    }
    if (pv->patch_size[c] == -1) {
      pv->patch_size[c] = c ? NLMEANS_PATCH_SIZE_LUMA_DEFAULT
                            : NLMEANS_PATCH_SIZE_CHROMA_DEFAULT;
    }
    if (pv->range[c] == -1) {
      pv->range[c] =
          c ? NLMEANS_RANGE_LUMA_DEFAULT : NLMEANS_RANGE_CHROMA_DEFAULT;
    }
    if (pv->nframes[c] == -1) {
      pv->nframes[c] =
          c ? NLMEANS_FRAMES_LUMA_DEFAULT : NLMEANS_FRAMES_CHROMA_DEFAULT;
    }
    if (pv->prefilter[c] == -1) {
      pv->prefilter[c] =
          c ? NLMEANS_PREFILTER_LUMA_DEFAULT : NLMEANS_PREFILTER_CHROMA_DEFAULT;
    }
    ...
    if (pv->max_frames < pv->nframes[c])
      pv->max_frames = pv->nframes[c];
    ...
    const float weight_factor = 1.0 / pv->patch_size[c] / pv->patch_size[c] /
                                (pv->strength[c] * pv->strength[c]);
  ...
  if (taskset_init(&pv->taskset, pv->threads, sizeof(nlmeans_thread_arg_t)) ==
      0) {
...
static void nlmeans_filter_thread(void *thread_args_v) {
  ...
  while (1) {
    ...
    hb_buffer_t *buf;
    buf = hb_frame_buffer_init(pv->output.pix_fmt, frame->width, frame->height);
    ...
    for (int c = 0; c < 3; c++) {
      ...
      nlmeans_plane(functions, frame, pv->prefilter[c], c, pv->nframes[c],
                    buf->plane[c].data, buf->plane[c].width,
                    buf->plane[c].stride, buf->plane[c].height, pv->strength[c],
                    pv->origin_tune[c], pv->patch_size[c], pv->range[c],
                    pv->exptable[c], pv->weight_fact_table[c], pv->diff_max[c]);
    }
...
static void nlmeans_add_frame(hb_filter_private_t *pv, hb_buffer_t *buf) {
  for (int c = 0; c < 3; c++) {
    ...
    nlmeans_alloc(buf->plane[c].data, buf->plane[c].width, buf->plane[c].stride,
                  buf->plane[c].height, &pv->frame[pv->next_frame].plane[c],
                  border);
...
static hb_buffer_t *nlmeans_filter_flush(hb_filter_private_t *pv) {
  ...
  for (int f = 0; f < pv->next_frame; f++) {
    ...
    for (int c = 0; c < 3; c++) {
      ...
      // Process current plane
      nlmeans_plane(functions, frame, pv->prefilter[c], c, nframes,
                    buf->plane[c].data, buf->plane[c].width,
                    buf->plane[c].stride, buf->plane[c].height, pv->strength[c],
                    pv->origin_tune[c], pv->patch_size[c], pv->range[c],
                    pv->exptable[c], pv->weight_fact_table[c], pv->diff_max[c]);
