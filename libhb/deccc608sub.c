...
/*
 * From ccextractor, leave this file as intact and close to the original as
 * possible so that it is easy to patch in fixes - even though this file
 * contains code that we don't need.
 *
...
static const char *color_text[][2] = {
    {"white", "&HFFFFFF&"},   {"green", "&H00FF00&"},
    {"blue", "&HFF0000&"},    {"cyan", "&HFFFF00&"},
    {"red", "&H0000FF&"},     {"yellow", "&H00FFFF&"},
    {"magenta", "&HFF00FF&"}, {"userdefined", "&HFFFFFF&"}};
  ...
  for (i = first; i <= last; i++) {
    ...
    if ((font_style & FONT_STYLE_MASK) == 0 && font_color == COL_WHITE) {
      ...
      if (font_color != wb->prev_font_color) {
        ...
        buffer += encode_line(buffer, (uint8_t *)color_text[font_color][1]);
      }
...
static struct eia608_screen *get_writing_buffer(struct s_write *wb) {
  ...
  case MODE_TEXT:
    ...
    break;
  default:
    hb_error("Caption mode has an illegal value at get_writing_buffer(), this "
             "is a bug.");
  }
  return use_buffer;
}
...
static void write_char(const unsigned char c, struct s_write *wb) {
  if (wb->data608->mode != MODE_TEXT) {
    ...
    if (use_buffer != NULL) {
      ...
      use_buffer
          ->characters[wb->data608->cursor_row][wb->data608->cursor_column] = c;
      use_buffer->colors[wb->data608->cursor_row][wb->data608->cursor_column] =
          wb->data608->color;
      use_buffer->fonts[wb->data608->cursor_row][wb->data608->cursor_column] =
          wb->data608->font;
...
static void handle_text_attr(const unsigned char c1, const unsigned char c2,
  ...
  if (((c1 != 0x11 && c1 != 0x19) || (c2 < 0x20 || c2 > 0x2f)) && debug_608) {
...
static int write_cc_buffer_as_ssa(struct eia608_screen *data,
  ...
  aspect = (double)wb->width * wb->par.num / (wb->height * wb->par.den);
  ...
  for (i = 0; i < 15; i++) {
    if (data->row_used[i]) {
      ...
      if (line == 1) {
        wb->enc_buffer_used +=
            encode_line(wb->enc_buffer + wb->enc_buffer_used, (uint8_t *)pos);
        wb->enc_buffer_used +=
            stuff_space(wb->enc_buffer + wb->enc_buffer_used, space);
        wb->enc_buffer_used += get_decoder_line_encoded(
            wb, wb->enc_buffer + wb->enc_buffer_used, i, data);
        line = 2;
      } else {
        wb->enc_buffer_used +=
            encode_line(wb->enc_buffer + wb->enc_buffer_used, (uint8_t *)"\\N");
        wb->enc_buffer_used +=
            stuff_space(wb->enc_buffer + wb->enc_buffer_used, space);
        wb->enc_buffer_used += get_decoder_line_encoded(
            wb, wb->enc_buffer + wb->enc_buffer_used, i, data);
...
static void move_roll_up(struct s_write *wb, int row) {
  ...
  if (row < wb->data608->rollup_base_row) {
    ...
    for (ii = 0; ii < keep_lines; ii++) {
      memcpy(use_buffer->characters[dst], use_buffer->characters[src],
             CC608_SCREEN_WIDTH + 1);
      memcpy(use_buffer->colors[dst], use_buffer->colors[src],
             CC608_SCREEN_WIDTH + 1);
      memcpy(use_buffer->fonts[dst], use_buffer->fonts[src],
             CC608_SCREEN_WIDTH + 1);
...
static void handle_pac(unsigned char c1, unsigned char c2, struct s_write *wb) {
  ...
  if (debug_608)
    hb_log("  --  Position: %d:%d, color: %s,  font: %s\n", row, indent,
           color_text[wb->data608->color][0], font_text[wb->data608->font]);
...
static void process608(const unsigned char *data, int length,
  ...
  if (data != NULL) {
    for (i = 0; i < length; i = i + 2) {
      ...
      if (debug_608 && !textprinted &&
          wb->data608->channel == cc_channel) { // Current FTS information after
...
static int decccInit(hb_work_object_t *w, hb_job_t *job) {
  ...
  if (pv) {
    ...
    if (!retval) {
      ...
      hb_subtitle_add_ssa_header(w->subtitle, HB_FONT_MONO, .08 * safe_height,
                                 width, height);
    }
  }
  return retval;
...
hb_work_object_t hb_deccc608 = {WORK_DECCC608, "Closed Caption (608) decoder",
                                decccInit, decccWork, decccClose};
