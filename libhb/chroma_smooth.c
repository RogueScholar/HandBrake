...
static int chroma_smooth_work(hb_filter_object_t *filter, hb_buffer_t **buf_in,
                              hb_buffer_t **buf_out);
...
static void chroma_smooth(const uint8_t *src, uint8_t *dst, const int width,
                          const int height, const int stride,
                          chroma_smooth_plane_context_t *ctx,
                          chroma_smooth_thread_context_t *tctx) {
  uint32_t **SC = tctx->SC;
  uint32_t SR[CHROMA_SMOOTH_SIZE_MAX - 1], Tmp1, Tmp2;
...
static int chroma_smooth_init(hb_filter_object_t *filter,
  ...
  for (int c = 0; c < 3; c++) {
    ...
    if (ctx->size % 2 == 0)
      ctx->size--;
    if (ctx->size < CHROMA_SMOOTH_SIZE_MIN)
      ctx->size = CHROMA_SMOOTH_SIZE_MIN;
    if (ctx->size > CHROMA_SMOOTH_SIZE_MAX)
      ctx->size = CHROMA_SMOOTH_SIZE_MAX;
...
static int chroma_smooth_work_thread(hb_filter_object_t *filter,
  ...
  for (c = 0; c < 3; c++) {
    ...
    chroma_smooth(in->plane[c].data, out->plane[c].data, in->plane[c].width,
