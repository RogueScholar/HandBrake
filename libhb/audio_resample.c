...
hb_audio_resample_t *hb_audio_resample_init(enum AVSampleFormat sample_fmt,
                                            int sample_rate, int hb_amixdown,
                                            int normalize_mix) {
...
static int is_mono(uint64_t layout) {
  ...
  for (ii = 0, channel_count = 0, mask = 1; ii < 64 && channel_count < 2;
       ii++, mask <<= 1) {
    if (layout & mask) {
      channel_count++;
    }
...
int hb_audio_resample_update(hb_audio_resample_t *resample) {
  ...
  if (resample_changed ||
      (resample->resample_needed && resample->swresample == NULL)) {
    ...
    if ((ret = swr_init(resample->swresample))) {
      ...
      hb_error("hb_audio_resample_update: swr_init() failed (%s)", err_desc);
...
hb_buffer_t *hb_audio_resample(hb_audio_resample_t *resample,
  ...
  if (resample->resample_needed) {
    out_samples =
        nsamples * resample->out.sample_rate / resample->in.sample_rate + 1;
    out_size = av_samples_get_buffer_size(
        NULL, resample->out.channels, out_samples, resample->out.sample_fmt, 0);
    ...
    out->size =
        (out_samples * resample->out.sample_size * resample->out.channels);
  } else {
