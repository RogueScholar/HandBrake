...
static inline int allbits_set(uint32_t *bitmap, int num_words) {
  unsigned int i;
...
static inline int bit_is_set(uint32_t *bit_map, int bit_pos) {
  return ((bit_map[bit_pos >> 5] & (0x1 << (bit_pos & 0x1F))) != 0);
}

static inline int bit_is_clear(uint32_t *bit_map, int bit_pos) {
  return ((bit_map[bit_pos >> 5] & (0x1 << (bit_pos & 0x1F))) == 0);
}

static inline void bit_set(uint32_t *bit_map, int bit_pos) {
  bit_map[bit_pos >> 5] |= 0x1 << (bit_pos & 0x1F);
}

static inline void bit_clear(uint32_t *bit_map, int bit_pos) {
  bit_map[bit_pos >> 5] &= ~(0x1 << (bit_pos & 0x1F));
}

static inline void bit_nclear(uint32_t *bit_map, int start_pos, int stop_pos) {
...
static inline void bit_nset(uint32_t *bit_map, int start_pos, int stop_pos) {
  int start_word = start_pos >> 5;
  int stop_word = stop_pos >> 5;
