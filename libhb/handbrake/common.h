...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_rate_s.cs when
...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_mixdown_s.cs when
// changing this struct
struct hb_mixdown_s {
  const char *name;
  const char *short_name;
...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_encoder_s.cs when
// changing this struct
struct hb_encoder_s {
...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_container_s.cs when
// changing this struct
...
// Update win/CS/HandBrake.Interop/Interop/HbLib/hb_image_s.cs when changing
// this struct
struct hb_image_s {
  int format;
  int max_plane;
...
// Update
// win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_subtitle_config_s.cs when
// changing this struct
...
int hb_audio_dither_get_default_method(
    void); // default method, if enabled && supported
...
int hb_container_get_from_extension(
    const char *extension); // not really a container name
...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_title_set_s.cs when
// changing this struct
 ...
 * Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_job_s.cs when
 *changing this struct
 *****************************************************************************/
struct hb_job_s {
...
#define HB_VCODEC_FFMPEG_MASK                                                  \
  (0x00000F0 | HB_VCODEC_FFMPEG_VCE_H264 | HB_VCODEC_FFMPEG_VCE_H265 |         \
   HB_VCODEC_FFMPEG_NVENC_H264 | HB_VCODEC_FFMPEG_NVENC_H265 |                 \
   HB_VCODEC_FFMPEG_VT_H264 | HB_VCODEC_FFMPEG_VT_H265)
...
#define HB_VCODEC_H264_MASK                                                    \
  (HB_VCODEC_X264_MASK | HB_VCODEC_QSV_H264 | HB_VCODEC_FFMPEG_VCE_H264 |      \
   HB_VCODEC_FFMPEG_NVENC_H264 | HB_VCODEC_FFMPEG_VT_H264)
...
#define HB_VCODEC_H265_MASK                                                    \
  (HB_VCODEC_X265_MASK | HB_VCODEC_QSV_H265_MASK | HB_VCODEC_FFMPEG_VCE_H265 | \
   HB_VCODEC_FFMPEG_NVENC_H265 | HB_VCODEC_FFMPEG_VT_H265)
...
#define HB_COLR_TRA_BT709                                                      \
  1 // also use for bt470m, bt470bg, smpte170m, bt2020_10 and bt2020_12
...
/* Audio starts here */
/* Audio Codecs: Update
 * win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/NativeConstants.cs when
 * changing these consts */
...
#define HB_ACODEC_PASS_MASK                                                    \
  (HB_ACODEC_AC3 | HB_ACODEC_DCA | HB_ACODEC_DCA_HD | HB_ACODEC_FFAAC |        \
   HB_ACODEC_FFEAC3 | HB_ACODEC_FFFLAC | HB_ACODEC_MP3 | HB_ACODEC_FFTRUEHD)
...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_audio_config_s.cs
...
struct hb_audio_config_s {
  ...
  struct {
    ...
    PRIVATE uint32_t
        substream_type;            /* Substream type for multiplexed streams */
    ...
    PRIVATE int matrix_encoding;   /* Source matrix encoding mode, set by the
                                      audio decoder */
    PRIVATE uint64_t
        channel_layout; /* Source channel layout, set by the audio decoder */
    PRIVATE hb_chan_map_t
        *channel_map; /* Source channel map, set by the audio decoder */
  ...
  struct {
    ...
    PRIVATE uint32_t
        attributes; /* normal, visually impaired, director's commentary */
  } lang;
};

#ifdef __LIBHB__
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_audio_s.cs when
// changing this struct
struct hb_audio_s {
  int id;
...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_chapter_s.cs when
// changing this struct
struct hb_chapter_s {
  int index;
 ...
 * > format
 *     - format of the packets the subtitle decoder work-object sends to
 * sub->fifo_raw
 *     - for passthru subtitles, is also the format of the final packets sent to
 * sub->fifo_out
 *     - PICTURESUB for banded 8-bit YAUV pixels; see decvobsub.c documentation
 * for more info
 *     - TEXTSUB for UTF-8 text marked up with <b>, <i>, or <u>
 *     - read by the muxers, and by the subtitle burn-in logic in the
 * hb_sync_video work-object > source
 ...
 *       to pass it through its own subtitle track in the output container
 * (PASSTHRUSUB)
 *     - all newly created non-VOBSUB tracks should default to PASSTHRUSUB
 *     - all newly created VOBSUB tracks should default to RENDERSUB, for legacy
 * compatibility > lang
 *     - user-readable description of the subtitle track
 *     - may correspond to the language of the track (see the 'iso639_2' field)
 *     - may correspond to the type of track (see the 'type' field; ex: "Closed
 * Captions") > iso639_2
 *     - language code for the subtitle, or "und" if unknown
 *
 * Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_subtitle_s.cs when
 * changing this struct
 */
 ...
 * An attachment.
 *
 * These are usually used for attaching embedded fonts to movies containing SSA
 * subtitles.
 */
...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_title_s.cs when
// changing this struct
struct hb_title_s {
...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_state_s.cs when
// changing this struct
struct hb_state_s {
...
struct hb_work_object_s {
  ...
  int (*work)(hb_work_object_t *, hb_buffer_t **, hb_buffer_t **);
   ...
   * other work objects. */
  int (*bsinfo)(hb_work_object_t *, const hb_buffer_t *, hb_work_info_t *);
  ...
  /* Pointer hb_audio_t so we have access to the info in the audio worker
   * threads. */
  hb_audio_t *audio;

  /* Pointer hb_subtitle_t so we have access to the info in the subtitle worker
   * threads. */
...
struct hb_filter_object_s {
  ...
  int (*work)(hb_filter_object_t *, hb_buffer_t **, hb_buffer_t **);
  int (*work_thread)(hb_filter_object_t *, hb_buffer_t **, hb_buffer_t **, int);
...
// Update win/CS/HandBrake.Interop/HandBrakeInterop/HbLib/hb_filter_ids.cs when
// changing this enum
enum {
  HB_FILTER_INVALID = 0,
