...
hb_dvd_func_t hb_dvdnav_func = {
    hb_dvdnav_init,        hb_dvdnav_close,       hb_dvdnav_name,
    hb_dvdnav_title_count, hb_dvdnav_title_scan,  hb_dvdnav_start,
    hb_dvdnav_stop,        hb_dvdnav_seek,        hb_dvdnav_read,
    hb_dvdnav_chapter,     hb_dvdnav_angle_count, hb_dvdnav_set_angle,
...
hb_dvd_func_t *hb_dvdnav_methods(void) { return &hb_dvdnav_func; }
...
static hb_dvd_t *hb_dvdnav_init(hb_handle_t *h, const char *path) {
  ...
  d->path = strdup(
      path); /* hb_dvdnav_title_scan assumes UTF-8 path, so not path_ccp here */
  free(path_ccp);
...
static uint64_t PttDuration(ifo_handle_t *ifo, int ttn, int pttn, int *blocks,
                            int *last_pgcn) {
  int pgcn, pgn;
  pgc_t *pgc;
  uint64_t duration = 0;
...
static hb_title_t *hb_dvdnav_title_scan(hb_dvd_t *e, int t,
  ...
  if (title->name == NULL || title->name[0] == 0) {
    ...
    if (DVDUDFVolumeInfo(d->reader, name, sizeof(name), unused,
                         sizeof(unused))) {
  ...
  for (i = 0; i < d->ifo->vts_c_adt->nr_of_vobs; ++i) {
    if ((d->ifo->vts_c_adt->cell_adr_table[i].start_sector & 0xffffff) ==
      ...
      hb_log("scan: cell_adr_table[%d].start_sector invalid (0x%x) "
             "- skipping title",
             i, d->ifo->vts_c_adt->cell_adr_table[i].start_sector);
      goto fail;
    }
    if ((d->ifo->vts_c_adt->cell_adr_table[i].last_sector & 0xffffff) ==
      ...
      hb_log("scan: cell_adr_table[%d].last_sector invalid (0x%x) "
             "- skipping title",
             i, d->ifo->vts_c_adt->cell_adr_table[i].last_sector);
      goto fail;
    }
  }
  ...
  /* Get pgc */
  if (d->pgcn < 1 || d->pgcn > d->ifo->vts_pgcit->nr_of_pgci_srp ||
      d->pgcn >= MAX_PGCN) {
  ...
  for (i = 0; i < d->ifo->vtsi_mat->nr_of_vts_subp_streams; i++) {
    ...
    spu_control = d->ifo->vts_pgcit->pgci_srp[d->pgcn - 1].pgc->subp_control[i];
...
static int try_button(dvdnav_t *dvdnav, int button, hb_list_t *list_title) {
  ...
  if (result != DVDNAV_STATUS_OK) {
    hb_log("dvdnav_button_select_and_activate: %s",
    ...
    if (cur_title > 0) {
      ...
      if (hbtitle != NULL) {
        if (hbtitle->duration / 90000 > 10 * 60) {
          hb_deep_log(3,
                      "dvdnav: Found candidate feature title %d duration "
                      "%02d:%02d:%02d on button %d",
  ...
  if (longest != -1) {
    ...
    if (hbtitle != NULL) {
      hb_deep_log(3,
                  "dvdnav: Found candidate feature title %d duration "
                  "%02d:%02d:%02d on button %d",
                  longest, hbtitle->hours, hbtitle->minutes, hbtitle->seconds,
                  button + 1);
    }
  }
  return longest;
...
static int try_menu(hb_dvdnav_t *d, hb_list_t *list_title, DVDMenuID_t menu,
  ...
  if (menu != DVD_MENU_Escape) {
    ...
    if (result != DVDNAV_STATUS_OK) {
      ...
      result = dvdnav_menu_call(d->dvdnav, menu);
      if (result != DVDNAV_STATUS_OK) {
        hb_error("dvdnav: Can not set dvd menu, %s",
                 dvdnav_err_to_string(d->dvdnav));
        goto done;
      }
    }
...
static int hb_dvdnav_main_feature(hb_dvd_t *e, hb_list_t *list_title) {
  ...
  if (title) {
    hb_deep_log(2, "dvdnav: Longest title %d duration %02d:%02d:%02d",
                longest_fallback, title->hours, title->minutes, title->seconds);
  }

  dvdnav_reset(d->dvdnav);
  if (skip_to_menu(d->dvdnav, 2000)) {
    longest_root =
        try_menu(d, list_title, DVD_MENU_Escape, longest_duration_fallback);
    if (longest_root >= 0) {
      ...
      if (title) {
        ...
        hb_deep_log(2,
                    "dvdnav: Found first-play title %d duration %02d:%02d:%02d",
  ...
  if (longest_root < 0 ||
    ...
    longest_root =
        try_menu(d, list_title, DVD_MENU_Root, longest_duration_fallback);
    if (longest_root >= 0) {
      index = find_title(list_title, longest_root);
      title = hb_list_item(list_title, index);
      if (title) {
  ...
  if (longest_root < 0 ||
      (float)longest_duration_fallback * 0.7 > longest_duration_root) {
    longest_title =
        try_menu(d, list_title, DVD_MENU_Title, longest_duration_fallback);
    if (longest_title >= 0) {
...
static int hb_dvdnav_start(hb_dvd_t *e, hb_title_t *title, int c) {
  ...
  if (result != DVDNAV_STATUS_OK) {
    hb_error("dvd: dvdnav_*_play failed - %s", dvdnav_err_to_string(d->dvdnav));
    return 0;
  }
  d->stopped = 0;
  d->chapter = 0;
...
static int hb_dvdnav_seek(hb_dvd_t *e, float f) {
  ...
  for (ii = 0; ii < hb_list_count(d->list_dvd_chapter); ++ii) {
    ...
    if (chap_offset <= sector && sector < chap_offset + chap_len) {
      ...
      if (dvdnav_current_title_program(d->dvdnav, &title, &pgcn, &pgn) !=
          DVDNAV_STATUS_OK)
      ...
      if (d->title != title || chapter->pgcn != pgcn) {
        ...
        if (dvdnav_program_play(d->dvdnav, d->title, chapter->pgcn,
                                chapter->pgn) != DVDNAV_STATUS_OK)
...
static int hb_dvdnav_chapter(hb_dvd_t *e) {
  ...
  if (dvdnav_current_title_program(d->dvdnav, &t, &pgcn, &pgn) !=
      DVDNAV_STATUS_OK) {
    return -1;
  }
  c = FindChapterIndex(d->list_dvd_chapter, pgcn, pgn);
...
static int hb_dvdnav_angle_count(hb_dvd_t *e) {
  ...
  if (dvdnav_get_angle_info(d->dvdnav, &current, &angle_count) !=
      DVDNAV_STATUS_OK) {
...
static int FindNextCell(pgc_t *pgc, int cell_cur) {
  ...
  if (pgc->cell_playback[cell_cur].block_type == BLOCK_TYPE_ANGLE_BLOCK) {
    ...
    hb_log("dvd: Skipping multi-angle cells %d-%d", cell_cur, cell_next - 1);
  } else {
...
static int NextPgcn(ifo_handle_t *ifo, int pgcn,
  ...
  if (next_pgcn < 1 || next_pgcn >= MAX_PGCN ||
      next_pgcn > ifo->vts_pgcit->nr_of_pgci_srp)
    return 0;
